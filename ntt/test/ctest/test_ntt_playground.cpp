...
//     // init
//     auto ntt_tensor_lhs =  make_tensor<ntt::vector<int,
//     8>>(ntt::fixed_shape_v<1>); NttTest::init_tensor(ntt_tensor_lhs, -10,
//     10);
...
//     3, 1, 16>); ntt::binary<ntt::ops::add>(ntt_tensor_lhs, ntt_tensor_rhs,
//     ntt_output1);
...
//     // // ort
//     auto [ort_input_lhs, ort_input_rhs] =
//     NttTest::convert_and_align_to_ort(ntt_tensor_lhs, ntt_tensor_rhs); auto
//     ort_output = ortki_Add(ort_input_lhs, ort_input_rhs);
//     // ortki_Add(ort_input_lhs, ort_input_rhs);
//     // // compare
//     auto ntt_output2 = make_tensor<ntt::vector<uint32_t,
//     8>>(ntt::fixed_shape_v<1, 3, 1, 16>); NttTest::ort2ntt(ort_output,
//     ntt_output2); EXPECT_TRUE(NttTest::compare_tensor(ntt_output1,
//     ntt_output2));

// }

TEST(BinaryTestAddint,
     fixed_fixed_fixed_broadcast_lhs_1D_vector_rhs_2D_vector) {
    // init
    auto ntt_tensor_lhs =
        make_tensor<ntt::vector<double, 8>>(ntt::fixed_shape_v<1>);
    ...
    auto [ort_input_lhs, ort_input_rhs] =
        NttTest::convert_and_align_to_ort(ntt_tensor_lhs, ntt_tensor_rhs);
    ...
    // auto ort_output = ortki_Div(ortki_Add(ortki_Add(ort_input_rhs,ort_neg1),
    // ort_input_lhs), ort_input_rhs); const size_t num_inputs = 2;
    ...
    // auto ort_output = ortki_Clip(ortki_Pow(ort_input_lhs, ort_input_rhs),
    // ort_zero, ort_max);
...
TEST(BinaryTestAddint, are_close_fp16_behavior) {
    ...
    std::cout << "Testing are_close behavior for fp16 (half) values:"
              << std::endl;
    ...
    // Test case 1: lhs = 922, rhs = 922.5
    {
        ...
        std::cout << "Test 1 - lhs: " << lhs_f
                  << " (fp16:5.1875), rhs: " << rhs_f << " (fp16:5.16406)"
    ...
    // Test case 2: lhs = -59.1875, rhs = -59.2188
    {
        ...
        float lhs_f = static_cast<float>(lhs);
        float rhs_f = static_cast<float>(rhs);
        std::cout << "Test 2 - lhs: " << lhs_f
                  << " (fp16: 1.4375f), rhs: " << rhs_f << " (fp16: 1.43652)"
                  << ", are_close result: " << (result ? "true" : "false")
                  << ", diff: " << std::abs(lhs_f - rhs_f) << std::endl;
    }
    ...
    // Test case 3: lhs = -7192, rhs = -7196
    {
        ...
        float lhs_f = static_cast<float>(lhs);
        float rhs_f = static_cast<float>(rhs);
        std::cout << "Test 3 - lhs: " << lhs_f
                  << " (fp16: 321.5), rhs: " << rhs_f << " (fp16: 321.25)"
                  << ", are_close result: " << (result ? "true" : "false")
                  << ", diff: " << std::abs(lhs_f - rhs_f) << std::endl;
    }
    ...
    // Test case 4: lhs = 6996, rhs = 6992
    {
        ...
        float lhs_f = static_cast<float>(lhs);
        float rhs_f = static_cast<float>(rhs);
        std::cout << "Test 4 - lhs: " << lhs_f
                  << " (fp16: 6996), rhs: " << rhs_f << " (fp16: 6992)"
                  << ", are_close result: " << (result ? "true" : "false")
                  << ", diff: " << std::abs(lhs_f - rhs_f) << std::endl;
    }
    ...
    // Test with different tolerance values for fp16
    {
        ...
        bool result_loose = NttTest::are_close(
            lhs, rhs, 1.0, 1e-3); // More loose tolerance for fp16
        bool result_tight =
            NttTest::are_close(lhs, rhs, 1e-12, 1e-9); // Tighter tolerance

        float lhs_f = static_cast<float>(lhs);
        float rhs_f = static_cast<float>(rhs);

        std::cout << "\nTolerance test for fp16 - lhs: " << lhs_f
                  << ", rhs: " << rhs_f << std::endl;
        std::cout << "  Default tolerance (1e-9, 1e-5): "
                  << (result_default ? "true" : "false") << std::endl;
        std::cout << "  Loose tolerance (1.0, 1e-3): "
                  << (result_loose ? "true" : "false") << std::endl;
        std::cout << "  Tight tolerance (1e-12, 1e-9): "
                  << (result_tight ? "true" : "false") << std::endl;

        // Calculate what the actual tolerance check values are
        double abs_diff = std::abs(lhs_f - rhs_f);
        double rel_tol_default =
            1e-5 * std::max(std::abs(lhs_f), std::abs(rhs_f));
        ...
        std::cout << "  Default relative tolerance: " << rel_tol_default
                  << std::endl;
        std::cout << "  Default absolute tolerance: " << abs_tol_default
                  << std::endl;
        std::cout << "  Default threshold: " << threshold_default << std::endl;
        std::cout << "  Passes default threshold: "
                  << (abs_diff <= threshold_default ? "true" : "false")
                  << std::endl;
    }
    ...
    // Test fp16 precision limitations
    {
        ...
        std::cout << "Difference after fp16 conversion: " << std::abs(f1 - f2)
                  << std::endl;

        // Show raw fp16 values
        std::cout << "Raw fp16 value for 922.0: 0x" << std::hex << h1.raw()
                  << std::dec << std::endl;
        std::cout << "Raw fp16 value for 922.5: 0x" << std::hex << h2.raw()
                  << std::dec << std::endl;
    }
}
...
//                             (fixed_shape_v<1, 3, 16, 16>), (fixed_shape_v<1,
//                             3, 16, 16>), (fixed_shape_v<1, 3, 16, 16>),
//                            int, add, Add)

// GENERATE_BINARY_TEST(BinaryTestAddint,
// fixed_fixed_fixed_broadcast_lhs_scalar,
//                             (fixed_shape_v<1>), (fixed_shape_v<1, 3, 16,
//                             16>), (fixed_shape_v<1, 3, 16, 16>),
//                            int, add, Add)

// GENERATE_BINARY_TEST(BinaryTestAddint,
// fixed_fixed_fixed_broadcast_rhs_scalar,
//                             (fixed_shape_v<1, 3, 16, 16>),
//                             (fixed_shape_v<1>), (fixed_shape_v<1, 3, 16,
//                             16>),
//                            int, add, Add)
...
//                             (fixed_shape_v<16>), (fixed_shape_v<1, 3, 16,
//                             16>), (fixed_shape_v<1, 3, 16, 16>),
//                            int, add, Add)
...
//                             (fixed_shape_v<1, 3, 1, 16>), (fixed_shape_v<3,
//                             1, 16, 1>), (fixed_shape_v<3, 3, 16, 16>),
//                            int, add, Add)

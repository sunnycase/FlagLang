...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_contiguous_unpack_axis_2_3D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<C, H, W>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim1_mul2_unpack_axis_2_3D) {
    ...
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<C, (H) *2, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    
    auto ntt_input = ntt::make_tensor_view_from_address<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_0_1_3D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<C, H, W>);
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<C * 4, H * P, W>);
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<C, H, W>);
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_1_2_3D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<C, H * 4, W * P>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_3D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<C, (H) *2, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    
    auto ntt_input = ntt::make_tensor_view_from_address<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(
        big_tensor.elements().data(),
        ntt::fixed_shape_v<C, H, W>,
        ntt::canonicalize_strides(ntt::fixed_shape_v<C, H, W>, big_tensor.strides())
        );
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_contiguous_unpack_axis_2_3D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(C, H, W));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim1_mul2_unpack_axis_2_3D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(C, (H) *2, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_0_1_3D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(C, H, W));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(C * 4, H * P, W));
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(C, H, W));
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_1_2_3D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(C, H * 4, W * P));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_3D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(C, (H) *2, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_contiguous_unpack_axis_3_4D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W>);
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim2_add5_unpack_axis_3_4D) {
    ...
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, (H) +7, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim2_mul2_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, (H) *2, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim1_mul2_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) *2, H, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim1_add5_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) +7, H, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_0_1_4D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W>);
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N * 4, C * P, H, W>);
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W>);
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_1_2_4D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N, C * 4, H * P, W>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_2_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N, C, H * 4, W * P>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim2_add5_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, (H) +7, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim2_mul2_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, (H) *2, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) *2, H, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim1_add5_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) +7, H, W>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_contiguous_unpack_axis_3_4D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim2_add5_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, (H) +7, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim2_mul2_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, (H) *2, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim1_mul2_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) *2, H, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim1_add5_unpack_axis_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) +7, H, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_0_1_4D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N * 4, C * P, H, W));
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W));
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_1_2_4D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N, C * 4, H * P, W));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_2_3_4D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N, C, H * 4, W * P));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim2_add5_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, (H) +7, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim2_mul2_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 2)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, (H) *2, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) *2, H, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim1_add5_unpack_axis_0_1_4D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) +7, H, W));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_contiguous_unpack_axis_4_5D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W, D>);
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_1D_vector_non_contiguous_dim1_mul2_unpack_axis_4_5D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) *2, H, W, D>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_0_1_5D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W, D>);
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N * 4, C * P, H, W, D>);
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, C, H, W, D>);
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_1_2_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N, C * 4, H * P, W, D>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_2_3_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N, C, H * 4, W * P, D>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_contiguous_unpack_axis_3_4_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::fixed_shape_v<N, C, H, W * 4, D * P>);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_fixed_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_5D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::fixed_shape_v<N, (C) *2, H, W, D>);
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_contiguous_unpack_axis_4_5D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W, D));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_1D_vector_non_contiguous_dim1_mul2_unpack_axis_4_5D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) *2, H, W, D));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_0_1_5D) {
    ...
    // 1. create NTT input 
    // ------------------------------------------------------------------
    auto ntt_input = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W, D));
    NttTest::init_tensor(ntt_input, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N * 4, C * P, H, W, D));
    ...
    // 1. build ORT input tensor
    // ------------------------------------------------------------------
        auto ntt_input_uint8 = ntt::make_tensor<ntt::vector<uint8_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, C, H, W, D));
        NttTest::reinterpret_cast_fp8_to_uint8(ntt_input, ntt_input_uint8);
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_1_2_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N, C * 4, H * P, W, D));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_2_3_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N, C, H * 4, W * P, D));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_contiguous_unpack_axis_3_4_5D) {
    ...
    // ------------------------------------------------------------------
    // Create output tensor
    auto ntt_output1 = ntt::make_tensor<float_e5m2_t>(ntt::make_shape(N, C, H, W * 4, D * P));
...
TEST(UnpackTest_Float8e5m2, Float8e5m2_dynamic_2D_vector_non_contiguous_dim1_mul2_unpack_axis_0_1_5D) {
    ...
    // ------------------------------------------------------------------
    // Create non-contiguous tensor (on dimension 1)
    auto big_tensor = ntt::make_tensor<ntt::vector<float_e5m2_t, 4, NTT_VLEN / (sizeof(float_e5m2_t) * 8)>>(ntt::make_shape(N, (C) *2, H, W, D));
    NttTest::init_tensor(big_tensor, float_e5m2_t(-32.0f), float_e5m2_t(32.0f), true, false);

...
#define PRINT_TENSOR(tensor)                                                   \
    nncase::ntt::apply(tensor.shape(), [&](auto index) {                       \
        printf("c[%ld, %ld] = %f\n", index[0], index[1], tensor(index));       \
    });

#define DECLARE_NTT_TENSOR(element_type, tensor_name, shape_param)             \
    /* is fixed_shape */                                                       \
    auto ntt_tensor_##tensor_name =                                            \
        make_unique_tensor<element_type>(shape_param);                         \
    NttTest::init_tensor(*ntt_tensor_##tensor_name, -10.f, 10.f)

#define BINARY_TEST_BODY(lhs_name, rhs_name, out_name, golden_name, ntt_op,    \
                         ort_op)                                               \
    /* ntt */                                                                  \
    ntt::binary<ntt::ops::ntt_op>(*ntt_tensor_##lhs_name,                      \
                                  *ntt_tensor_##rhs_name,                      \
                                  *ntt_tensor_##out_name);                     \
    /* ort */                                                                  \
    ...
    EXPECT_TRUE(NttTest::compare_tensor(*ntt_tensor_##out_name,                \
                                        *ntt_tensor_##golden_name));

#define GENERATE_BINARY_TEST(test_group, test_name, lhs_shape, rhs_shape,      \
                             output_shape, element_type, ntt_op, ort_op)       \
...
#define MAKE_SUFFIX_IMPL(s1, s2, s3, suffix_type)                              \
    s1##_##s2##_##s3##_##suffix_type
#define MAKE_SUFFIX(s1, s2, s3, suffix_type)                                   \
    MAKE_SUFFIX_IMPL(s1, s2, s3, suffix_type)

#define GENERATE_BINARY_TEST_GROUP(TestNamePrefix, LhsShapeType, RhsShapeType, \
                                   OutShapeType, DataType, Ntt_op, Ort_op)     \
    GENERATE_BINARY_TEST(                                                      \
        TestNamePrefix,                                                        \
        MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType, normal),         \
        GET_SHAPE_##LhsShapeType(1, 3, 16, 16),                                \
        GET_SHAPE_##RhsShapeType(1, 3, 16, 16),                                \
        GET_SHAPE_##OutShapeType(1, 3, 16, 16), DataType, Ntt_op, Ort_op)      \
    GENERATE_BINARY_TEST(                                                      \
        TestNamePrefix,                                                        \
        MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType,                  \
                    broadcast_lhs_scalar),                                     \
        GET_SHAPE_##LhsShapeType(1), GET_SHAPE_##RhsShapeType(1, 3, 16, 16),   \
        GET_SHAPE_##OutShapeType(1, 3, 16, 16), DataType, Ntt_op, Ort_op)      \
    GENERATE_BINARY_TEST(                                                      \
        TestNamePrefix,                                                        \
        MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType,                  \
                    broadcast_rhs_scalar),                                     \
        GET_SHAPE_##LhsShapeType(1, 3, 16, 16), GET_SHAPE_##RhsShapeType(1),   \
        GET_SHAPE_##OutShapeType(1, 3, 16, 16), DataType, Ntt_op, Ort_op)      \
    GENERATE_BINARY_TEST(                                                      \
        TestNamePrefix,                                                        \
        MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType,                  \
                    broadcast_lhs_vector),                                     \
        GET_SHAPE_##LhsShapeType(16), GET_SHAPE_##RhsShapeType(1, 3, 16, 16),  \
        GET_SHAPE_##OutShapeType(1, 3, 16, 16), DataType, Ntt_op, Ort_op)      \
    GENERATE_BINARY_TEST(                                                      \
        TestNamePrefix,                                                        \
        MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType,                  \
                    broadcast_rhs_vector),                                     \
        GET_SHAPE_##LhsShapeType(1, 3, 16, 16), GET_SHAPE_##RhsShapeType(16),  \
        GET_SHAPE_##OutShapeType(1, 3, 16, 16), DataType, Ntt_op, Ort_op)      \
    GENERATE_BINARY_TEST(TestNamePrefix,                                       \
                         MAKE_SUFFIX(LhsShapeType, RhsShapeType, OutShapeType, \
                                     broadcast_multidirectional),              \
                         GET_SHAPE_##LhsShapeType(1, 3, 1, 16),                \
                         GET_SHAPE_##RhsShapeType(3, 1, 16, 1),                \
                         GET_SHAPE_##OutShapeType(3, 3, 16, 16), DataType,     \
                         Ntt_op, Ort_op)

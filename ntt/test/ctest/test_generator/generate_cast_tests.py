...
class CastTestGenerator(BaseTestGenerator):
    ...
    def __init__(self):
        ...
        self.element_type_lengths = {
            'uint8_t': 1, 'int8_t': 1, 'bool': 1, 'uint16_t': 2, 'int16_t': 2, 'half': 2, 'bfloat16': 2, 'uint32_t': 4,
            'int32_t': 4, 'float': 4, 'uint64_t': 8, 'int64_t': 8, 'double': 8, 'float_e4m3_t': 1, 'float_e5m2_t': 1
        }
    ...
    def generate_ort_output(self, to_type):
        ...
        return ["// ORT reference implementation", f"auto ort_output = ortki_Cast(ort_input, 1, {ort_type});", ""]

    def generate_ntt_ops(self, repackedAxes=None):
        """Generate NTT cast operation code"""
        if repackedAxes is not None:
            axes_str = f"ntt::fixed_shape_v<{repackedAxes[0]}>"
            return ["// Execute cast operation", f"ntt::cast(ntt_input, ntt_output1, {axes_str});", ""]
        else:
            return ["// Execute cast operation", "ntt::cast(ntt_input, ntt_output1);", ""]

    def generate_ntt_output_to_test(self, from_type, to_type, shape_type, dims_spec, continuity, vector_rank, P,
                                    repackedAxes=None):
        ...
        code.extend(
            self.generate_tensor_init(datatype=cast_data_type, shape_type=shape_type, dim_spec=dims_spec,
                                      continuity=continuity, vector_rank=vector_rank, P=P, var_name="ntt_input",
                                      name_suffix=""))
    ...
    def generate_ntt_cast_golden_output_fp8(self, from_type, to_type, shape_type, dims_spec, continuity, P,
                                            vector_rank):
        code = []
        from_ele_len = self.element_type_lengths.get(from_type.cpp_type, 4)
        to_ele_len = self.element_type_lengths.get(to_type.cpp_type, 4)
        ...
        if not continuity.is_contiguous:
            copy_code, continuous_input_var_name = self.generate_copy_to_contiguous_code(
                tensor_element_type, shape_type, dims_spec)
        ...
        if 'vector' in tensor_element_type:
            ...
            out_vec_dims_spec[-1] = ((dims_spec[-1] // scale) if from_ele_len > to_ele_len else (dims_spec[-1] * scale))

            output_element_type = self.get_element_cpp_type(
                to_type.cpp_type, vector_rank, (f"{P} * {scale}" if from_ele_len > to_ele_len else f"{P} / {scale}"))

            unpacked_dims = self.get_unpacked_dims(dims_spec, unpack_axes)
            code.append(
                f"auto ntt_scalar_input = ntt::make_tensor<{from_type.cpp_type}>({self.generate_shape_init(shape_type, unpacked_dims)});"
            )
            code.append(
                f"ntt::unpack({continuous_input_var_name}, ntt_scalar_input, {self.generate_pack_axes_str(unpack_axes)});"
            )
        else:
        ...
        code.append(f"ntt::apply(ntt_golden_scalar.shape(), [&](auto& index){{\n"
        ...
        if "vector" in tensor_element_type:
            code.append(
                f"auto ntt_golden_vector = ntt::make_tensor<{output_element_type}>({self.generate_shape_init(shape_type, out_vec_dims_spec)});"
            )
    ...
    def generate_ort_golden_output(self, from_type, to_type, shape_type, dims_spec, continuity, P, vector_rank,
                                   deal_fp8, repackedAxes=None):
        ...
        if not is_fp8_cast:
            ...
            continuity_var_name, copy_code = self._prepare_contiguous_input("ntt_input", from_type, vector_rank, P,
            ...
            code.extend(
                self.generate_ntt_cast_golden_output_fp8(from_type, to_type, shape_type, dims_spec, continuity, P,
                                                         vector_rank))
    ...
    def generate_test_case(self, from_type, to_type, shape_type, vector_rank, continuity, ndim, repackedAxes=None):
        ...
        golden_output_code = self.generate_ort_golden_output(from_type, to_type, shape_type, dims_spec, continuity, P,
                                                             vector_rank, deal_fp8, repackedAxes)
        ...
        if is_fp8_cast:
            ...
            code.extend(
                ["    // Compare results", "    EXPECT_TRUE(NttTest::compare_tensor(ntt_output1, ntt_golden));", "}"])
        else:
            # ORT-based comparison
            compare_code = self.generate_ort_back2ntt_and_compare_section(to_type, output_element_type,
                                                                          output_shape_expr, deal_fp8,
    ...
    def generate_all_tests_for_from_type(self, from_type):
        ...
        for to_type in ALL_DATATYPES:
            ...
            for ndim in [3, 4]:
                ...
                for shape_type, vector_rank, continuity in itertools.product(shape_types, vector_ranks,
                                                                             current_continuities):
                    ...
                    for repackedAxes in repackedAxes_choices:
                        test_code = self.generate_test_case(from_type, to_type, shape_type, vector_rank, continuity,
                                                            ndim, repackedAxes)
...
if __name__ == "__main__":
    ...
    generate_cmake_list(generated_directory, generated_filenames, "generated_cast_tests.cmake",
                        "GENERATED_CAST_TEST_SOURCES")

...
class UnaryTestGenerator(BaseTestGenerator):
    ...
    def __init__(self):
        ...
        self.op_str_map_exhaustive = {
            "abs": f"auto ort_output = ortki_Abs(ort_input);", "cos": f"auto ort_output = ortki_Cos(ort_input);", "sin":
            f"auto ort_output = ortki_Sin(ort_input);", "exp": f"auto ort_output = ortki_Exp(ort_input);"
        ...
        self.dims_specs_options = {"default": [[2, 3, 16, 16], [2, 1, 16, 7]]}
        ...
        self.OPRATOR_SPECICAL_TYPES = {
            "sin": {"float": DataType("float", "Float32", "-1e6", "1e6", False)}, "cos": {
                "float": DataType("float", "Float32", "-1e6", "1e6", False),
                "double": DataType("double", "Float64", "-1e6", "1e6", False),
            }, "exp": {
                "float": DataType("float", "Float32", "-70.0", "70.0",
                                  False), "double": DataType("double", "Float64", "-700.0", "700.0", False), "half":
                DataType("half", "Float16", "half(-10.0f)", "half(10.0f)",
                         False), "bfloat16": DataType("bfloat16", "Bfloat16", "-70.0_bf16", "70.0_bf16", False),
                "float_e4m3_t": DataType("float_e4m3_t", "Float8e4m3", "float_e4m3_t(-6.0f)", "float_e4m3_t(6.0f)",
                                         False), "uint8_t": DataType('uint8_t', 'Uint8', '0', '5', True), "int8_t":
                DataType('int8_t', 'Int8', '-4', '4',
                         True), "uint16_t": DataType('uint16_t', 'Uint16', '0', '11',
                                                     True), "int16_t": DataType('int16_t', 'Int16', '-10', '10', True),
                "uint32_t": DataType('uint32_t', 'Uint32', '0', '22',
                                     True), "int32_t": DataType('int32_t', 'Int32', '-21', '21', True), "uint64_t":
                DataType('uint64_t', 'Uint64', '0', '30',
                         True), "int64_t": DataType('int64_t', 'Int64', '-30', '30', True)
            }
        }

        self.ulp_tolerances = {"sin": {"default": 2}, "cos": {"default": 2}, "default": {"default": 1}}
    ...
    def _get_param_combinations(self, op_str):
        ...
        param_combinations_exhaustive = itertools.product(is_dynamic_options,  # input_dynamic_shape 2
    ...
    def generate_test_name(self, datatype, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank,
                           input_continuity):
        parts = []
    ...
    def get_op_call_lines(self, ntt_op_str):
        ...
        return ["// Execute unary operation", f"ntt::unary<ntt::ops::{ntt_op_str}>(ntt_input, ntt_output);", ""]

    def generate_ntt_output_to_test(self, datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec, vec_rank,
                                    vec_param, input_continuity):
        code = []

        code.append(f"{self.indent}//---init ntt_input---")
        tensor_init_lhs_code = self.generate_tensor_init(datatype=datatype, shape_type=input_is_dynamic,
                                                         dim_spec=dims_spec, continuity=input_continuity,
                                                         var_name="ntt_input", name_suffix="", vector_rank=vec_rank,
        ...
        ntt_output_and_op_code = self.generate_ntt_output_and_op_section(datatype=datatype,
                                                                         ...
                                                                         cast_mode=0, ntt_op_call_lines=op_call_lines,
    ...
    def _generate_vector_to_scalar_conversion(self, code, datatype, input_var_name, input_is_dynamic, input_dims_spec,
                                              vec_rank):
        assert (vec_rank > 0)
        ...
        code.append(
            f"auto {input_var_name}_unsqueezed = ({input_var_name}).unsqueeze(fixed_shape_v<{unsqueeze_dims}>);")

        # 2. unpack
        code.append(
            f"auto {input_var_name}_scalar = ntt::make_tensor<{datatype.cpp_type}>(fixed_shape_v<{','.join(map(str,scalar_dims))}>);"
        )
        code.append(f"ntt::unpack({input_var_name}_unsqueezed, {input_var_name}_scalar, fixed_shape_v<{unpack_dims}>);")
        return scalar_dims

    def _prepare_double_scalar_input(self, code, datatype, input_is_dynamic, input_dims_spec, vector_rank, vec_param,
                                     input_continuity, cast_target="double"):
        """
        ...
        if input_is_vec:
            input_scalar_dims = self._generate_vector_to_scalar_conversion(code, datatype, input_var_name,
                                                                           input_is_dynamic, input_dims_spec,
                                                                           vector_rank)
        else:
    ...
    def _generate_ntt_cast_golden_output(self, datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec,
                                         vector_rank, vec_param, input_continuity, cast_target):
        """Special handling for types that cannot be cast in ORT"""
        code = []

        self._prepare_double_scalar_input(code, datatype, input_is_dynamic, dims_spec, vector_rank, vec_param,
                                          input_continuity, cast_target)

        ort_golden_double_var = self._execute_ort_operation(code, datatype, op_str, cast_target)

        self._cast_ort_golden_double_into_ntt_shape(code, datatype, op_str, output_is_dynamic, dims_spec, vector_rank,
                                                    vec_param, ort_golden_double_var, cast_target)

        return code

    def _generate_ort_cast_golden_output(self, datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec,
                                         vector_rank, vec_param, input_continuity, cast_target):
        ...
        if need_cast_in_ort:
            code.append(
                f"auto ort_input = ortki_Cast({ort_input_origin_type}, 1, ortki::{self.ort_datatype_map[cast_target]});"
            )
        else:
        ...
        cast_code, golden_var_name = self.generate_ort_back2ntt(datatype, output_element_type, output_shape_expr,
                                                                ...
                                                                ort_output_var_name="ort_golden", ort_type=cast_target)
    ...
    def generate_ntt_golden_output(self, datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank,
                                   vec_param, input_continuity):
        code = []
        ...
        if need_cast_in_ntt:
            # cast in ntt
            code.extend(
                self._generate_ntt_cast_golden_output(datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec,
                                                      vector_rank, vec_param, input_continuity, cast_target))
        else:
            # cast in ort or need not cast
            golden_output_code, golden_var_name = self._generate_ort_cast_golden_output(
                datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank, vec_param,
                input_continuity, cast_target)
            code.extend(golden_output_code)

        return code, golden_var_name

    def generate_test_case(self, datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank,
                           input_continuity):
        test_name = self.generate_test_name(datatype, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank,
                                            input_continuity)
        ...
        ntt_output_code, output_shape_expr, output_element_type = self.generate_ntt_output_to_test(
            datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec, vector_rank, vec_param, input_continuity)
        code.extend(ntt_output_code)

        golden_output_code, golden_var_name = self.generate_ntt_golden_output(datatype, op_str, input_is_dynamic,
                                                                              output_is_dynamic, dims_spec, vector_rank,
                                                                              vec_param, input_continuity)
    ...
    def generate_all_tests_for_type(self, datatype, op_str):
        ...
        for input_is_dynamic, output_is_dynamic, dims_spec, input_vector_rank, input_continuity in param_combinations:
            ...
            test_case = self.generate_test_case(datatype, op_str, input_is_dynamic, output_is_dynamic, dims_spec,
                                                input_vector_rank, input_continuity)
...
if __name__ == "__main__":
    ...
    generate_cmake_list(generated_directory, generated_filenames, "generated_unary_tests.cmake",
                        "GENERATED_UNARY_TEST_SOURCES")

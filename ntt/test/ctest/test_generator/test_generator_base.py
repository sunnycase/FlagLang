...
class BaseTestGenerator:
    ...
    def __init__(self):
        ...
        self.ulp_tolerances = {"default": {"default": 1}}
    ...
    def _need_cast_in_ort(self, datatype, op_str):
        ...
        if not hasattr(self, 'types_need_cast_in_ort'):
            raise NotImplementedError(f"{self.__class__.__name__} must implement 'types_need_cast_in_ort' attribute")
...
#dim_spec: dim_names(list[str]) or dim_spec(list[int])
    ...
    def generate_tensor_init(self, datatype, shape_type, dim_spec, continuity, vector_rank, var_name, name_suffix,
                             P=None, integer_only=False):
        code = []
        ...
        if continuity.is_contiguous:
            ...
            code.append(
                f"NttTest::init_tensor({var_name}, {datatype.min_val}, {datatype.max_val}, {allow_zr}, {integer_only_str});"
            )
        else:  # non-contiguous
            ...
            integer_only_str = "true" if integer_only else "false"
            code.append(
                f"NttTest::init_tensor(big_tensor{name_suffix}, {datatype.min_val}, {datatype.max_val}, {allow_zr}, {integer_only_str});"
            )
            code.append(f"")
    ...
    def generate_copy_to_contiguous_code(self, input_element_type, shape_type, dim_names, input_var_name="ntt_input",
                                         output_var_name="continuous_input"):
        code = []
        input_dims_expr = [f"{name}" for name in dim_names]
        code.append("    // Copy to contiguous tensor for ORT reference")
        code.append(
            f"    auto {output_var_name} = ntt::make_unique_tensor<{input_element_type}>({self.generate_shape_init(shape_type, input_dims_expr)});"
        )
        code.append("    ")
        ...
        for i, name in enumerate(dim_names):
            code.append(
                f"    {'    ' * i}for (size_t {iter_var_names[i]} = 0; {iter_var_names[i]} < {name}; {iter_var_names[i]}++) {{"
            )
        indices = [f"{iter_var_names[i]}" for i in range(len(dim_names))]
        code.append(
            f"    {'    ' * len(dim_names)}({output_var_name})({', '.join(indices)}) = {input_var_name}({', '.join(indices)});"
        )
    ...
    def generate_reference_and_comparison_code(self, datatype, continuity, dim_names, shape_type, is_fp8,
                                               input_element_type, output_element_type, output_shape_expr, ort_ref_code,
                                               ntt_output_var_name="ntt_output1", ntt_output_var_is_vector=False):
        code = []
        ...
        if not continuity.is_contiguous:
            if is_fp8:
                ...
                copy_code, ort_input_tensor = self.generate_copy_to_contiguous_code(input_element_type, shape_type,
                                                                                    dim_names)
        ...
        if is_fp8:
            ...
            code.append(
                f"    auto ntt_output2_uint8 = ntt::make_tensor<{output_element_type_uint8}>({output_shape_expr});")
...
'''
    ...
    # -------------------------------------------------------------------------
    def generate_ntt_input_section(self, datatype: DataType, shape_type: str, dims_spec, continuity: Continuity,
                                   vector_rank: int = 0, P: Optional[str] = None, var_name: str = "ntt_input",
        ...
        body = self.generate_tensor_init(datatype, shape_type, dims_spec, continuity, vector_rank, var_name,
                                         name_suffix, P)
        return comment_lines + body + [""]

    def generate_ntt_operation_section(self, operation_lines: list[str]) -> list[str]:
    ...
    def generate_ntt_output_and_op_section(self, datatype: DataType, output_shape_expr: str, cast_mode: int,
                                           ntt_op_call_lines: List[str], output_var_name: str = "ntt_output1",
        ...
        if cast_mode == 1:
            uint8_type = "uint8_t" if "vector" not in output_element_type else output_element_type.replace(
                datatype.cpp_type, "uint8_t")
            op_section.extend([
    ...
    def _cast_ort_golden_double_into_ntt_shape(self, code, datatype, ntt_op_str, output_is_dynamic, output_dims_spec,
                                               output_vector_rank, output_vec_param, ort_golden_double_var,
                                               cast_target="double"):
        ...
        golden_scalar_dims = self.generate_ntt_golden_double_scalar_dims_spec(ntt_op_str, output_dims_spec,
                                                                              output_vector_rank)

        golden_scalar_shape_expr = self.generate_shape_init(output_is_dynamic, golden_scalar_dims)

        code.append(
            f"auto ntt_golden_{cast_target}_scalar = ntt::make_unique_tensor<{cast_target}>({golden_scalar_shape_expr});"
        )
        code.append(f"NttTest::ort2ntt({ort_golden_double_var}, *ntt_golden_{cast_target}_scalar);")
        code.append("")
        code.append(
            f"auto ntt_golden_{datatype.cpp_type}_scalar = ntt::make_unique_tensor<{datatype.cpp_type}>({golden_scalar_shape_expr});"
        )
        ...
        if output_vector_rank > 0:
            ...
            code.append(
                f"ntt::pack(*ntt_golden_{datatype.cpp_type}_scalar, ntt_golden_unsqueeze, fixed_shape_v<{pack_dims}>);")
    ...
    def generate_ort_output(self, datatype, ntt_op_str):
        ...
        if ntt_op_str in self.op_str_map_exhaustive:
            ...
            raise KeyError(
                f"Operation '{ntt_op_str}' not found in either op_str_map_exhaustive or op_str_map_simplified")

        if callable(op_str):
            op_str = op_str(datatype)
        return [f"// Execute Ort operation", f"{op_str}", ""]

    def _prepare_contiguous_input(self, input_name, datatype, vector_rank, vec_param, is_dynamic_shape, dims_spec,
                                  continuity):
        ...
        if not continuity.is_contiguous:
            ...
            copy_code, _ = self.generate_copy_to_contiguous_code(element_type, is_dynamic_shape, dims_spec, input_name,
                                                                 continuity_var_name)
    ...
    def generate_ort_input_section(self, datatype: DataType, shape_type, dims_spec, continuity, cast_mode: int,
                                   P: Optional[str] = None, vector_rank: int = 0, ort_input_var_name: str = "ort_input",
                                   ntt_input_var_name: str = "ntt_input", name_suffix: str = "") -> list[str]:
        ...
        if cast_mode == 1:
            ...
            lines.append(
                f"    auto {ntt_input_var_name}_uint8 = ntt::make_tensor<{uint8_cpp_type}>({input_shape_expr});")
            lines.append(
                f"    NttTest::reinterpret_cast_fp8_to_uint8({ntt_input_var_name}, {ntt_input_var_name}_uint8);")
            lines.append(f"")
            ...
            for i, iter_end in enumerate(dims_spec):
                indent = "    " * i
                lines.append(
                    f"    {indent}for (size_t {iter_var_names[i]} = 0; {iter_var_names[i]} < {iter_end}; {iter_var_names[i]}++) {{"
                )
            indices = ", ".join([iter_var_names[i] for i in range(len(dims_spec))])
            lines.append(
                f"    {'    ' * len(dims_spec)}continuous_input{name_suffix}({indices}) = {ntt_input_var_name}({indices});"
            )
    ...
    def generate_ort_back2ntt_and_compare_section(self, datatype: DataType, output_element_cpp_type: str,
                                                  output_shape_expr: str, cast_mode: int,
        ...
        if cast_mode == 0:  #  no cast
            ...
            golden_cpp_type = "uint8_t" if "vector" not in output_element_cpp_type else output_element_cpp_type.replace(
                datatype.cpp_type, "uint8_t")
            ...
            lines.append(
                f"auto {golden_ntt_in_ort_type_var} = ntt::make_unique_tensor<{golden_cpp_type}>({output_shape_expr});")
            ...
            if datatype.cpp_type in ["uint8_t", "uint16_t", "uint32_t", "uint64_t"]:
                ...
                lines.append(
                    f"auto {golden_signed_int_var} = ntt::make_unique_tensor<{int_tensor_cpp_type}>({output_shape_expr});"
                )
            ...
            lines.append(
                f"auto {golden_origin_var} = ntt::make_unique_tensor<{output_element_cpp_type}>({output_shape_expr});")
    ...
    def generate_ort_back2ntt(self, datatype: DataType, output_element_cpp_type: str, output_shape_expr: str,
                              cast_mode: int, ntt_output_var_name: str = "ntt_output1",
                              ort_output_var_name: str = "ort_output", ort_type: str = "double"):
        ...
        if cast_mode == 0:  #  no cast
            ...
            golden_var_name = "*ntt_golden"
            lines.append(
                f"auto {generate_var_name} = ntt::make_unique_tensor<{output_element_cpp_type}>({output_shape_expr});")
            lines.append(f"ntt::cast(*{golden_cast_source_var}, {golden_var_name});")

        return lines, golden_var_name

    def generate_compare(self, ntt_output_var_name: str = "ntt_output", golden_var_name: str = "ntt_golden",
                         ulp_tolerances=1) -> list[str]:
        """Generate comparison code between NTT output and golden output."""
        lines = []
        lines.append(
            f"EXPECT_TRUE(NttTest::compare_tensor({ntt_output_var_name}, {golden_var_name}, {ulp_tolerances}));")
        lines.append("}")
        lines.append("")
        return lines

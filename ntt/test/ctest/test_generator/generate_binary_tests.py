...
class BinaryTestGenerator(BaseTestGenerator):
    ...
    def __init__(self):
        ...
        self.types_need_cast_in_ort = {
            "swishb": [
                'bool', 'uint8_t', 'uint16_t', 'uint32_t', 'uint64_t', 'int8_t', 'int16_t', 'bfloat16', 'half',
                "int32_t", "int64_t", 'float_e4m3_t', 'float_e5m2_t'
            ], "default":
            ['bool', 'int8_t', 'int16_t', 'bfloat16', 'half', 'uint8_t', 'uint16_t', 'uint32_t', 'uint64_t']
        }
        self.types_need_cast_in_ntt = {'float_e4m3_t', 'float_e5m2_t'}
        ...
        self.ALL_POW_OPRANDS = {
            ...
            "float_e4m3_t": {
                "lhs_min": "float_e4m3_t(-3.0)", "lhs_max": "float_e4m3_t(2.0)", "rhs_min": "float_e4m3_t(-2.0f)",
                "rhs_max": "float_e4m3_t(3.0f)"
            },
            "float_e5m2_t": {
                "lhs_min": "float_e5m2_t(-3.0)", "lhs_max": "float_e5m2_t(3.0)", "rhs_min": "float_e5m2_t(-3.0f)",
                "rhs_max": "float_e5m2_t(3.0f)"
            },
            "bfloat16": {
                "lhs_min": "bfloat16(-64.0)", "lhs_max": "bfloat16(64.0)", "rhs_min": "-10.0_bf16", "rhs_max":
                "10.0_bf16"
            },
            "half":
            {"lhs_min": "half(-32.0)", "lhs_max": "half(32.0)", "rhs_min": "half(-3.0)", "rhs_max": "half(3.0)"},
    ...
    def _generate_aligned_ntt_scalar_input(self, ntt_op_str, datatype, input_var_name, var_name, is_dynamic_shape,
                                           dims_spec, vector_rank, other_vector_rank):
        ...
        if vector_rank >= other_vector_rank:
            ...
            if vector_rank == 0:
                ...
                if vector_rank == 1:
                    ...
                    unpack_dims = unsqueeze_dims
                code.append(
                    f"auto {input_var_name}_unsqueezed = ({var_name}).unsqueeze(fixed_shape_v<{unsqueeze_dims}>);")
        else:
        ...
        if (vector_rank == 0):
            ...
            code.append(
                f"auto {input_var_name}_aligned = ntt::make_tensor<{datatype.cpp_type}>(fixed_shape_v<{','.join(map(str, aligned_dims))}>);"
            )
            code.append(
                f"ntt::unpack({input_var_name}_unsqueezed, {input_var_name}_aligned, fixed_shape_v<{unpack_dims}>);")
        return code, aligned_dims

    def _prepare_double_input_alignment(self, code, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec,
                                        rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity, rhs_continuity,
                                        lhs_vec_param, rhs_vec_param, ntt_op_str):
        ...
        if need_alignment:
            ...
            lhs_code, lhs_aligned_dims = self._generate_aligned_ntt_scalar_input(ntt_op_str, datatype, "ntt_input_lhs",
                                                                                 lhs_var_name, lhs_is_dynamic_shape,
                                                                                 lhs_dims_spec, lhs_vector_rank,
                                                                                 rhs_vector_rank)
            code.extend(lhs_code)

            # Generate aligned rhs input
            rhs_code, rhs_aligned_dims = self._generate_aligned_ntt_scalar_input(ntt_op_str, datatype, "ntt_input_rhs",
                                                                                 rhs_var_name, rhs_is_dynamic_shape,
                                                                                 rhs_dims_spec, rhs_vector_rank,
                                                                                 lhs_vector_rank)
    ...
    def _execute_ort_operation(self, code, datatype, ntt_op_str):
        ...
        code.append(
            f"auto [ort_input_lhs, ort_input_rhs] = NttTest::convert_and_align_to_ort(ntt_lhs_double, ntt_rhs_double, false, false);"
        )
    ...
    def _generate_ntt_cast_golden_output(self, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec,
                                         rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity,
                                         rhs_continuity, lhs_vec_param, rhs_vec_param, ntt_op_str):
        ...
        lhs_aligned_dims, rhs_aligned_dims = self._prepare_double_input_alignment(
            code, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec, rhs_dims_spec, lhs_vector_rank,
            rhs_vector_rank, lhs_continuity, rhs_continuity, lhs_vec_param, rhs_vec_param, ntt_op_str)

        # 2. Execute ORT operation
        ort_golden_double_var = self._execute_ort_operation(code, datatype, ntt_op_str)

        # Calculate output shape for scalar tensor
        output_is_dynamic_shape, output_dims_spec = self.get_binary_output_shape(lhs_is_dynamic_shape,
                                                                                 rhs_is_dynamic_shape, lhs_dims_spec,
                                                                                 rhs_dims_spec)
        ...
        self._cast_ort_golden_double_into_ntt_shape(code, datatype, ntt_op_str, output_is_dynamic_shape,
                                                    output_dims_spec, output_vector_rank, output_vec_param,
                                                    ort_golden_double_var)

        return code

    def _generate_ort_cast_golden_output(self, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec,
                                         rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity,
                                         rhs_continuity, lhs_vec_param, rhs_vec_param, output_element_type,
                                         output_shape_expr, ntt_op_str):
        """
        ...
        lhs_var_name, lhs_copy_code = self._prepare_contiguous_input("ntt_input_lhs", datatype, lhs_vector_rank,
                                                                     lhs_vec_param, lhs_is_dynamic_shape, lhs_dims_spec,
                                                                     lhs_continuity)
        code.extend(lhs_copy_code)
        ntt2ort_lhs = lhs_var_name

        rhs_var_name, rhs_copy_code = self._prepare_contiguous_input("ntt_input_rhs", datatype, rhs_vector_rank,
                                                                     rhs_vec_param, rhs_is_dynamic_shape, rhs_dims_spec,
                                                                     rhs_continuity)
        code.extend(rhs_copy_code)
        ...
        code.extend([
            f"auto [ort_input_lhs, ort_input_rhs] = NttTest::convert_and_align_to_ort({ntt2ort_lhs},{ntt2ort_rhs}, {need_cast_str}, {is_outer_product});"
        ])
        ...
        cast_code, golden_var_name = self.generate_ort_back2ntt(datatype, output_element_type, output_shape_expr,
    ...
    def generate_test_name(self, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec, rhs_dims_spec,
                           lhs_vector_rank, rhs_vector_rank, lhs_continuity, rhs_continuity, test_name_suffix):
    ...
    def get_binary_output_shape(self, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_shape, rhs_shape):
    ...
    def generate_ntt_golden_output(self, datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec,
                                   rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity, rhs_continuity,
                                   lhs_vec_param, rhs_vec_param, output_element_type, output_shape_expr, ntt_op_str):
        code = []
        ...
        if need_cast_in_ntt:
            ...
            code.extend(
                self._generate_ntt_cast_golden_output(datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
            ...
            golden_output_code, golden_var_name = self._generate_ort_cast_golden_output(
                datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec, rhs_dims_spec, lhs_vector_rank,
                rhs_vector_rank, lhs_continuity, rhs_continuity, lhs_vec_param, rhs_vec_param, output_element_type,
                output_shape_expr, ntt_op_str)
            code.extend(golden_output_code)

        return code, golden_var_name

    def generate_ntt_output_to_test(self, lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
                                    lhs_dims_spec, rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity,
                                    rhs_continuity, lhs_vec_param, rhs_vec_param, ntt_op_str):
        indent = "    "
        code = []
        ...
        tensor_init_lhs_code = self.generate_tensor_init(datatype=lhs_datatype, shape_type=lhs_is_dynamic_shape,
                                                         dim_spec=lhs_dims_spec, continuity=lhs_continuity,
                                                         var_name="ntt_input_lhs", name_suffix="_lhs",
                                                         vector_rank=lhs_vector_rank, P=lhs_vec_param,
                                                         integer_only=lhs_datatype.integer_only)
        code.extend([f"{indent}{line}" for line in tensor_init_lhs_code])

        code.append(f"{indent}//---init ntt_input_rhs---")
        tensor_init_rhs_code = self.generate_tensor_init(datatype=rhs_datatype, shape_type=rhs_is_dynamic_shape,
                                                         dim_spec=rhs_dims_spec, continuity=rhs_continuity,
                                                         var_name="ntt_input_rhs", name_suffix="_rhs",
                                                         vector_rank=rhs_vector_rank, P=rhs_vec_param,
                                                         integer_only=rhs_datatype.integer_only)
        ...
        ntt_output_and_op_code = self.generate_ntt_output_and_op_section(datatype=datatype,
    ...
    def generate_test_case(self, lhs_datatype, rhs_datatype, lhs_is_dynamic_shape: bool, rhs_is_dynamic_shape: bool,
                           lhs_dims_spec: List[int], rhs_dims_spec: List[int], lhs_vector_rank: int,
                           rhs_vector_rank: int, lhs_continuity: Continuity, rhs_continuity: Continuity, ntt_op_str,
        ...
        test_name = self.generate_test_name(datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec,
                                            rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity,
                                            rhs_continuity, test_name_suffix)
        ...
        ntt_output_code, output_shape_expr, output_element_type = self.generate_ntt_output_to_test(
            lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape, lhs_dims_spec, rhs_dims_spec,
            lhs_vector_rank, rhs_vector_rank, lhs_continuity, rhs_continuity, lhs_vec_param, rhs_vec_param, ntt_op_str)
        ...
        compare_code = self.generate_compare(ntt_output_var_name="ntt_output", golden_var_name=golden_var_name,
    ...
    def _generate_pow_test_case_pair(self, lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
                                     lhs_dims_spec, rhs_dims_spec, lhs_vector_rank, rhs_vector_rank, lhs_continuity,
                                     rhs_continuity, ntt_op_str):
        ...
        if lhs_datatype.cpp_type in self.integer_types:
            ...
            lhs_datatype = lhs_datatype._replace(min_val=pow_ranges["lhs_min"], max_val=pow_ranges["lhs_max"])
            rhs_datatype = rhs_datatype._replace(integer_only=True, min_val=pow_ranges["rhs_min"],
                                                 max_val=pow_ranges["rhs_max"])
            test_code = self.generate_test_case(lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
                                                lhs_dims_spec, rhs_dims_spec, lhs_vector_rank, rhs_vector_rank,
                                                lhs_continuity, rhs_continuity, ntt_op_str)
            test_cases.append(test_code)
        else:
            # Case 2.1: floating point types - rhs as integer
            pow_ranges = self.ALL_POW_OPRANDS.get(lhs_datatype.cpp_type)
            lhs_datatype = lhs_datatype._replace(integer_only=False, min_val=pow_ranges["lhs_min"],
                                                 max_val=pow_ranges["lhs_max"])
            rhs_datatype = rhs_datatype._replace(integer_only=True, min_val=pow_ranges["rhs_min"],
                                                 max_val=pow_ranges["rhs_max"])
            test_code1 = self.generate_test_case(lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
                                                 lhs_dims_spec, rhs_dims_spec, lhs_vector_rank, rhs_vector_rank,
                                                 lhs_continuity, rhs_continuity, ntt_op_str, "rhs_int")
            ...
            lhs_datatype = lhs_datatype._replace(min_val=zero_val_map.get(lhs_datatype.cpp_type, "0.0"))
            rhs_datatype = rhs_datatype._replace(integer_only=False)

            test_code2 = self.generate_test_case(lhs_datatype, rhs_datatype, lhs_is_dynamic_shape, rhs_is_dynamic_shape,
                                                 lhs_dims_spec, rhs_dims_spec, lhs_vector_rank, rhs_vector_rank,
                                                 lhs_continuity, rhs_continuity, ntt_op_str, "rhs_float")
    ...
    def _get_param_combinations(self, op_str):
        ...
        param_combinations_exhaustive = itertools.product(is_dynamic_options,  # lhs_is_dynamic_shape 2
    ...
    def generate_all_tests_for_type(self, datatype, op_str):
        ...
        for lhs_is_dynamic, rhs_is_dynamic, (
                lhs_shape, rhs_shape), lhs_vec_rank, rhs_vec_rank, lhs_continuity, rhs_continuity in param_combinations:
            ...
            if (op_str == "pow"):
                ...
                test_code = self._generate_pow_test_case_pair(
                    datatype, datatype, lhs_is_dynamic_shape=lhs_is_dynamic, rhs_is_dynamic_shape=rhs_is_dynamic,
                    lhs_dims_spec=lhs_shape, rhs_dims_spec=rhs_shape, lhs_vector_rank=lhs_vec_rank,
                    rhs_vector_rank=rhs_vec_rank, lhs_continuity=lhs_continuity, rhs_continuity=rhs_continuity,
                    ntt_op_str=op_str)
                code.append(test_code)
            else:
                test_code = self.generate_test_case(datatype, datatype, lhs_is_dynamic_shape=lhs_is_dynamic,
...
if __name__ == "__main__":
    ...
    generate_cmake_list(generated_directory, generated_filenames, "generated_binary_tests.cmake",
                        "GENERATED_BINARY_TEST_SOURCES")

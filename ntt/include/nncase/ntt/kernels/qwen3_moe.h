...
//  gate/up proj scale:    [num_expert, moe_intermediate_size, 1] or
//  [num_expert, 1] gate/up input scale:   [num_expert, hidden_size, 1] or
//  [num_expert, 1] (optional, can be empty) down proj weight:      [num_expert,
//  hidden_size, moe_intermediate_size] down proj scale:       [num_expert,
//  hidden_size, 1] or [num_expert, 1] down input scale:      [num_expert,
//  moe_intermediate_size, 1] or [num_expert, 1] (optional, can be empty) gate
//  (router) weight:  [num_expert, hidden_size] input q:               [seq_len,
//  hidden_size] output:                [seq_len, hidden_size]
//
...
template <class T> inline T sigmoid(T x) noexcept {
    return (T)1 / ((T)1 + (T)std::exp((double)-x));
}

template <Tensor TQ, Tensor TGateW, Tensor TGateInputScale, Tensor TGateProjW,
          Tensor TGateProjScale, Tensor TDownInputScale, Tensor TDownProjW,
          Tensor TDownProjScale, Tensor TUpInputScale, Tensor TUpProjW,
...
void qwen3_moe_impl(const TQ &q, const TGateW &moeGateW,
    ...
    // 1. router logits : [seq_len, num_expert] = q [seq_len, hidden_size] @
    // gateW [num_expert, hidden_size]
    ...
    for (size_t i = 0; i < seq_len; i++) {
        for (size_t e = 0; e < num_expert; e++) {
            ...
            router_logits[i * num_expert + e] =
                acc; // cast to float already in evaluator, kept same type here.
        }
    }
    ...
    for (size_t i = 0; i < seq_len; i++) {
        ...
        // max
        auto m = row_in[0];
        for (size_t e = 1; e < num_expert; e++)
            m = ntt::max(m, row_in[e]);
        // exp and sum
        float sum = 0.f;
        for (size_t e = 0; e < num_expert; e++) {
            float v = row_in[e] - m;
    ...
    for (size_t i = 0; i < seq_len; i++) {
        ...
        for (size_t e = 0; e < num_expert; e++) {
            ...
            for (size_t k = 0; k < num_top_k; k++) {
                if (pos == num_top_k &&
                    (topk_indices[i * num_top_k + k] == -1 ||
                     p > topk_probs[i * num_top_k + k])) {
                    pos = k;
                    break;
                }
            }
            if (pos < num_top_k) {
                ...
                for (size_t k = num_top_k - 1; k > pos; k--) {
                    topk_probs[i * num_top_k + k] =
                        topk_probs[i * num_top_k + k - 1];
                    topk_indices[i * num_top_k + k] =
                        topk_indices[i * num_top_k + k - 1];
                }
        ...
        if (is_norm_topk_prob) {
            float sum = 0.f;
            for (size_t k = 0; k < num_top_k; k++)
                sum += topk_probs[i * num_top_k + k];
            float inv = sum != 0.f ? 1.f / sum : 0.f;
            for (size_t k = 0; k < num_top_k; k++)
                topk_probs[i * num_top_k + k] *= inv;
        }
    }
    ...
    for (size_t i = 0; i < seq_len; i++) {
        ...
        for (size_t k = 0; k < num_top_k; k++) {
            ...
            constexpr bool gate_scale_is_2d =
                moeExpertGateProjScale.shape().rank() == 2;
            ...
            constexpr bool gate_input_scale_is_empty =
                moeExpertGateInputScale.rank() == 0;
            constexpr bool gate_input_scale_is_2d =
                moeExpertGateInputScale.rank() == 2;
            float gate_input_scale_val = 1.f;
            if constexpr (!gate_input_scale_is_empty &&
                          gate_input_scale_is_2d) {
                gate_input_scale_val =
                    (float)moeExpertGateInputScale(expert, 0);
            }
            ...
            for (size_t d = 0; d < moe_intermediate_size; d++) {
                ...
                for (size_t h = 0; h < hidden_size; h++) {
                    ...
                    if constexpr (!gate_input_scale_is_empty) {
                        if constexpr (gate_input_scale_is_2d) {
                            ...
                            input_val /=
                                (float)moeExpertGateInputScale(expert, h, 0);
                        }
                    }
                ...
                if constexpr (gate_scale_is_2d) {
                    ...
                    acc *= ((float)moeExpertGateProjScale(expert, d, 0) *
                            gate_input_scale_val);
                }
            ...
            // Check if up proj scale is 2D [num_expert, 1] or 3D [num_expert,
            ...
            constexpr bool up_input_scale_is_empty =
                moeExpertUpProjInputScale.rank() == 0;
            constexpr bool up_input_scale_is_2d =
                moeExpertUpProjInputScale.rank() == 2;
            float up_input_scale_val = 1.f;
            if constexpr (!up_input_scale_is_empty && up_input_scale_is_2d) {
                up_input_scale_val =
                    (float)moeExpertUpProjInputScale(expert, 0);
            }
            ...
            for (size_t d = 0; d < moe_intermediate_size; d++) {
                ...
                for (size_t h = 0; h < hidden_size; h++) {
                    ...
                    if constexpr (!up_input_scale_is_empty) {
                        if constexpr (up_input_scale_is_2d) {
                            ...
                            input_val /=
                                (float)moeExpertUpProjInputScale(expert, h, 0);
                        }
                    }
                ...
                if constexpr (up_scale_is_2d) {
                    ...
                    acc *= ((float)moeExpertUpProjScale(expert, d, 0) *
                            up_input_scale_val);
                }
            ...
            // down: (gate*up)[moe_intermediate_size] @ downW[hidden_size,
            // moe_intermediate_size] Check if down proj scale is 2D
            // [num_expert, 1] or 3D [num_expert, hidden_size, 1]
            constexpr bool down_scale_is_2d =
                (moeExpertDownProjScale.rank() == 2);
            ...
            constexpr bool down_input_scale_is_empty =
                moeExpertDownProjInputScale.rank() == 0;
            constexpr bool down_input_scale_is_2d =
                moeExpertDownProjInputScale.rank() == 2;
            float down_input_scale_val = 1.f;
            if constexpr (!down_input_scale_is_empty &&
                          down_input_scale_is_2d) {
                down_input_scale_val =
                    (float)moeExpertDownProjInputScale(expert, 0);
            }
            ...
            for (size_t h = 0; h < hidden_size; h++) {
                ...
                for (size_t d = 0; d < moe_intermediate_size; d++) {
                    ...
                    if constexpr (!down_input_scale_is_empty) {
                        if constexpr (down_input_scale_is_2d) {
                            ...
                            down_in /= (float)moeExpertDownProjInputScale(
                                expert, d, 0);
                        }
                    }
                ...
                if constexpr (down_scale_is_2d) {
                    ...
                    acc *= ((float)moeExpertDownProjScale(expert, h, 0) *
                            down_input_scale_val);
                }
...
void qwen3_moe(const TQ &q, const TGateW &moeGateW,
               ...
               const TUpProjW &moeExpertUpProjW,
               const TUpProjScale &moeExpertUpProjScale, TOut &&output,
               size_t /* layer_id */, size_t hidden_size,
               size_t intermediate_size, size_t moe_intermediate_size,
               size_t num_expert, size_t num_top_k,
               size_t is_norm_topk_prob) noexcept {
    detail::qwen3_moe_impl(q, moeGateW, moeExpertGateInputScale,
                           moeExpertGateProjW, moeExpertGateProjScale,
                           moeExpertDownInputScale, moeExpertDownProjW,
                           moeExpertDownProjScale, moeExpertUpInputScale,
                           moeExpertUpProjW, moeExpertUpProjScale, hidden_size,
                           intermediate_size, moe_intermediate_size, num_expert,
                           num_top_k, is_norm_topk_prob, output);
}

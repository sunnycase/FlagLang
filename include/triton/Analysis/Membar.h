...
struct BlockInfo {
    ...
    bool isIntersected(const BlockInfo &other, MembarFilterFn filter) const {
        return /*RAW*/ isIntersected(syncWriteIntervals,
                                     other.syncReadIntervals, filter) ||
               /*WAR*/
...
class MembarOrFenceAnalysis {
  ...
  public:
    ...
    /// it is considered as the problem of the operation itself but not the
    /// membar analysis.
    MembarOrFenceAnalysis() = default;
    explicit MembarOrFenceAnalysis(Allocation *allocation,

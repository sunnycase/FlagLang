...
class AxisInfo {
  ...
  public:
    ...
    // contiguity[d] is the length of the shortest sequence of contiguous
    // integers along dimension d.
    //
    // If we have an array of N elements with a contiguity value C, then the
    // array can be divided into a list of N/C sequences of C contiguous
    // elements. Since we have N = 2^k, C must be a power of two.
    //
    ...
    // divisibility[d] is the largest power of two that divides the first
    // element of all groups of length contiguity[d] along dimension d.
    //
    // For example,
    //
    //   [[10, 11, 12, 13, 18, 19, 20, 21],
    ...
    //
    // If we have an array of N elements, with a constancy value C, then the
    // array can be divided into a list of N/C sequences of C elements with the
    // same value.  Since we have N = 2^k, C must be a power of two.
    //
    // For example
    //
    //   [[8, 8, 8, 8, 12, 12, 12, 12],
...
class AxisInfoVisitor {
  public:
    ...
    bool isContiguousDim(const AxisInfo &info, ArrayRef<int64_t> shape,
                         int dim) {
...
class ModuleAxisInfoAnalysis : public CallGraph<AxisInfoMapT> {
  public:
    ...
    // lowering to hardware intrinsics that require a scalar/warp-uniform base
    // ptr with separate per lane offsets like AMD buffer operations.
    //

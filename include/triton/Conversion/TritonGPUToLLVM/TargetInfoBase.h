...
class TargetInfoBase {
  public:
    ...
    virtual Value getClusterCTAId(RewriterBase &rewriter,
                                  Location loc) const = 0;

    virtual Value ballot(RewriterBase &rewriter, Location loc, Type type,
                         Value cmp) const = 0;

    // Insert a synchronization barrier. If isWarpSync is true, emit a
    // warp-level synchronization when supported by the backend; otherwise emit
    // a block/CTA level barrier. Backends that do not support warp-level
    // barriers should conservatively emit a block-level barrier.
    ...
    //
    // A target that does not support cross-CTA transfers will assert if ctaId
    // is non-nullopt.
    //
    ...
    virtual Value permute(RewriterBase &rewriter, Location loc, Value a,
                          Value b, Value selector) const = 0;
    ...
    // Emits LLVM code with |rewriter| to print a message, particularly useful
    // for backend debug. |msg| is the message to print, |args| are the
    // arguments to fill placeholders in the |msg|. NOTE: This function is used
    // for backend debug. DO NOT DELETE. Example use:
    // targetInfo.printf(rewriter,"index: %d, value: %f", {index, value});
    virtual void printf(RewriterBase &rewriter, StringRef msg, ValueRange args,
                        ArrayRef<bool> isSigned = {}) const = 0;

    // Emits LLVM code with |rewriter| to perform assertion failure with the
    // given |message| from the given |func| in |file|.

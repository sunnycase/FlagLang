  ...
  public:
    ...
    SmallVector<Value> maybeDeduplicate(SourceOp op,
        ...
        if (isa<AMDMfmaEncodingAttr>(baseEncoding) ||
            ...
            // TODO: this logic seems incorrect for mfma and wmma layout. Skip
            // for now. We saw mismatches for some flash-attention and dot tests
            // on AMD backend. Note that this logic works for sliced layout
            // whose parent is mfma layout. Therefore, this is not combined with
            // the following check.
            return resultVals;
        ...
        for (int i = 0; i < resultVals.size(); ++i) {
            ...
            for (int j = 0; j < rank; ++j) {
                ...
                dedup_idx +=
                    (coord_j / constancy[j] * constancy[j]) * strides[j];
    ...
    matchAndRewrite(SourceOp op, OpAdaptor adaptor,
        ...
        for (auto it = allOperands.begin(), end = allOperands.end();
             it != end;) {
            auto curr = static_cast<const ConcreteT *>(this)->createDestOps(
                op, adaptor, rewriter, elemTy, MultipleOperandsRange(it, end),
                loc);
            if (curr.size() == 0)
                return failure();
            for (auto v : curr) {

...
template <typename T> constexpr T highestPowOf2Divisor(T n) {
    // When n is 0 or min, return the highest power of 2. The min case is
    // handled separately to avoid underflow when T is a signed integer.
    // Technically in that case the correct divisor is -n, but this value is
    // outside the range of possible values, so we take the next best
    // alternative.

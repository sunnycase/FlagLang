...
class WarpSchedule {
  ...
  public:
    ...
    // Schedule an operation to a partition if it is not already scheduled.
    // Return true if the operation was scheduled.
    ...
    // Iterate the inputs of the partition. Input values are those that
    // originate from a different partition or a previous iteration of the
    // current partition. E.g. partition B(i) may have inputs from A(i) or
    // B(i-1). Note that the same value may be visited more than once.
    ...
    // Iterate the defining ops of the inputs to the partition in the current
    // and previous iterations, including the distance in the past.

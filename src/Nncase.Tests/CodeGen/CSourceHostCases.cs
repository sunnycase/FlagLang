...
public class SubCase : ICodeGenCase
{
    public override PrimFunction GetEntry()
    {
        var func = T.PrimFunc("sub",
                  T.Buffer(TensorType.Scalar(DataTypes.Float32), BufferStorage.GlobalInput(), out var x),
                  T.Buffer(TensorType.Scalar(DataTypes.Float32), BufferStorage.GlobalInput(), out var y)).Body(
          x - y
        );
        return func;
    }
...
public class ForCase : ICodeGenCase
{
    ...
    public override PrimFunction GetEntry()
    {
        return T.PrimFunc("for_loop",
               T.Buffer(new(DataTypes.Int32, new[] { 100 }), BufferStorage.GlobalInput(), out var A),
               T.Buffer(TensorType.Scalar(DataTypes.Int32), BufferStorage.GlobalInput(), out var n)
               ).Body(

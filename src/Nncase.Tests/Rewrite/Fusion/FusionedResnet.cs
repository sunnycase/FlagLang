...
internal sealed record ForwardFusion(Func<Expr[], Fusion> Creator) : IForwardable
{
    ...
    public static ForwardFusion Binary(BinaryOp binaryOp)
    {
        var creator = (Expr[] inputs) =>
        {
            ...
            return new Fusion("BinaryFusion", BaseFunction.CPUModuleKind, IR.F.Math.Binary(BinaryOp.Add, in_a, in_b), new[] { in_a, in_b });
        };
        return new(creator);
    }
    ...
    public static ForwardFusion Conv3x3(int in_planes, int out_planes, int stride = 1, int groups = 1, int dilation = 1)
    {
        var creator = (Expr[] inputs) =>
        {
            ...
            return new Fusion("Conv3x3Fusion", BaseFunction.CPUModuleKind, IR.F.NN.Conv2D(v_input, weights, bias, new[] { stride, stride }, new[,] { { dilation, dilation }, { dilation, dilation } }, new[] { dilation, dilation }, PadMode.Constant, groups), new[] { v_input });
        };
        return new(creator);
    }
        ...
        var creator = (Expr[] inputs) =>
          {
              ...
              var bias = IR.F.Random.Normal(DataTypes.Float32, 0, 1, 1, new[] { out_planes }).Evaluate().AsTensor();
              return new Fusion("Conv1x1Fusion", BaseFunction.CPUModuleKind, IR.F.NN.Conv2D(v_input, weights, bias, new[] { stride, stride }, new[,] { { 0, 0 }, { 0, 0 }, }, new[] { 1, 1 }, PadMode.Constant, 1), new[] { v_input });
          };
        return new(creator);
    }
}
...
internal sealed class ResNet
{
        ...
        bool[]? replace_stride_with_dilation = null)
    {
        ...
        var maxpool_creator = (Expr[] inputs) =>
        {
            ...
            return new Fusion("ReduceWindowFusion", BaseFunction.CPUModuleKind, IR.F.NN.ReduceWindow2D(ReduceOp.Max, v_input, 0.0f, new[] { 3, 3 }, new[] { 2, 2 }, new[,] { { 1, 1 }, { 1, 1 } }, new[] { 1, 1 }, false, false), new[] { v_input });
        };

...
{
    public static DataType GetDataType(IRType iRType) => iRType switch
    {
        DataType dt => dt,
        TensorType tt => tt.DType,
        DistributedType dt => dt.TensorType.DType,
        _ => throw new InvalidOperationException($"Cannot get DataType from {iRType}"),
    };
...
public sealed record ModuleType : IRType
{
    public static readonly ModuleType Default = new();
}

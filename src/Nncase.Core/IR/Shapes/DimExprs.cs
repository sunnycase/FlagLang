...
public sealed class DimSum : Dimension, IEquatable<DimSum?>
{
    ...
    private static Dimension CreateWithSimplify(Dimension[] operands, long bias = 0)
    {
        ...
        foreach (var operand in operands)
        {
            ...
            else if (operand is DimProduct dimProduct && dimProduct.Count == 1)
            {
                ref var value = ref CollectionsMarshal.GetValueRefOrAddDefault(scales, dimProduct.Operands[0], out _);
                value += dimProduct.Scale;
            }
            else
            {
        ...
        var newOperands = scales.Where(kvp => kvp.Value != 0).Select(kvp => kvp.Key * kvp.Value).ToArray();

...
{
    public If(Expr condition, IRBlock then, IRBlock @else, ReadOnlySpan<BaseExpr> arguments)
        : base(ArrayUtility.Concat(condition, then, @else, arguments))
    {
    }

    public If(Expr condition, IRBlock then, IRBlock @else, params BaseExpr[] arguments)
    ...
    public IRBlock Then => (IRBlock)Operands[1];

    public IRBlock Else => (IRBlock)Operands[2];
    ...
    public If With(Expr? condition = null, IRBlock? then = null, IRBlock? @else = null, BaseExpr[]? arguments = null)

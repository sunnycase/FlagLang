...
public static partial class TypePatternUtility
{
    ...
    /// <summary>
    /// check the tensor is pointer.
    /// </summary>
    public static TypePattern IsPointer() => new TypePattern(
      x => x switch
      {
          TensorType ttype => DataTypes.IsPointer(ttype.DType),
          DistributedType distributedType => DataTypes.IsPointer(distributedType.TensorType.DType),
          _ => false,
      },
      "IsPointer");
    ...
    public static TypePattern IsPointerScalar() => IsScalar() & IsPointer();

...
internal sealed partial class TypeInferenceVisitor : ExprVisitor<IRType, Unit>
{
    ...
    protected override IRType VisitLeafCall(Call expr)
    {
        ...
        var type = expr.Target switch
        {
            ...
            Callable callable => InferCallableType(expr, callable),
    ...
    {
        _context.CurrentCall = expr;
        return TypeInference.CommonType(InferCallableType(expr, expr.Then), InferCallableType(expr, expr.Else));
    }
    ...
    protected override IRType VisitLeafIRModule(IRModule expr)
    {
    ...
    protected override IRType VisitIRBlock(IRBlock expr)
    {
        foreach (var p in expr.Parameters)
        {
            VerifySubField(expr, (BaseExpr)p);
    ...
    private IRType InferCallableType(BaseCall call, Callable callable)
    {
        if (callable.CheckedType is InvalidType)
        {
            return callable.CheckedType;
        }

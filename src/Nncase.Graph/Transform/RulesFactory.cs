...
internal sealed class ExprGeneratorVisitor : PatternVisitor<BaseExpr, IRType>
{
    ...
    /// <inheritdoc/>
    public override BaseExpr VisitLeaf(IRBlockPattern pattern)
    {
        return new IRBlock((Expr)PatternMemo[pattern.Body], pattern.Parameters.Select(p => (Var)PatternMemo[p]).ToArray());
    }

    /// <inheritdoc/>
    public override BaseExpr VisitLeaf(FunctionPattern pattern)
    {
        return new Function((IRBlock)PatternMemo[pattern.Body]);
    }

...
clr::tensor_type::tensor_type(datatype element_type, clr::shape shape) {
    obj_ =
        nncase_clr_api()->tensor_type_create(element_type.get(), shape.get());
}

clr::shape clr::tensor_type::shape() {
    return {std::in_place, nncase_clr_api()->tensor_type_get_shape(obj_.get())};
}

clr::shape clr::expr::get_shape() {
    return {std::in_place, nncase_clr_api()->base_expr_get_shape(obj_.get())};
}

...
public sealed class AddPostProcess : ModulePass
{
    ...
    protected override Task<IRModule> RunCoreAsync(IRModule module, RunPassContext options)
    {
        ...
        if (preProcess && modelLayout != outputLayout && outputLayout != string.Empty)
        {
            ...
            var entryBlock = entry.Body;
            BaseExpr newOutput;
            if (entryBlock.Body is IR.Tuple tuple)
            {
                newOutput = new IR.Tuple(tuple.Fields.ToArray().Select((field, i) => AddTranspose(outputLayout, modelLayout, (Expr)field)).ToArray());
            }
            else
            {
                newOutput = AddTranspose(outputLayout, modelLayout, (Expr)entryBlock.Body);
            }

            var newEntry = entry.With(body: entryBlock.With(body: newOutput));
            module.Remove(entry);
            module.Add(newEntry);

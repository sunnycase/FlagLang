...
internal sealed class AutoDistributedRewriter : ExprVisitor<Unit, Unit>
{
    ...
    public Function Rewrite(Function function)
    {
        ...
        return function.With(body: function.Body.With(body: post));
    }
        ...
        if (!_memo.TryGetValue(root, out var expr))
        {
            ...
            switch (root.Expr)
            {
                case Var or TensorConst or TupleConst or None or Shape or Padding or Paddings or Dimension or Call:
                    ...
                    break;
                case IRBlock block:
                    expr = block.With(body: children[0]);
                    break;
                case Fusion fusion:
                    expr = fusion;
                    break;
                ...
                case IR.Tuple tp:
                    ...
                    break;
                case IR.If @if:
                    expr = @if.With(condition: (Expr)children[^3], then: (IRBlock)children[^2], @else: (IRBlock)children[^1], arguments: children[..^3].ToArray());
                    break;
                default:

...
public sealed class ModulePartitionPass : ModulePass
{
    ...
    {
        var funcs = module.Functions.Length;
        for (int i = 0; i < funcs; i++)
        {
            ...
            var preBlock = function.Body;
            var postBody = PerformPartition(module, function.Name, preBlock.Body);
            ReplaceUtility.ReplaceAllUsesWith(preBlock.Body, postBody);
        }
...
internal sealed class DistributedReconstructor : ExprReconstructor<ExprVertex, ExprEdge>
{
    ...
    protected override Expr OnComplexCluster(ClusteredBidirectionalGraph<ExprVertex, ExprEdge> cluster, int sortIndex)
    {
        ...
        var func = new Function($"{FuncName}_{sortIndex}_kernel", ModuleCompiler.ModuleKind, new IRBlock(cloned, @params.ToArray()));
        Module.Add(func);

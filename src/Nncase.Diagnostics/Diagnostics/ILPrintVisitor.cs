...
internal sealed class ILPrintVisitor : ExprFunctor<string, string>
{
    ...
    public override string VisitType(TensorType type) => type.DType switch
    {
        ...
        PointerType { ElemType: PrimType etype } => $"*{etype.GetDisplayName()}" + (type.Shape.IsScalar ? string.Empty : VisitShape(type.Shape)),
        ReferenceType { ElemType: DataType rtype } => $"&{rtype.GetDisplayName()}" + (type.Shape.IsScalar ? string.Empty : VisitShape(type.Shape)),
        ValueType => $"{type.DType}" + (type.Shape.IsScalar ? string.Empty : VisitShape(type.Shape)),
    ...
    /// <inheritdoc/>
    protected override string VisitIRModule(IRModule expr)
    {
        var name = "%module";
    ...
    protected override string VisitIRBlock(IRBlock expr)
    {
        ...
        // 1. Function signature
        _writer.WInd().Write($"{name} = ^({StringUtility.Join(", ", expr.Parameters.AsValueEnumerable().Select(Visit))})");
        AppendCheckedType(expr.CheckedType, expr.Metadata.Range);
        _writer.WInd().WriteLine("{");
    ...
    /// <inheritdoc/>
    protected override string VisitReturn(TIR.Return expr)
    {
        var fields = expr.Values.AsValueEnumerable().Select(Visit).ToArray();
        if (Flags.HasFlag(PrinterFlags.Inline))
        {
            return $"return ({string.Join(", ", fields)})";
        }
        else
        {
            var name = GetNextSSANumber();
            _writer.WInd().Write($"return ({string.Join(", ", fields)})");
            AppendCheckedType(expr.CheckedType, expr.Metadata.Range);
            _writer.WInd().WriteLine();
            return name;
        }
    ...
    /// <inheritdoc/>
    protected override string VisitSequential(TIR.Sequential expr)
    {
        if (Flags.HasFlag(PrinterFlags.Inline))
        {
            throw new NotSupportedException($"Inline Mode with {typeof(TIR.Sequential)}");
        }

        var name = GetNextSSANumber();

        // 1. Sequential signature
        _writer.Write($"{name} = Sequential()");
        AppendCheckedType(expr.CheckedType, expr.Metadata.Range);
        _writer.WInd().WriteLine(" {");

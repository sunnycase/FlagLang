...
createDistributedEncodings(MLIRContext &ctx) {
        ...
        for (auto ctaLayout : ctaLayouts) {
            for (const auto &order : orders) {
                ...
                distributedEncodings.push_back(
                    triton::gpu::SliceEncodingAttr::get(&ctx, 0,
                                                        blockedEncoding));
            }
        }
    }
    ...
    // layouts yet)
    {
        for (auto versionMajor : {2, 3}) {
            ...
            auto instrShape = versionMajor == 2
            ...
            for (unsigned opIdx = 0; opIdx < 2; ++opIdx) {
                ...
                distributedEncodings.push_back(
                    triton::gpu::DotOperandEncodingAttr::get(&ctx, opIdx, mma,
                                                             kWidth));
            }
        }
    }
...
void testReshape(RankedTensorType srcTy, RankedTensorType dstTy,
    ...
    // Capture any errors from calling inferReshapeNoOpReorderEncoding, so we
    // can print them if we expected the reshape to succeed but it failed.
    std::vector<std::string> diags;
    Attribute inferredEnc;
    LogicalResult result = success();
    {
        ScopedDiagnosticHandler scopedHandler(ctx, [&](Diagnostic &diag) {
            diags.push_back("  - " + diag.str());
        });
        result = inferLayout->inferReshapeOpEncoding(
            srcTy.getShape(), srcTy.getEncoding(), dstTy.getShape(),
            inferredEnc, UnknownLoc::get(ctx));
    }
    ...
    // Even more, we check that the inferred encoding is structurally the same
    // as the src encoding, showing that the inference is consistent.
    {
        std::vector<std::string> diags;
        ScopedDiagnosticHandler scopedHandler(ctx, [&](Diagnostic &diag) {
            diags.push_back("  - " + diag.str());
...
TEST_F(Fp4ToFpOpTest, Fp4ToFpOpLayoutPropagation) {
    ...
    for (auto enc : distributedEncodings) {
        for (auto shape : shapes) {
            ...
            auto axis = rank - 1;
            // Test that we can do a round trip from src to dst encoding and
            // back.
            Attribute dstEnc;
...
TEST_F(JoinOpTest, JoinOpLayoutPropagation) {
    ...
    for (auto enc : distributedEncodings) {
        for (auto shape : shapes) {
            ...
            result = inferLayout->inferSplitOpEncoding(dstEnc, newSrcEnc,
                                                       newShape, std::nullopt);
            EXPECT_TRUE(succeeded(result));
            // Structural equality.
            EXPECT_EQ(toLinearLayout(shape, newSrcEnc),
                      toLinearLayout(shape, enc));
            ...
            Attribute joinedEnc;
            result = inferLayout->inferDefaultJoinOpEncoding(
                enc, joinedEnc, shape, std::nullopt);
...
class AMDLayoutTest : public ::testing::Test {
  public:
    AMDLayoutTest() {
        ctx.getOrLoadDialect<TritonGPUDialect>();
        ctaLayout = triton::gpu::CTALayoutAttr::get(&ctx, ctaPerCGA, ctaSplit,
                                                    ctaOrder);
    ...
    createDotOperand(int idx, Attribute parent, int kWidth) {
        return triton::gpu::DotOperandEncodingAttr::get(&ctx, idx, parent,
                                                        kWidth);
    }
...
class AMDMfmaLayoutTest : public AMDLayoutTest {
  public:
    ...
    createMFMA(int mDim, int nDim, ArrayRef<unsigned> warpsPerCTA) {
...
TEST_F(LinearEncodingTest, DistributedEncodingToLinearEncoding) {
    ...
    for (const auto &distributedEncoding : distributedEncodings) {
        for (auto shape : shapes) {
            ...
            // Test that the canonical form of the LinearLayout is indeed
            // canonical by expanding it to the original shape
            ...
            if (!isa<triton::gpu::SliceEncodingAttr>(distributedEncoding)) {
                auto baseEncoding =
                    cast<LayoutEncodingTrait>(distributedEncoding);
                ASSERT_EQ(baseEncoding.getCTASplitNum(),
                          linearEncoding.getCTASplitNum());
                ASSERT_EQ(baseEncoding.getCTAsPerCGA(),
                          baseEncoding.getCTAsPerCGA());
                // If we are not using CGAs, the order is meaningless
                auto useCGA = baseEncoding.getCTAsPerCGA() !=
                              SmallVector<unsigned>(rank, 1);
                if (useCGA &&
                    !is_dot_op_with_block_parent(distributedEncoding)) {
                    ASSERT_EQ(baseEncoding.getCTAOrder(),

...
void populateMathPatternsAndLegality(TritonGPUTypeConverter &typeConverter,
    ...
    // Rewrite rule
    patterns
        .add<GenericOpPattern<math::ExpOp>, GenericOpPattern<math::Exp2Op>,
...
struct TritonExpandDimsPattern
    ...
    matchAndRewrite(triton::ExpandDimsOp op, OpAdaptor adaptor,
        ...
        auto retCTAsPerCGA =
            insertOne(argCTALayout.getCTAsPerCGA(), op.getAxis());
        auto retCTASplitNum =
            insertOne(argCTALayout.getCTASplitNum(), op.getAxis());
        auto retCTAOrder =
            insertOrder(argCTALayout.getCTAOrder(), op.getAxis());
        ...
        triton::gpu::BlockedEncodingAttr retEncoding =
            triton::gpu::BlockedEncodingAttr::get(
                getContext(), retSizePerThread, retThreadsPerWarp,
...
struct TritonDotPattern : public OpConversionPattern<triton::DotOp> {
    ...
    matchAndRewrite(triton::DotOp op, OpAdaptor adaptor,
        ...
        retSizePerThread[rank - 1] =
            std::min(retSizePerThread[rank - 1],
                     static_cast<unsigned>(origShape[rank - 1]));
        retSizePerThread[rank - 2] =
            std::min(retSizePerThread[rank - 2],
                     static_cast<unsigned>(origShape[rank - 2]));
        ...
        if (!mlir::isa<triton::gpu::DotOperandEncodingAttr>(aEncoding)) {
            ...
            a = rewriter.create<triton::gpu::ConvertLayoutOp>(a.getLoc(),
                                                              dstType, a);
        }
        if (!mlir::isa<triton::gpu::DotOperandEncodingAttr>(bEncoding)) {
            ...
            b = rewriter.create<triton::gpu::ConvertLayoutOp>(b.getLoc(),
                                                              dstType, b);
        }
        c = rewriter.create<triton::gpu::ConvertLayoutOp>(c.getLoc(), retType,
                                                          c);
...
struct TritonCatPattern : public OpConversionPattern<triton::CatOp> {
    ...
    matchAndRewrite(triton::CatOp op, OpAdaptor adaptor,
        ...
        // next_power_of_2(lhs.total_elems_per_thread +
        // rhs.total_elems_per_thread) For now, this behaves like generic, but
        // this will evolve when we add support for `can_reorder=False`.
        ...
        auto lhsTotalElemsPerThread =
            triton::gpu::getTotalElemsPerThread(lhsType);
        auto rhsTotalElemsPerThread =
            triton::gpu::getTotalElemsPerThread(rhsType);
        auto retTotalElemsPerThread =
            triton::gpu::getTotalElemsPerThread(retType);
        ...
        // We have to round it up to the next power of 2 due to triton's tensor
        // size constraint.
        auto newRetTotalElemsPerThread =
            nextPowOf2(lhsTotalElemsPerThread + rhsTotalElemsPerThread);
        auto newRetSizePerThread =
            llvm::to_vector(retEncoding.getSizePerThread());
...
struct TritonJoinOpPattern : public OpConversionPattern<triton::JoinOp> {
    ...
    LogicalResult matchAndRewrite(JoinOp op, OpAdaptor adaptor,
        ...
        // Simply rely on type inference for this op.  (Notably,
        // GenericOpPattern does not do this, instead it assigns the default
        // layout to the ins and outs.)
...
struct TritonSplitOpPattern : public OpConversionPattern<triton::SplitOp> {
    ...
    LogicalResult matchAndRewrite(SplitOp op, OpAdaptor adaptor,
        ...
        //  - threadsPerWarp, warpsPerCTA, and CTAsPerCGA = 1 in the last dim,
        //  and
        ...
        if (!srcEnc || srcEnc.getSizePerThread().back() != 2 ||
            ...
            // If we take the default encoding for the op's result (i.e.
            // post-split) and add 1 to the end of each dim, that gives us what
            // we want.  Other than making a legal src encoding, our choice of
            // layout doesn't matter; it'll get fixed by
            ...
            auto defaultEnc = getDefaultBlockedEncoding(
                ...
                typeConverter->getThreadsPerWarp(),
...
struct TritonTransPattern : public OpConversionPattern<TransOp> {
    ...
    matchAndRewrite(TransOp op, OpAdaptor adaptor,
        ...
        if (!srcEnc)
            return failure();
        addNamedAttrs(
            rewriter.replaceOpWithNewOp<TransOp>(op, src, op.getOrder()),
            adaptor.getAttributes());
        return success();
    }
};
...
struct TritonScanPattern : public OpConversionPattern<triton::ScanOp> {
    ...
    matchAndRewrite(triton::ScanOp op, OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        auto newScan =
            rewriter.create<triton::ScanOp>(op.getLoc(), adaptor.getOperands(),
...
struct TritonMapElementwisePattern
    ...
    matchAndRewrite(triton::MapElementwiseOp op, OpAdaptor adaptor,
        ...
        auto err =
            converter->convertTypes(op.getResults().getType(), resultTys);
        if (failed(err)) {
            return err;
        }
  ...
  public:
    ...
    matchAndRewrite(triton::FuncOp op, OpAdaptor adaptor,
        ...
        rewriter.inlineRegionBefore(op.getBody(), newOp.getBody(),
                                    newOp.getBody().end());
        // Convert just the entry block. The remaining unstructured control flow
        // is converted by br patterns.
...
struct SCFForPattern : public OpConversionPattern<scf::ForOp> {
    ...
    matchAndRewrite(scf::ForOp op, OpAdaptor adaptor,
        ...
        // replaces each block with a new block containing the updated
        // signature. The entry block may have a special conversion if
        // `entryConversion` is provided. On success, the new entry block to the
        // region is returned for convenience. Otherwise, failure is returned.
        if (failed(rewriter.convertRegionTypes(&newOp.getRegion(),
                                               *getTypeConverter()))) {
            return rewriter.notifyMatchFailure(op,
                                               "could not convert body types");
        }
        // Change the clone to use the updated operands. We could have cloned
        // with a IRMapping, but this seems a bit more direct.
...
class SCFIfPattern : public OpConversionPattern<scf::IfOp> {
  public:
    ...
    matchAndRewrite(scf::IfOp op, OpAdaptor adaptor,
        ...
        // For example, it's possible that one result type converts to 0 types
        // and another to 2 types, so newResultTypes would at least be the right
        // size to not crash in the llvm::zip call below, but then we would set
        // the the wrong type on the SSA values! These edge cases are also why
        // we cannot safely use the TypeConverter::convertTypes helper here.
        SmallVector<Type> newResultTypes;
        ...
        // See comments in the ForOp pattern for why we clone without regions
        // and then inline.
        scf::IfOp newOp =
  ...
  public:
    ...
    matchAndRewrite(scf::WhileOp op, OpAdaptor adaptor,
        ...
        for (auto i : {0u, 1u}) {
            ...
            rewriter.inlineRegionBefore(op.getRegion(i), dstRegion,
                                        dstRegion.end());
            if (failed(rewriter.convertRegionTypes(&dstRegion, *converter)))
                return rewriter.notifyMatchFailure(
                    op, "could not convert body types");
        }
        rewriter.replaceOp(op, newOp.getResults());
        return success();
    }
  ...
  public:
    ...
    matchAndRewrite(cf::BranchOp op, cf::BranchOp::Adaptor adaptor,
        ...
        if (failed(rewriter.convertRegionTypes(
                newOp.getSuccessor()->getParent(), *converter)))
            return failure();
        return success();
    }
};
  ...
  public:
    ...
    matchAndRewrite(cf::CondBranchOp op, cf::CondBranchOp::Adaptor adaptor,
        ...
        if (failed(rewriter.convertRegionTypes(newOp.getTrueDest()->getParent(),
            ...
            return failure();
        if (failed(rewriter.convertRegionTypes(
                newOp.getFalseDest()->getParent(), *converter)))
            return failure();
        return success();
    }
};
...
class TleDSLRegionOpPattern : public OpConversionPattern<tle::DSLRegionOp> {
  public:
    ...
    matchAndRewrite(tle::DSLRegionOp op, tle::DSLRegionOp::Adaptor adaptor,
        ...
        if (failed(
                rewriter.convertRegionTypes(&newBody, *getTypeConverter()))) {
            return rewriter.notifyMatchFailure(op,
                                               "could not convert body types");
        }
        newOp->setOperands(adaptor.getOperands());
...
void populateTleRawPatterns(TritonGPUTypeConverter &typeConverter,
                            RewritePatternSet &patterns) {
    MLIRContext *context = patterns.getContext();
    patterns.add<TleDSLRegionOpPattern, GenericOpPattern<tle::YieldOp>,
                 ...
                 GenericOpPattern<tle::ExtractPtrOp>,
                 GenericOpPattern<tle::PackOp>>(typeConverter, context);
}
    ...
    void runOnOperation() override {
        if (target.getValue().empty()) {
            mlir::emitError(getOperation().getLoc(),
                            "'convert-triton-to-tritongpu' requires 'target' "
                            "option to be set");
        ...
        // TODO: can we use
        //    mlir::scf::populateSCFStructurealTypeConversionsAndLegality(...)
        //    here?

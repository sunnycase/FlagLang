...
TritonGPUTypeConverter::TritonGPUTypeConverter(MLIRContext *context,
    ...
    if (enableSourceRemat) {
        addSourceMaterialization([](OpBuilder &builder,
            ...
            return builder
                .create<UnrealizedConversionCastOp>(loc, tensorType, inputs)
                .getResult(0);
        });
    }
    ...
    addTargetMaterialization([](OpBuilder &builder, RankedTensorType tensorType,
        ...
        auto cast = builder.create<triton::gpu::ConvertLayoutOp>(
            loc, tensorType, inputs);
...
static RankedTensorType getNewIndicesType(RankedTensorType type,
    ...
    // Technically any layout where we have a pack of 4 neighbouring elements
    // plus broadcasted over the warp dimension is okay but for now we just pick
    // a layout.
...
static LogicalResult convertGatherScatterIndices(Operation *op,
    ...
    if (!newType)
        return failure();
    Value index =
        b.create<ConvertLayoutOp>(op->getLoc(), newType, indices.get());
    indices.set(index);
    return success();
}

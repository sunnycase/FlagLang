...
applyLinearLayout(Location loc, RewriterBase &rewriter,
    ...
    assert(
        llvm::equal(layout.getInDimNames(), llvm::make_first_range(indices)));
    // Trivial layout
    ...
    // compilation slow.  (We think this shouldn't be the case -- it's not
    // *that* much code -- but we're not clear on how to fix the slowness, which
    // happens in the bowels of MLIR.)
    //
...
Value getThreadId(OpBuilder &rewriter, Location loc) {
    ...
    // If this is being created inside a warp specialize op, compute the
    // relative thread ID within the warp group.
...
lowerLdStShared(Location loc, MLIRContext *ctx, LinearLayout cvt,
    ...
    return lowerLdSt(loc, ctx, cvt, valsArray, llvmElemTy, smemBase,
                     ...
                     laneId, warpId, rewriter, targetInfo, {}, emitLdSt);
}
...
SmallVector<Value> lowerLdSt(
    ...
    for (int i = 0; i < cvt.getInDimSize(kReg); i += nAdditive) {
        ...
        for (int j = 0; j < nAdditive; j += elemsPerVec) {
            ...
            llvm::append_range(
                outVals, lowerInst(rewriter, loc, vals, vecAddr, i + j, vecTy));
        }
    }
...
lowerLocalLdSt(Location loc, MLIRContext *ctx,
    ...
    if (!removeBroadcastSrc.isIdentity()) {
        ...
        auto outVals =
            lowerLocalLdSt(loc, ctx, prmtCvt, inVals, llvmElemTy, srcTy,
...
bool emitTransferBetweenRegistersAndShared(
    ...
    for (auto &indices : indicesVec) {
        ...
        auto vecAddr = b.gep(smemBase.getType(), elemLlvmTy, smemBase,
                             smemOffset, LLVM::GEPNoWrapFlags::inbounds);
...
Value packLLElements(Location loc, const LLVMTypeConverter *typeConverter,
    ...
    for (auto [i, value] : llvm::enumerate(resultVals)) {
        ...
        if (value.getType() != elementTypes[i]) {
            ...
            LDBG("value " << value);
            emitError(loc)
                << "invalid element type in packLLElements. Expected "
...
Value SharedMemoryObject::getShmemOffset(Location loc, RewriterBase &rewriter,
    ...
    // We return the offset without the padding. The padding will be added in
    // the lowering
    if (auto paddedSharedEncoding =
            dyn_cast<triton::gpu::PaddedSharedEncodingAttr>(
                srcTy.getEncoding())) {
        auto allocShape64 = srcTy.getAllocShape();
        SmallVector<unsigned> allocShape(allocShape64.begin(),
                                         allocShape64.end());
...
Value pext_i32(RewriterBase &rewriter, Location loc, Value a, uint32_t mask) {
    ...
    while (mskConst) {
        ...
        extcnt += grplen;
        result = b.or_(result,
                       b.lshr(b.and_(b.i32_val(bitgrp), a), b.i32_val(shift)));
    }
    return result;
}
    ...
    for (Block &origBlock : region) {
        ...
        if (terminator->getRegisteredInfo()->getTypeID() == terminatorTypeId) {
            ...
            rewriter.replaceOpWithNewOp<LLVM::BrOp>(
                terminator, terminatorOperands, remainingOpsBlock);
        }
    }
...
void finalizeTensorAtomicResults(Operation *op, RankedTensorType tensorTy,
    ...
    dstLayout = dstLayout.sublayout(
        {kReg, kLane, kWarp}, llvm::to_vector(dstLayout.getOutDimNames()));
    ...
    auto emitSt = [&](RewriterBase &rewriter, Location loc,
                      ArrayRef<Value> vals, Value shmemAddr, int idx,
                      VectorType vecTy) -> SmallVector<Value> {
        auto length = vecTy.getNumElements();
        Value valsVec = packLLVector(
    ...
    auto emitLd = [&](RewriterBase &rewriter, Location loc,
                      ArrayRef<Value> vals, Value shmemAddr, int idx,
                      VectorType vecTy) -> SmallVector<Value> {

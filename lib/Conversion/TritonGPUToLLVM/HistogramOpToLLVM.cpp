...
static SmallVector<Value> computeWarpLevelHistogram(
    ...
    for (int i = 0; i < numElementsPerThreads; ++i) {
        ...
        for (int i = 0; i < numBitsLaneId; i++) {
            ...
            mask = b.and_(mask, b.xor_(ballotBits[i + numBits - numBitsLaneId],
                                       updateMask));
        }
        // save a ballot bit to capture the input mask
        Value inputMaskBit = fullMask;
        if (maskValues.size() > 0) {
            inputMaskBit = targetInfo.ballot(
                rewriter, loc, int_ty(numThreadPerWarp), maskValues[i]);
        }
        ...
        for (int k = 0; k < warpLevelHistogram.size(); k++) {
            ...
            for (int j = 0; j < numBits - numBitsLaneId; j++) {
                Value updateMask = b.int_val(
                    numThreadPerWarp, ((k & (1 << j)) ? 0 : fullMaskValue));
...
static SmallVector<Value> computeCrossWarpHistogram(
    ...
    for (int i = 0; i < warpLevelHistogram.size(); ++i) {
        ...
        Value offset = b.add(
            b.mul(laneId, b.i32_val(warpLevelHistogram.size())), b.i32_val(i));
        Value sharedMemPtr =
            b.gep(baseSharedMemPtr.getType(), i32_ty, baseSharedMemPtr, offset);
...
struct HistogramOpConversion
  ...
  public:
    ...
    matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
        ...
        // First compute a warp local histogram based on values owned by each
        // warps.
        ...
        // generate the right layout. Currently the warp level histogram
        // generates data in the default blocked layout.
        Value baseSharedMemPtr = LLVM::getSharedMemoryBase(
            loc, rewriter, targetInfo, op.getOperation());
        auto dstType = op.getType();
        Attribute dstEncoding = dstType.getEncoding();
        auto indices = emitIndices(op.getLoc(), rewriter, targetInfo,
        ...
        SmallVector<Value> histogramValue = computeCrossWarpHistogram(
            loc, rewriter, srcType, baseSharedMemPtr, warpLevelHistogram,
            numBins, numThreadsPerWarp, innerDimIndices, threadId, numWarps);

        // Depending on the layout, some threads may have duplicate data. We can
        // account for this by calculating a "replication factor" and dividing
        // the results by it to avoid overcounting.
        ...
        replicationFactor /=
            std::accumulate(threadsPerWarp.begin(), threadsPerWarp.end(), 1,
                            std::multiplies<>());
        replicationFactor /= std::accumulate(
            warpsPerCTA.begin(), warpsPerCTA.end(), 1, std::multiplies<>());

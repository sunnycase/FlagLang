...
class GenericFMAVectorMultiplier : public FMAVectorMultiplier {
  ...
  public:
    ...
    Value multiplyVectors(ArrayRef<Value> a, ArrayRef<Value> b,
        ...
        for (auto it = llvm::zip(a, b).begin(); it != llvm::zip(a, b).end();
             ++it) {
            ...
            // to avoid: 'llvm.intr.fmuladd' op operand #0 must be floating
            // point LLVM type or LLVM dialect-compatible vector of floating
            // point LLVM type, but got 'i32'
            llvm::TypeSwitch<Type>(tgtTy)
                .Case<FloatType>([&](auto) {
                    accum = builder.create<LLVM::FMulAddOp>(loc, aElem, bElem,
                                                            accum);
                })
                .Case<IntegerType>([&](auto) {
                    accum = builder.create<LLVM::AddOp>(
                        loc, builder.create<LLVM::MulOp>(loc, aElem, bElem),
                        accum);
                });
        }
        return accum;

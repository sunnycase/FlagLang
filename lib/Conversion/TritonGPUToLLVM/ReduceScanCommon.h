...
inline SmallVector<Value> applyCombineOp(Location loc,
    ...
    // Allows for passing an uninitialized acc and use cur as the neutral
    // element
    if (acc.size() == 0) {
        return cur;
    }
    ...
    // Create a new copy of the combine block, and try to speculatively inline
    // it
    ...
    if (!pred || isRegionSpeculatable) {
        // Fast path, region has no side effects so we can unconditionally
        // execute
...
class ConvertTritonGPUReduceScanToLLVMPattern
  ...
  public:
    ...
    SmallVector<Value> getSmemBases(SourceOp op, unsigned elems,
        ...
        auto basePtr = LLVM::getSharedMemoryBase(loc, rewriter, targetInfo,
                                                 op.getOperation());

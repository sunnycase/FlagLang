...
static void warpScan(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        unsigned elementIdx =
            (srcIndex / elementStride) % scanElementsPerThreads;
...
static void storeWarpAccumulator(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        index =
            b.add(index, b.i32_val(chunkId * numParallelLane * axisNumWarps));
        for (unsigned i = 0; i < lastElement.size(); ++i) {
            Value writePtr = b.gep(smemBases[i].getType(), smemTypes[i],
                                   smemBases[i], index);
            targetInfo.storeShared(rewriter, loc, writePtr, lastElement[i],
                                   mask);
        }
        chunkId++;
    }
...
static void AddPartialReduce(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        // accumulator to combine based on whether the elements belong to the
        // same dimension along axis.
        ...
        for (unsigned i = 0; i < axisNumWarps; ++i) {
            Value index =
                b.add(parallelLaneId, b.i32_val(numParallelLane *
            ...
            for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                ...
                Value ptr =
                    b.gep(smemBases[j].getType(), elemTy, smemBases[j], index);
            ...
            for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                accumulator.maskedAcc[j] = b.select(mask, accumulator.acc[j],
        ...
        for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
            auto elem =
                targetInfo.shuffleUp(rewriter, loc, temp[i], threadStride);
            lastElement[i] =
        ...
        for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
            ...
            laneValue =
                accumulate(helper, rewriter, lastElement, laneValue, pred);
            if (axisBlockId == 0) {
                // For the first warp and first chunk we don't have anything to
                // accumulate.
                for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                    laneValue[j] =
                        b.select(maskNotFirstThread, laneValue[j],
...
static void AddPartialReduceOneWarp(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
            ...
            if (axisBlockId == 0) {
                for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                    // For the first warp and first chunk we don't have anything
                    // to accumulate.
                    laneValue[j] =
                        b.select(maskFirstThread,
                                 srcValues[srcIndex - i * elementStride][j],
                                 laneValue[j]);
                }
            }
...
ScanOpConversion::getDelinearizedIds(ConversionPatternRewriter &rewriter,
    ...
    warpsPerCTA[axis] = 1;
    Value warpIdParallel = linearize(rewriter, loc, multiDimWarpId, warpsPerCTA,
                                     helper.getOrder());
        ...
        for (unsigned j = 0; j < op.getNumOperands(); ++j) {
            for (unsigned k = iWarpSize / 2; k >= 1; k = k / 2) {
                srcValues[revIndex][j] = targetInfo.shuffleXor(
                    rewriter, loc, srcValues[revIndex][j], k);
            }
...
ScanOpConversion::emitFastScan(triton::ScanOp op, triton::ScanOpAdaptor adaptor,
    ...
    if (axisNumWarps > 1) {
        // Slow path for the case where there are multiple warps with unique
        // data on the axis.
        ...
        storeWarpAccumulator(srcValues, rewriter, helper, laneIdAxis,
                             warpIdAxis, smemBases, smemTypes, flatIdParallel,
                             isRepresentative, targetInfo);
        b.barrier();
        // Read back the partial reduction of each warp and accumulate them
        // based on warpId. Then update each chunk of contiguous elements by
        ...
        // Fast path for the case where there is only one warp with unique data
        // on the axis.
        unsigned scanDim = helper.getAxisNumThreadsPerWarpWithUniqueData();
        auto multiDimLaneId =
        ...
        auto laneIdLast = linearize(rewriter, loc, multiDimLaneId,
                                    threadsPerWarp, helper.getOrder());
        AddPartialReduceOneWarp(srcValues, rewriter, targetInfo, helper,
                                warpIdAxis, laneIdAxis, laneIdLast);
    ...
    auto transpose = [](const SmallVector<SmallVector<Value>> &v) {
        ...
        auto ret = SmallVector<SmallVector<Value>>(
            v[0].size(), SmallVector<Value>(v.size()));

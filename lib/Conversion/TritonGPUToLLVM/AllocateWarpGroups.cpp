...
struct AllocateWarpGroups
    ...
    void runOnOperation() override {
        ...
        mod.walk([&](WarpSpecializeOp op) {
            maxExtraWarps =
                std::max<int>(maxExtraWarps, op.getTotalPartitionWarps());
        });

        // Round this up to the nearest warpgroup (multiple of 4) and then pad
        // each `ttg.warp_specialize` to the nearest warpgroup.
        ...
        mod.walk([&](WarpSpecializeOp op) {
            ...
            // Allocate the start IDs such that the largest warpgroups have
            // lower starting warp IDs.
            ...
            llvm::sort(idxAndSize, [&](auto lhs, auto rhs) {
            ...
            // Require that an estimate has been set and that we have even
            // warpgroups.
            ...
            for (auto [startId, partition, estRegs, numWarps] :
                ...
                orderedPartitions.push_back(
                    {startId, partition, estRegs, numWarps});
            llvm::sort(orderedPartitions, [&](auto lhs, auto rhs) {
                return lhs.startId < rhs.startId;
            });

            // Iterate over the partitions and assign them to warp groups.
            // Determine the maximum number of requested registers per warp
            // group.
            ...
            for (auto [startId, partition, estRegs, numWarps] :
                ...
                warpGroups.back().maxRequestedRegs = std::max<int>(
                    warpGroups.back().maxRequestedRegs, estRegsCeil8);
            ...
            for (const WarpGroupInfo &wg : warpGroups) {
                ...
                registerBudget += (maxnreg - wg.maxRequestedRegs) *
                                  wg.numWarps * threadsPerWarp;
            }
            if (registerBudget <= 0)
                return;

            // Determine the number of extra registers that we can distribute to
            // the default warp group.
            ...
            // Generate setmaxnreg in each partition according to its warp
            // group.
            ...
            // Set the initial max number of registers. This is needed for PTXAS
            // to cooperate.

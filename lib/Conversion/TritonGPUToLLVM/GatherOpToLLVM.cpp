...
class GatherOpConversion : public ConvertOpToLLVMPattern<GatherOp> {
  ...
  private:
    // Codegen the gather by storing the source tensor into shared memory and
    // then gathering directly from shared memory.
...
GatherOpConversion::matchAndRewrite(GatherOp op, OpAdaptor adaptor,
    ...
    // Specialize the lowering based on the source layout. Given that the cost
    // of a warp shuffle is approximately half the cost of a roundtrip to shared
    // memory with zero bank conflicts, we will need a more precise heuristic to
    // choose between the two codegen paths and rely on the middle end to pick
    // the right layout.
...
void GatherOpConversion::emitGatherInShared(
    ...
    // Store the src values owned by the thread into their respective location
    ...
    // For each src element owned by the thread, index into the scratch memory
    // and then store it.
    Type elemType = getTypeConverter()->convertType(srcType.getElementType());
    for (auto [value, indices] : llvm::zip(srcValues, srcIndices)) {
        // Convert the index at each dim into a single offset given the shape of
        // the tensor.
...
void GatherOpConversion::emitWarpLocalGather(
    ...
    // the thread that contains the required element and `src_reg` is the
    // register within that thread.
    //
    ...
    // Invert the source layout. It doesn't matter whether it is fully
    // invertible with respect to anything except the register input dimension,
    // since we know those don't vary in ways that matter for codegen.
    LinearLayout invSrcLayout = srcLayout.pseudoinvert();

    // Sanity check: the warp must be invariant to the index because otherwise
    // the gather would need to read across warps!
    ...
    // Given a index value, we need to know which sources register values it
    // could index into. This is invariant to anything other than the register,
    // which we checked already. Compute the full reverse map from
    //
    ...
    for (auto [idxReg, idxVal] : llvm::enumerate(idxValues)) {
        ...
        for (unsigned i = 0; i != numRegsPerColumn; ++i) {
            ...
            Value value = targetInfo.shuffleIdx(rewriter, loc,
                                                srcValues[srcRegIdx], srcLane);
            result = b.select(b.icmp_eq(b.i32_val(srcRegIdx), srcReg), value,
                              result);
        }

...
struct SplatOpConversion : public ConvertOpToLLVMPattern<triton::SplatOp> {
    ...
    static Value convertSplatLikeOp(Type elemType, Type resType, Value constVal,
        ...
        // Check the converted type for the tensor as depending on the encoding
        // the converter may pick different element types.
...
struct ArithConstantSplatOpConversion
    ...
    matchAndRewrite(arith::ConstantOp op, OpAdaptor adaptor,
        ...
        if (type::isFloat(elemType)) {
            ...
            llvm::errs()
                << "ArithConstantSplatOpConversion get unsupported type: "
                << value.getType() << "\n";
            return failure();
        }
        // Lower FP8 constant to int8 constant since FP8 types are not supported
        // on LLVM IR.
...
struct ArithConstantArrayOpConversion
    ...
    matchAndRewrite(arith::ConstantOp op, OpAdaptor adaptor,
        ...
        if (elemsPerThread != llVals.size()) {
            op->emitError("Right now we only support constant arrays with the "
                          "same number of "
                          "elements as the number of threads per warp");
            return failure();
        }
        auto llStruct = packLLElements(loc, getTypeConverter(), llVals,
                                       rewriter, op.getType());
        rewriter.replaceOp(op, {llStruct});
        return success();
    }
...
struct CatOpConversion : public ConvertOpToLLVMPattern<CatOp> {
    ...
    matchAndRewrite(CatOp op, OpAdaptor adaptor,
        ...
        // pack and replace
        Value ret =
            packLLElements(loc, typeConverter, retVals, rewriter, resultTy);
...
struct JoinOpConversion : public ConvertOpToLLVMPattern<JoinOp> {
    ...
    matchAndRewrite(JoinOp op, OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        // We rely on the following invariants of this op (which are checked by
        // its verifier):
        //
        ...
        //
        // With these invariants, join is trivial: We can count how many
        // contiguous registers belong to the same chunk then we merge the
        // registers between two different chunks.
        Location loc = op->getLoc();
        ...
        auto typeConverter = getTypeConverter();
        Value ret =
            packLLElements(loc, typeConverter, joinedVals, rewriter, dstTy);
        rewriter.replaceOp(op, ret);
        return success();
    }
};
struct SplitOpConversion : public ConvertOpToLLVMPattern<SplitOp> {
    ...
    matchAndRewrite(SplitOp op, OpAdaptor adaptor,
        ...
        //
        // With these invariants, split is trivial: We can count how many
        // contiguous registers belong to the same chunk then we separate the
        // registers between two different chunks.
    ...
    matchAndRewrite(ReshapeOp op, OpAdaptor adaptor,
        ...
        if (triton::gpu::isExpensiveView(op.getSrc().getType(), op.getType())) {
            return emitOptionalError(
                loc, "expensive view not supported on reshape op");
        }
        auto resultTy = cast<RankedTensorType>(op.getType());
        auto srcTy = cast<RankedTensorType>(op.getSrc().getType());
        auto typeConverter = getTypeConverter();
        ...
        Value ret =
            packLLElements(loc, typeConverter, vals, rewriter, resultTy);
        rewriter.replaceOp(op, ret);
        return success();
    }
};
struct ExpandDimsOpConversion : public ConvertOpToLLVMPattern<ExpandDimsOp> {
    ...
    matchAndRewrite(ExpandDimsOp op, OpAdaptor adaptor,
        ...
        if (!srcLayout) {
            return emitOptionalError(
                loc,
                "ExpandDimsOp only supports SliceEncodingAttr as its input");
        }
...
struct MemDescTransOpConversion
    ...
    matchAndRewrite(MemDescTransOp op, OpAdaptor adaptor,
        ...
        auto retVal =
            getStructFromSharedMemoryObject(loc, dstSmemObj, rewriter);
...
struct TransOpConversion : public ConvertOpToLLVMPattern<TransOp> {
    ...
    matchAndRewrite(TransOp op, OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        // By construction, TransOp::inferReturnTypes ensures that the src
        // encoding is the same as the dst encoding so that this op is a no-op.
...
struct MemDescIndexOpConversion
    ...
    matchAndRewrite(triton::gpu::MemDescIndexOp op, OpAdaptor adaptor,
        ...
        // getAllocationShapePerCTA returns the correct number fp4 elements that
        ...
        if (auto padEnc =
                dyn_cast<PaddedSharedEncodingAttr>(dstTy.getEncoding())) {
            auto bitwidth = dstTy.getElementTypeBitWidth();
            Value padOffset =
                emitPadding(loc, rewriter, padEnc, bitwidth, offset,
                            /*offsetInBytes=*/false);

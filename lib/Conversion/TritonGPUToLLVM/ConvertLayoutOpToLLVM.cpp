...
struct ConvertLayoutOpConversion
    ...
    // conversions.  TODO(jlebar): Eventually we want this to be the only
    // pattern.
    ...
    matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
        ...
        if (llvm::is_contained(dims, kBlock)) {
            ...
            //          This requires moving values through distributed shared
            //          memory.
            return rewriter.notifyMatchFailure(
                op, "NYI: Transfer between different CTAs");
        } else if (llvm::is_contained(dims, kWarp)) {
            // Case 2: Transfer between values in the same CTA, in which case we
            // move
            //         values through shared memory.
            transferWithinBlockSwizzling(op, adaptor.getSrc(), rewriter);
            return success();
        } else if (llvm::is_contained(dims, kLane)) {
            // Case 3. Transfer between values in the same warp, in which case
            // we try
            //         to move values using warp shuffles, though if the pattern
            //         is expensive enough we fall back to using shared memory
            ...
            // Case 4. Transfer between values in the same thread, in which case
            // we
    ...
    SmallVector<Value> transferWithinBlockSwizzlingImpl(
        ...
        // We handle transformations recursively as they all need a
        // preprocessing and a postprocessing step.

        // Handle pointer types as 64-bit integers
        if (isa<LLVM::LLVMPointerType>(llvmElemTy)) {
            auto llvmElemTyPtr = i64_ty;
            auto newInVals =
                llvm::to_vector(llvm::map_range(inVals, [&](Value v) {
        ...
        if (llvmElemTy.getIntOrFloatBitWidth() < 8) {
            ...
            auto newInVals =
                llvm::to_vector(llvm::map_range(inVals, [&](Value v) {
                    return b.zext(i8ElemTy, v).getResult();
                }));
            auto outVals = transferWithinBlockSwizzlingImpl(
        ...
        if (!removeBroadcastSrc.isIdentity()) {
            ...
            return transferWithinBlockSwizzlingImpl(loc, rewriter, prmtSrc,
                                                    dstLayout, newInVals,
                                                    llvmElemTy, smemBase);
        }
        ...
        if (!removeBroadcastDst.isIdentity()) {
            ...
            auto outVals = transferWithinBlockSwizzlingImpl(
                loc, rewriter, srcLayout, prmtDst, inVals, llvmElemTy,
                smemBase);
        ...
        storeCvt =
            storeCvt.reshapeOuts({{kOffset, storeCvt.getTotalOutDimSize()}});
        loadCvt =
            loadCvt.reshapeOuts({{kOffset, loadCvt.getTotalOutDimSize()}});
        ...
        for (int i = 0; i < nReps; ++i) {
            ...
            lowerLdStShared(loc, ctx, storeCvt, tileInVals, llvmElemTy,
                            smemBase, noPaddingOffset, affineOffset,
    ...
    transferWithinBlockSwizzling(ConvertLayoutOp op, Value src,
        ...
        // Remove the kBlock dimension from the layout as it's the identity in
        // the cvt
    ...
    LogicalResult
    transferWithinWarp(ConvertLayoutOp op, OpAdaptor adaptor,
                       ConversionPatternRewriter &rewriter) const {
        auto loc = op.getLoc();
        auto *ctx = op.getContext();
        auto b = TritonLLVMOpBuilder(loc, rewriter);
        ...
        auto factors =
            getWarpLayoutConvertDecomposition(srcTy, dstTy, bitwidth);
        ...
        assert((m > 0 || !pLaneIsTrivial) &&
               "Shuffles not needed for conversion");

        // The desired layout conversion can be expressed as a permutation P of
        // hardware index bits for the `kLane` and `kReg` dimensions. The
        // `factors` of P describe a decomposition
        //
        //                 P = P_mixed \circ P_lane \circ P_reg,
        //
        // where P_reg and P_lane are permutations involving only register or
        // only lane index bits and P_mixed is a product of disjoint
        // transpositions of register index bits with lane index bits. Our goal
        // is to implement P using predicated selects and warp-shuffles. We have
        // two tools for this:
        //  - An out-of-place `Ship` method which implements one mixed
        //  transposition
        //    at a time using 1.5 * R selects/permutes and .5 * R shuffles each.
        //  - An in-place `Swap` method which can simultaneously implement
        //  P_lane
        //    and multiple mixed transpositions at a time using 2 * m * R
        //    selects/ permutes and either (1 - (1/2)^m) * R shuffles if
        //    `pLaneIsTrivial` and R shuffles otherwise.
        // Here, R denotes the number of 32-bit registers in use after packing
        // (or splitting, if applied to 64-bit types or pointers), and in the
        // `Swap` method, `m` denotes the number of mixed transpositions passed
        // in.
        auto inVals = unpackLLElements(loc, adaptor.getSrc(), rewriter);
        ...
        // layout, then we broadcast along the register dimension to match size.
        // The removal of broadcasting above and introduction here is expected
        // by the `factors`.
        ...
        if (elemsPerVec > 1) {
            ...
            if (bitwidth == 8 && bitsPerVecElem == 16) {
                // TODO: Can remove `if` part of `if-else` once ptxas bugfix
                // lands.
                for (int i = 0; i < regDim; i += elemsPerVec) {
                    Value x0 =
                        b.zext(i32_ty, b.bitcast(inVals[i], int_ty(bitwidth)));
                    Value x1 = b.zext(
                        i32_ty, b.bitcast(inVals[i + 1], int_ty(bitwidth)));
        ...
        auto isShippable = [](const TranspositionInfo &t) {
            // The `Ship` method cannot mix elements from different registers in
            // the same lane, so we are restricted to cycles like (l0 r1), (l0
            // r2), and (l0 r0 r1) which do not use both high and low register
            // bits.
        ...
        if (m == 1 && pLaneIsTrivial && isShippable(mixedTranspositions[0])) {
            ...
            outVals =
                transferWithinWarpSwapImpl(loc, rewriter, inVals, nPack, pLane,
        ...
        // broadcasting is removed, then drop the extra registers from
        // `outVals`.
        auto dstLayout = toLinearLayout(dstTy);
        auto removeBroadcastDst = actionRemoveBroadcastedRegs(dstLayout);
    ...
    SmallVector<Value> transferWithinWarpSwapImpl(
        Location loc, ConversionPatternRewriter &rewriter,
        ArrayRef<Value> inVals, int nPack, const LinearLayout &pLane,
        bool pLaneIsTrivial,
        ...
        // on, resulting in
        // `(1 - (1/2)^m) * numRegs` shuffles in total. If `pLane` is
        // nontrivial, then we can conjugate its effects through the first two
        // stages and fuse it with the second stage, resulting in `numRegs`
        // shuffles instead.
        ...
        auto applySwap = [&](TranspositionInfo t, bool preShuf) {
            ...
            int lIdx =
                preShuf ? llvm::Log2_32(pLInvBases[origLIdx][0]) : origLIdx;
            ...
            for (int tileIdx = 0; tileIdx < numTiles; ++tileIdx) {
                ...
                for (int i = 0; i < tileSize / 2; ++i) {
                    ...
                    if (topSel == 0x3210 && botSel == 0x7654) {
                        ...
                        Value sel01 =
                            b.i32_val(preShuf ? botSel : (topSel ^ 0x4444));
                        Value sel10 = b.i32_val(botSel);
                        Value sel11 =
                            b.i32_val(preShuf ? topSel : (botSel ^ 0x4444));
                        Value sel1 = b.select(lBitOff, sel00, sel01);
                        Value sel2 = b.select(lBitOff, sel10, sel11);
                        newVals[r0] =
                            targetInfo.permute(rewriter, loc, v0, v1, sel1);
                        newVals[r1] =
                            targetInfo.permute(rewriter, loc, v0, v1, sel2);
                    }
                }
            }
            return newVals;
        ...
        for (int r = 0; r < numRegs; ++r) {
            ...
            if (pLaneIsTrivial) {
                if (mask != 0)
                    vals[r] =
                        targetInfo.shuffleXor(rewriter, loc, vals[r], mask);
            } else {
    ...
    SmallVector<Value> transferWithinWarpShipImpl(
        Location loc, ConversionPatternRewriter &rewriter,
        ArrayRef<Value> inVals, int nPack, TranspositionInfo t) const {
        // Implements the effects of a single mixed transposition as in
        // `transferWithinWarpSwapImpl`, but uses auxiliary registers to hold
        // the values to be shuffled, resulting in fewer emitted instructions.
        int numRegs = inVals.size();
        int rIdx = t.transposition.first - nPack;
        int lIdx = t.transposition.second;
        int tileSize = 1 << (rIdx + 1);
        ...
        for (int tileIdx = 0; tileIdx < numTiles; ++tileIdx) {
            ...
            for (int i = 0; i < tileSize / 2; ++i) {
                ...
                if (t.topPreSel == 0x3210 && t.topPostSel == 0x3210) {
                    ...
                    Value shippedVal = targetInfo.shuffleXor(
                        rewriter, loc, valToShip, (1 << lIdx));
                    outVals[r0] = b.select(lBitOff, v0, shippedVal);
                    outVals[r1] = b.select(lBitOff, shippedVal, v1);
                } else {
                    Value shipSel = b.select(lBitOff, b.i32_val(t.botPreSel),
                                             b.i32_val(t.topPreSel));
                    Value valToShip =
                        targetInfo.permute(rewriter, loc, v0, v1, shipSel);
                    Value shippedVal = targetInfo.shuffleXor(
                        rewriter, loc, valToShip, (1 << lIdx));
                    ...
                    Value sel2 = b.select(lBitOff, sel10, sel11);
                    outVals[r0] =
                        targetInfo.permute(rewriter, loc, v0, shippedVal, sel1);
                    outVals[r1] =
                        targetInfo.permute(rewriter, loc, v1, shippedVal, sel2);
                }
            }
        }
        return outVals;

    ...
    matchAndRewrite(triton::ReturnOp op, OpAdaptor adaptor,
        ...
        if (funcOp->hasAttr("nvvm.kernel")) {
            ...
            if (adaptor.getOperands().size() < 2) {
                ...
                newOp = rewriter.create<LLVM::ReturnOp>(op.getLoc(),
                                                        adaptor.getOperands());
            } else {
                // Pack the results into a struct.
                auto packedResultsTy =
                    this->getTypeConverter()->packFunctionResults(
                        funcOp.getResultTypes());
                Value packedResults = rewriter.create<LLVM::UndefOp>(
                    op.getLoc(), packedResultsTy);
                for (auto it : llvm::enumerate(adaptor.getOperands())) {
                    packedResults = b.insert_val(packedResultsTy, packedResults,
                                                 it.value(), it.index());
                }
                newOp =
                    rewriter.create<LLVM::ReturnOp>(op.getLoc(), packedResults);
            }
...
struct CallOpConversion : public ConvertOpToLLVMPattern<triton::CallOp> {
  ...
  private:
    ...
    promoteOperands(triton::CallOp callOp,
        ...
        // Get the last argument of the caller, which is the current stack
        // pointer of shared memory and append it to the operands of the callOp.
        ...
        if (!caller->hasAttr("allocation.offset") ||
            ...
            auto base =
                LLVM::getSharedMemoryBase(loc, rewriter, targetInfo, callOp);
            promotedOperands.push_back(base);
        }
    ...
    getCallOpResults(triton::CallOp callOp, LLVM::CallOp newCallOp,
        ...
        if (numResults < 2) {
            // If < 2 results, packing did not do anything and we can just
            // return.
            results.append(newCallOp.result_begin(), newCallOp.result_end());
        } else {
            // Otherwise, it had been converted to an operation producing a
            // structure. Extract individual results from the structure and
            // return them as list.

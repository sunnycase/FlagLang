...
struct FuncOpConversion : public ConvertOpToLLVMPattern<triton::FuncOp> {
    ...
    triton::FuncOp amendFuncOp(triton::FuncOp funcOp,
        ...
        // Push back two new arguments that indicate the current pointer to
        // shared memory and global scratch memory.
            ...
            for (const auto &attr : attrs) {
                if (attr.getName() == "tt.nv_tma_desc") {
                    ...
                    assert(attr.getValue() ==
                           mlir::IntegerAttr::get(i32_type, 1));
                    assert(
                        isKernel &&
                    ...
                    llvmFuncOp.setArgAttr(i,
                                          LLVM::LLVMDialect::getByValAttrName(),
                                          mlir::TypeAttr::get(arrayType));
                    llvmFuncOp.setArgAttr(
                        i, NVVM::NVVMDialect::getGridConstantAttrName(),
    ...
    matchAndRewrite(triton::FuncOp funcOp, OpAdaptor adaptor,
        ...
        if (triton::isKernel(funcOp)) {
            ...
            // The noinline attribute will be used by the LLVM codegen to
            // prevent inlining.
        ...
        if (Attribute maxnregAttr = funcOp.getParentOp()->getAttr(
                triton::gpu::AttrMaxRegistersName))
            newFuncOp->setAttr(NVVM::NVVMDialect::getMaxnregAttrName(),
                               maxnregAttr);

        // Set an attribute for reqntidx, it could be used in latter LLVM
        // codegen for `nvvm.annotation` metadata.

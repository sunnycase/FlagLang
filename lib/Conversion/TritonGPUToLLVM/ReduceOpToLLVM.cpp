...
struct ReduceOpConversion
  ...
  public:
    ...
    matchAndRewrite(triton::ReduceOp op, OpAdaptor adaptor,
        ...
        SmallVector<Value> smemBases = getSmemBases(
            op, product<unsigned>(smemShape), rewriter, targetInfo);

        storeWarpReduceToSharedMemory(helper, accs, indices, smemBases,
                                      rewriter);
        ...
        // We could avoid this barrier in some of the layouts, however this is
  ...
  private:
    ...
    void reduceWithinThreads(
        ...
        // indices in `offsets` that hold unique values, and only accumulate
        // over those.
        ...
        for (const auto &[_, i] : uniqueOffsets) {
            ...
            accumulate(op.getLoc(), rewriter, *combineOp, accs[key],
                       srcValues[i]);
            if (isFirst)
    ...
    void storeWarpReduceToSharedMemory(
        ...
        Value write = b.and_(b.and_(isRepresentativeLane, isRepresentativeWarp),
                             laneZero);
        ...
        for (auto it : accs) {
            ...
            for (unsigned i = 0; i < op.getNumOperands(); ++i) {
                ...
                Value writePtr = b.gep(smemBases[i].getType(), elemTy,
    ...
    accumulatePartialReductions(ReduceOpHelper &helper,
        ...
        for (unsigned round = 0; round < elemsPerThread; ++round) {
            ...
            for (unsigned i = 0; i < op.getNumOperands(); ++i) {
                auto elemTy = getElementType(op, i);
                Value readPtr = b.gep(smemBases[i].getType(), elemTy,
            ...
            for (unsigned i = 0; i < op.getNumOperands(); ++i) {
                auto elemTy = getElementType(op, i);
                writePtrs[i] = b.gep(smemBases[i].getType(), elemTy,
                                     smemBases[i], writeOffset);
            }
            ...
            for (unsigned i = 0; i < op.getNumOperands(); ++i) {
                targetInfo.storeShared(rewriter, loc, writePtrs[i], acc[i],
                                       pred);
            }
                ...
                for (size_t j = 0; j < resultElems; ++j) {
                    ...
                    for (size_t resultIdx = 0, resultDim = resultShape.size();
                        ...
                        if (resultShape[resultIdx] > smemShape[smemIdx]) {
                            // When srcShape smaller than src sizePerThread,
                            // only srcShape elements is accumulated in smem.
                            // Modulo smemShape effectively replicates srcShape
                            // elements to src sizePerThread.
                            readIdx[smemIdx] =
                                b.urem(readIdx[smemIdx],
                                       b.i32_val(smemShape[smemIdx]));
                        }
                    }
                    Value readOffset =

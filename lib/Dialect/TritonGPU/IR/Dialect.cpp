...
bool isExpensiveView(Type srcType, Type dstType) {
    ...
    for (auto [srcMask, dstMask] : llvm::zip(llSrc.getFreeVariableMasks(),
                                             llDst.getFreeVariableMasks())) {
    ...
    if (fwdInference) {
        ...
        if (!found)
            return emitOptionalError(
                loc, "Fp4ToFpOp/SplitOp requires at least 2 elements "
...
static LogicalResult parseBoolAttrValue(AsmParser &parser, Attribute attr,
    ...
    if (!boolAttr) {
        parser.emitError(parser.getNameLoc(), "expected a bool type in ")
            << desc;
        return failure();
    }
...
LinearEncodingAttr::verify(function_ref<InFlightDiagnostic()> emitError,
    ...
    for (auto [i, dim] : llvm::enumerate(linearLayout.getOutDimNames())) {
        if (dim.str() != ("dim" + llvm::Twine(i)).str()) {
            return emitError() << "Expected output dimensions to be ['dim0', "
                                  "'dim1', ...]. Got "
                               << dim << " at position " << i;
        }
    }
    ...
    for (const auto &dimBases : llvm::make_second_range(bases)) {
        if (!llvm::all_of(dimBases, [&](const auto &basis) {
            ...
            return emitError() << "In a distributed layout, each base must "
                                  "move in at most one "
                                  "dimension.";
        }
    }
...
basesPerDimImpl(const LinearLayout::BasesT &namedBases, StringAttr dimName,
    ...
    for (const auto &basis : bases) {
        ...
        if (it != basis.end()) {
            ...
            // If we've seen a non-zero basis, we double the size of the
            // previous dim This is just needed to count the CTAsPerCGA
            ret[nonZeroIdx] *= 2;
        }
    }
    return ret;
...
SmallVector<unsigned> LinearEncodingAttr::getRepOrder() const {
    ...
    // - getRepOrder does not make sense for LinearEncodingAttr as it already
    // has
...
LinearLayout LinearEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
    ...
    ll =
        ensureLayoutNotLargerThan(ll, namedShape, /*broadcastRegisters=*/false);
...
LinearEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape) const {
    ...
    // We can either have BroadcastOp with SameOperandsAndResultEncoding, or
    // keep the invariant that the shape of the LL is that of the tensor We
    // choose the former for BC
...
Attribute PaddedSharedEncodingAttr::parse(AsmParser &parser, Type type) {
    ...
    auto parseIntervalPaddingPair = [&]() {
        ...
        if (failed(parser.parseInteger(interval)) ||
            failed(parser.parseColon()) || failed(parser.parsePlus()) ||
            failed(parser.parseInteger(padding)))
            return failure();
...
Attribute NVMMASharedEncodingAttr::parse(AsmParser &parser, Type type) {
    ...
    for (const NamedAttribute &attr : dict) {
        if (attr.getName() == "swizzlingByteWidth") {
            if (parseUInt(parser, attr, swizzlingByteWidth,
                          "swizzlingByteWidth")
                    .failed())
                return {};
...
SwizzledSharedEncodingAttr AMDMfmaEncodingAttr::composeSharedLayoutForOperand(
    ...
    if (operandIdx >= 2) {
        return SwizzledSharedEncodingAttr::get(getContext(), 1, 1, 1,
    ...
    // GFX950 supports LDS transpose load instructions, so we need swizzling
    // even when K dimension is not the contiguous dimension.
    ...
    int maxPhase = std::max(
        std::min(simdWidth / perPhase, innerDimLength / vectorSize), 1u);
...
NvidiaMmaEncodingAttr::getRepForOperand(ArrayRef<int64_t> shape, int bitwidth,
    ...
    if (opIdx == 0) {
        ...
        // RF so it's fine if the n is incorrect here
        tileSize.push_back(tileBitWidthK / bitwidth);
...
SmallVector<unsigned> DotOperandEncodingAttr::getRepOrder() const {
    ...
    } else if (auto blocked =
                   mlir::dyn_cast<BlockedEncodingAttr>(getParent())) {
...
LogicalResult DotOperandEncodingAttr::verify(
    ...
    if (auto parentAttr = mlir::dyn_cast<NvidiaMmaEncodingAttr>(parent)) {
        ...
        if (kWidth == 0 && (parentAttr.isAmpere() || parentAttr.isHopper()))
            return emitError()
                   << "ttg.dot_op kWidth parameter is mandatory for "
    ...
    if (auto parentAttr = mlir::dyn_cast<AMDWmmaEncodingAttr>(parent)) {
        if (kWidth != 8 && kWidth != 16 && parentAttr.getVersion() == 1 ||
            ...
            return emitError()
                   << "ttg.dot_op kWidth parameter must be 8/16 for "
    ...
    if (auto parentAttr = mlir::dyn_cast<BlockedEncodingAttr>(parent)) {
        if (kWidth != 0)
            return emitError()
                   << "ttg.dot_op kWidth parameter is not supported "
    ...
    AliasResult getAlias(Attribute attr, raw_ostream &os) const override {
        ...
        if (auto mmaAttr = mlir::dyn_cast<MmaEncodingTrait>(attr)) {
            ...
            return AliasResult::FinalAlias;
                       ...
                       mlir::dyn_cast<SharedEncodingTrait>(attr)) {
            os << "shared";
            return AliasResult::FinalAlias;
...
struct TritonGPUInferLayoutInterface
    ...
    inferTransOpEncoding(Attribute operandEncoding, ArrayRef<int64_t> shape,
        ...
        auto checkRank = [&](unsigned rank) {
            if (rank != order.size()) {
                return emitOptionalError(
                    loc, "rank of encoding does not match order: ", rank,
                    " vs ", order.size());
            }
            return success();
        };
    ...
    verifyLayoutsAreEqual(ArrayRef<int64_t> shape, Attribute expected,
        ...
        if (!areLayoutsEquivalent(shape,
                                  cast<DistributedEncodingTrait>(expected),
    ...
    inferReshapeOpEncoding(ArrayRef<int64_t> srcShape, Attribute srcEnc,
        ...
        if (!isa<DistributedEncodingTrait>(srcEnc)) {
            return emitOptionalError(
                loc, "Failed MemDescReshapeOp encoding inference");
        }
        ...
        // HACK: We create a dummy tensor type to pass to
        // inferReshapeLinearLayout.
    ...
    inferDefaultJoinOpEncoding(Attribute srcEnc, Attribute &dstEnc,
        ...
        if (auto enc = mlir::dyn_cast<SliceEncodingAttr>(srcEnc);
            ...
            Attribute splitEnc;
            auto result =
                inferSplitOpEncoding(parent, splitEnc, joinedShape, loc);
            if (succeeded(result) &&
                areLayoutsEquivalent(shape,
                                     cast<DistributedEncodingTrait>(splitEnc),
            ...
            // elems per thread in the new dimension. The new dimension is the
            ...
            dstEnc = BlockedEncodingAttr::get(
                ...
                append(enc.getWarpsPerCTA(), 1), appendMajorDim(enc.getOrder()),
    ...
    inferSplitOpEncoding(Attribute srcEnc, Attribute &dstEnc,
        ...
        // encoding is the same as the input, but with the last dimension
        // removed.
        ...
        if (isSimpleSplit) {
            ...
            newOrder.erase(
                std::remove(newOrder.begin(), newOrder.end(), splitDim),
                newOrder.end());
            dstEnc = BlockedEncodingAttr::get(
                enc.getContext(), //
    ...
    inferFp4ToFpOpEncoding(ArrayRef<int64_t> shape, int axis, Attribute inEnc,
        ...
        // fastest running dimension.
        // FIXME: We should make sure that there are enough elements along the
        // axis axis whenever fwdInference is false
        if (getOrder(cast<DistributedEncodingTrait>(inEnc), shape)[axis] == 0) {
            ...
            if (auto dotEnc = mlir::dyn_cast<DotOperandEncodingAttr>(inEnc)) {
                ...
                if (fwdInference) {
                    ...
                    if (kWidth > 1) {
                        ...
                        return emitOptionalError(
                            loc, "Fp4ToFpOp requires at least 2 elements "
                                 "per thread in the axis dimension");
                    }
                }
                outEnc = DotOperandEncodingAttr::get(
                    ctx, dotEnc.getOpIdx(), dotEnc.getParent(), kWidth);
                return success();
            }
            ...
            if (auto blockedEnc = mlir::dyn_cast<BlockedEncodingAttr>(inEnc)) {
                auto sizePerThread =
                    llvm::to_vector(blockedEnc.getSizePerThread());
                if (fwdInference) {
...
struct TritonGPUVerifyTensorLayoutInterface
    ...
    using DialectVerifyTensorLayoutInterface::
        DialectVerifyTensorLayoutInterface;
    ...
    LogicalResult verifyTensorLayout(
        ...
        if (llvm::any_of(rankedTy.getShape(),
            ...
            return makeErr()
                   << "Layout has shape " << rankedTy.getShape()
                   << ", but the tensor it's attached to has shape "
                   << rankedTy.getShape() << " which is not a power of two.";
        }
        ...
        if (!moduleWarpsPerCTA) {
            return makeErr() << "Could not determine the number of warps per "
                                "CTA. Operation "
...
std::string getSharedLayoutStr(RankedTensorType type, bool useHWPointOfView) {
    ...
    for (int32_t block = 0; block < numBlocks; block++) {
        for (int32_t offset = 0; offset < blockSize; offset++) {
            ...
            for (int i = 0; i < outputs.size(); i++) {
                // Based on the formatting from LinearLayout::toString, the
                // format for the hw layout is slightly different. HW layouts
                // use "," vs ":".
                if (i > 0) {
                    sharedInfo += ",";
                    value += ":";
    ...
    if (!useHWPointOfView) {
        ...
        for (int32_t block = 0; block < numBlocks; block++) {
            ...
            for (int32_t offset = 0; offset < (tensorSize / numBlocks);
                 offset++) {
            ...
            for (int tid = 0; tid < threadsPerWarp; ++tid) {
                for (int idx = 0; idx < numElementsPerThreads; ++idx) {
                    ...
                    if (!value.empty())
                        value += "|";
                    int padding =
                        numCharacterPadding(blockId, numBlocks) +
                    ...
                    value += "T" +
                             std::to_string(tid + warpId * threadsPerWarp) +
                             ":" + std::to_string(idx);
                    ...
                    for (int i = 0; i < outputs.size(); i++) {
                        if (i > 0)
                            threadInfo += ",";
                        threadInfo += paddedString(outputs[i].second,
...
struct MemDescModel
    ...
    int64_t getElementTypeBitWidth(Type pointer) const {
        return cast<MemDescType>(pointer)
            .getElementType()
...
int triton::gpu::lookupThreadsPerWarp(OpBuilder &rewriter) {
    ...
    Operation *op = rewriter.getInsertionBlock()
                        ->getParentOp()

...
struct CanonicalizeConvertFromHistogram
    ...
    matchAndRewrite(triton::HistogramOp op,
        ...
        // If mask is present, convert the layout of mask to match new src
        // layout
        ...
        if (mask) {
            ...
            mask =
                rewriter.create<ConvertLayoutOp>(op.getLoc(), sharedType, mask);
        }
...
struct CanonicalizeConvertFromGatherSource : public OpRewritePattern<GatherOp> {
    ...
    matchAndRewrite(GatherOp op, PatternRewriter &rewriter) const override {
        ...
        rewriter.replaceOpWithNewOp<GatherOp>(op, convert.getSrc(),
                                              op.getIndices(), op.getAxis());
        return success();
    }
};
...
struct CanonicalizeConvertFromSplit
    ...
    matchAndRewrite(triton::SplitOp op,
        ...
        // layout of the convert gives the same destination layout we can skip
        // the convert.
...
struct CanonicalizeConvertFromConvert
    ...
    matchAndRewrite(ConvertLayoutOp op,
        ...
        if (auto sharedLoad = dyn_cast<LocalLoadOp>(arg)) {
            ...
            // into it. We insert at the point of the original op as there could
            // be ops with memory side-effects between the LocalLoad op and the
            // ConvertLayout op
...
MemDescTransOp::inferReturnTypes(MLIRContext *context,
    ...
    inferredReturnTypes.push_back(MemDescType::get(
        retShape, retEltTy, retEncoding, argTy.getMemorySpace(),
...
LogicalResult MemDescReshapeOp::inferReturnTypes(
    ...
    if (Attribute srcEnc = srcTy.getEncoding()) {
        if (failed(inferMemDescReshapeOpEncoding(srcTy.getShape(), srcEnc,
                                                 dstShape, dstEncoding)))
            return failure();
    }

    SmallVector<int64_t> dstAllocShape =
        to_vector(srcTy.getAllocShape().take_front(
            srcTy.getAllocShape().size() - srcTy.getShape().size()));
...
LogicalResult verifyAllocOp(Operation *op, Value src, MemDescType dstTy) {
    ...
    return verifyMemoryOpTypes(op, cast<RankedTensorType>(src.getType()),
                               dstTy);
}
...
static LogicalResult verifySharedMemoryRank(Operation *op,
    ...
    if (!enc)
        return op->emitOpError(
            "expected memdesc to have a shared memory encoding");
...
LogicalResult MemDescIndexOp::verify() {
    ...
    if (srcTy.getAllocShape().size() != srcTy.getRank()) {
        return emitError(
            "We don't allow taking memdesc_index of a memdesc_index");
    }
    ...
    if (isa<triton::nvidia_gpu::TensorMemoryEncodingAttr>(srcEnc)) {
        // We support only 3D -> 2D subviews with only first offset being
        // non-zero.
...
LogicalResult MemDescSubsliceOp::verify() {
    ...
    if (!isa<SharedEncodingTrait>(srcEnc) ||
        !isa<SharedEncodingTrait>(dstEnc)) {
    ...
    for (auto [dim, offset] : llvm::enumerate(offsets)) {
        if (!splitDims.contains(dim)) {
            if (offset != 0) {
                return emitError(
                    "A non zero offset found in a dimension that is "
                    "not being split");
            }
        } else {
    ...
    for (auto dim : splitDims) {
        ...
        for (int dimSize = dstTy.getDimSize(dim);
             dimSize < srcTy.getDimSize(dim); dimSize *= 2) {
...
LogicalResult WarpSpecializeOp::verify() {
    ...
    if (!isa<WarpSpecializePartitionsOp>(
            getPartitionOpHolder().front().front())) {
        return emitOpError("expected to find only a "
                           "`ttg.warp_specialize.partitions` op inside "
                           "its second region");
    }
    ...
    for (auto [i, region] : llvm::enumerate(getPartitionRegions())) {
        ...
        for (auto [argIdx, argType, capType] :
             llvm::enumerate(region->getArgumentTypes(),
                             getExplicitCaptures().getTypes())) {
            if (argType == capType)
                continue;
            return emitOpError("partition region #")
    ...
    if (unusedResults.any()) {
        for (Block &block : op.getDefaultRegion()) {
            if (auto yield = dyn_cast<WarpYieldOp>(block.getTerminator())) {
                b.modifyOpInPlace(yield,
                                  [&] { yield->eraseOperands(unusedResults); });
            }
        }
...
ParseResult WarpSpecializeOp::parse(OpAsmParser &p, OperationState &result) {
    ...
    while (succeeded(p.parseOptionalKeyword(
        ...
        if (p.parseArgumentList(partitionArgs, AsmParser::Delimiter::Paren,
            ...
            p.parseInteger(partitionNumWarps.emplace_back()) ||
            p.parseRParen() ||
    ...
    for (auto [i, result, type] :
        ...
        if (result != type) {
            return emitOpError("operand #")
                   << i << " has type " << type << " but parent op expected "
                   << result;
        }
    }
    return success();

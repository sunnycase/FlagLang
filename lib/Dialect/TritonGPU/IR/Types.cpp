...
Type MemDescType::parse(AsmParser &parser) {
    ...
    if (failed(parser.parseComma()) ||
        failed(parser.parseAttribute(memorySpace)))
        return Type();
...
LogicalResult MemDescType::verify(function_ref<InFlightDiagnostic()> emitError,
    ...
    // Every dimension but the first (to allow for pipelining) must be a power
    // of
    // 2
    ...
    if (auto enc = dyn_cast<nvidia_gpu::TensorMemoryEncodingAttr>(encoding)) {
        ...
        if (!enc.getUnpacked() && bitwidth > 16) {
            return emitError()
                   << "bitwidth must be <= 16 for packed tensor memory";
        }
        if (enc.getUnpacked() && (16 != bitwidth && 32 != bitwidth)) {
            return emitError() << "bitwidth must be either 16 or 32 for "
                                  "unpacked tensor memory";
        }
        ...
        if (memorySpace != SharedMemorySpaceAttr::get(ctx)) {
            return emitError() << "memorySpace must be SharedMemorySpace for "
                                  "shared encoding. "
                               << "Got " << memorySpace;
        }

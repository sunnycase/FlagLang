...
LinearLayout combineCtaCgaWithShape(LinearLayout ctaLayout,
    ...
    for (auto dim : ctaLayout.getOutDimNames()) {
        ctaShape[dim] = std::max(int64_t{1}, labeledShape[dim] /
                                                 cgaLayout.getOutDimSize(dim));
    }
...
LinearLayout swizzledSharedToLinearLayout(ArrayRef<int64_t> shape,
    ...
    for (int row = 1; row < numRows; row *= 2) {
        ...
        bases2D.push_back(
            {row, (vec * ((row / perPhase) % maxPhase)) % numCols});
    }
...
LinearLayout getCoreMatrixLinearLayout(NVMMASharedEncodingAttr shared,
    ...
    for (int col = 1; col < tileCols; col *= 2) {
        if (isFp4Padded) {
            // Each group of 16 offsets consists of 8 "real" and 8 "padded"
            ...
            // the inverse of this LL, the offsets correspoding to the real ones
            // are picked in the image by invertAndCompose.
...
LinearLayout nvmmaSharedToLinearLayout(ArrayRef<int64_t> shape,
    ...
    if (shared.getSwizzlingByteWidth() == 0) {
        ...
        LinearLayout layout = LinearLayout::identity1D(
            tmaShape[rank - 1], kOffset, outDimNames[rank - 1]);
        for (int i = rank - 2; i >= 0; --i) {
            layout *=
                LinearLayout::identity1D(tmaShape[i], kOffset, outDimNames[i]);
        }
    ...
    // Collapse all the outer dim into one. We will then create a layout for
    // this shape and reshape it to the original shape.
    ...
    if (shared.getTransposed()) {
        ...
        // TODO: we should move back to using `order` instead of transposed to
        // make the order more explicit.
...
static LinearLayout broadcastedDotOperandLayout(MLIRContext *ctx,
    ...
    // As such, choosing the order of A {1, 0}, gives us the correct
    // broadcasting Same happens if the warpOrder is {0, 1}, like in Hopper
    for (auto d : order) {
        if (d == kDim) {
            layout *= LinearLayout::zeros1D(shape[d], inDimName, dimNames[d]);
        } else {
            layout *=
                LinearLayout::identity1D(shape[d], inDimName, dimNames[d]);
        }
    }
    return layout;
}
...
AMDMfmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
    ...
    if (!isTransposed) {
        ...
        // exceeds the mDim, duplicate elements across lanes - this can happen
        // for 4x4 output.
        LinearLayout lanes =
    ...
    // combineCtaCgaWithShape function to extend it to the whole tensor, we take
    // a different approach. Suppose tilesPerWarp is 2x2—meaning a warp computes
    // a 2x2 block of MFMA tiles. If we define the layout only on the CTA tile
    // and extend it across the tensor, the resulting tile order won’t be
    // N-contiguous (i.e., row-major). Due to the 2x2 shape, the third tile
    // would fall in the M dimension. While defining the layout per CTA tile
    // might seem more intuitive, the current dot op lowering assumes an
    // N-contiguous ordering of MFMA tiles across the entire tensor. In other
    // words, the lowering logic isn't layout-aware, it only supports a fixed
    // N-contiguous MFMA tile ordering. Supporting other orderings would require
    // extending the dot lowering implementation. For now, we conform to the
    // current lowering algorithm by defining the MFMA linear layout globally,
    // with N-contiguous tiles across the tensor and across CTA tile boundaries.
    ...
    // the layout along the remaining N dimension, and only then proceed along
    // M, following the tilesPerWarp configuration. If the N dimension is not
    // large enough to span multiple CTA tiles (i.e., the first argument is 0),
    // an empty layout is created, so this identity layout will not introduce
    // any new registers.
    ...
    if (hasBatchDim) {
        ...
        tileLayout *=
            LinearLayout::identity1D(1, kRegister, outDimNames[order[2]]);
...
LinearLayout chooseDotDsReadB64TrLayout(DotOperandEncodingAttr dotMfmaLayout,
    ...
    auto mfmaLayout =
        llvm::cast<AMDMfmaEncodingAttr>(dotMfmaLayout.getParent());
    ...
    SmallVector<unsigned> order = getOrderForDotOperand(
        dotMfmaLayout.getOpIdx(), rank, /*kContig*/ false);

    std::vector<std::vector<int32_t>> registerBase;
    std::vector<std::vector<int32_t>> laneBase;
    auto populateFP4LL = [&registerBase, &laneBase](int kSize, int mDim) {
        const bool isMfma32 = (mDim == 32);
        // ds_read_b64_tr4 operates on FP4 values swapping the packing of them.
        // Look at i8 values for the ownership of register/lane since it's the
        // data type of the tensor. Register dimension: what i8 in the tile are
        // held by thread 0? Lane dimension: what i8 in the tile are held in
        // register 0 of each thread?
    ...
    auto populateLL = [&registerBase, &laneBase](int elemBitWidth, int kSize,
        ...
        // For ds_read_b64_tr_* instructions, each thread accesses 64 bits (8
        // bytes) of data. The smallest unit for transposition is a [non-K, K] =
        // {16, kWidthTransRead} sub-tile of elements, where each thread reads
        // kWidthTransRead elements along the non-K dimension. Due to the
        // transposition mechanism, each thread ends up with kWidthTransRead
        // elements along the K dimension.
        //
        // The MFMA selection logic prioritizes double-rate MFMA instructions
        // whenever possible:
        //
        // - For MFMA operations where M = N = 16, when blockK > k,
        // mfma16x16x2*k
        //   is selected; otherwise (blockK ≤ k), mfma16x16xk remains the
        //   choice.
        //
        // - For MFMA operations where M = N = 32, when blockK > k,
        // mfma32x32x2*k is
        ...
        //
        // In "double-rate" MFMA instructions, each thread holds
        // 2*kWidthTransRead elements along the K dimension:
        ...
        // for one MFMA instruction. The shape of these tiles depends on the
        ...
        if (isMfma32) {
            laneBaseExt = {{16, 0},
                           {0, numSubtilesPerTile * threadsPerSubtileK}};
        } else {
...
LinearLayout mfmaDotToLinearLayout(DotOperandEncodingAttr dotMfmaLayout,
    ...
    if (kSize > kTileSize) {
        tileLayout *=
            LinearLayout::identity1D(kSize / kTileSize, kRegister, dimK);
    }
...
AMDWmmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
    ...
    if (hasBatchDim) {
        ...
        tileLayout *=
            LinearLayout::identity1D(1, kLane, outDimNames[batchIndex]);
    }

    // And each warp takes the same register and lane sub-layout. So multiply
    // with an identity layout for the warp.
...
LinearLayout wmmaDotOperandToLinearLayout(DotOperandEncodingAttr dotWmmaLayout,
                                          ArrayRef<int64_t> shape) {
    auto wmmaLayout =
        llvm::cast<AMDWmmaEncodingAttr>(dotWmmaLayout.getParent());
    auto rank = shape.size();
    bool hasBatchDim = rank == 3;
    ...
    case 1:
        ...
        break;
    case 2:
        // WMMA version 2 offset values in lanes 0-15 and 16-31 across k
        // dimensions
    ...
    if (hasBatchDim) {
        ...
        tileLayout *=
            LinearLayout::identity1D(1, kLane, outDimNames[laneOrder[2]]);
    }
    ...
    LinearLayout warpLayout = broadcastedDotOperandLayout(
        ctx, warpsPerCTA, warpOrder, kDim, S("warp"));
...
LinearLayout fmaDotToLinearLayout(DotOperandEncodingAttr operandLayout,
    ...
    auto lanesLayout = broadcastedDotOperandLayout(ctx, threadShape,
                                                   threadOrder, kDimIdx, kLane);
    auto warpsLayout =
    ...
    return combineCtaCgaWithShape(ctaLayout, getCTALayout(operandLayout),
                                  shape);
}
...
NvidiaMmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
    ...
    auto ctaLayout =
        nvidiaMmaTile(ctx, tileShape, kWidth, order, getRepOrder());
...
LinearLayout nvidiaDotToLinearLayout(ArrayRef<int64_t> shape,
    ...
    ctaLayout *=
        broadcastedDotOperandLayout(ctx, mma.getWarpsPerCTA(), warpOrder, kDim,
                                    S("warp"))
            .transposeOuts(llvm::to_vector(ctaLayout.getOutDimNames()));
...
LinearLayout SliceEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
    ...
    //  2. Compute linearSlice = parent.compose(transform).  Now linearSlice
    //  maps
    ...
    for (auto [idx, outDim] : llvm::enumerate(parentLL.getOutDimNames())) {
        if (idx == getDim()) {
            // Because we're multiplying by all zeros, we could replace
            // outDimNames[0] with any other valid out-dim; the layout will be
            // the same.
...
LinearLayout tensorMemoryToLinearLayout(ArrayRef<int64_t> shape,
    ...
    if (encoding.getCTASplitN() > 1) {
        ...
        auto newEncoding = TensorMemoryEncodingAttr::get(
            ...
            encoding.getUnpacked(), encoding.getCTASplitM(), 1);
        return tensorMemoryToLinearLayout(
                   {shape[0], shape[1] / encoding.getCTASplitN()},
                   newEncoding) *
               split;
    }
    if (encoding.getCTASplitM() > 1) {
        ...
        auto newEncoding = TensorMemoryEncodingAttr::get(
            ctx, encoding.getBlockM(), encoding.getBlockN(),
            encoding.getUnpacked(), 1, encoding.getCTASplitN());
        return tensorMemoryToLinearLayout(
                   {shape[0] / encoding.getCTASplitM(), shape[1]},
                   newEncoding) *
               split;
    }
...
tensorMemoryScalesToLinearLayout(ArrayRef<int64_t> shape,
    ...
    auto tile =
        ...
        // reps
        LinearLayout::identity1D(std::max<int>(1, shape[0] / 32), kCol,
                                 dims[0]) *
...
LinearLayout TritonGPUDialect::toLinearLayout(ArrayRef<int64_t> shape,
    ...
    if (auto distributed = dyn_cast<DistributedEncodingTrait>(layout)) {
        ...
        if (auto shared = dyn_cast<SwizzledSharedEncodingAttr>(layout)) {
            ...
            result = tensorMemoryScalesToLinearLayout(
                shape, tensorMemoryScalesEncoding);
        } else {
...
LinearLayout chooseScaledMfmaScaleLayout(MLIRContext *ctx, int dotOperandIdx,
    ...
    // Fetch the tilesPerWarp value in the M dimension for operand A, or in the
    // N dimension for operand B.
    ...
    auto ctaLay =
        CTALayoutAttr::get(/*context=*/ctx, /*CTAsPerCGA=*/{1, 1},
...
chooseMfmaLikeStoreLayout(RankedTensorType valType) {
    ...
    // now, it is only possible for FA-like kernels since during mfma
    // generation, the WarpsPerCTA of the head dot in the chain will be reshaped
    // to [numWaprs, 1].
...
LinearLayout getScaleTMEMStoreLinearLayout(RankedTensorType scaleType,
    ...
    // Pick a layout that will be trivial to store into the following TMEM
    // layout:
    // https://docs.nvidia.com/cuda/parallel-thread-execution/#tcgen05-mma-scale-factor-a-layout-1x
    ...
    // If we have 8 warps distribute the last dimension on the second warp
    // group.
    if (numWarps == 8) {
    ...
    auto regLanes = LinearLayout(
        {{kRegister, regBase}, {kLane, laneBase}, {kWarp, warpBase}},
...
getTmemLoadStoreLayout16x256(int M, int N, RankedTensorType oldType,
    ...
    // in getTmemLoadStoreLayout32x32b. This allows us to use the same lowering
    // to tmem for load and store. This part could be generalized by making the
    ...
    if (maxRegN / nBase > 1) {
        innerTile = innerTile * LinearLayout::identity1D(
                                    maxRegN / nBase, kRegister, outDimNames[1]);
    }
    if (M != 64) {
        innerTile =
            innerTile * LinearLayout::identity1D(2, kRegister, outDimNames[0]);
    ...
    if (numMRegDim > 1) {
        innerTile = innerTile * LinearLayout::identity1D(numMRegDim, kRegister,
                                                         outDimNames[0]);
    }
    ...
    if (distributeMAlongWarps) {
        innerTile =
            innerTile * LinearLayout::identity1D(2, kWarp, outDimNames[0]);
        nextDim <<= 1;
    }
    ...
    if (numNRegDim > 1) {
        innerTile = innerTile * LinearLayout::identity1D(numNRegDim, kRegister,
                                                         outDimNames[1]);
    }
    if (distributeNAlongWarps) {
        innerTile =
            innerTile * LinearLayout::identity1D(2, kWarp, outDimNames[1]);
    }

...
class LayoutPropagation {
  public:
    ...
    // Set the encoding to all the values and fill out the values with new
    // layout in `changed`.
...
class LayoutRematerialization {
  public:
    ...
    // TODO: Merge the three hoistConvert*(); functions as they are duplicate
    // code
    ...
    void rewriteSlice(SetVector<Value> &slice,
  ...
  private:
    ...
    // Existing tuples of (value, layout) that needs to be updated when
    // recreating scf ops. This prevents keeping track of Values that have been
    // delete when rewriting slices.
...
void LayoutPropagation::initAnchorLayout() {
    ...
    // -- you can pass a tensor with an encoding as an arg, instead of
    // explicitly calling tt.load.
    ...
    for (Value value : values) {
        ...
        for (auto encoding : info.encodings) {
            ...
            if (isa<ConvertLayoutOp>(op)) {
                // Try to remove the convert by making the dst encoding match
                // the source encoding.
...
SmallVector<Value> LayoutPropagation::propagateToUsers(Value value,
    ...
    for (OpOperand &use : value.getUses()) {
        ...
        if (auto gatherOp = dyn_cast<GatherOp>(user)) {
            // Propagate the layout through the indices only, and if the layout
            // does not have an efficient layout set.
    ...
    while (!queue.empty()) {
        ...
        LLVM_DEBUG({
            DBGS() << "propagateLayout considering " << currentValue
                   << ", which has " << info.encodings.size()
                   << " candidate encoding(s):\n";
...
void LayoutPropagation::map(Value old, Value newV) {
    rewriteMapping[{
        old, cast<RankedTensorType>(newV.getType()).getEncoding()}] = newV;
}
...
Value LayoutPropagation::getValueAs(Value value, Attribute encoding) {
    if (auto tensorType = dyn_cast<RankedTensorType>(value.getType())) {
        ...
        Value converted = rewriter.create<ConvertLayoutOp>(
...
Operation *LayoutPropagation::cloneElementwise(OpBuilder &rewriter,
    ...
    if (op->getNumOperands() > 0) {
        for (auto operand : op->getOperands()) {
            auto ty = dyn_cast<RankedTensorType>(
                getRewrittenValue(operand).getType());
            if (!ty)
                continue;
...
Operation *LayoutPropagation::rewriteForOp(scf::ForOp forOp) {
    ...
    for (auto [oldArg, newArg] :
         llvm::zip(forOp.getBody()->getArguments(),
...
Operation *LayoutPropagation::rewriteWhileOp(scf::WhileOp whileOp) {
    ...
    for (auto [operand, arg] :
        ...
        if (layouts.count(arg))
            convertedOperand =
                getValueAs(operand, *layouts[arg].encodings.begin());
        operands.push_back(convertedOperand);
    }
    ...
    rewriter.createBlock(&newWhileOp.getBefore(), {}, argsTypesBefore,
                         bbArgLocsBefore);
    rewriter.createBlock(&newWhileOp.getAfter(), {}, returnTypes,
                         bbArgLocsAfter);
    ...
    for (auto [oldArg, newArg] : llvm::zip(whileOp.getBeforeArguments(),
...
void LayoutPropagation::rewriteYieldOp(scf::YieldOp yieldOp) {
    ...
    for (OpOperand &operand : yieldOp->getOpOperands()) {
        ...
        if (isa<scf::ForOp, scf::IfOp>(parentOp))
            yieldType =
                parentOp->getResult(operand.getOperandNumber()).getType();
        if (auto whileOp = dyn_cast<scf::WhileOp>(parentOp))
            yieldType = whileOp.getBeforeArguments()[operand.getOperandNumber()]
                            .getType();
...
void LayoutPropagation::rewriteConditionOp(scf::ConditionOp conditionOp) {
    ...
    for (unsigned i = 1; i < conditionOp->getNumOperands(); ++i) {
        ...
        Type argType =
            whileOp->getResult(operand.getOperandNumber() - 1).getType();
...
Operation *LayoutPropagation::rewriteOp(Operation *op) {
    ...
    if (op->hasTrait<OpTrait::SameOperandsAndResultEncoding>() ||
        ...
        isa<ReduceOp, ExpandDimsOp, ReshapeOp, TransOp, JoinOp, SplitOp,
            GatherOp, ConvertLayoutOp, nvidia_gpu::WarpGroupDotWaitOp>(op)) {
...
void LayoutRematerialization::updateRematMapping(
    ...
    for (auto [old, newV] : values) {
        ...
        if (it != mappedValues.end()) {
            ...
            // Loop through the replacement value to find the new version of
            // remat value. This should be okay as the number of values should
            // be small.
    ...
    for (Value v : slice) {
        ...
        if (v.getDefiningOp()) {
            ...
            if (auto loopOp = cast<LoopLikeOpInterface>(parentOp)) {
                ...
                yieldOperandsMap[yieldOp].push_back(
    ...
    for (Operation *op : opsToRewrite) {
        if (auto forOp = dyn_cast<scf::ForOp>(op)) {
            ...
            for (auto m : argMapping) {
                mapping.map(forOp.getResult(m.first),
        ...
        if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
            ...
            scf::IfOp newIfOp = replaceIfOpWithNewSignature(
                builder, ifOp, newTypes, replacements);
            unsigned oldIdx = 0;
            unsigned newIdx = ifOp.getNumResults();
            for (auto res : ifOp.getResults()) {
                if (slice.count(res)) {
                    // Why can't we use res instead of ifOp.getResult(oldIdx)?
                    mapping.map(ifOp.getResult(oldIdx),
        ...
        if (auto yieldOp = dyn_cast<scf::YieldOp>(op)) {
            ...
            for (int operandIdx : operandsToRewrite) {
                yieldOperands.push_back(
                    mapping.lookup(yieldOp.getOperand(operandIdx)));
            }
        ...
        if (isa<arith::ConstantOp>(op)) {
            ...
            auto newType =
                tensorType.cloneWithEncoding(layout[op->getResult(0)]);
        ...
        for (auto [old, newV] :
            ...
            if (it == layout.end())
                continue;
            auto newType = cast<RankedTensorType>(old.getType())
                               .cloneWithEncoding(it->second);
...
LogicalResult LayoutRematerialization::getConvertBackwardSlice(
    ...
    auto getExistingConversion = [&](OpOperand &value, Attribute encoding) {
        ...
        // FIXME: If the current user is a conversion, then we know it will
        // become a no-op when its operand is replaced with `remat`, but we need
        // to check that its users are all dominated by `remat` so the IR is
        // valid. if (isa<ConvertLayoutOp>(user) && remat.getDefiningOp() &&
...
void LayoutRematerialization::backwardRematerialization() {
    ...
    for (ConvertLayoutOp convertOp : convertOps) {
        ...
        if (!opToDelete.contains(convertOp)) {
            // If the conversion didn't get removed, consider it for reuse in
            // future backward slices.
...
static bool isExpensiveMathOp(Operation *op) {
    ...
    return isa<
        arith::DivFOp, math::ErfcOp, math::SinhOp, math::CoshOp, math::TanhOp,
        math::AsinhOp, math::AcoshOp, math::AtanhOp, math::CtPopOp,
        math::CountLeadingZerosOp, math::CountTrailingZerosOp, math::ExpOp,
        math::Exp2Op, math::ExpM1Op, math::LogOp, math::Log2Op, math::Log10Op,
        math::Log1pOp, math::SinOp, math::CosOp, math::TanOp, math::AsinOp,
        math::AcosOp, math::AtanOp, math::Atan2Op, math::PowFOp, math::SqrtOp,
        math::RsqrtOp, math::ErfOp, math::CbrtOp>(op);
}
    ...
    for (Operation *op : sliceOps) {
        ...
        if (isOpSingleUse(op)) {
            ...
            if (!helper.isAssociative()) {
                ...
                // multiple use chain.
                LDBG("  skipped rematerialization due to non-associative "
                     "reduce in the "
                     "slice");
                return;
            }
            ...
            rematerialisationCost +=
                8 * helper.getInterWarpSizeWithUniqueData();
        }
    }
...
void LayoutRematerialization::hoistConvertDotOperand(
    ...
    auto canBePipelined = [&](ConvertLayoutOp convertOp) {
        ...
        parent->walk([&](Operation *op) {
            ...
            if (!opType)
                return;
            auto dotEnc =
                dyn_cast<DotOperandEncodingAttr>(opType.getEncoding());
            if (!dotEnc)
                return;
    ...
    LogicalResult result =
        getConvertBackwardSlice(convertOp.getSrcMutable(),
                                targetType.getEncoding(), slice, layout, stop);
    if (result.failed())
        return;
    ...
    for (Value v : slice) {
        ...
        if (!isa<LoadOp, DescriptorLoadOp>(v.getDefiningOp())) {
            ...
            if (isa<arith::ConstantOp>(op) || noDataMovement(op)) {
                ...
                LLVM_DEBUG({
                    DBGS() << "  Leaves must be Load, DescriptorLoad or "
                              "Constant. Got "
                           << v << "\n";
                });
                return;
...
void LayoutRematerialization::hoistConvertOnTopOfExtOrBroadcast(
    ...
    auto isExtOrBroadcastOp = [](Operation *op) {
        ...
        if (auto fpToFpOp = dyn_cast<FpToFpOp>(op)) {
            auto srcType =
                cast<RankedTensorType>(fpToFpOp.getOperand().getType());
            return getElementBitWidth(srcType) <
                   getElementBitWidth(
                       cast<RankedTensorType>(fpToFpOp.getType()));
        }
        return false;
    };
    ...
    for (unsigned i = 0; i < sliceSize; i++) {
        ...
        if (isExtOrBroadcastOp(op)) {
            ...
            // If we can rematerialize the rest of the ext slice we can ignore
            // this ext as it won't need a convert.
            if (result.succeeded()) {
                slice.insert(tempSlice.begin(), tempSlice.end());
                layout.insert(tempLayout.begin(), tempLayout.end());
                continue;
            }
            // Only apply it if there is a single ext op otherwise we would have
            // to duplicate the convert.
...
void LayoutRematerialization::hoistConvertIntoConditionals(
    ...
    for (unsigned i = 0; i != slice.size(); ++i) {
        ...
        // If propagation across both edges of this conditional succeeded, then
        // we don't need to hoist across it. Merge into the current slice.
        ...
        // Only hoist into conditionals inside loops. The assumption is that an
        // if inside a loop executes fewer than the total number of loop
        // iterations, making this hoist profitable.
        if (!isa<scf::ForOp>(ifOp->getParentOp())) {
            terminals.push_back(cast<OpResult>(v));
            continue;
        }

        // The layout conversion can be rematerialized along one edge but not
        // the other. We can hoist the conversion into the other branch. Push
        // this into the subslice list for analysis.
    ...
    auto hoistRemat = [&](OpBuilder &b, Value v, Attribute encoding) {
        ...
        auto newType = tensorType.cloneWithEncoding(encoding);
        Value newCvt =
            b.create<ConvertLayoutOp>(convertOp.getLoc(), newType, v);
...
class TritonGPURemoveLayoutConversionsPass
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        // 4. Apply clean up patterns to remove remove dead convert and dead
        // code generated by the previous transformations.

        ...
        if (isa<RankedTensorType>(curr.getType())) {
            auto retType = RankedTensorType::get(
                ...
                cast<RankedTensorType>(
                    curr.getDefiningOp()->getOperand(0).getType())
                    .getEncoding());
...
LogicalResult Prefetcher::initialize() {
    ...
    for (triton::DotOp dot : dotsInFor) {
        ...
        auto aEnc = mlir::cast<triton::gpu::DotOperandEncodingAttr>(
            aType.getEncoding());
        auto bEnc = mlir::cast<triton::gpu::DotOperandEncodingAttr>(
            bType.getEncoding());
...
scf::ForOp Prefetcher::createNewForOp() {
    ...
    for (Operation &op : forOp.getBody()->without_terminator()) {
        ...
        if (dot && dots.contains(dot)) {
            ...
            int64_t kRem =
                dot.getA().getType().getShape().back() - prefetchWidth;
            Operation *prevDot = firstDot;
            if (kRem == 0) {
                // There is only one dot while prefetchWidth == kSize so delay
                // issuing it. Meanwhile, newOp should be set to firstDot to
                // make sure the dot result is updated to yield.
            ...
            while (kRem != 0) {
                ...
                builder.setInsertionPoint(prevDot);
                Value aRem =
                    generatePrefetch(mapping.lookup(dot2aLoopArg[dot]), 0,
                                     false, dotEncoding, builder, kOff, kShape);
                cloneElementwiseOps(aRem, dot2aVals[dot], builder);
                Value bRem =
                    generatePrefetch(mapping.lookup(dot2bLoopArg[dot]), 1,
                                     false, dotEncoding, builder, kOff, kShape);
                ...
                kRem -= kShape;
                if (kRem == 0) {
                    // We want to delay issuing the last dot as long as
                    // possible, ideally until after the prefetch.  To
                    // accomplish this, set the insertion point above the dot.
                    // If we find anything dependent on the dot (at the top of
                    // this loop), we resume inserting after it.
                    builder.setInsertionPoint(prevDot);
                }
            }
        }
    ...
    for (triton::DotOp dot : dots) {
        Attribute dotEncoding = dot.getType().getEncoding();
        Value aToYield = generatePrefetch(mapping.lookup(dot2aYield[dot]), 0,
                                          true, dotEncoding, builder);
        cloneElementwiseOps(aToYield, dot2aVals[dot], builder);
        yieldValues.push_back(aToYield);
        // bToYield
        Value bToYield = generatePrefetch(mapping.lookup(dot2bYield[dot]), 1,
                                          true, dotEncoding, builder);

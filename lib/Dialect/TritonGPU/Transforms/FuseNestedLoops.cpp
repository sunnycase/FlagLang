...
struct LoopNest {
    ...
    // outermost loop is contained within an if, its preconditions relative to
    // the loop nest are empty.
    LoopNestNode *root;
};
} // namespace
...
static void constructLoopNest(LoopNestNode *parent, LoopNest &nest,
    ...
    parent->loop->walk<mlir::WalkOrder::PreOrder>([&](Operation *op) {
        ...
        // If the traversal encounters any other operation with regions, restart
        // the traversal and construct new loop nests. This means ops like
        // `scf.while` divide the analysis domain, but it also means loop fusion
        // won't "see" across `scf.if`, for example.
...
struct Logue {
    ...
    // Replace all uses of the logue results with the given values, where
    // `logue` comprises all the ops in `containingRegion`.
    void replaceAllUsesWith(ValueRange values, Region &containingRegion) {
        for (auto [newOut, output] : llvm::zip(values, outputs)) {
            // Replace uses of the prologue outputs that are not in the
            // prologue, i.e. inside the `then` region where it got spliced.
            output.replaceUsesWithIf(newOut, [&](OpOperand &use) {
                return !containingRegion.isAncestor(
    ...
    // The outputs of the logue. These are the SSA value results of `ops` that
    // are used by ops outside of `ops`.
...
static void fuseOneLevel(LoopNestNode *parent, mlir::DominanceInfo &domInfo) {
    ...
    for (LoopNestNode *child : parent->children) {
        ...
        // Check if the inner loop bounds are or can be made invariant to the
        // outer loop. Check them all at once to avoid adding ops to `toHoist`
        // if not necessary.
    ...
    // children of the current loop node. Child loops that cannot be fused are
    // now treated opaquely by the rest of the analysis. This allows partial
    // fusing of the constructed loop nest.
    ...
    Value totalIters = b.create<arith::MulIOp>(
        castIntIfNecessary(b, lenOuter, intTy), innerLen);

    // The outputs of the prologue, each epilogue, and all inner loop bodies
    // need to carried through the fused loop.
    ...
    ArrayRef<BlockArgument> ivars =
        fused.getRegionIterArgs().slice(ivarStartIdx);
    auto bodyOutsIt =
    ...
    for (unsigned k = 0; k <= N; ++k) {
        ...
        // The `scf.if` outputs will be `jk` and the outputs of prologuek. We
        // also have to initialize the inner loop iter args.
        ...
        // Yield the initialized jk, the prologue outputs, and the initial
        // values of the inner loop.
        ...
        // In the `else` region, just yield the last values of jk, the outputs,
        // and the iter args.
        ...
        Value lt =
            b.create<arith::CmpIOp>(arith::CmpIPredicate::slt, T, innerEndT);
        Value bodyCond = b.create<arith::AndIOp>(ge, lt);

        // The outputs will be the outputs of the inner loop body and the next
        // jk.
        ...
        bodyForwardedOuts.append(bodyOutsIt,
                                 bodyOutsIt + inner.getNumResults());
        bodyOutsIt += inner->getNumResults();
        b.create<scf::YieldOp>(bodyForwardedOuts);

        // Now we can replace the results of the inner loop with the outputs of
        // the body if.
        inner.replaceAllUsesWith(
            bodyIf.getResults().slice(1, inner.getNumResults()));

        // If the inner loop must execute, then its body does not have to be
    ...
    for (auto [logueIf, logue] :
         llvm::zip(prologueIfs, llvm::drop_end(logues))) {
        llvm::append_range(
            outerOuts, logueIf.getResults().slice(1, logue.getNumOutputs()));
    }
    ...
    // inner loop iter args to the outer loop when possible, and then placing
    // the reset of these values in the epilogue.
    ...
    auto fusedYieldIt =
        getYield(fused.getBodyRegion())->getOpOperands().begin() +
        innerOutsStartIdx;
    ...
    if (!yieldsToUpdate.empty()) {
        ...
        MutableOperandRange(getYield(epilogueIf.getElseRegion()))
            .append(forwarded);
        b.setInsertionPoint(epilogueIf);
        TypeRange newTypes =
            getYield(epilogueIf.getThenRegion()).getOperandTypes();
    ...
    // Propagate the `tt.disallow_acc_multi_buffer` attribute to the parent
    // loop.
    ...
    // Update the parent's loop to the fused loop. Set the new stage count to
    // the max stage count of the inner loops.
    int numStages = 1;
    if (auto stageAttr = outer->getAttrOfType<IntegerAttr>(kNumStagesAttrName))
        numStages = stageAttr.getInt();
    for (scf::ForOp loop : innerLoops) {
        if (auto stageAttr =
                loop->getAttrOfType<IntegerAttr>(kNumStagesAttrName))
...
static void optimizeEpilogueDependencies(scf::ForOp outerLoop,
    ...
    auto inEpilogue = [&](Operation *op) {
        return domInfo.properlyDominates(innerLoop, op,
                                         /*enclosingOpOk=*/false);
    };
...
static LogicalResult speculateInnerLoopLength(scf::ForOp outerLoop,
    ...
    // In the `then` branch, the inner loop does not execute. Clone the loop
    // nest into it and remove the inner loop.
    mlir::IRMapping map;

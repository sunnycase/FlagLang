...
struct CoalescePass : public impl::TritonGPUCoalesceBase<CoalescePass> {
    ...
    setCoalescedEncoding(ModuleAxisInfoAnalysis &axisInfoAnalysis,
        ...
        if (ptr.getDefiningOp()) {
            for (Operation *use : mlir::multiRootGetSlice(op)) {
                ...
                if (!val || !matchesShape(val) ||
                    memAccessesSameOrder.contains(use))
                    continue;
                auto currOrder =
        ...
        perThread =
            std::min<int>(perThread, std::max(numElems / numThreads, 1));
        LDBG("perThread: " << perThread);

        if (!dyn_cast<triton::LoadOp>(op)) {
            // For ops that can result in a global memory write, we should
            // enforce that each thread handles at most 128 bits, which is the
            // widest available vectorized store op; otherwise, the store will
            // have "gaps" in the memory write at the warp level, resulting in
            // worse performance. For loads, we can expect that the gaps won't
            // matter due to the L1 cache.
    ...
    void coalesceOp(Attribute encoding, Operation *op) {
        ...
        for (auto operand : op->getOperands()) {
            ...
            if (tensorType && !isa<triton::gpu::SharedEncodingTrait>(
    ...
    void runOnOperation() override {
        ...
        moduleOp.walk([&](Operation *curr) {
            ...
            setCoalescedEncoding(axisInfoAnalysis, curr, numWarps,

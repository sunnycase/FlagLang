        ...
        for (auto user : users) {
            if (auto load = dyn_cast<triton::nvidia_gpu::TMEMLoadOp>(user)) {
                tmemLoad = load;
                           ...
                           dyn_cast<triton::nvidia_gpu::MMAv5OpInterface>(
                               user)) {
                mmaOp = mma;
            }
        }
...
std::pair<Value, Operation *> getAccumulatorUseAndDef(Operation *op) {
    ...
    if (auto tc05MmaOp = dyn_cast<triton::nvidia_gpu::MMAv5OpInterface>(op)) {
        ...
        auto tmemAlloc =
            accVal.getDefiningOp<triton::nvidia_gpu::TMEMAllocOp>();
        if (!tmemAlloc ||
...
findZeroInitOp(Value accUse, scf::ForOp forOp, bool &loopArgIsZero) {
    ...
    if (auto arg = dyn_cast<BlockArgument>(v)) {
        ...
        v = forOp.getBody()->getTerminator()->getOperand(arg.getArgNumber() -
                                                         1);
    }
    ...
    if (auto ifOp = dyn_cast<scf::IfOp>(defOp)) {
        ...
        if (isConstantZeroTensor(thenVal) || isConstantZeroTensor(elseVal)) {
            // Make sure that the other value is not defined in the if itself,
            // but passed from outside
  ...
  public:
    void runOnOperation() override {
        ...
        for (Operation *mmaOp : mmaOps) {
            ...
            Value vTrue = rewriter.create<arith::ConstantOp>(
                loc, rewriter.getBoolAttr(true));
            Value vFalse = rewriter.create<arith::ConstantOp>(
            ...
            if (auto useAccValue = getUseAccFlag(mmaOp)) {
                ...
                if (!useAcc || *useAcc == false) {
                    // Do not run this optimization if there is already a
                    // non-constant flag (this pass has already run), or if this
                    // MMA does not use the accumulator (e.g. the peeled MMA in
                    // the prologue, the first dot in attention)
                    continue;
                }
            }
            ...
            if (zeroInitOp) {
                ...
                if (auto selOp = dyn_cast<arith::SelectOp>(zeroInitOp->first)) {
                    ...
                    zeroValue = isConstantZeroTensor(selOp.getTrueValue())
                                    ...
                                    : selOp.getFalseValue();
                    thenInitsToZero =
                        isConstantZeroTensor(selOp.getTrueValue());
                } else {
                    assert(isa<scf::IfOp>(*zeroInitOp->first) &&
                           "Expected an if op");
                    ...
                    Value elseVal = ifOp.elseYield()->getOperand(resultIndex);
                    oldValue =
                        isConstantZeroTensor(thenVal) ? elseVal : thenVal;
                    zeroValue =
                        isConstantZeroTensor(thenVal) ? thenVal : elseVal;
                ...
                bool zeroingBeforeMMA =
                    zeroInitOp->first->isBeforeInBlock(mmaOp);
                Value prevFlagValue =
                    zeroingBeforeMMA ? loopArgFlagValue : vTrue;
                ...
                setUseAccFlag(mmaOp, zeroingBeforeMMA ? selectFlagOp
                                                      : loopArgFlagValue);
                auto forYield =
                    cast<scf::YieldOp>(forOp.getBody()->getTerminator());
                forYield->insertOperands(
                    forYield->getNumOperands(),

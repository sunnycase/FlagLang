  ...
  public:
    ...
    Operation *getFirstUse(Operation *op) {
        ...
        for (auto user : op->getUsers()) {
            if (Operation *ancestor =
                    op->getBlock()->findAncestorOpInBlock(*user))
                users.push_back(ancestor);
        }
        auto minOpIt = llvm::min_element(
            users, [](mlir::Operation *a, mlir::Operation *b) {
    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::gpu::LocalLoadOp op) {
            auto dstEncoding =
                mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(
            ...
            if (!AOp)
                return;
            // Check that the conversion to OpIdx=1 happens before and can be
            // moved after the conversion to OpIdx=0.

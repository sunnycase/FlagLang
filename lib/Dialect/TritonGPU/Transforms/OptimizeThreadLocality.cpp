...
static LogicalResult setOptimizedGatherLayout(GatherOp op, RewriterBase &b) {
    ...
    // number of) elements, which is `product(srcSizePerThread[i] for i !=
    // axis)`.
    //
    ...
    // warp or the number of warps, `product(srcSizePerThread)` is just a
    // function of the shape.
    //
    // So we want to minimize `idxSizePerThread[axis]`. Note that broadcasting
    // is forbidden in the source tensor but allowed in the index tensor. Choose
    // the smallest value while still ensuring that a warp spans whole columns.
    //
    ...
    //
    // For all i != axis in the source tensor. The same relationship must hold
    // for the index tensor. This means we can't just set
    // `idxSizePerThread[axis]` to 1 and compute the rest from that. Find the
    // smallest value where this relationship is still respected.
    ...
    // Minimize `sizePerThread[axis]` by putting as many theads along the axis
    // as possible, limited to the actual size of the dimension.
    ...
    // handling it. This also means we can use the same layout between the
    // source and index tensors for simplicity.
    ...
    auto ctaLayout = CTALayoutAttr::get(ctx, baseLayout.getCTAsPerCGA(),
                                        baseLayout.getCTASplitNum(),
    ...
    b.modifyOpInPlace(op, [&] {
        ...
        // Mark the layout as optimized on the op to prevent it from being
        // changed.
    ...
    void runOnOperation() override {
        ...
        mod.walk([&](triton::ReduceOp reduce) -> void {
            ...
            if (!(isa<triton::gpu::BlockedEncodingAttr>(srcEncoding) &&
                  rank > 1))
                return;
            // The code currently assumes that the reduction is happening on the
            // most inner dim.
            ...
            // Not worth applying this optimization if there is only one element
            // per thread on the reduction axis
            if (elemsPerThread == 1)
                return;
            if (!reduce->hasOneUse())
                return;
        ...
        for (auto reduce : reduceOps) {
            ...
            auto slice2d = triton::gpu::SliceEncodingAttr::get(mod.getContext(),
                                                               rank, blocked3d);
            // Get forOp
            ...
            auto newAccum = createAccum(builder, reduce, oldAccum,
                                        viewOpTensorShape, slice2d);
            ...
            auto newUpdate =
                createUpdate(builder, newLoop, newReduce, oldUpdate);
            // create new yield
            ...
            // add convert_layout to get back to original layout, the result
            // layout should now match the layout of the old accumulator (%cst)
            ...
            auto finalOp = incorporateOriginalAccumulatorValue(
                builder, oldUpdate, cvtLayout, oldAccum);
            // Replace the old loop user with the final result
            loopUser->setOperand(loopUse.getOperandNumber(),
                                 finalOp->getResult(0));
  ...
  private:
    ...
    Operation *createYield(OpBuilder &builder, scf::ForOp &loop,
        ...
        SmallVector<Value> yieldValues =
            llvm::to_vector(oldYield.getOperands());
    ...
    Operation *createAccum(OpBuilder &builder, triton::ReduceOp reduce,
        ...
        auto elemType =
            cast<RankedTensorType>(oldAccum.getType()).getElementType();
    ...
    getThreadLocalityOptimizedEncoding(triton::ReduceOp reduce) const {
        ...
        auto threadsPerWarp3d =
            insertValue(blocked.getThreadsPerWarp(), rank, 1);
        ...
        auto blocked3d = triton::gpu::BlockedEncodingAttr::get(
            reduce.getContext(), sizePerThread3d, threadsPerWarp3d,
            warsPerCTA3d, order3d, ctaLayout3d);
        return blocked3d;
    }

    template <typename T>
    SmallVector<T> insertValue(ArrayRef<T> vec, unsigned index,
                               int value) const {

            ...
            for (auto *resUser : resVal.getUsers()) {
                if (ifOp->isProperAncestor(resUser)) {
                    if (ifOp.getThenRegion().findAncestorOpInRegion(*resUser) !=
                        ...
                        usersNeedreplaced[std::make_pair(resVal, trueVal)]
                            .push_back(resUser);
                    } else {
                        // The user is inside the ElseRegion of the scf.if.
                        usersNeedreplaced[std::make_pair(resVal, falseVal)]
                            .push_back(resUser);
                    }
                }
            }
  ...
  public:
    void runOnOperation() override {
        ...
        m.walk([&](arith::SelectOp selectOp) {
            // Apply only to selects with a tensor result. Scalars are cheap
            // enough to predicate.
            if (!isa<RankedTensorType>(selectOp.getResult().getType()))
                return;
            // Look if there is an if in the same block, with the same
            // condition.
            auto *parentBlock = selectOp->getBlock();
            Value condition = selectOp.getOperand(0);
        ...
        for (auto [ifOp, selectOps] : selectToIf) {
            // Add new return value to the if (and create else block if
            // necessary), then yield the select value in the then block and the
            // else block.
            ...
            SmallVector<Value> ifYieldOperands =
                newIfOp.thenYield().getOperands();
            SmallVector<Value> elseYieldOperands =
                newIfOp.elseYield().getOperands();
            for (arith::SelectOp selectOp : selectOps) {
            ...
            // Update yields
            auto updateYield = [&](scf::YieldOp yield,

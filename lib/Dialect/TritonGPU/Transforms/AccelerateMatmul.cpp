...
static int getMMAVersionSafe(int computeCapability, DotOp op) {
    ...
    for (int baseVersion : versionsSupported) {
        ...
        if (baseVersion == 3) {
            ...
            remark.attachNote() << "Target compute capability ("
                                << computeCapability << ") supports MMA v3.";
        }
        ...
        if (baseVersion == 5) {
            auto remark = op.emitRemark()
                          << "MMA version 5 acceleration not applied due to "
                             "unsupported shapes or data types.";
            remark.attachNote() << "Target compute capability ("
                                << computeCapability << ") supports MMA v5.";
        }
    }
    return 0;
}
...
getSharedMemoryMMAOperand(Value v, mlir::PatternRewriter &rewriter, int opIdx,
    ...
    // If the MMA op doesn't support transpose pick the layout expected by the
    // MMA op.
    ...
    for (auto op : slice) {
        if (isa<LoadOp, DescriptorLoadOp>(op)) {
            if (auto tensorTy =
                ...
                origBitWidth = std::min<int>(origBitWidth,
  ...
  public:
    ...
    matchAndRewrite(triton::DotOp dotOp,
        ...
        if (computeCapability < 80) {
            dotOp.emitRemark()
                << "Dot op using MMA for compute capability "
                << computeCapability
        ...
        // Enable F64 MMA only on SM80/SM90 with high performance F64
        // tensorcore. Otherwise, fallback to F64 FMA for better performance.
        ...
        rewriter.replaceOpWithNewOp<ConvertLayoutOp>(
            origDotOp, origDotOp.getType(), newDot->getResult(0));
        return success();
    }
};
...
class BlockedToMMAv5 : public mlir::OpRewritePattern<DotOp> {
  ...
  public:
    ...
    matchAndRewrite(triton::DotOp dotOp,
        ...
        Value cvtAcc = rewriter.create<ConvertLayoutOp>(loc, newAccType,
                                                        dotOp.getOperand(2));
        ...
        auto vTrue =
            rewriter.create<arith::ConstantIntOp>(dotOp.getLoc(), 1, 1);
...
Value addSmemStageToScaleLoad(Value scale, mlir::PatternRewriter &rewriter) {
    /*
      ...
      not needs to be put into SMEM. But in practice, the software pipeliner
      puts loading of scales into multi-buffered SMEM. At that point, the SMEM
    ...
    while (!isa<LoadOp, DescriptorLoadOp>(op)) {
        if (auto reshape = dyn_cast<ReshapeOp>(op)) {
            ...
            // Unrecognized pattern, bail out. In practice, this implies that
            // MMA pipelining will not apply to the scaled dot op, since scales
            // will not be in passed through SMEM to tc_gen5_mma_scaled.
            return scale;
        }
    }
...
class ScaledBlockedToMMAv5
  ...
  public:
    ...
    matchAndRewrite(triton::DotScaledOp dotOp,
        ...
        // For mixed-precision fp4 operands, set allowTranspose = false, to
        // force the packed axis, K, to be contiguous in SMEM
        ...
        Attribute accEncoding =
            triton::nvidia_gpu::TensorMemoryEncodingAttr::get(
                context, m, n, /*unpacked=*/true, CTASplitNum[0],
                CTASplitNum[1]);
        Attribute tensorMemorySpace =
            triton::nvidia_gpu::TensorMemorySpaceAttr::get(context);
        Type accMemDescType = triton::gpu::MemDescType::get(
        ...
        Type scaleAType = triton::gpu::MemDescType::get(
            ...
            scaleEncoding, tensorMemorySpace,
            /*mutableMemory=*/false);
        ...
        // We don't need to track memory dependencies for the scale operands
        // since they are not pipelined.
        ...
        auto mmaOp = rewriter.create<triton::nvidia_gpu::TCGen5MMAScaledOp>(
            loc, tokType, a, b, acc.getResult(), acc.getToken(),
...
static void decomposeMixedModeDotOp(ModuleOp mod, int computeCapability) {
    mod.walk([=](DotOp dotOp) -> void {
        ...
        if (mmaLayout) {
            bool isNativeFP8 =
                llvm::isa<Float8E5M2Type, Float8E4M3FNType>(AElType);
...
static void transposeDotOp(DotScaledOp dotOp) {
    ...
    Value lhsTransposed =
        builder.create<TransOp>(lhs.getLoc(), lhs, transOrder);
    Value rhs = dotOp.getB();
    Value rhsTransposed =
        builder.create<TransOp>(rhs.getLoc(), rhs, transOrder);
    Value c = dotOp.getC();
...
static void transposeDots(ModuleOp m) {
    // TODO: extend to regular dot when it is profitable. For instance when we
    // may want to use rhs from register for mmav3.
...
class TritonGPUAccelerateMatmulPass
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        // reverted in these two PRs
        // https://github.com/triton-lang/triton/pull/5834
        ...
        // Now that we have picked the mma type, decompose dot that are not
        // natively supported.

  ...
  public:
    ...
    LogicalResult matchAndRewrite(ttng::TMEMStoreOp store,
        ...
        if (valueFromTMEM == kTrue) {
            Value one =
                rewriter.create<arith::ConstantIntOp>(select.getLoc(), 1, 1);
  ...
  public:
    ...
    LogicalResult matchAndRewrite(ttng::TMEMLoadOp load,
        ...
        if (!llvm::all_of(uses, [&](OpOperand *use) {
                return postDomInfo.properlyPostDominates(use->getOwner(),
                                                         domOp);
            }))
            return failure();
        // In order to not re-ordering multiple tmem load in a loop, don't sink
        // if all the ops between the load and the domOp are tmem loads.
        ...
        Value newToken = sinkValueRedefinition(
            rewriter, load.getDep(), load.getToken(), domOp->getBlock());
...
class HoistTMEMAllocOutOfIf : public OpRewritePattern<ttng::TMEMAllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttng::TMEMAllocOp alloc,
        ...
        if (yieldArgs.empty())
            return failure();
        // Since init is used in the else terminator we know that it dominates
        // the if op.
        alloc->moveBefore(ifOp);
        rewriter.setInsertionPointAfter(ifOp);
        for (int argNo : yieldArgs) {
            auto load = cast<TMEMTokenLoadOp>(
                thenOp->getOperand(argNo).getDefiningOp());
...
class RotateTMEMStoreInLoop : public OpRewritePattern<ttng::TMEMStoreOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttng::TMEMStoreOp store,
        ...
        if (!store.getDep())
            return failure();
        // Pattern match stores whose source comes from a loop region argument
        // and whose predicate is loop-invariant.
...
class RotateTMEMLoadInLoop : public OpRewritePattern<ttng::TMEMLoadOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttng::TMEMLoadOp load,
        ...
        auto tokType = rewriter.getType<AsyncTokenType>();
        auto initStore = rewriter.create<ttng::TMEMStoreOp>(
            load.getLoc(), tokType, load.getSrc(), initAlloc.getToken(),
            initVal, vTrue);
        forOp.getInitArgsMutable()[tokArgNo].assign(initStore.getToken());
...
static void findLastMemoryUses(OpResult token,
    ...
    for (Operation *user : token.getUsers())
        findLastMemoryUses(cast<OpResult>(getTokenFromOp(user)), lastUses,
                           seen);
}
...
struct HoistTMEMAlloc
    ...
    void runOnOperation() override {
        ...
        if (!hoistOutOfIf) {
            ...
            m.walk(
                [&](ttng::MMAv5OpInterface mmaOp) { mmaOps.push_back(mmaOp); });
            for (auto mmaOp : mmaOps) {
                ...
                // Only hoist the TMEM alloc feeding into the accumulator. Leave
                // the ones for the scales in the loop.
                auto alloc =
                    mmaOp.getAccumulator().getDefiningOp<TMEMTokenAllocOp>();
                if (!alloc ||
                    alloc->getParentRegion() != mmaOp->getParentRegion()) {
                    continue;
                }
        ...
        // TODO: currently some code assumes that a mutable tmem alloc doesn't
        // have an initial value. As a workaround we break up the op in order to
        // keep this form for the downstream passes. We should remove this once
        // the downstread passes are fixed.

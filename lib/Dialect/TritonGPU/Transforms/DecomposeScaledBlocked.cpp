...
class DecomposeScaledBlocked : public OpRewritePattern<DotScaledOp> {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(DotScaledOp scaledDotOp,
        ...
        auto newDot = rewriter.create<DotOp>(scaledDotOp.getLoc(), scaledA,
                                             scaledB, scaledDotOp.getC());
  ...
  private:
    ...
    TypedValue<RankedTensorType> scaleTo16(PatternRewriter &rewriter,
        ...
        // getFpMantissaWidth() returns the number of bits in the mantissa plus
        // the sign bit!
    ...
    broadcastScale(PatternRewriter &rewriter, DotScaledOp scaledDotOp,
        ...
        // 2.1) Expand dims along the last dimension
        {
            ...
            auto blockedEnc = getDefaultBlockedEncoding(
                ctx, shape, nWarps, threadsPerWarp, numCTAs);
    ...
    TypedValue<RankedTensorType> maskNan(PatternRewriter &rewriter,
        ...
        auto result =
            rewriter.create<arith::SelectOp>(loc, cond, constNan, mxfp);
    ...
    TypedValue<RankedTensorType> scaleArg(PatternRewriter &rewriter,
        ...
        auto scale =
            opIdx == 0 ? scaledDotOp.getAScale() : scaledDotOp.getBScale();
        auto isFp4 =
            ScaleDotElemType::E2M1 == (opIdx == 0 ? scaledDotOp.getAElemType()
                                                  : scaledDotOp.getBElemType());
        ...
        // For some weird reason, we take the scale with shape as if it were
        // coming from the lhs even when it's the rhs. In a normal world, we
        // should accept this parametre transposed, as we do with the mxfp.
        if (opIdx == 1) {

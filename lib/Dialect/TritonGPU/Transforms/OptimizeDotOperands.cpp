...
class SwizzleShmemConvert : public OpRewritePattern<ConvertLayoutOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ConvertLayoutOp cvtOp,
        ...
        auto newCTALayout =
            permuteCTALayout(ctx, oldCTALayout, trans.getOrder());
        auto newInnerCvtEnc = SwizzledSharedEncodingAttr::get(
            ctx, cvtEncoding, srcTy.getShape(),
            /*order=*/getOrderForMemory(srcTy), newCTALayout,
        ...
        auto localLoadOp = rewriter.create<LocalLoadOp>(trans.getLoc(),
...
class FuseTransMMAV3Plus : public OpRewritePattern<LocalAllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(LocalAllocOp allocOp,
        ...
        auto allocEncoding =
            cast<NVMMASharedEncodingAttr>(allocType.getEncoding());
        RankedTensorType srcTy = trans.getSrc().getType();
        ...
        MemDescType innerTy =
            MemDescType::get(srcTy.getShape(), srcTy.getElementType(),
                             newInnerEnc, allocType.getMemorySpace());
...
class ReshapeMemDesc : public OpRewritePattern<LocalAllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(LocalAllocOp allocOp,
        ...
        auto newAlloc = rewriter.create<LocalAllocOp>(allocOp.getLoc(), innerTy,
                                                      reshapeOp.getSrc());
        rewriter.replaceOpWithNewOp<MemDescReshapeOp>(
            allocOp, allocOp.getType(), newAlloc);
        return success();
    }
};

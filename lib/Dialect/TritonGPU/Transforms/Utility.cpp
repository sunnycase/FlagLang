...
SmallVector<unsigned, 3> mmaVersionToInstrShape(int version,
    ...
    if (version == 1)
        ...
        if (llvm::isa<Float8E5M2Type, Float8E4M3FNType, Float8E4M3FNUZType>(
            ...
            validN.assign({256, 248, 240, 232, 224, 216, 208, 200,
        ...
        if (eltType.isInteger(8)) {
            validN.assign({224, 208, 192, 176, 160, 144, 128, 112, 96, 80, 64,
                           48, 32, 24, 16, 8});
        }
        ...
        // Right now default to distributing along N. TODO: For cases where we
        // have dot followed by reduction we need to be able to distribute along
        // M.
        //    if (numWarps > 4)
        //      m = 64;
...
std::string GraphDumper::emitValueNode(Value value) const {
    ...
    if (info.find("label") == info.end()) {
        ...
        if (auto arg = mlir::dyn_cast<BlockArgument>(value))
            info["label"] = "BlockArg" + std::to_string(arg.getArgNumber()) +
                            " " + shapeStr;
        else
            info["label"] = shapeStr;
    }
...
static Attribute inferSrcEncoding(triton::ReshapeOp op, Attribute encoding) {
    // The encoding of x given the encoding of y in `reshape(x) -> y` is the
    // same as the encoding of x given the encoding of y in `reshape(y) -> x`.
    // It's an invariant of inferReshapeOpNoReorderEncoding that it's symmetric
    // in this way.
...
bool isExpensiveToRemat(Operation *op, Attribute &targetEncoding) {
    ...
    if (isa<triton::gpu::AsyncCopyGlobalToLocalOp, triton::AtomicRMWOp,
        ...
        return true;
    if (isa<scf::YieldOp, scf::ForOp, scf::IfOp, scf::WhileOp,
            scf::ConditionOp>(op))
        return true;
    return false;
}
...
scf::ForOp replaceForOpWithNewSignature(OpBuilder &rewriter, scf::ForOp loop,
    ...
    auto newForOp = replaceForOpWithNewSignature(rewriter, loop,
                                                 newIterOperands, replacements);
...
scf::WhileOp replaceWhileOpWithNewSignature(
    ...
    for (auto [oldArg, newArg] : llvm::zip(
             loop.getAfterArguments(), newLoop.getAfterArguments().take_front(
...
scf::IfOp replaceIfOpWithNewSignature(
    ...
    for (auto it : llvm::zip(ifOp.getResults(), newIf.getResults().take_front(
                                                    ifOp.getNumResults())))
        replacements.push_back(it);
    return newIf;
}
...
scf::IfOp replaceIfOpWithNewSignature(OpBuilder &rewriter, scf::IfOp ifOp,
                                      TypeRange newResultTypes) {
    SmallVector<std::tuple<Value, Value>> replacements;
    auto newIfOp = replaceIfOpWithNewSignature(rewriter, ifOp, newResultTypes,
                                               replacements);
    for (auto &kv : replacements)
        std::get<0>(kv).replaceAllUsesWith(std::get<1>(kv));
    return newIfOp;
    ...
    while (!queue.empty()) {
        ...
        if (auto ifOp = currentValue.getDefiningOp<scf::IfOp>()) {
            if (stopPropagation && stopPropagation(ifOp))
                continue;
            unsigned argIdx =
                mlir::cast<OpResult>(currentValue).getResultNumber();
        ...
        if (auto *definingOp = currentValue.getDefiningOp()) {
            // If the op has multiple results we need to update all results
            // layout.
            ...
            if (auto gather = dyn_cast<GatherOp>(definingOp)) {
                // Specially handle gather since its transfer function only
                // applies between its index operand and result.
            ...
            for (auto [i, operand] :
                 llvm::enumerate(definingOp->getOpOperands())) {
                auto srcEncoding = inferSrcEncoding(definingOp, encoding);
                if (!srcEncoding)
                    return failure();
                // If the infered layout matches the original one we don't need
                // to keep propagating.
                if (auto operandType =
...
SmallVector<Value> delinearize(OpBuilder &b, Location loc, Value linear,
    ...
    if (rank == 1) {
        ...
        for (auto &&en : llvm::enumerate(shape.drop_back())) {
            ...
            multiDim[en.index()] =
                b.create<arith::RemSIOp>(loc, remained, dimSize);
...
Value linearize(OpBuilder &b, Location loc, ArrayRef<Value> multiDim,
    ...
    if (rank > 0) {
        ...
        for (auto [dim, dimShape] : llvm::reverse(
                 llvm::zip(multiDim.drop_back(), shape.drop_back()))) {
...
swizzleDotOperandLike(RankedTensorType type, ttg::CTALayoutAttr ctaLayout) {
    // We want to see if the linear layout has the same order as an mma
    // microtile of shape (8, 4*kWidth) or (4*kWidth, 8). If so, we return a
    ...
    // All the LinearLayouts contained within LinearEncoidngAttr have order [0,
    // 1, 2, ...]
    auto repOrder = to_vector(llvm::seq<unsigned>(rank));
    auto tile =
        ttg::nvidiaMmaTile(ctx, microtileShape, kWidth, order, repOrder);
...
getSharedEncIfAllUsersAreDotEnc(Value val, bool &incompatible) {
    ...
    for (Operation *user : val.getUsers()) {
        ...
        if (user->getNumResults() != 1)
            return std::nullopt;
        if (auto memDesc = dyn_cast<triton::gpu::MemDescType>(
            ...
            tempAttr = dyn_cast<ttg::SwizzledSharedEncodingAttr>(
                memDesc.getEncoding());
            if (!tempAttr)
                return std::nullopt;
            ...
            // FIXME This may not be correct for multiple CTA, but getCTALayout
            // is NYI for LinearEncodingAttr
            ...
            if (auto dot = dyn_cast<ttg::DotOperandEncodingAttr>(
                ...
                tempAttr = ttg::SwizzledSharedEncodingAttr::get(
                    val.getContext(), dot, srcTy.getShape(), order, CTALayout,
                    bitWidth,
                    /*needTrans=*/false);
            } else {
...
struct ForOpDeadArgElimination : public OpRewritePattern<scf::ForOp> {
    ...
    LogicalResult matchAndRewrite(scf::ForOp forOp,
        ...
        // Mark all yield operands as live if the associated forOp result has
        // any use.
        ...
        if (aliveValues.size() == forOp.getNumResults())
            return failure();
        // Operations with side-effects are always live. Mark all theirs
        // operands as live.
        ...
        while (!queue.empty()) {
            ...
            // If an argument block is live then the associated yield operand
            // and forOp operand are live.
            ...
            if (auto forOwner =
                ...
                if (arg.getArgNumber() < forOwner.getNumInductionVars())
                    continue;
                unsigned iterIdx =
                    arg.getArgNumber() - forOwner.getNumInductionVars();
                Value yieldOperand =
        ...
        for (auto yieldOperand : llvm::enumerate(yieldOp->getOperands())) {
            ...
            if (yieldOperand.value() ==
                block.getArgument(yieldOperand.index() + 1))
                continue;
            ...
            // otherwise it returns %init.  We cowardly refuse to remove this
            // operand from the yield.  (We could, but we'd need to prove that
            // the loop runs 0 or >=1 times.)
            //
            // As a special case, if it doesn't matter whether the loop runs 0
            // or >=1 times (because the loop returns the same value in both
            // cases) then we can still mark the operand as dead. This occurs in
            // the above example when %init is the same as %x.
            if (!forOp->isAncestor(
                    yieldOperand.value().getParentRegion()->getParentOp()) &&
                yieldOperand.value() !=
                    forOp.getInitArgs()[yieldOperand.index()])
                continue;
        ...
        rewriter.modifyOpInPlace(forOp, [&]() {
            ...
            // associated argument and leave the operations and argument removal
...
ttg::LocalAllocOp findShmemAlloc(Value operand) {
    ...
    if (auto subView = dyn_cast_or_null<ttg::MemDescIndexOp>(
        ...
        // Single bufferred operand that does not require a subview (not loaded
        // in the loop)
        return dyn_cast_or_null<ttg::LocalAllocOp>(
            transitiveOperand.getDefiningOp());
    }
...
void visitNestedOperands(Operation *op,
    ...
    op->walk([&](Operation *nestedOp) {
        for (OpOperand &operand : nestedOp->getOpOperands()) {
            if (operand.get().getParentBlock()->getParentOp()->isProperAncestor(
                    op))
                visitor(operand);
        }
    });
...
void visitNestedOperands(Operation *op, function_ref<void(Value)> visitor) {
    visitNestedOperands(op,
                        [&](OpOperand &operand) { visitor(operand.get()); });
}
...
void replaceUsesAndPropagateType(
    ...
    for (OpOperand &use : oldUse->getUses()) {
        ...
        if (auto wsOp = dyn_cast<ttg::WarpSpecializeOp>(use.getOwner())) {
            for (Region *region : wsOp.getPartitionRegions())
                region->getArgument(use.getOperandNumber())
                    .setType(val.getType());
        }
        ...
        if (auto subview = dyn_cast<ttg::MemDescIndexOp>(user)) {
            ...
            bool isMutable =
                cast<ttg::MemDescType>(val.getType()).getMutableMemory();
            Type newDstType = ttg::MemDescType::get(
                oldType.getShape(), oldType.getElementType(),
                oldType.getEncoding(), oldType.getMemorySpace(), isMutable,
                oldType.getAllocShape());
            newVal = builder.create<ttg::MemDescIndexOp>(
                subview.getLoc(), newDstType, val, subview.getIndex());
    ...
    for (OpOperand *operand : operandsToReplace) {
        if (auto wait =
                dyn_cast<ttng::WarpGroupDotWaitOp>(operand->getOwner())) {

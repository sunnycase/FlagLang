...
static OwningOpRef<ModuleOp> takeIntoFunction(ModuleAxisInfoAnalysis &axisInfo,
    ...
    FunctionType funcType =
        b.getFunctionType(partition->getArgumentTypes(), {});
...
static LogicalResult relayoutWarps(ModuleAxisInfoAnalysis &axisInfo,
    ...
    replacer.addReplacement(
        [](TensorDescType ty) -> std::pair<Type, WalkResult> {
    ...
    if (!target)
        return mlir::emitError(mod.getLoc(),
                               "module missing target specification");
...
static unsigned getTensorNumI32Regs(RankedTensorType ty) {
    ...
    unsigned elSize = isa<PointerType>(ty.getElementType())
                          ? 64
...
static LogicalResult optimizePartitionNumWarps(ModuleAxisInfoAnalysis &axisInfo,
        ...
        partition->walk([&](Operation *op) {
            for (Type type : llvm::concat<Type>(op->getOperandTypes(),
                ...
                if (auto tensor = dyn_cast<RankedTensorType>(type))
                    tensorRegs =
                        std::max(tensorRegs, getTensorNumI32Regs(tensor));
            }
        });
        // Assume that the largest tensor accounts for half of the registers
        // used by a warpgroup.
        tensorRegs *= 2;
    }
    ...
    for (auto [minWarps, region] :
        ...
        region->walk([minWarps = &minWarps](Operation *op) {
            // Some instructions have critical throughput if have low register
            // usage. Make sure there are enough warps for these ops to execute
            // quickly.
            ...
            else if (isa<ttng::TMEMLoadOp, ttng::TMEMStoreOp,
                         ttng::TMEMAllocOp>(op))
                *minWarps = 4;
        });
    }
    ...
    do {
        ...
        // Assuming even distribution of registers, given the total number of
        // warps currently allocated, we can guess the number of registers PTXAS
        // will distribute to each warp.
        //
        // For example, given 18 warps and a tensor<128x256xf32> contained in an
        // 8-warp partition, we have (nTotalRegs/32/18) = ~113 regs per thread,
        // and the tensor requires 128 regs per thread in its partition. In this
        // case, nothing can be done.
        //
        // However, given a tensor<128x128xf32>, this requires only 64 regs per
        // thread in 8 warps. If we reduce the size of the warp to 4, the
        // overall regs per thread increases to (nTotalRegs/32/14) = ~146 regs
        // per thread, while the tensor now requires 128 regs per thread. This
        // works.
        //
        // The next iteration sees ~170 regs per thread, but the tensor will
        // require 256, which is too many. So the algorithm stops at 4 warps.
        // Evidently, if there are other partitions that can be reduced, we have
        // to iterate this algorithm.
        int32_t curTotalNumWarps =
            std::accumulate(partitionNumWarps.begin(), partitionNumWarps.end(),
                            defaultNumWarps);

        for (auto [minWarps, numWarps, tensorRegs] : llvm::zip(
                 minWarpsForPartition, partitionNumWarps, maxTensorRegs)) {
            if (numWarps <= minWarps)
                continue;
            // Check if reducing the number of warps will still fit the tensor.
            // If it didn't fit to begin with, it won't fit after shrinking.
            unsigned reqRegsPerThread =
                tensorRegs / threadsPerWarp / (numWarps / 2);
    ...
    for (auto [partition, newNumWarps, prevNumWarps, tensorRegs, estRegs] :
        ...
        // Layouts need to be reassigned if the number of warps changed and
        // there are tensor computations.
...
void OptimizePartitionWarps::runOnOperation() {
    ...
    auto runPipelineFn = [&](OpPassManager &pm, ModuleOp container) {
        // The module must be directly nested under the current op for
        // `runPipeline` to work.

...
static Operation *findDefOpInLoop(scf::ForOp loop, Value value,
    ...
    if (auto arg = dyn_cast<BlockArgument>(value)) {
        ...
        if (distance == 1)
            return {};
        return findDefOpInLoop(loop,
                               loop.getYieldedValues()[arg.getArgNumber() - 1],
                               distance + 1);
    }
...
static bool hasDefPartition(scf::ForOp loop, Operation *op,
    ...
    while (!worklist.empty()) {
        ...
        iterateDefs(loop, op, [&](OpResult def) {
            worklist.push_back(def.getDefiningOp());
        });
    }
    return false;
}
...
static std::optional<WarpSchedule> getInitialSchedule(scf::ForOp loop) {
    ...
    // Start by creating the default partition, a partition for for all loads,
    // and a partition for all MMAs.
    WarpSchedule schedule;
    ...
    for (auto mmaOp : loop.getOps<ttng::MMAv5OpInterface>()) {
        ...
        // If the store is unrelated to the use of the MMA, then it gets placed
        // in the MMA partition.
        ...
        while (!operandViews.empty()) {
            ...
            // Duplicate the op if necessary to ensure that the MMA partition is
            // the only user.
            if (!llvm::all_of(op->getUsers(), [&](Operation *user) {
                ...
                op->replaceUsesWithIf(newOp->getResults(), [&](OpOperand &use) {
                    return schedule.getPartition(use.getOwner()) ==
                           mmaPartition;
                });
                op = newOp;
            }
...
void propagatePartitions(scf::ForOp loop, WarpSchedule &schedule) {
    ...
    for (Partition &partition : schedule.getPartitions()) {
        ...
        auto defCallback = [&](OpResult result, unsigned distance) {
            ...
            if (!schedule.isScheduled(defOp) &&
                ...
                opClusters.getOrCreate(defOp)->sinkPartitions.insert(
                    &partition);
            }
        };
        ...
        auto useCallback = [&](OpResult result, OpOperand &use,
                               unsigned distance) {
            Operation *user =
                loop.getBody()->findAncestorOpInBlock(*use.getOwner());
    ...
    while (!worklist.empty()) {
        ...
        iterateDefs(loop, op, [&](OpResult def) {
            ...
            if (schedule.isScheduled(defOp)) {
                ...
                if (!defCluster) {
                    // This operation has not yet been added to a cluster. Add
                    // it to the current cluster and recurse on it.
                    defCluster = cluster;
                    cluster->ops.insert(defOp);
                    worklist.push_back(defOp);
                } else if (defCluster != cluster) {
                    // This operation is part of another cluster. Merge the two
                    // clusters together and continue.
        ...
        iterateUsers(loop, op, [&](Operation *user) {
            if (schedule.isScheduled(user)) {
                // If the user is already assigned to a partition, add that
                // partition as one of the sink partitions.
                Partition *userPartition = schedule.getPartition(user);
                cluster->sinkPartitions.insert(userPartition);
                return;
            }
            // If the user does not already have a cluster, add it to the
            // current cluster. We don't have to handle merging here because
            // when the user visits the current op, it will trigger the merge.
    ...
    for (OpCluster &cluster : llvm::make_pointee_range(opClusters.clusters)) {
        ...
        assert(llvm::all_of(cluster.ops, [&](Operation *op) {
            return !schedule.isScheduled(op);
        }));

        // If there are multiple def or sink partitions, don't know what to do.
        // Assign the whole cluster to its own partition.
        if (cluster.defPartitions.size() > 1 ||
            cluster.sinkPartitions.size() > 1) {
        ...
        DenseSet<Operation *> opsInCluster(cluster.ops.begin(),
                                           cluster.ops.end());
        ...
        // If all ops are on the critical path, assign them to the def
        // partition.
        ...
        // Rematerialize the critical path ops into the sink partition. Leave
        // the rest in the def partition and rely on DCE to remove them.

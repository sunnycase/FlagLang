...
static std::pair<Value, Value> postIncrementModulo(ImplicitLocOpBuilder &b,
    ...
    Value rollover = b.create<arith::CmpIOp>(arith::CmpIPredicate::eq,
                                             nextIndex, intCst(numStages));
...
addIndexAndPhase(PartitionBuilder &b, scf::ForOp &loop, unsigned numStages,
    ...
    auto [nextIndex, nextPhase] =
        postIncrementModulo(b, index, phase, numStages);
    if (epilogue) {
    ...
    for (Operation *user : value.getUsers()) {
        if (isa<ttng::MMAv5OpInterface, LocalLoadOp>(user)) {
            ...
            return mlir::emitWarning(
                user->getLoc(), "failed to warp specialize: cannot handle sink "
...
LogicalResult PipelinedLoad::determineLiveRange(Block &container,
    ...
    for (Operation *user : loadOp->getUsers()) {
        ...
        if (it == allocOps.end()) {
            // This is an in-register use of the load. The result must be live
            // before the op. Since it will be loaded out of shared memory, it
            // only needs to be live until the op as well.
    ...
    for (Partition *userPartition : userPartitions) {
        ...
        auto sinks =
            llvm::to_vector(llvm::concat<Operation *>(regSink, shmemSink));
        ...
        for (Operation *sinkOp : shmemSink) {
            ...
            // Async operations require the memory to be live as long as the
            // operation is in-flight. Each async operation is treated as a
            // separate consumer.
            if (isa<ttng::MMAv5OpInterface>(sinkOp)) {
                asyncUsers.push_back(sinkOp);
                continue;
            }
            // The sink operation is synchronous and the memory is released
            // after the operation.
        ...
        // The memory only needs to be live until before the first register
        // user.
        ...
        // The memory is live until before the first register user or after the
        // last shmem terminal, whichever is later.
...
void PipelinedLoadGroup::allocateAref(scf::ForOp &loop, int numStages) {
    ...
    for (PipelinedLoad &load : loads) {
        loadBuffers.push_back(createAlloc(
            loop, load.type, load.loadOp->getLoc(), load.sharedEnc, numStages));
    }
    ...
    for (PipelinedLoad &load : loads) {
        distinctAsyncUsers.insert(load.asyncUsers.begin(),
                                  load.asyncUsers.end());
        int numLiveUntil =
...
static void lowerTMACopy(PartitionBuilder &b, Partition &loadPartition,
    ...
    if (auto load = dyn_cast<DescriptorLoadOp>(op)) {
        auto indices = ttng::translateTMAIndices(
            b, load.getLoc(),
            load.getDesc().getType().getBlockType().getEncoding(),
            load.getIndices());
        b.createInto<ttng::AsyncTMACopyGlobalToLocalOp>(
            loadPartition, stageCluster, load.getDesc(), indices, barrier, view,
            truePred);
    } else {
...
LogicalResult PipelinedLoadGroup::lowerLoads(WarpSchedule &schedule,
    ...
    // Set up the consumer wait. We know the live before ops are the same for
    // all loads since that's how they were grouped.
    ...
    for (Operation *asyncUser : distinctAsyncUsers) {
        ...
        llvm::report_fatal_error(
            "FIXME: unhandled async user of pipelined load: " +
    ...
    for (auto [load, buffer] : llvm::zip(loads, loadBuffers)) {
        ...
        lowerTMACopy(b, loadPartition, stageCluster, load.loadOp, curLoadBar,
                     view);
        ...
        for (auto &[partition, uses] : regUses) {
            ...
            Operation *loadBeforeOp =
                findNearestCommonDominator(users, domInfo);
            ...
            Value loaded = b.createInto<LocalLoadOp>(
                *partition, userStageCluster, load.type, view);
...
static LogicalResult pipelineMMA(scf::ForOp &loop, PipelinedMMA &mma,
    ...
    // Replace uses of the accumulator before the loop with buffer 0, and
    // replace those after the loop with the last buffer.
    ...
    if (readOp == mmaOp) {
        ...
        userPred =
            b.create<arith::CmpIOp>(arith::CmpIPredicate::eq,
    ...
    if (firstAfterInc) {
        ...
        if (firstAfterInc->op == mmaOp) {
            Value firstBar =
                createSingleBufferView(b, firstAfterInc->barPrev, 0);
            b.create<ttng::ArriveBarrierOp>(firstBar, /*arriveCount=*/1);
        } else {
            assert(firstAfterInc->op ==
                   dyn_cast<ttng::TMEMStoreOp>(overwriteOp));
            for (auto i : llvm::seq(numMmaStages)) {
                Value firstBar =
                    createSingleBufferView(b, firstAfterInc->barPrev, i);
                b.create<ttng::ArriveBarrierOp>(firstBar, /*arriveCount=*/1);
            }
        }
    }
    ...
    while (!incomingOperands.empty()) {
        ...
        if (!defPartition || defPartition == schedule.getRootPartition()) {
            // If the MMA operand is coming from outside the loop, move the
            // alloc out.
        ...
        if (auto allocOp = operand.getDefiningOp<LocalAllocOp>()) {
            ...
            propagateMutability(allocOp.getResult());
                       ...
                       operand.getDefiningOp<ttng::TMEMAllocOp>()) {
            ...
            auto store = b.createInto<ttng::TMEMStoreOp>(
                *defPartition, stageCluster, Type(), tmemAllocOp.getResult(),
                Value(), tmemAllocOp.getSrc(), b.boolCst(true));
            operandDefs.emplace_back(body.findAncestorOpInBlock(*store),
                                     defPartition);
            tmemAllocOp->moveBefore(loop);
            tmemAllocOp->removeAttr(kPartitionAttrName);
            tmemAllocOp.getSrcMutable().clear();
            tmemAllocOp.getResult().setType(
                getAsMutable(tmemAllocOp.getType()));
    ...
    for (Node &node : nodes) {
        ...
        // Find operand defs that come from the same partition and incorporate
        // them in this synchronization edge.
        decltype(operandDefs) nextOperandDefs;
        for (auto &[defOp, defPartition] : operandDefs) {
            if (defPartition == partition &&
                inBody(node.op)->isBeforeInBlock(mmaOp))
        ...
        if (node.barPrev) {
            if (!isa<ttng::TMEMLoadOp>(node.op)) {
                // If the user precondition is defined after the MMA, we need to
                // peel the wait for the user.
                if (incrementPt &&
                    domOp->isBeforeInBlock(&*incrementPt->getPoint()) &&
                    domInfo.properlyDominates(mmaOp,
                                              userPred.getDefiningOp())) {
                    b.restoreInsertionPoint(*incrementPt);
                    Value bar =
                        createSingleBufferView(b, node.barPrev, curIndex);
                    b.createInto<ttng::WaitBarrierOp>(
                        *partition, nodeStageCluster, bar, curPhase, userPred);
                } else {
                    b.setInsertionPoint(domOp);
                    Value bar =
                        createSingleBufferView(b, node.barPrev, node.index);
                    b.createInto<ttng::WaitBarrierOp>(*partition,
                                                      nodeStageCluster, bar,
                                                      node.phase, userPred);
                }
            } else {
                b.setInsertionPoint(domOp);
                ...
                b.createInto<ttng::WaitBarrierOp>(*partition, nodeStageCluster,
                                                  bar, node.phase);
            }
        }
        if (node.barNext) {
            if (mmaOp == node.op) {
                ...
                Value bar = createSingleBufferView(b, node.barNext, node.index);
                b.createInto<ttng::ArriveBarrierOp>(*partition,
                                                    nodeStageCluster, bar, 1);
            }
        }
    }
    ...
    for (auto &[partition, defs] : operandDefsMap) {
        ...
        b.createInto<ttng::WaitBarrierOp>(*partition, srcStageCluster,
                                          emptyView, phase);

        b.setInsertionPointAfter(lastOp);
        Value readyView = createSingleBufferView(b, readyBar, index);
        b.createInto<ttng::ArriveBarrierOp>(*partition, srcStageCluster,
                                            readyView, 1);
    ...
    if (nodes.back().barNext) {
        ...
        Value lastPhase = loop.getResult(phase.getArgNumber() - 1);
        Value lastBar =
            createSingleBufferView(b, nodes.back().barNext, lastIndex);
        b.create<ttng::WaitBarrierOp>(lastBar, lastPhase);
    }

    llvm::SetVector<Operation *> predOps;
    Operation *hoistPt = findNearestCommonDominator(
        llvm::to_vector(userPred.getUsers()), domInfo);
    if (!hoistPt)
        return success();
...
LogicalResult lowerLoops(scf::ForOp &loop, MutableArrayRef<PipelinedLoad> loads,
    ...
    for (PipelinedLoad &load : loads) {
        if (failed(
                load.determineLiveRange(body, domInfo, postDomInfo, schedule)))
            return failure();
...
struct LoadMMASpecialization
    ...
    using TritonGPULoadMMASpecializationBase::
        TritonGPULoadMMASpecializationBase;

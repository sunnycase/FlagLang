...
static Operation *getEarliestUser(ArrayRef<OpOperand *> uses) {
    OpOperand *use =
        *llvm::min_element(uses, [](OpOperand *lhs, OpOperand *rhs) {
    ...
    for (const Partition &partition : schedule.getPartitions()) {
        ...
        auto callback = [&](Value output, OpOperand &use, unsigned distance) {
            ...
            if (usePartition == &partition) {
                assert(distance > 0 &&
                       "self-recursion must occur in the future");
                return;
            }
            Operation *owner =
                loop.getBody()->findAncestorOpInBlock(*use.getOwner());
        ...
        for (auto &[output, info] : useInfo) {
            ...
            while (true) {
                if (auto arg = dyn_cast<BlockArgument>(tmp)) {
                    tmp = loop.getBody()->getTerminator()->getOperand(
                        arg.getArgNumber() - 1);
                    continue;
                }
            ...
            for (auto &[key, uses] : info.consumers) {
                ...
                auto [view, exitOp] =
                    aref.getView(b, *usePartition, sinkSrcCluster);
                // Load the value at the current index and replace uses in this
                // partition with it.
                Value value = b.createInto<LocalLoadOp>(
                    *usePartition, sinkSrcCluster, tensorType, view);
            ...
            b.createInto<LocalStoreOp>(partition, srcStageCluster, output,
                                       view);
            exitOp(b);
        }
    }

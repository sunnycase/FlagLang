...
enum class LoopVarCategory {
    ...
    Unused,
    // The given loop variable is used by the given partition. For example, a
    // loop
    ...
    // partition. If the original loop results include a tensor which is
    // computed
...
getLoopVarIndicesToKeep(scf::ForOp loop, const Partition *partition,
    ...
    SmallVector<std::optional<size_t>> reverseIndices(
        loop.getNumRegionIterArgs(), std::nullopt);
    for (auto [i, arg] : llvm::enumerate(loop.getRegionIterArgs())) {
...
void cloneIfOp(scf::IfOp ifOp, SmallVector<WarpGroupBuilder> &builders,
    ...
    for (size_t idx : partitionIndices) {
        ...
        auto newIfOp =
            b.create<scf::IfOp>(ifOp.getLoc(), ifOp.getResultTypes(), cond,
...
void cloneReduceOp(triton::ReduceOp reduceOp,
    ...
    for (size_t idx : partitionIndices) {
        ...
        for (auto arg :
             reduceOp.getRegion().getBlocks().front().getArguments()) {
                ...
                if (auto forOp = dyn_cast<scf::ForOp>(yieldOp->getParentOp())) {
                    newOperandIndices =
                        getLoopVarIndicesToKeep(
                            forOp, schedule.getPartition(builder.partitionId),
                            schedule)
                            .first;
                } else {
...
LogicalResult triton::gpu::partitionLoop(scf::ForOp loop) {
    ...
    for (const Partition &partition : schedule.getPartitions()) {
        bool failed = false;
        auto callback = [&](OpResult output, OpOperand &use,
                            unsigned distance) {
            Operation *owner =
    ...
    for (auto &op_ : *loop->getBlock()) {
        ...
        if (!wsTag || wsTag.getInt() != schedule.getTag())
            continue;
        if (auto partitionId =
                op->getAttrOfType<IntegerAttr>(kPartitionAttrName)) {
    ...
    for (auto [b, region, partition] : llvm::zip(
        ...
        if (b.partitionId == 0) {
            ...
            // Tensor results computed by non-default partitions are
            // communicated back via SMEM. The calls to getLoopVarIndicesToKeep
            // and isTensorResultComputedBy below are unnecessary if we can
            // encode the partition index and the corresponding result tensor
            // index of newForOp in
            // LoopVarCategory::TensorResultFromOtherPartition. In the absence
            // of such language support, we end up computing the same
            ...
            for (size_t i = 0; i < loop.getNumRegionIterArgs(); ++i) {
                if (loopVarCategories[i] ==
                    ...
                    b.create<LocalStoreOp>(wgOp.getLoc(), result,
    ...
    for (auto [i, res] : llvm::enumerate(loop.getResults())) {
        ...
        if (loopVarCategories[i] ==
            ...
            auto output =
                topBuilder.create<LocalLoadOp>(ty, tensorResultAllocs[i]);

...
class AssignLoadLatencies {
  public:
    ...
    void run() {
        bool pipelineWithoutDot =
            forOp->hasAttr(mlir::triton::kNumStagesAttrName);
        ModuleOp moduleOp = forOp->getParentOfType<ModuleOp>();
        tt::ModuleAxisInfoAnalysis axisInfoAnalysis(moduleOp);

        llvm::MapVector<Operation *, std::pair<int, Operation *>>
            loadOpToIndLevel = loadOpsToIndirectionLevel(
        ...
        if (llvm::any_of(op->getUsers(), [&](Operation *user) {
            ...
            for (auto user : op->getUsers()) {
                ...
                if (enc != localAllocEnc) {
                    // If the load is used by a LocalAllocOp, all the users need
    ...
    void run() {
        ...
        for (auto &op : forOp.getBody()->without_terminator()) {
            ...
            if (auto mma = dyn_cast<ttng::MMAv5OpInterface>(&op)) {
                // Try to push out the wait by one stage even if the operands
                // are not pipelineable, but we know where the loads are
                // scheduled, so we can place the wait right before the loads.

                if (hasSyncDots(forOp)) {
                    // Skip pipelining MMA in the loops where sync dots are
                    // used. This is a dirty heuristic for performance drops in
                    // kernels where we would rather want to have last iteration
                    // peeled instead of having a full iteration of masked
                    // operations only to execute single wait.
                    continue;
                }
                ...
                if (pipeHelper.isPipelineable ||
                    ...
                    // HACK: A pipelined MMA's latency should equal the number
                    // of buffers for the accumulator, but when the user is in
                    // an `scf.if` in SWP, the `scf.if` is pushed to the end of
                    // the loop rather than peeled before the MMA op, requiring
                    // an extra buffer due to liverange overlap. WS does not
                    // have this problem because the MMA is placed in a
                    // different partition than the MMA, so we can correctly set
                    // the latency.
        ...
        serializeSelfLatencies(forOp->getParentOfType<ModuleOp>(),
                               mmaSelfLatency);
    }
        ...
        [&](Operation *op, Operation *finalUser, int distance) {
            ...
            if (isa<tt::LoadOp, tt::DescriptorLoadOp, tt::DescriptorGatherOp>(
                ...
                if (loadOpToIndLevel.count(op)) {
                    ...
                    if (level != distance) {
                        // If we have multiple uses at different distances, we
                    ...
                    LDBG("Load " << *op
                                 << " considered for pipelining with distance "
                                 << distance);

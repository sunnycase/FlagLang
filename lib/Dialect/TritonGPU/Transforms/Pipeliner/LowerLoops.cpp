...
bool mustLoadToRegisters(Operation *op) {
    if (auto loadOp = dyn_cast<tt::LoadOp>(op)) {
        ...
        // With consumer consuming directly the shared memory, there would be no
        // way to replace masked values with the "other" value.
    ...
    if (auto descLoad = dyn_cast<DescriptorLoadOp>(op)) {
        loadEncoding = nvidia_gpu::getEncodingFromDescriptor(
            op, descLoad.getType(), descLoad.getDesc());
...
int getDefUseStageDiff(Operation *op, scf::ForOp forOp,
    ...
    // we must consider the uses of the local_alloc, as it may be removed and
    // its uses will become direct uses of the async load.
    ...
    return createTMAAsyncCopy(
        ...
        [&](OpBuilderForStage &builder, Value tmaPtr, Value barrier, Value view,
            ...
            builder.create<ttng::AsyncTMAGatherOp>(
                gatherOp.getLoc(), tmaPtr, gatherOp.getXOffsets(),
...
void createTMABarrierAndWait(
    ...
    for (auto &[loadOp, asyncLoad] : asyncLoads) {
        ...
        auto addToGroup = [&](Operation *loadOp) {
            ...
            for (Operation *user : loadOp->getUsers()) {
                ...
                // only consider uses of the alloc op since it will be removed.
                if (!mustLoadToRegisters(loadOp)) {
                    assert(loadOp->hasOneUse());
                    auto alloc =
                        cast<ttg::LocalAllocOp>(*loadOp->getUsers().begin());
        ...
        while (nextOp) {
            ...
            if (isTMALoad(nextOp) && asyncLoads.count(nextOp)) {
                if (asyncLoads[nextOp].stageDiff != numBuffers)
                    break;
                if (group.size() > 0 &&
                    schedule[group[0]] == schedule[nextOp]) {
                    addToGroup(nextOp);
                }
            }
    ...
    for (SmallVector<Operation *> &group : commonWaitGroups) {
        ...
        auto wait = builder.create<ttng::WaitBarrierOp>(barrierViewWait,
                                                        loadGroup.phase);
...
scf::ForOp lowerLoads(scf::ForOp forOp, CoarseSchedule &schedule,
    ...
    for (auto &op : forOp.getBody()->without_terminator()) {
        if (isa<tt::LoadOp, tt::DescriptorLoadOp, tt::DescriptorGatherOp>(op)) {
            ...
            if (stageDiff == 0) {
                // Don't care about non-pipelined loads. Scalar loads will be
                // converted to tensor loads if they are pipelined.
                continue;
            }
            SharedEncodingTrait sharedEncoding;
            ...
            if (!isa<RankedTensorType>(op.getResultTypes()[0])) {
                canUseAsyncCp =
                    op.getResultTypes()[0].getIntOrFloatBitWidth() >= 32;
                ...
                // Do not create async loads for small loads (cp.async requires
                // at least 4 bytes)
                canUseAsyncCp = isa<tt::LoadOp>(op) &&
            ...
            if (canUseAsyncCp || isTMALoad(&op)) {
                if (loadRequiresAdditionalBuffer(&op)) {
                    // Allocate additional buffer required by the wgmma
                    // pipelining.
                    stageDiff += 1;
                }
                ...
                // Distance-1 loads can in most cases be pipelined in registers
                // without any performance degradation, as the schedule will
                // usually reorder the user and the producer so there is no
                // liverange overlap, and no copy needed.
    ...
    // NOTE: We create two duplicates values, insertIdx and extractIdx so that
    // the pipeliner will re-materialize the value in later stages of the
    // pipeline instead of carrying it as a dependency across multiple
    // iterations.
    ...
    for (auto [_, loadGroup] : loadGroups) {
        ...
        if (loadGroup.hasTMALoad) {
            ...
            // overlap, provided the phases are differentiated with an
    ...
    for (auto &[numBuffers, loadGroup] : loadGroups) {
        ...
        if (phase) {
            ...
            phase =
                builder.create<arith::SelectOp>(loc, cndExt, nextPhase, phase);
    ...
    for (auto [op, asyncLoad] : asyncLoads) {
        auto [insertIdx, extractIdx, phase, _] =
            loadGroups[asyncLoad.stageDiff];
        if (auto loadOp = dyn_cast<tt::LoadOp>(op)) {
            createAsyncCopy(forOp, loadOp, asyncLoad.alloc, insertIdx,
                            extractIdx, schedule);
            hasAsyncLoads = true;
        } else if (auto loadOp = dyn_cast<tt::DescriptorLoadOp>(op)) {
            createTMAAsyncLoad(forOp, loadOp, asyncLoad.alloc, insertIdx,
            ...
            createTMAAsyncGather(forOp, loadOp, asyncLoad.alloc, insertIdx,
                                 extractIdx, asyncLoad.barrier,
                                 asyncLoad.waitOp, schedule);
        }
    }
    // Patch the yield with the updated counters. Subtract to account for the
    // loop counter.
    ...
    // Automatically discover dependencies and schedule new insert/extract ops
    // to correct stages.
    scheduleDependencies(forOp, schedule);

    if (hasAsyncLoads) {
        // Insert sync point for any possibly outstanding loads after the loop.
        // This can happen as we speculatively execute loads in the loop.
...
Operation *hoistBufferOutOfLoop(scf::ForOp forOp, Operation *op,
    ...
    auto newType = triton::gpu::MemDescType::get(
        ...
        allocType.getEncoding(), allocType.getMemorySpace(),
        /*mutableMemory=*/true);
    ...
    if (auto tmemAlloc = dyn_cast<ttng::TMEMAllocOp>(newAlloc)) {
        ...
        newStore = builder.create<ttng::TMEMStoreOp>(
            tmemAlloc.getResult(), op->getOperand(0), trueVal);
    } else {
...
void createBarrierAndWaitOps(scf::ForOp forOp, CoarseSchedule &schedule,
    ...
    for (auto user : alloc->getUsers()) {
        if (auto load = dyn_cast<ttng::TMEMLoadOp>(user)) {
            ...
            if (!latestSyncPoint ||
                schedule.isOpBefore(load, *latestSyncPoint)) {
    ...
    if (!mmaPipeHelper.isPipelineable &&
        ...
        for (auto def : updatedDefs) {
            if (!latestSyncPoint ||
                schedule.isOpBefore(def, *latestSyncPoint)) {
                latestSyncPoint = def;
            }
        }
    }
    ...
    for (auto user : alloc->getUsers()) {
        if (auto load = dyn_cast<ttng::TMEMLoadOp>(user)) {
            ...
            if (loadStage < mainWaitStage) {
                ...
                builder.create<ttng::WaitBarrierOp>(barrierSlice, phase,
                                                    waitBuffers);
            }
        }
    }
    ...
    if (numStages > 1) {
        Value barWrap;
        newBarrierIdx =
            createIncrementModulo(builder, builder.getLoc(), barrierIdx,
    ...
    for (auto user : allocUsers) {
        if (auto store = dyn_cast<ttng::TMEMStoreOp>(user)) {
            ...
            if (forOp->isAncestor(store)) {
                ...
                Value newBufIdx =
                    createIncrementModulo(builder, forOp.getLoc(), curBufIdx,
                                          numStagesVal, zero, one);
                if (Value pred = store.getPred()) {
                    newBufIdx = builder.create<arith::SelectOp>(
                        newBufIdx.getType(), pred, newBufIdx, curBufIdx);
                }
                ...
                auto tmemSlice = triton::createSingleBufferView(
                    builder, newAlloc, newBufIdx);
                store.getDstMutable().assign(tmemSlice);
            } else {
                // Store before the loop
                assert(store->isBeforeInBlock(forOp) &&
                       "Store is not before the loop");
                builder.setInsertionPoint(store);
                auto tmemSlice =
            ...
            if (forOp->isAncestor(load)) {
                ...
                // Load after the loop
                assert(forOp->isBeforeInBlock(load) &&
                       "Load is not after the loop");
                builder.setInsertionPoint(load);
                auto tmemSlice = triton::createSingleBufferView(
            ...
            // We can legally switch to next buffer index if the mma does not
            // use the accumulator
            auto isConstTrue = [](Value v) {
                if (auto constOp = v.getDefiningOp<arith::ConstantOp>()) {
                    if (auto attr =
                            dyn_cast<BoolAttr>(constOp.getValueAttr())) {
            ...
            newBufIdx = builder.create<arith::SelectOp>(newBufIdx.getType(),
                                                        mma.useAccumulator(),
                                                        curBufIdx, newBufIdx);

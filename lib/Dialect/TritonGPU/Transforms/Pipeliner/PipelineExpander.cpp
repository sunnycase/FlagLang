...
struct LoopPipelinerInternal {
  ...
  protected:
    ...
    /// Assign a value to `valueMapping`, this means `val` represents the
    /// version `idx` of `key` in the epilogue.
    void setValueMapping(Value key, Value el, int64_t idx);

    /// Return the defining op of the given value, if the Value is an argument
    /// of the loop return the associated defining op in the loop and its
  ...
  public:
    ...
    /// Gather liverange information for Values that are used in a different
    /// stage than its definition.
...
bool LoopPipelinerInternal::initializeLoopInfo(
    ...
    // Currently, we do not support assigning stages to ops in nested regions.
    // The block of all operations assigned a stage should be the single
    // `scf.for` body block.
    ...
    // Support only loop-carried dependencies with a distance of one iteration
    // or those defined outside of the loop. This means that any dependency
    // within a loop should either be on the immediately preceding iteration,
    // the current iteration, or on variables whose values are set before
    // entering the loop.
    for (auto &op : forOp.getBody()->without_terminator()) {
        for (auto operand : getNestedOperands(&op)) {
            ...
            if (!def)
                continue;
            if (distance > 1) {
                LDBG("--only support loop carried dependency with a distance "
                     "of 1 or "
...
LogicalResult LoopPipelinerInternal::emitPrologue(RewriterBase &rewriter) {
    ...
    // If the incoming value to an iter arg from the loop yield is defined
    // outside the loop, then that means the iter arg takes that value for all
    // stages after the first stage.
    ...
    for (int64_t i = 0; i < maxStage; i++) {
        ...
        for (Operation *op : opOrder) {
            ...
            Operation *newOp = cloneAndUpdateOperands(
                rewriter, op, [&](OpOperand *newOperand) {
            ...
            if (annotateFn)
                annotateFn(newOp,
                           triton::PipeliningOption::PipelinerPart::Prologue,
                           i);
            for (unsigned destId :
                 llvm::seq(unsigned(0), op->getNumResults())) {
                Value source = newOp->getResult(destId);
                // If the value is a loop carried dependency update the loop
                // argument
                for (OpOperand &operand : yield->getOpOperands()) {
                    ...
                    if (predicates[predicateIdx] &&
                        !forOp.getResult(operand.getOperandNumber())
                             .use_empty()) {
                        // If the value is used outside the loop, we need to
                        // make sure we return the correct version of it.
                        Value prevValue =
                            valueMapping[forOp.getRegionIterArgs()
...
scf::ForOp LoopPipelinerInternal::createKernelLoop(
    ...
    for (auto escape : crossStageValues) {
        ...
        for (unsigned stageIdx = 0;
             stageIdx < info.lastUseStage - info.defStage; stageIdx++) {
            Value valueVersion =
    ...
    auto newForOp =
        rewriter.create<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(),
                                    newUb, forOp.getStep(), newLoopArg);
...
LogicalResult LoopPipelinerInternal::createKernel(
    ...
    if (!peelEpilogue) {
        ...
        for (unsigned i = 0; i < maxStage; i++) {
            ...
            predicates[i] = emitPredicateStageFn(
                rewriter, newForOp.getInductionVar(), ub, step, maxStage, i);
        }
    }
    for (Operation *op : opOrder) {
        ...
        for (OpOperand *operand : operands) {
            ...
            // Special case for the induction variable uses. We replace it with
            // a version incremented based on the stage where it is used.
            ...
            if (arg && arg.getOwner() == forOp.getBody()) {
                ...
                if (forOp.isDefinedOutsideOfLoop(ret)) {
                    // Special case for values defined outside the loop accessed
                    // with distance 1.
                ...
                if (stageDep == stages.end() || stageDep->second == useStage)
                    continue;
                // If the value is a loop carried value coming from stage N + 1
                // remap, it will become a direct use.
            ...
            // For operands defined in a previous stage we need to remap it to
            // use the correct region argument. We look for the right version of
            // the Value based on the stage where it is used.
        ...
        if (annotateFn)
            annotateFn(newOp, triton::PipeliningOption::PipelinerPart::Kernel,
                       0);
    }
    ...
    for (OpOperand &yieldOperand :
        ...
        // When we don't peel the epilogue and the yield value is used outside
        // the loop we need to make sure we return the version from numStages -
        // defStage.
        if (!peelEpilogue &&
            ...
            if (def) {
                ...
                if (defStage != stages.end() && defStage->second < maxStage) {
                    ...
                    source = rewriter.create<arith::SelectOp>(
                        ...
                        newForOp.getBody()
                            ->getArguments()[yieldOperand.getOperandNumber() +
                                             1]);
                }
            }
        }
    ...
    for (auto &it : crossStageValues) {
        ...
        // If there is a live range spanning across more than 2 stages we need
        // to add extra arg.
...
LoopPipelinerInternal::emitEpilogue(RewriterBase &rewriter,
    ...
    Value stepDecr = rewriter.create<arith::SelectOp>(loc, stepLessZero, one,
                                                      createConst(-1));
    ...
    Value totalIterations =
        rewriter.create<arith::DivSIOp>(loc, rangeDecr, step);
    ...
    for (int64_t i = 1; i <= maxStage; i++) {
        ...
        for (Operation *op : opOrder) {
            ...
            if (annotateFn)
                annotateFn(newOp,
                           triton::PipeliningOption::PipelinerPart::Epilogue,
                           i - 1);
            for (auto [opRes, newRes] :
                 llvm::zip(op->getResults(), newOp->getResults())) {
                setValueMapping(opRes, newRes, currentVersion);
                // If the value is a loop carried dependency update the loop
                // argument mapping and keep track of the last version to
                // replace the original forOp uses.
                for (OpOperand &operand :
                     forOp.getBody()->getTerminator()->getOpOperands()) {
                    if (operand.get() != opRes)
                        continue;
                    // If the version is greater than maxStage it means it maps
                    // to the original forOp returned value.
        ...
        if (dynamicLoop) {
            ...
            // predication. If the stage is valid select the peeled value, else
            // use previous stage value.
    ...
    if (it == valueMapping.end())
        it = valueMapping
                 .insert(std::make_pair(key,
                                        llvm::SmallVector<Value>(maxStage + 1)))
                 .first;
    it->second[idx] = el;
}

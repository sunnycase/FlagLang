...
CoarseSchedule scheduleKeyOps(scf::ForOp forOp,
    ...
    std::function<int(Operation *)> computeDistance =
        [&](Operation *op) -> int {
        ...
        for (Operation *user : op->getUsers()) {
            ...
            Operation *inBlockUser =
                forOp.getBody()->findAncestorOpInBlock(*user);
    ...
    for (auto [op, stage] : opToStage) {
        ...
        if (llvm::any_of(slice,
                         [&](Operation *op) { return opToStage.count(op); }))
            continue;
        ...
        auto isLatencyOp = [&](Operation &op) {
            return opLatency.count(&op) ||
                   isa<LoadOp, DescriptorLoadOp, DescriptorGatherOp,
                       LocalStoreOp, LocalLoadOp, ttng::TMEMLoadOp,
                       ttng::TMEMStoreOp, AsyncCopyGlobalToLocalOp,
...
CoarseSchedule::Cluster schedulePrologueAndEpilogue(scf::ForOp forOp,
    ...
    if (!ifsToStage.empty()) {
        CoarseSchedule::Cluster prologueCluster =

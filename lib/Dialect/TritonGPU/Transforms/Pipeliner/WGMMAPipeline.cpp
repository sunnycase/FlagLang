...
static int minNumInterleavedCommitOps(Operation *waitOp) {
    ...
    std::function<int(Value, Operation *, int)> minOverHistories =
        ...
        if (auto arg = mlir::dyn_cast<BlockArgument>(val)) {
            ...
            // get the value assigned to the argument coming from outside the
            // loop
...
std::vector<ttng::WarpGroupDotOp> splitRSDot(ttng::WarpGroupDotOp dotOp) {
    ...
    assert(lhss.size() == numSplits &&
           "lhs must have the same number of splits");
    assert(rhss.size() == numSplits &&
...
static std::optional<int> dotCanBeProperlyAsync(ttng::WarpGroupDotOp dotOp,
    ...
    auto checkOperand = [&](Value operand) {
        // We can always make RSGEMM async s long as the RHS can be
        // multi-buffered
        if (isa<RankedTensorType>(operand.getType())) {
            return true;
        }
        // If it's a shmem operand, it must either be defined outside the loop,
        // or come from an MemDescIndex op.  Only ConvertLayout and view ops are
        // allowed in between.
    ...
    // reaches a WarpGroupDotOp (with use index 2), possibly after passing
    // through a chain of noops
    ...
    // properly async, but we have to thread its result from iteration i-1
    // through the wait.
...
static void insertAsyncWarpGroupDotWaitInLoop(
    ...
    for (auto asyncDot : llvm::make_first_range(properlyAsyncDots)) {
        ...
        if (rsDotNeedsWait(asyncDot, forOp)) {
            ...
            auto newWait = builder.create<ttng::WarpGroupDotWaitOp>(
                asyncDot->getLoc(), ArrayRef<Value>{},
...
void triton::asyncLaunchDots(scf::ForOp forOp) {
    ...
    for (auto WarpGroupDotOp :
         forOp.getBody()->getOps<ttng::WarpGroupDotOp>()) {

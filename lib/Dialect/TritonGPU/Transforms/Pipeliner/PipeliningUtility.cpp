...
bool triton::getDominatingValueSetOpsToHoist(
    ...
    // Climb the use-def chain breadth-first so that operations can be hoisted
    // in the reverse visitation order.
    ...
    while (!queue.empty()) {
        ...
        // If the value properly dominates the outer loop, then it must be
        // invariant to it.
        ...
        if (visited.contains(op))
            continue;
        // If the defining op cannot be hoisted, then the value cannot be made
        // loop invariant.
        if (!canHoist(op))
            return false;
        visited.insert(op);
    ...
    // added to `toHoist` unless all of `values` can be hoisted. This is to
    // avoid hoisting operations for loops that don't end up getting fused if
    // one of their bounds operands cannot be hoisted.
...
Value triton::sinkValueRedefinition(RewriterBase &rewriter, Value in, Value out,
    ...
    for (; block != in.getParentBlock();
        ...
        llvm::report_fatal_error(
            "FIXME: sinking into unhandled control flow op: " +
...
Operation *mlir::triton::predicateOp(RewriterBase &rewriter, Operation *op,
    ...
    if (auto waitBarrier = dyn_cast<ttng::WaitBarrierOp>(op)) {
        ...
        if (currentPred) {
            mask =
                getPredMask(rewriter, currentPred.getType(), currentPred, pred);
        }
...
void mlir::triton::resolveMaskOp(ModuleOp moduleOp,
    ...
    for (auto maskOp : peeledMaskOps) {
        ...
        while (&maskOp.getBody()->front() !=
               maskOp.getBody()->getTerminator()) {
...
int mlir::triton::getCopyVecBytes(RankedTensorType registerTy,
    ...
    // FIXME: Here we should pass a MemDescType instead of a
    // SharedEncodingTrait!! This is currently broken for memdesc_subslice!
...
bool mlir::triton::canBeAsyncLoad(Operation *op) {
    ...
    ttg::SharedEncodingTrait sharedEncoding =
        mlir::triton::getSharedEncoding(op);
...
void mlir::triton::combineRedundantWaitOps(
    ...
    for (auto waitOp : waitOps) {
        ...
        // group or a branching op (forOp, ifOp, whileOp) in between the waits
        while (next &&
...
ttg::MemDescType mlir::triton::getBufferViewType(ttg::MemDescType allocTy,
    ...
    return ttg::MemDescType::get(
        ...
        allocTy.getEncoding(), allocTy.getMemorySpace(), mutableMemory,
    ...
    if (llvm::any_of(op->getUsers(), [&](Operation *user) {
        ...
        for (auto user : op->getUsers()) {
            ...
            if (enc != localAllocEnc) {
                ...
                // Use one of the encodings, and warn about the performance
                // issue.
                op->emitRemark()
                    << "Pipelining load with different use encodings. This "
                       "will lead "
...
static LogicalResult rewriteTMABufferUpdates(
    ...
    for (auto [iOp, pair] : llvm::enumerate(tmaBufferMapping)) {
        ...
        IRRewriter rewriter(forOp);
        nextCounter = triton::sinkValueRedefinition(
            rewriter, counter, nextCounter, op->getBlock());
...
scf::ForOp triton::lowerTMADescriptors(scf::ForOp forOp,
    ...
    for (auto &op : forOp.getBody()->without_terminator()) {
        if (auto wgMmaOp = dyn_cast<ttng::WarpGroupDotOp>(&op)) {
            // Hopper only: Add one more buffer slice if there is a
            // WarpGroupDotOp, as if it will be pipelined, we will effectively
            // make the pipeline one stage longer.
            maxStage += 1;
            break;
        }
    }
...
static Operation *getUseOfPipelinedOp(
    ...
    for (Operation *topLevelUser : topLevelUsers) {
        assert(schedule.count(topLevelUser) &&
        ...
        [&](Operation *candidate, Operation *current) {
            ...
            return candidateStage < currentStage ||
                   (candidateStage == currentStage &&
                    schedule.clusters.isBefore(candidateCluster,
                                               currentCluster)) ||
                   (candidateStage == currentStage &&
...
triton::getLastUseOfPipelinedOp(ArrayRef<Operation *> ops, scf::ForOp forOp,
    ...
    return getUseOfPipelinedOp(
        ...
        [&](Operation *candidate, Operation *current) {
            ...
            return candidateStage > currentStage ||
                   (candidateStage == currentStage &&
                    schedule.clusters.isBefore(currentCluster,
                                               candidateCluster)) ||
                   (candidateStage == currentStage &&
                    candidateCluster == currentCluster &&

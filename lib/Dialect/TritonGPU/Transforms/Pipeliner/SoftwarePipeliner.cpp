            ...
            if (isEpilogue) {
                ...
                if (predOp.getStage() == predOp.getMaxStage() - 1) {
                    ...
                    return triton::emitPredicateForStage(
                               rewriter, predOp.getIv(), predOp.getUb(),
                               predOp.getStep(), predOp.getMaxStage(),
                               predOp.getStage())
                        .getDefiningOp();
                }
            }
                ...
                [](RewriterBase &rewriter, Value inductionVar, Value upperBound,
                   Value step, uint64_t maxStage, uint64_t stage) {
                    return rewriter.create<triton::gpu::PredicateStageOp>(
                        inductionVar.getLoc(), inductionVar, upperBound, step,
                        maxStage, stage);
                };
        }
...
struct PipelinePass : public impl::TritonGPUPipelineBase<PipelinePass> {
    ...
    void runOnOperation() override {
        ...
        if (dumpIntermediateSteps) {
            llvm::dbgs() << "// -----// SoftwarePipeliner internal IR Dump "
                            "After: LowerLoops\n"
                         << moduleOp << "\n\n\n";
        }
        ...
        // simplify the IR.
        auto arithDialect = getOperation()
                                .getContext()

...
class CombineBroadcastMulReducePattern : public RewritePattern {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(Operation *op,
        ...
        auto expandLhsOp =
            broadcastLhsOp.getSrc().getDefiningOp<ExpandDimsOp>();
        if (!expandLhsOp)
            return failure();
        auto expandRhsOp =
            broadcastRhsOp.getSrc().getDefiningOp<ExpandDimsOp>();
        if (!expandRhsOp)
            return failure();
        ...
        Type newAccType = RankedTensorType::get(
            ...
            cast<ShapedType>(broadcastLhsOp.getSrc().getType())
                .getElementType());
...
class RankedReduceDescriptorLoads : public mlir::OpRewritePattern<ReshapeOp> {
  public:
    ...
    matchAndRewrite(triton::ReshapeOp reshapeOp,
                    mlir::PatternRewriter &rewriter) const override {
        auto loadDef =
            reshapeOp.getSrc().getDefiningOp<triton::DescriptorLoadOp>();
        ...
        if (loadShape.take_back(reshapeRank) != reshapeShape)
            return failure();
        rewriter.modifyOpInPlace(loadDef, [&]() {
            loadDef.getResult().setType(reshapeOp.getType());
        });
  ...
  public:
    ...
    matchAndRewrite(OpTy addOp,
        ...
        rewriter.modifyOpInPlace(dotOp, [&] {
            dotOp.getCMutable().assign(isDotLHS ? addOp.getRhs()
                                                : addOp.getLhs());

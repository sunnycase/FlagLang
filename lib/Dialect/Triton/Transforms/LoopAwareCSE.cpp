...
class ValueEquivalence {
  public:
    std::optional<bool> getKnownEquivalence(Value a, Value b) {
        if (auto it = equalValues.find(normalizeKey(a, b));
            it != equalValues.end())
            return it->second;
        return std::nullopt;
    }
  ...
  private:
    ...
    std::pair<Value, Value> normalizeKey(Value a, Value b) {
        if ((uintptr_t)a.getAsOpaquePointer() <
            (uintptr_t)b.getAsOpaquePointer())
            return {a, b};
        return {b, a};
    }
...
static void loopCSE(scf::ForOp loop) {
    ...
    for (auto it = equivalentArgs.begin(), end = equivalentArgs.end();
         it != end; ++it) {
        if (!(*it)->isLeader())
            continue;
        ...
        for (int j : llvm::drop_begin(eqArgs)) {
            ...
            // Short-circuit the value. The canonicalizer will clean this up.
            // Leftover subcomputations can now be removed by normal CSE.
...
struct LoopAwareCSE
    ...
    void runOnOperation() override {
        ...
        // first to make sure values from outside loops that are equivalent are
        // made pointer equal.
        ...
        // Now that equivalent iter args have been made pointer equal, run CSE
        // again to clean up the loop body.
        eliminateCommonSubExpressions(rewriter, domInfo, getOperation());

        // Run the `scf.for` canonicalizer to clean up the loops
        // (short-circuited values, unused results, etc.).

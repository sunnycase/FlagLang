...
struct CanonicalizeMaskedLoadPattern : public OpRewritePattern<LoadOp> {
    ...
    LogicalResult matchAndRewrite(LoadOp loadOp,
        ...
        auto splatMask =
            mlir::dyn_cast<SplatElementsAttr>(constantMask.getValue());
        if (!splatMask)
            return failure();
...
OpFoldResult TransOp::fold(FoldAdaptor adaptor) {
    ...
    if (isIota(getOrder())) {
        ...
        // If their layout is different (even if structurally equivalent), we
        // need to insert a convert_layout in between as otherwise ::fold
        // complains We do this in CanonicalizeConvertFromTranspose
...
LogicalResult DotOp::verify() {
    ...
    Dialect &dialect = retEnc.getDialect();
    auto interface = cast<DialectInferLayoutInterface>(&dialect);
    return interface->verifyDotOpEncodingCompatibility(getOperation(),
                                                       aEncoding, bEncoding);
}
    ...
    if (retShape.empty()) {
        ...
        if (argEncoding) {
            ...
            auto inferLayoutInterface =
                cast<DialectInferLayoutInterface>(&dialect);
...
template <class Op> LogicalResult verifyReduceScan(Op &op) {
    ...
    if (op.getNumOperands() != op.getNumResults()) {
        return op.emitOpError()
               << "must have the same number of inputs as outputs";
    }
    ...
    for (auto [opElemTy, resTy] :
        ...
        if (opElemTy != getElementTypeOrSelf(resTy)) {
            return op.emitOpError()
                   << "operand types and result types must agree";
        }
    }
    return success();
}
...
static LogicalResult verifyRegionsImpl(Op &op) {
    ...
    for (unsigned i = 0; i < numArgs; ++i) {
        ...
        if (blockArgTy != argElemTy) {
            return op.emitOpError()
                   << "type mismatch on combine operation. Expected argument "
                   << i << " to have type " << argElemTy << " but got "
                   << blockArgTy;
        }
    }
...
LogicalResult MapElementwiseOp::verifyRegions() {
    ...
    auto walkRes = getRegion().walk([&](Operation *op) -> WalkResult {
        ...
        // Ban stores as we won't get the redundant masking correct by treating
        // it as a scalar.
        ...
        if (auto srcEnc = srcTy.getEncoding()) {
            ...
            if (failed(inferLayoutInterface->inferExpandDimsOpEncoding(
                ...
                return emitOptionalError(
                    op.getLoc(), "failed to infer layout for ExpandDimsOp");
            }
        }
...
OpFoldResult FpToFpOp::fold(FoldAdaptor adaptor) {
    ...
    if (matchPattern(srcVal, m_NegZeroFloat())) {
        llvm::APFloat negZero =
            llvm::APFloat::getZero(semantic, /*negative=*/true);
        if (auto tensorTy = dyn_cast<RankedTensorType>(dstTy))
...
LogicalResult BitcastOp::verify() {
    ...
    if (dstIsPtr || srcIsPtr) {
        ...
        // pointer-to-scalar.
        if (dstIsPtr && srcIsPtr) {
            if (triton::getAddressSpace(dstType) !=
                triton::getAddressSpace(srcType))
                return emitError(
...
void JoinOp::build(OpBuilder &builder, OperationState &state, Value lhs,
    ...
    auto retTy =
        RankedTensorType::get(retShape, lhsTy.getElementType(), retEnc);
...
LogicalResult JoinOp::verify() {
    ...
    if (retTy.getElementType() != srcTy.getElementType()) {
        return emitOpError(
            "result element type must match the input element type");
    }
    ...
    // There are multiple correct destination layout for a given source layout
    // but there is only one correct source layout for a given destination
    // layout. So we verify that the source layout match the destination layout.
    Attribute srcEnc;
    ...
    if (cast<triton::DialectInferLayoutInterface>(&srcEnc.getDialect())
            ->verifyLayoutsAreEqual(srcTy.getShape(), srcEnc,
                                    srcTy.getEncoding(), {})
            .failed()) {
...
LogicalResult SplitOp::inferReturnTypes(
    ...
    if (srcEnc) {
        if (cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                ->inferSplitOpEncoding(srcEnc, retEnc, srcTy.getShape(),
                                       location)
                .failed()) {
            return failure();
        }
    ...
    if (getNumOperands() >= 1) {
        ...
        if (numInputElems % this->getPackedElement() != 0) {
            return emitError("number of input elements ")
                   << numInputElems
                   << " must be a multiple of the op's packed_element "
                      "attribute, "
                   << getPackedElement();
        }
    }
...
DescriptorGatherOp::verifyResultType(Operation *op, ShapedType resultType,
    ...
    // TODO: We can support smaller gather sizes by padding the `local_alloc`
    // this lowers to to the nearest minimum tile size.
    ...
    if (unsigned cols = resultType.getShape()[1]; cols < minCols) {
        return op->emitOpError("gather of ")
               << dtype << " must have at least " << minCols
               << " columns, but got " << cols;
    }

    if (resultType.getShape()[0] != indicesType.getShape()[0]) {
        return op->emitOpError(
                   "result tensor must have as many rows as indices (")
...
static LogicalResult verifyGatherScatterOp(Operation *op,
    ...
    if (blockType.getRank() != 2) {
        return op->emitOpError("block must be a 2D tensor, but got ")
               << blockType;
    }
    ...
    if (failed(
            DescriptorGatherOp::verifyResultType(op, resultType, indicesType)))
        return failure();

    if (resultType.getShape()[1] != blockType.getShape()[1]) {
        return op->emitOpError(
                   "result tensor number of columns must match block (")
...
LogicalResult DescriptorGatherOp::verify() {
    return verifyGatherScatterOp(
        *this, getDesc().getType().getSignlessBlockType(),
...
static LogicalResult verifyDescriptorLoadStoreType(Operation *op,
    ...
    if (blockShape.size() > tensorShape.size()) {
        ...
        for (int i = 0; i < blockShape.size() - tensorShape.size(); ++i) {
            if (blockShape[i] != 1)
                return op->emitOpError("ranked reduce load only allowed for "
                                       "unit dimension leading dim.");
        }
    ...
    if (blockShape == tensorShape &&
        ...
        return success();
    return op->emitOpError(
        "tensor descriptor block and tensor types must match");
}

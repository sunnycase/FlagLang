...
struct FenceInsertionPass
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        mod.walk([&](DotOpInterface dotOp) {
            Value a = dotOp.getA();
            Value b = dotOp.getB();
            SmallVector<Operation *> copyRegToSharedOpsA =
                findCopyRegToSharedOps(a);
            SmallVector<Operation *> copyRegToSharedOpsB =
                findCopyRegToSharedOps(b);
            ...
            auto fence = builder.create<FenceAsyncSharedOp>(dotOp.getLoc(),
                                                            /*bCluster=*/false);
            // If there is all the dependencies are outside of the loop try to
            // hoist the fence.
            while (auto loopOp =
                       fence->getParentOfType<LoopLikeOpInterface>()) {
                if (!copyRegToSharedOpsA.empty() &&
                    llvm::any_of(copyRegToSharedOpsA, [&](Operation *op) {
                        return loopOp->isAncestor(op);
                    }))
                    break;
            ...
            if (auto lastFence = dyn_cast_or_null<FenceAsyncSharedOp>(
                    fence->getPrevNode())) {
  ...
  private:
    ...
    void findCopyRegToSharedOps(Value operand, DenseSet<Value> &visited,
                                llvm::SetVector<Operation *> &result) {
        // If the value has already been visited we can safely return false as
        // we would early return when true.
        ...
        if (auto forOp = dyn_cast<scf::ForOp>(argOwner)) {
            ...
            --argNum;
            // prologue
            findCopyRegToSharedOps(forOp.getInitArgs()[argNum], visited,
                                   result);
            // yield
        ...
        if (auto wsOp = dyn_cast<ttg::WarpSpecializePartitionsOp>(argOwner)) {
            findCopyRegToSharedOps(
                wsOp.getParentOp().getExplicitCaptures()[argNum], visited,
                result);
            return;
        }

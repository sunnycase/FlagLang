...
class SyncMMALowering : public OpRewritePattern<TCGen5MMAOpTy> {
  public:
    ...
    LogicalResult matchAndRewrite(TCGen5MMAOpTy op,
        ...
        Value barrierAlloc = rewriter.create<ttg::LocalAllocOp>(
            loc, barrierMemDescType, Value());
        rewriter.create<InitBarrierOp>(loc, barrierAlloc, 1);
        op.addCompletionBarrier(
            barrierAlloc, rewriter.create<arith::ConstantIntOp>(loc, 1, 1));
        op.setIsAsync(true);
...
struct TCGen5MMAScaleSharedToTmemConversion
    ...
    bool lowerScaleToTmem(OpOperand &operand, PatternRewriter &rewriter,
        ...
        Type scaleAType = ttg::MemDescType::get(shape, elType, scaleEncoding,
                                                tensorMemorySpace,
                                                /*mutableMemory=*/true);
    ...
    LogicalResult matchAndRewrite(TCGen5MMAScaledOp op,
        ...
        if (isa<ttg::SharedMemorySpaceAttr>(aScaleType.getMemorySpace())) {
            anyChanged =
                lowerScaleToTmem(op.getAScaleMutable(), rewriter, blockM);
        }
...
class TritonNvidiaGPUMMALoweringPass
  ...
  public:
    void runOnOperation() override {
        ...
        patterns.add<SyncMMALowering<TCGen5MMAOp>,
                     SyncMMALowering<TCGen5MMAScaledOp>,

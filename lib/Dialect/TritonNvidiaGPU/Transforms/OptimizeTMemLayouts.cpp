...
class TMemSplitLoadPattern : public OpRewritePattern<SplitOp> {
  public:
    ...
    LogicalResult matchAndRewrite(SplitOp splitOp,
        ...
        // Peel off convert_layouts *below* the reshape as well.  This is
        // required for the recursive case where the producer of the reshape is
        // the result of an earlier optimisation pass (i.e. a convert_layout of
        // a previous tmem_load).
        ...
        if (shape[0] !=
            cast<RankedTensorType>(reshapeSrc.getType()).getShape()[0])
            return failure();
...
class TMemStoreJoinPattern : public OpRewritePattern<TMEMStoreOp> {
  public:
    ...
    LogicalResult matchAndRewrite(TMEMStoreOp storeOp,
        ...
        // We found a tmem_store that is joined on the N dimension. We can split
        // it into multiple tmem_stores.
        ...
        auto newStoreType =
            joinOp.getLhs().getType().cloneWithEncoding(distLayout);
...
class TMemLoadReducePattern : public OpRewritePattern<TMEMLoadOp> {
  public:
    ...
    LogicalResult matchAndRewrite(TMEMLoadOp tmemLoadOp,
        ...
        // If there is only 1 warpgroup there is nothing to optimize as the
        // layout is already reduction friendly.
        if (numWarps != 8)
            return failure();
  ...
  public:
    ...
    LogicalResult matchAndRewrite(TMEMStoreOp tmemStoreOp,
        ...
        for (Value v : slice) {
            ...
            if (localLoad.getType().getElementType().getIntOrFloatBitWidth() !=
                16)
                return failure();
            ...
            // If we find a 16bits load that cannot be vectorized use the
            // alternative layout.
            if (vecDim != 1)
                return failure();
        ...
        if (!foundImprovedLoad)
            return failure();
        // Use the new layout and rely on RemoveLayoutConversions pass to
        // propagate the convert_layout.
        auto cvt = rewriter.create<ttg::ConvertLayoutOp>(

...
static TMemChunk allocFirstFit(MemoryBitMap &memoryMap,
    ...
    // the same time as the current allocation plus what is known to be
    // currently live. Union those allocations with a copy of the current memory
    // map and use that to find the actual offsets.
...
static SmallVector<Operation *> getAlloc(Value value) {
    ...
    while (!worklist.empty()) {
        ...
        if (auto arg = dyn_cast<BlockArgument>(v)) {
            ...
            if (!block->isEntryBlock()) {
                for (Block *pred : block->getPredecessors()) {
                    ...
                    unsigned idx =
                        std::distance(br->getSuccessors().begin(), it);
                    SuccessorOperands args = br.getSuccessorOperands(idx);
                    Value operand = args.getForwardedOperands()
                                        [arg.getArgNumber() -
        ...
        if (isa<TMEMAllocOp>(defOp)) {
            ...
            llvm::report_fatal_error(
                "unhandled op when looking for TMEM alloc: " +
...
allocateTMem(Operation *parentOp,
    ...
    parentOp->walk<WalkOrder::PostOrder>([&](Operation *op) {
        ...
        if (auto mmaOp = dyn_cast<MMAv5OpInterface>(op)) {
            if (isa<TensorMemoryEncodingAttr>(
                    mmaOp.getA().getType().getEncoding())) {
                TMemAllocation allocSize =
                    getTmemAllocSizes(mmaOp.getA().getType());
                if (allocSize.numRows == 64) {
                    // HW restriction, the A alloc and accumulator needs to be
                    // in the same rows.
                    SmallVector<Operation *> lhsAllocs = getAlloc(mmaOp.getA());
                    SmallVector<Operation *> accAllocs =
                    ...
                    // TODO: we need to handle cases where the format is blockM
                    // and we have multiple blocks.
                    assert((cast<TensorMemoryEncodingAttr>(
                            ...
                            cast<TensorMemoryEncodingAttr>(
                                mmaOp.getAccumulator().getType().getEncoding())
                                    .getBlockM() != 64) &&
                           "interleaved layout with TMEM operand is not "
                           "supported yet.");
                }
            }
        }
    ...
    for (auto it = allocs.begin(), e = allocs.end(); it != e; ++it) {
        ...
        Interval<int> liveInterval =
            getLiveIntervals(alloc, liveness, operationId);
        ...
        // TODO: clarify the alignment requirements for different allocations.
        // For now enforce an alignment of 4 columns.
        const int columnAlignment = 4;
        TMemChunk chunkAllocated =
            allocFirstFit(memoryMap, allocSize, rowIdConstraint,
                          coexistingChunks, columnAlignment);
        ...
        alloc->setAttr(
            ...
            IntegerAttr::get(IntegerType::get(parentOp->getContext(), 32),
                             rowOffset));
        totalMemorySize =
            std::max(totalMemorySize, colOffset + allocSize.numCols);
    }
...
class TritonTensorMemoryAllocationPass
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        // NOTE: if totalMemorySize > 512 we exceeded the maximum amount of
        // tensor memory, but we let the compilation finish so that we can raise
        // an exception in python for the auto-tuner.
        ...
        if (totalMemorySize > 0) {
            // We use a small smem allocation to get the tensor memory base
            // address from tcgen05.alloc, ensure the block has at least 4 bytes
            // of smem
            int shared = 0;

...
bool tmemMayAlias(Value a, Value b) {
    ...
    if (aAlloc != bAlloc)
        return false;
    // If the access ranges along any dimension are known to not overlap, then
    // the accesses don't alias.
    for (auto [aRange, bRange] : llvm::zip(aRanges.ranges, bRanges.ranges)) {
        // If either access range at this dim is unknown, we can't determine if
        // they don't overlap.
        if (!aRange || !bRange)
            continue;
        ...
        if (!isMemoryEffectFree(next)) {
            ...
            for (auto effect : effects) {
                ...
                if (isa<TensorMemory>(effect.getResource()) &&
                    (!effect.getValue() ||
                     tmemMayAlias(effect.getValue(), buffer))) {
                    dep = true;
                    break;
                }
            }

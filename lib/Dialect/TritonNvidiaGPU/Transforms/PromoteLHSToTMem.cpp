...
Attribute getLHSTMemLayout(MMAOpTy tcGen5MMAOp, RankedTensorType srcType) {
    ...
    // M has to follow the MMA size, as it is related to the message we are
    // using. N has to follow the number of columns in the LHS.
  ...
  public:
    ...
    LogicalResult matchAndRewrite(MMAOpTy tcGen5MMAOp,
        ...
        ttg::MemDescType lhsMemDescType = ttg::MemDescType::get(
            ...
            /*mutableMemory=*/false);
        bool layoutTmemCompatible = isDistributedLayoutTMemCompatible(
        ...
        Value tMemAlloc =
            rewriter.create<TMEMAllocOp>(loc, lhsMemDescType, src);

...
void ProxyFenceAnalysis::insertFence(Operation *op, OpBuilder *builder) {
    ...
    builder->create<triton::nvidia_gpu::FenceAsyncSharedOp>(op->getLoc(),
                                                            false);
}
...
void ProxyFenceAnalysis::update(Operation *op, BlockInfo *blockInfo,
                ...
                if (auto value = effectInstance.getValue()) {
                    for (auto bufferId : allocation->getBufferIds(value)) {
                        if (bufferId != Allocation::InvalidBufferId) {
                            // TODO: handle proxy read cases. Those are
                            // currently handled in FenceInsertionPass where it
                            // can generate better placement for the fence. But
                            // we should support a safe fallback here.
    ...
    // memory read/write operations, mark them as a read.

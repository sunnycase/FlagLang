...
std::optional<UseInfo> getUseInfo(Operation *op) {
    ...
    if (auto load = dyn_cast<DescriptorLoadOp>(op)) {
        ...
        auto encoding = info.desiredSharedEncoding
...
void assignMemoryLayouts(FuncOp &func) {
    ...
    func.walk([&](Operation *op) {
        if (auto info = getUseInfo(op)) {
            updateEncoding(info->descriptor,
                           EncodingInfo{info->desiredSharedEncoding,
                                        info->ctaLayout, info->shape});
        } else {
            bool forcedToDefault =
                isa<CallOp, ReturnOp, ReinterpretTensorDescOp>(op);
            auto einfo = internEncoding(
                encodings, EncodingInfo{{}, {}, {}, forcedToDefault});
    ...
    while (!worklist.empty()) {
        ...
        for (OpOperand &use : desc.getUses()) {
            ...
            if (isa<scf::ForOp, scf::WhileOp>(op)) {
                ...
                auto vals =
                    getTiedArgs(op->getParentOp(), use.getOperandNumber());
                updateEncoding(vals, EncodingInfo{});
            }
        }
    ...
    for (auto &[desc, einfo] : valueToEncodingInfo) {
        ...
        if (einfo->desiredEncoding) {
            ...
            newEncoding = getFallbackSharedEncoding(
                existingTy, einfo->ctaLayout, einfo->shape);
        }
        desc.setType(getTensorDescTypeWithEncoding(desc.getDefiningOp(),

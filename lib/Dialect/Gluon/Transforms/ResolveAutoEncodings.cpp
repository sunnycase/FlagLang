...
LogicalResult inferAutoLayouts(FuncOp func) {
    ...
    while (!worklist.empty()) {
        ...
        for (OpOperand &use : val.getUses()) {
            ...
            if (isa<scf::ForOp, scf::WhileOp>(op)) {
                ...
                auto tiedArgs =
                    getTiedArgs(op, use.getOperandNumber() - offset);
                ...
                if (dstEnc) {
                    ...
                    if (failed(updateEncoding(
                            llvm::to_vector_of<Value>(op->getResults()),
                            dstInfo)))
                        return failure();
                }
            }
        ...
        if (auto opResult = dyn_cast<OpResult>(val)) {
            ...
            if (isa<scf::ForOp, scf::WhileOp, scf::IfOp>(definingOp)) {
                auto tiedArgs =
                    getTiedArgs(definingOp, opResult.getResultNumber());
                if (failed(updateEncoding(tiedArgs, info)))
                    return failure();
            } else {
    ...
    for (auto &[val, info] : valueToEncoding) {
        ...
        if (auto opResult = dyn_cast<OpResult>(val)) {
            if (auto constantOp =
                    dyn_cast<arith::ConstantOp>(opResult.getOwner())) {
                auto value = cast<SplatElementsAttr>(constantOp.getValueAttr());
                auto newValue = SplatElementsAttr::get(
                    ty, value.getSplatValue<Attribute>());

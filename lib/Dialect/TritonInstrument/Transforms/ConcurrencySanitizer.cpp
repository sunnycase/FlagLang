...
bool canAllocBeInstrumented(Operation *op) {
    if (llvm::any_of(op->getUsers(),
        ...
        op->emitWarning(
            "Allocation is used in a function call, cannot instrument");
        return false;
    }
  ...
  public:
    void runOnOperation() override {
        ...
        if (!barrierValues.empty()) {
            barrierValues.resize(llvm::NextPowerOf2(barrierValues.size() - 1),
                                 0);
        }
        ...
        if (!barrierValues.empty()) {
            ...
            barriers = createBufferPointersTensor(b, MemType::SHARED_MEM,
                                                  barrierValues);
  ...
  private:
    ...
    void addReadChecks(ImplicitLocOpBuilder &b, Value buf, Value pred,
        ...
        if (memType == MemType::SHARED_MEM && wgmmaCommitsAlloc) {
            b.create<tti::ExperimentalCheckOutstandingCommitsOp>(
                ...
                wgmmaCommitsType, "warpgroup_mma operand read", pred);
        }
    }
    ...
    SmallVector<MemEffects> getMemEffects(Operation *op) {
        ...
        if (auto storeOp = dyn_cast<ttng::AsyncTMACopyLocalToGlobalOp>(op)) {
            effects.emplace_back(MemEffects{
                ...
                .trackingKind =
                    MemEffects::TrackingKind::None, // async tma writes
                                                    // not modelled yet
                .buf = storeOp.getSrc()});
        }
        ...
        if (auto mmav5Op = dyn_cast<ttng::TCGen5MMAOp>(op)) {
            SmallVector<std::tuple<Value, Value>> barriersAndPreds =
                llvm::to_vector(llvm::zip(mmav5Op.getBarriers(),
                                          mmav5Op.getBarrierPreds()));
    ...
    void instrumentMemoryOperations(ImplicitLocOpBuilder &b) {
        module.walk([&](Operation *op) {
            ...
            if (!effects.empty()) {
                for (MemEffects effect : effects) {
                    ...
                    if (effect.rw == MemEffects::RW::Read) {
                        // For op that is reading, we only need to check if
                        // anything else is writing to the same buffer.
                        addWriteChecks(b, buf, effect.pred, memType,
                                       effect.hwPipelined);
                        if (effect.trackingKind ==
                            MemEffects::TrackingKind::Barrier) {
                            for (auto [barrier, pred] :
                                 effect.barriersAndPreds) {
                                if (pred && effect.pred) {
                                    pred = b.create<arith::AndIOp>(effect.pred,
                                                                   pred);
                                }
                        ...
                        if (effect.trackingKind ==
                            ...
                            b.create<tti::ExperimentalStageAccessForCommitOp>(
                                buf, buffersTensor[(int)memType],
                                wgmmaCommitsAlloc, wgmmaCommitsType,
                                effect.pred);
                        }
                    ...
                    if (effect.rw == MemEffects::RW::Write) {
                        if (isa<ttg::LocalAllocOp, ttng::TMEMAllocOp>(op)) {
                            // For allocs place insert point after the alloc -
                            // we want to check if it is not overwriting any
                            // earlier allocation, but the memref value can be
                            // referenced only after it is created.
                            b.setInsertionPointAfter(op);
                        }
                        // Op is writing to the buffer, we need to check if
                        // anything else is reading or writing to the same
                        // buffer.
                        addWriteChecks(b, buf, effect.pred, memType,
                                       effect.hwPipelined);
                        ...
                        if (effect.trackingKind ==
                            ...
                            for (auto [barrier, pred] :
                                ...
                                b.create<
                                    tti::ExperimentalCommitWriteWithBarrierOp>(
                                    barrier, barriers,
                                    writeBarriersAlloc[(int)memType],
                        ...
                        if (effect.trackingKind ==
                            ...
                            b.create<tti::ExperimentalStageAccessForCommitOp>(
                                buf, buffersTensor[(int)memType],
                                asyncCpCommitsAlloc, asyncCpCommitsType,
                                effect.pred);
                        }
                        assert(effect.trackingKind !=
                               MemEffects::TrackingKind::wgmmaCommit);
            ...
            if (auto commitOp = dyn_cast<ttng::TCGen5CommitOp>(op)) {
                // Workaround: scan towards the beginning of the current block
                // looking for mmav5s and mark their operands as reads guarded
                // by the barrier.
                ...
                while (prevOp) {
                    auto setBarrier = [&](TypedValue<ttg::MemDescType> buf) {
                        MemType memType = MemType::TENSOR_MEM;
                        if (isa<ttg::SharedEncodingTrait>(
                                buf.getType().getEncoding())) {
                            memType = MemType::SHARED_MEM;
                        }
                        b.create<tti::ExperimentalSetReadBarrierOp>(
                            buf, commitOp.getBarrier(),
                            buffersTensor[(int)memType], barriers,
            ...
            if (auto expectOp = dyn_cast<ttng::BarrierExpectOp>(op)) {
                if (writeBarriersAlloc[(int)MemType::SHARED_MEM]) {
                    b.create<tti::ExperimentalCheckBarrierWritesClearedOp>(
                        ...
                        writeBarriersType[(int)MemType::SHARED_MEM],
                        expectOp.getPred());
                }
            ...
            if (auto wgmmaOp = dyn_cast<ttng::WarpGroupDotOp>(op)) {
                if (wgmmaOp.getIsAsync() == true) {
                    // Add commit (implicit in ttgir) after staging wgmma's
                    // operand for read
                    b.create<tti::ExperimentalCommitAccessesOp>(
    ...
    Value createInitializedScratchMemory(ImplicitLocOpBuilder &b,
        ...
        auto alloc = b.create<tt::gpu::GlobalScratchAllocOp>(
            ptrType, sizeInBytes, elSize);
        createStoreScratchMemory(b, b.getLoc(), alloc, tensor,
                                 tensor.getType());
        return alloc;
    }

...
std::unique_ptr<uint64_t[]> getMatrix(const LinearLayout &layout) {
    ...
    //  |    ↓         ↓         ↓         ↓      |
    //
    // Note `new uint64_t[n]()` is zero-initialized, but `new uint64_t[n]` is
    // not.
...
void assertDimsEqualIgnoringOrder(T &&a, U &&b) {
    ...
    if (as != bs) {
        llvm::report_fatal_error(
            "Dimensions must match, ignoring order, but they "
            "don't.  Got dims: [" +
            Twine(triton::join(a, ", ")) + "] and [" + triton::join(b, ", ") +
            "]");
    }
}
...
void assertDimsSubsetIgnoringOrder(T &&small, U &&big) {
    ...
    if (!llvm::set_is_subset(smallSet, bigSet)) {
        llvm::report_fatal_error(
            "Dimensions must be a subset, ignoring order, but "
...
LinearLayout::checkInvariants(bool requireSurjective) {
    ...
    for (const auto &[inDim, inDimBases] : bases) {
        for (const auto &basis : inDimBases) {
            if (llvm::any_of(basis, [](int32_t b) { return b < 0; })) {
                return "Invalid bases passed to LinearLayout.  Expected all "
                       "basis "
                       "values to be non-negative, but found a negative value "
                       "for "
                       "in dimension '" +
    ...
    for (const auto &[inDim, inDimBases] : bases) {
        for (const auto &basis : inDimBases) {
            if (basis.size() != outDims.size()) {
                return "Invalid bases passed to LinearLayout.  Expect all "
                       "bases to "
    ...
    for (const auto &[outDim, size] : outDims) {
        if (!llvm::isPowerOf2_32(size)) {
            return "Invalid out-dim size " + std::to_string(size) +
                   " for out-dim '" + outDim.str() +
                   "'.  Out-dim sizes must be powers of 2.\n";
        }
    }
        ...
        for (const auto &basis : inDimBases) {
            for (int i = 0; i < basis.size(); i++) {
                if (basis[i] >= outDims[outDimNames[i]]) {
                    return "Invalid basis " + std::to_string(basis[i]) +
                           " for in-dim '" + inDim.str() + "' and out-dim '" +
                           outDimNames[i].str() +
    ...
    if (requireSurjective && !isSurjective()) {
        return "Layout is expected to be surjective, i.e. every `out` "
               "coordinate "
...
LinearLayout LinearLayout::reshapeIns(
    ...
    assert(getTotalInDimSize() ==
           std::accumulate(
               newInDims.begin(), newInDims.end(), 1,
               [&](int32_t acc, auto &inDim) { return acc * inDim.second; }));
...
std::optional<LinearLayout> divideLeft(const LinearLayout &A,
    ...
    for (StringAttr inDim : A.getInDimNames()) {
        ...
        for (int i = 0; i < inB; ++i) {
            for (StringAttr outDim : A.getOutDimNames()) {
                int expected =
                    B.hasOutDim(outDim) ? B.getBasis(inDim, i, outDim) : 0;
        ...
        // Extract the candidate C bases from the remaining (shifted) entries in
        // A.
        for (int i = inB; i < inA; ++i) {
            std::vector<int32_t> candidateBasis;
            for (StringAttr outDim : llvm::make_first_range(cOutDimSizes)) {
                int outB =
                    B.hasOutDim(outDim) ? B.getOutDimSizeLog2(outDim) : 0;
...
std::optional<LinearLayout> divideRight(const LinearLayout &A,
    ...
    for (StringAttr inDim : A.getInDimNames()) {
        ...
        // The remaining inB basis vectors in A should correspond to B after
        // being shifted.
        for (int i = inC; i < inA; ++i) {
            ...
            for (StringAttr outDim : B.getOutDimNames()) {
                ...
                int outC =
                    outA - outB; // Expected log2 size for C in this output.
                int shift = outC;
                int v = A.getBasis(inDim, i, outDim);
        ...
        for (auto [outDimIdx, outDimNameAndSize] :
            ...
            if (inner.hasInDim(inDimName) && inner.hasOutDim(outDimName)) {
                for (int i = 0; i < inner.getInDimSizeLog2(inDimName); i++) {
                    inDimBases[i][outDimIdx] =
                        inner.getBasis(inDimName, i, outDimName);
                }
            }
            if (outer.hasInDim(inDimName) && outer.hasOutDim(outDimName)) {
                int offset = inner.hasInDim(inDimName)
                                 ? inner.getInDimSizeLog2(inDimName)
                                 : 0;
...
bool LinearLayout::isTrivialOver(ArrayRef<StringAttr> dimNames) const {
    ...
    // We can quotient out dimNames iff they don't affect the
    // remainingInDimNames in the result. In other words, we want to check that
    // B is zero, and C is zero, and D is the identity
...
LinearLayout LinearLayout::compose(const LinearLayout &outer) const {
    ...
    for (const auto &[inDim, inDimBases] : bases) {
        ...
        for (const auto &basis : inDimBases) {
            ...
            newInDimBases.push_back(std::vector<int32_t>(newBasesRange.begin(),
                                                         newBasesRange.end()));
        }
    }
...
LinearLayout LinearLayout::invertAndCompose(const LinearLayout &outer) const {
    ...
    // For this, we need to implement our LLVM lowerings by inverting the
    // "outer" layout, and then iterating over the elements from the "this"
    // layout and fetching the corresponding element from the "outer" layout.
    // This exercises the broadcasting that we incentivise via choosing the
    // minimum norm solution in lstsq.
    ...
    for (auto dim : outDims) {
        assert(A.getOutDimSize(dim) >= B.getOutDimSize(dim) &&
               ("A.invertAndCompose(B) called with incompatible output shapes "
                "in " +
    ...
    // TODO(Lezcano): We should return the reduced layout instead of re-adding
    // the identity maps. With this, we'll be able to kill `minimalCvtLayout`

    // Add the identity maps for the dimensions that are the same for both
    // layouts
...
LinearLayout LinearLayout::pseudoinvert() const {
    ...
    for (auto outDim : getOutDimNames()) {
        identity *=
            LinearLayout::identity1D(getOutDimSize(outDim), outDim, outDim);
    }
...
SmallVector<Value> ColumnAction::apply(ValueRange values) const {
    ...
    assert(inDim.str() == "register" &&
           "Values are in registers, so we can only "

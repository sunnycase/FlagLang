...
ensureLayoutNotLargerThan(const LinearLayout &layout,
    ...
    for (auto outDim : llvm::enumerate(layout.getOutDimNames())) {
        ...
        llvm::sort(sortedBases, [](auto a, auto b) {
...
LinearLayout ensureLayoutNotSmallerThan(
    ...
    for (StringAttr outDimName : layout.getOutDimNames()) {
        ...
        ret *= LinearLayout::identity1D(desiredSize / actualSize, kDim,
                                        outDimName);
...
std::optional<ColumnAction> regPermForDivide(const LinearLayout &A,
    ...
    // We can implement this generically for any dimension, but for now we only
    // do it for regs to keep the API simpler
    ...
    SmallVector<size_t> permOrder =
        to_vector(llvm::concat<size_t>(left ? bIndices : remainingIndices,
                                       left ? remainingIndices : bIndices));
...
actionAdditiveStrides(const LinearLayout &layout, const LinearLayout addrLayout,
    ...
    // Note this function assumes that if any registers are used in the
    // addrLayout of the layout (as in ldmatrix/stmatrix) they will be the first
    // non-zero registers within `layout`
    assert(layout.getNumInDims() != 0);
    auto kReg = *layout.getInDimNames().begin();
    assert(kReg.str() == "register");

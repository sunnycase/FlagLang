...
LinearLayout buildReps(MLIRContext *ctx, const LinearLayout &src,
    ...
    SetVector<int32_t> smemSegment(llvm::from_range_t{},
                                   flatten(smem, kSegment));
...
SmallVector<int32_t> intersectionBasis(ArrayRef<int32_t> b1,
    ...
    bool powerOf2 =
        llvm::all_of(b1, isPowerOf2) && llvm::all_of(b2, isPowerOf2);
    if (powerOf2) {
        SmallVector<int32_t> result;
...
std::optional<SmallVector<int32_t>> optimalSwizzlingTile(
    ...
    if (nRegA < nRegB) {
        return optimalSwizzlingTile(b, a, nRegB, nRegA, laneIdTileB,
                                    laneIdTileA);
    }
    assert(nRegA >= nRegB);
    ...
    // is composed of 32/bitwidth register elements and the bases 0, 1st as
    // given by the laneAddr The contiguous tile of ldmatrix.v4.trans for a
    // packed element of size 16 is composed of the bases 2, 3, 4th as given by
    // the laneAddr

    // Note that for register elements, we can choose any register basis we
    // want, but the lane bases are fixed
...
optimalSwizzling(const LinearLayout &src, const LinearLayout &dst,
    ...
    // We work on the flattened tensors as the tensor dimensions are not
    // relevant
    ...
    assert(regsNotZero(src) && "Remove register broadcasting from src. See "
                               "actionRemoveBroadcastedRegs");
    assert(regsNotZero(dst) && "Remove register broadcasting from dst. See "
                               "actionRemoveBroadcastedRegs");
    ...
    // The bank is the complement of the union of the vector and the start of
    // the segments
...
LinearLayout optimalSwizzlingLdSt(const LinearLayout &src,
    ...
    if ((1 << vbasis.size()) * bitwidth < 32) {
        ...
        if (regSrcWarp.size() == regDstWarp.size() && regSrcWarp.size() > 0) {
            ...
            // avoid PRMTs. The comparison of the mins will be strict as the
            // sets removeVec(regSrc) and removeVec(regDst) don't intersect
            if (*llvm::min_element(regSrcWarp) <
                *llvm::min_element(regDstWarp)) {
                largest = regSrcWarp;
                srcFillsBank = true;
            } else {
                largest = regDstWarp;
        ...
        while (vbasis.size() < basesPerBank &&
            ...
            // until we have 32 bits. They will at least avoid bank conflicts in
            // one direction
        ...
        // The idea here is that implementing asymmetric vectorisation without
        // bank conflicts is a bit tricky. Basically, in this case, you need to
        // use the vectorisation base in the swizzling pattern. As such, you
        // would not be able to vectorise all the `ld.shared` instructions that
        // you emit, but just about half of them (the ones that are not
        // swizzled). We don't implement this yet
        if (vbasis.size() > basesPerBank) {
...
optimalSwizzling(const LinearLayout &src, const LinearLayout &dst,
    ...
    if (tiles.empty()) {
        ...
        for (auto [instrs, vbasis, tileSrc, tileDst] : tiles) {
            auto smem = optimalSwizzling(srcFlat, dstFlat, bitwidth, vbasis,
                                         tileSrc, tileDst, src.getOutDims());
            auto [read, write] =
                logBankConflicts(tileSrc, tileDst, smem, bitwidth);
            smems.push_back(
                {read + write, smem, {instrs.first, instrs.second}});
        }
        // Current heuristic: Minimise total bank conflicts
        // We break ties looking at the number of rounds we do to move the data
        auto kReps = StringAttr::get(ctx, "reps");
        auto it =
            llvm::min_element(smems, [kReps](const auto &a, const auto &b) {

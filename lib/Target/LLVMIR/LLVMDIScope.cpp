...
struct LLVMDIScopePass : public impl::LLVMDIScopeBase<LLVMDIScopePass> {
    void setSubprogramAttr(LLVM::LLVMFuncOp funcOp) {
        ...
        if (ModuleOp module = funcOp->getParentOfType<ModuleOp>()) {
            auto fusedCompileUnitAttr =
                module->getLoc()
                    ->findInstanceOf<
                        mlir::FusedLocWith<LLVM::DICompileUnitAttr>>();
        ...
        // Figure out debug information (`subprogramFlags` and
        // `compileUnitAttr`) to attach to the function definition /
        // declaration. External functions are declarations only, and are
        // defined in a different compile unit, so mark them appropriately in
        // `subprogramFlags`, and set an empty `compileUnitAttr`.
        ...
        // DIScopeForLLVMFuncOpPass. I don't find reasons why scopeline should
        // be the column offset
    ...
    void setLexicalBlockFileAttr(Operation *op) {
        ...
        if (auto callSiteLoc = dyn_cast<CallSiteLoc>(opLoc)) {
            ...
            // We assemble the full inline stack so the parent of this loc must
            // be a function
            auto funcOp = op->getParentOfType<LLVM::LLVMFuncOp>();
            auto funcOpLoc = mlir::cast<FusedLoc>(funcOp.getLoc());
            scopeAttr =
                mlir::cast<LLVM::DISubprogramAttr>(funcOpLoc.getMetadata());
            auto loc = CallSiteLoc::get(getNestedLoc(op, scopeAttr, calleeLoc),
                                        callerLoc);
            op->setLoc(loc);
        }
    }

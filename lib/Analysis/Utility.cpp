...
bool shouldUseDistSmem(Attribute srcLayout, Attribute dstLayout) {
    ...
    // Case where CTAsPerCGA of dstLayout in the sliced dim is not 1 is
    // supported
...
bool ReduceOpHelper::isAssociative() {
    ...
    op.walk([&](Operation *nestedOp) -> WalkResult {
        if (isa<arith::AddFOp, arith::MulFOp>(nestedOp)) {
            // Only when the data type is float point and reduce size greater
            // than 2, and has addf or mulf op, we though it's a non-associative
            // reduce.
...
unsigned ScanLoweringHelper::getNonAxisNumBlocks() {
    ...
    for (unsigned i = 0; i < rank; i++) {
        if (i == axis)
            continue;
        numBlocks *=
            ceil<unsigned>(getShape()[i], (contigPerThread[i] *
                                           threadsPerWarp[i] * warpsPerCTA[i]));
    }
    return numBlocks;
}
...
getWarpLayoutConvertDecomposition(RankedTensorType srcTy,
    ...
    //
    // where each is represented by a 'subpermutation' matrix, i.e., a
    // permutation matrix with zero columns possibly inserted. A layout
    // conversion can be viewed as a map P': H_src -> H_dst which factors ll_src
    // = ll_dst \circ P'.
    //
    ...
    //
    // As a permutation, P can be viewed as a product of cycles permuting lane
    // and register index bits. Any such permutation can be expressed as a
    // composition
    //
    ...
    // involving lane bits and register bits, respectively. Such a
    // representation is not unique, and we choose the factorization method
    // which slices out subsequences of consecutive lane bits from cycles
    // involving both bit types. Further explanation of this method is below.
    //
    ...
    // factor out mixed transpositions to build `mixedTranspositions`, `pReg`,
    // and `pLane`. Finally, we determine any selectors needed for byte permute
    // instructions in place of `selp` instructions when packing registers.

    // We remove any broadcasting in the register dimensions of the layouts
    // before forming the permutation `P` as the components of the decomposition
    // directly inform the number of emitted instructions, and leaving
    // broadcasting in would unnecessarily inflate the count.
    ...
    // size in each of the layouts, we first pad the lane and register
    // dimensions with zero vectors if needed.
    auto *ctx = srcTy.getContext();
    StringAttr kReg = StringAttr::get(ctx, "register");
    StringAttr kLane = StringAttr::get(ctx, "lane");

    // Determine the target sizes of the register and lane dimensions for
    // padding.
    ...
    // fill in zero columns, prioritizing producing fixed points. As we only
    // need the basis vectors of `P`, we never actually produce the
    // LinearLayout.
    ...
    for (auto [srcZeroLoc, dstZeroLoc] :
         llvm::zip(srcFreeZeros, dstFreeZeros)) {
    ...
    for (auto dim : inDimNames) {
        ...
        for (int i = 0; i < inDimSize; ++i) {
            ...
            // Start a new cycle, tracking the entry basis vector and the
            // 'current' one as we walk the cycle.
            StringAttr entryDim = dim;
            int32_t entryIdx = i;
            StringAttr currDim = entryDim;
            int32_t currIdx = entryIdx;

            // We slice out subsequences of consecutive lane basis vectors
            // appearing in mixed cycles by factoring out transpositions (r_i
            // l_j) as in
            //
            // (.. r_m l_j .. l_k r_i ..) = (r_i l_j) * (.. r_m r_i ..)(l_j ..
            // l_k).
            //
            // The permutations are applied right-to-left, and the block `l_j ..
            // l_k` indicates a contiguous subsequence of lane basis vectors.
            // Note that the transposition does not commute with the other two
            // cycles.
            //
            // The following variables are used to track the start and end
            // points of such subsequences.
            ...
            do {
                ...
                // Set a `pReg` or `pLane` vector, or mark an r->l or l->r
                // transition.
                ...
                // If a subsequence of the form (.. r_m l_j .. l_k r_i ..) has
                // been found, perform the prescribed factorization.
                if (regEndIdx >= 0) {
...
getTranspositionSelectors(SmallVector<std::pair<int, int>> &mixedTranspositions,
    ...
    // with a mixed transposition, resulting in byte permute instructions
    // instead of `select` instructions. After processing, no low register bits
    // appear in the returned list of mixed transpositions.
    ...
    // is reduced to (r3 r2) and the mixed transposition recorded is (r3 l0),
    // with the effects of (r3 r0) and (r3 r1) encoded in the returned
    // selectors. In general, low bits occurring immediately before l_j modify
    // the selectors of the `prmt` before the shuffle, while low bits occurring
    // immediately after l_k modify the selectors of the `prmt` after the
    // shuffle. Unmodified selectors correspond to `select` instructions. Cases
    // like (l0 r0 r1) must be handled by selecting a 'partner' bit that is not
    // used in another mixed transposition and conjugating out a low bit:
    //
    ...
    // choice. We reorder the transpositions to guarantee this during
    // processing.
    ...
    std::stable_partition(mixedTranspositions.begin(),
                          mixedTranspositions.end(), nextHighFree);
    ...
    auto findPartner = [&](int lowBit, auto &preShufLoBits) {
        if (nPack == 2) {
            ...
            int b = next(otherLow);
            if (next(lowBit) == lowBit && b >= nPack &&
                !pairedRegBits.contains(b) &&
    ...
    for (auto p : mixedTranspositions) {
        ...
        if (forwardEnd != cycle.end()) {
            if (*forwardEnd == rBit || !pairedRegBits.contains(*forwardEnd)) {
                // End at original or unpaired high bit. E.g. (l0 r0 r2) or (l0
                // r2) No conjugation needed.
            ...
            if (next(rBit) != rBit && pairedRegBits.contains(next(rBit))) {
                ...
                // Isolated low bits with single mixed transposition. E.g. (l0
                // r0 r1)
        ...
        auto [topPreSel, botPreSel] =
            generateSelectors(head, tail, preShufLoBits);
        ...
        // In noncommutative cases, post-shuffle selectors of non-leading terms
        // come from a single low bit by design, so we can determine where to
        // insert a non-terminal factor by examining processed selectors.
        if (!preShufLoBits.empty()) {
            uint16_t sel =
                (nPack - preShufLoBits.back()) == 2 ? 0x6240 : 0x5410;
            auto it = llvm::find_if(
                ret, [&](auto &t) { return t.topPostSel == sel; });
            ret.insert(it, info);
        } else {
            ret.push_back(info);
        }
...
unsigned ScanLoweringHelper::getAxisThreadStride() {
    ...
    // to avoid this
    auto threadsPerWarp =
        encoding.basesPerDim(kThread, /*skipBroadcast=*/false);
    auto order = getOrder();
    unsigned stride = 1;
    for (unsigned dim : order) {
        if (dim == getAxis())
...
unsigned GatherLoweringHelper::getScratchSizeInBytes() {
    ...
    // Otherwise, performing the gather will require scratch space to
    // communicate the source tensor across threads. For now, assume the whole
    // source tensor is written back to shared memory.
...
bool GatherLoweringHelper::isWarpLocal() {
    ...
    // The tensor layouts must be distributed layouts, where the basis matrix is
    // a subpermutation matrix (permutation matrix plus zeros for broadcasting).
    ...
    //
    // But due to broadcasting, the matrix might not be invertible. But since
    // the matrix is a permutation matrix (checked below), we can instead query
    //
    //   srcLayout.sublayoutIsZero({kBlock, kWarp}, {kGatherDim})
    //
    // Which implies that changing the warp will not change the gather
    // dimension. And since there is no swizzling, this applies to all warps.
    ...
    // If the gather axis `dimN` is invariant to the warp, but the `(block,
    // warp)` mapping to all other dimensions must be the same for both layouts.
    // If so, then the warp that owns a particular index element also owns all
    // the source elements it could index into.
    ...
    // operation are contained within a warp. The subsequent constraints
    // simplify codegen.

    // Require that for any given gather column, the threads mapped to the
    // column in the index and source tensors are the same. This means we don't
    // need to xor shuffle across threads before emitting index shuffles; we
    // push warp shuffling to layout conversions.
...
bool supportMMA(triton::DotOp op, int version) {
    ...
    if (version == 3) {
        ...
        // We cannot use MMA_V3 if we need to accumulate in F32 within the MMA
        // op.
...
bool supportMMA(Value value, int version) {
    ...
    // We cannot get both the operand types(in TypeConverter), here we assume
    // the types of both the operands are identical here.
...
struct DFSState {
    ...
    /// We mark each op as ready if all its operands and parents ops are seen.
    /// If an op is ready, we add it to the queue. Otherwise, we keep adding its
...
class ConstantAnalysis : public DataFlowAnalysis {
  public:
    ...
    LogicalResult visit(ProgramPoint *point) override {
        ...
        // Dead code analysis requires every operands has initialized
        // ConstantValue state before it is visited.

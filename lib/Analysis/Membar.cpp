...
void MembarOrFenceAnalysis::resolve(FunctionOpInterface funcOp,
    ...
    while (!blockList.empty()) {
        ...
        Block::iterator startIt = block.second.isValid()
                                      ? std::next(block.second)
                                      : block.first->begin();
        ...
        if (outputBlockInfoMap.count(block) &&
            ...
            // If we have seen the block before and the inputBlockInfo is the
            // same as the outputBlockInfo, we skip the successors
            continue;
        }
        // Update the current block. The block transfer function is not
        // monotonic, so overwrite the output state entirely.
    ...
    funcOp.walk<WalkOrder::PreOrder>([&](triton::ReturnOp returnOp) {
        ...
        auto maxIt =
            llvm::max_element(virtualBlocks, [&](auto &lhs, auto &rhs) {
                assert(lhs.first.first == rhs.first.first);
                Block::iterator lhsIt = lhs.first.second,
                                rhsIt = rhs.first.second;
    ...
    if (br && isa<RegionBranchOpInterface>(br->getParentOp())) {
        ...
        for (RegionSuccessor &region : regions) {
            if (region.isParent()) {
                ...
                successors.emplace_back(parent->getBlock(),
                                        parent->getIterator());
            } else {
                Block &block = region.getSuccessor()->front();
                successors.emplace_back(&block, Block::iterator());
...
void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
    ...
    if (isa<triton::gpu::AsyncWaitOp, triton::nvidia_gpu::TMAStoreWaitOp>(op) &&
        ...
        // If the current op is an async wait and the next op is not a barrier
        // we insert a barrier op and sync
    ...
    if (isa<triton::CallOp>(op)) {
                ...
                if (auto value = effectInstance.getValue()) {
                    for (auto bufferId : allocation->getBufferIds(value)) {
                        if (bufferId != Allocation::InvalidBufferId) {
                            if (isa<MemoryEffects::Write>(
                                    effectInstance.getEffect()))
                                curBlockInfo
                                    .syncWriteIntervals
                                        [allocation->getAllocatedInterval(
                                            bufferId)]
                                    .insert(op);
                            else if (isa<MemoryEffects::Read>(
                                         effectInstance.getEffect()))
                                curBlockInfo
                                    .syncReadIntervals
                                        [allocation->getAllocatedInterval(
        ...
        // If this op is may be signalling other threads asynchronously, make
        // sure all shared memory transactions are complete beforehand.
        if (isa<triton::nvidia_gpu::ArriveBarrierOp>(op)) {
            Interval<size_t> allIntervals(0,
    ...
    // starting from a shared memory write, followed by a series of shared
    // memory read/write operations, and ending with a shared memory read, i.e.,
    // shared memory write -> ... -> shared memory read.
    if (scratchBufferId != Allocation::InvalidBufferId) {
        ...
        // the internal shared-memory write and read phases. For these ops, we
        // must not globally clear pending dependencies.

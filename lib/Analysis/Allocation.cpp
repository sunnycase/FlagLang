    ...
    if (!result.use_empty()) {
        if (auto tensorTy = dyn_cast<RankedTensorType>(result.getType())) {
            ...
            if (llvm::any_of(freeVariableMasks, [](auto variableMask) {
                ...
                smemShape =
                    convertType<unsigned>(gpu::getShapePerCTA(tensorTy));
            }
        } else {
...
class AllocationAnalysis {
  public:
    AllocationAnalysis(Operation *operation,
        ...
        : operation(operation),
          funcAllocMap(funcAllocMap),
          allocation(allocation),
          scratchSizeGetter(scratchSizeGetter) {
        run();
    }
  ...
  private:
    ...
    /// Initializes explicitly defined shared memory values for a given
    /// operation.
    void getExplicitValueSize(Operation *op) {
        ...
        int64_t numElems = 0;
        if (auto paddedEnc = dyn_cast<gpu::PaddedSharedEncodingAttr>(
    ...
    void getScratchValueSize(Operation *op) {
        ...
        if (auto func = dyn_cast<FunctionOpInterface>(op)) {
            ...
            // Warp specialization communicates states over shared memory to
            // each warp. Add space for an i8 for each warpgroup warp.
            func.walk([&](gpu::WarpSpecializeOp op) {
                numWarpIndices =
                    std::max(numWarpIndices, op.getTotalPartitionWarps());
            });
            maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(op,
                                                                numWarpIndices);
            return;
        }
    ...
    void getValuesAndSizes() {
        ...
        // Run the analysis rooted at every isolated from above operation,
        // including the top-level function but also any nested regions.
    ...
    /// Extends the liveness range by unionizing the liveness range of the
    /// aliased values because each allocated buffer could be an alias of
    /// others, if block arguments are involved.
    ...
    void resolveScratchBufferLiveness(
        ...
        auto processScratchMemory = [&](const auto &container) {
            for (auto [op, buffer] : container) {
                ...
                // function. This memory is used for warp specialization
                // codegen.
                // FIXME: Spooky-action-at-a-distance. Find a better way to
                // model this.
                if (op == operation) {
                    bufferRange.insert(
                        {buffer, Interval(size_t(),
                                          std::numeric_limits<size_t>::max())});
                    continue;
                }

                // Any scratch memory's live range is the current operation's
                // live range.
                bufferRange.insert({buffer, Interval(operationId.at(op),
                                                     operationId.at(op) + 1)});
                LLVM_DEBUG({
                    llvm::dbgs() << "-- buffer " << buffer->id << "; value: ";
                    op->dump();
    ...
    void resolveLiveness() {
        ...
        // otherwise %5 liveness range ends before the child operation's
        // liveness range ends.
    ...
    void computeOffsets() {
        ...
        // chance to overlap with multiple other buffers, and allocating them
        // first (by calculateStarts) ensures a higher chance that they will
        // occupy a standalone smem slot.
        llvm::stable_sort(
        ...
        // could interfere after offset bumping if their liveness ranges
        // overlap. Therefore, we rerun the interference graph algorithm after
        // bumping so that we regroup the buffers and color them again. Since we
        // always increase the buffer offset and keep reducing conflicts, we
    ...
    void calculateStarts(const SmallVector<BufferT *> &buffers) {
        ...
        // we won't know if there has been an overlap without using graph
        // coloring. Start -> Liveness Range
        ...
        while (!xBuffers.empty()) {
            ...
            auto bufferIt = std::find_if(
                xBuffers.begin(), xBuffers.end(), [&](auto *buffer) {
                    auto xRange = bufferRange[buffer];
                    bool res = xRange.intersects(range);
                    for (const auto &val : tripleMap)
                        res = res && !val.second.intersects(
                                         xRange); // only one buffer intersect
                    return res;
                });
            if (bufferIt != xBuffers.end()) {
                ...
                // TODO(Keren): A buffer's size shouldn't be determined here,
                // have to clean it up
                ...
                // We could either insert (range.start, xRange.start) or
                // (range.start, xRange.end), both are correct and determine the
                // potential buffer offset, and the graph coloring algorithm
                // will solve the interference, if any
                if (range.start() < xRange.start())
                    tripleMap.insert(
                        {offset, Interval{range.start(), xRange.end()}});
                if (xRange.end() < range.end())
                    tripleMap.insert(
                        {offset, Interval{xRange.start(), range.end()}});
    ...
    void buildInterferenceGraph(const SmallVector<BufferT *> &buffers,
        ...
        for (auto x : buffers) {
            for (auto y : buffers) {
                ...
                // Buffers interfere if their allocation offsets overlap and
                ...
                // Buffers also interfere if their allocation offsets overlap
                // and they exist within regions that may execute simultaneously
                // with respect to each other.
                auto wsx =
                    x->owner->getParentWithTrait<OpTrait::AsyncRegions>();
                auto wsy =
                    y->owner->getParentWithTrait<OpTrait::AsyncRegions>();
                if (wsx && wsy && wsx == wsy &&
                    x->owner->getParentRegion() !=
                        y->owner->getParentRegion() &&
                    xSizeRange.intersects(ySizeRange)) {
                    interference[x].insert(y);
                }
            }

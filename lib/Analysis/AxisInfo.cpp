...
int64_t getDivisibilityFromContiguity(const AxisInfo &lhs, const AxisInfo &rhs,
    ...
    if (lhs.getContiguity(d) == rhs.getContiguity(d) ||
        ...
        // If unresolved, we can first perform a loose bound gcd since the
        // unknown contiguity will be resolved in the end.
...
class BinaryOpVisitorImpl : public AxisInfoVisitorImpl<OpTy> {
    ...
    AxisInfo getAxisInfo(
        ...
        for (auto d = 0; d < rank; ++d) {
            if (constantValue.has_value()) {
                ...
                divisibility.push_back(
                    getDivisibility(op, lhsInfo, rhsInfo, d));
            }
        }
  ...
  protected:
    ...
    getConstantValue(OpTy op, const AxisInfo &lhs, const AxisInfo &rhs) {
        return {};
    }
};
  ...
  private:
    ...
    void setToEntryState(dataflow::Lattice<AxisInfo> *lattice) override {
        propagateIfChanged(lattice,
                           lattice->join(AxisInfo::getPessimisticValueState(
                               lattice->getAnchor())));
    }
  ...
  public:
    ...
    AxisInfo getAxisInfo(
        ...
        if (splatAttr && splatAttr.getElementType().isIntOrIndex()) {
            int64_t value =
                splatAttr.template getSplatValue<APInt>().getZExtValue();
            ...
            return AxisInfo(
                ...
                /*constancy=*/
                AxisInfo::DimVectorT(ty.getShape().begin(),
                                     ty.getShape().end()),
                /*knownConstantValue=*/{value});
        }
        return AxisInfo();
...
class AddSubOpAxisInfoVisitor final : public BinaryOpVisitorImpl<OpTy> {
  ...
  private:
    ...
    int64_t getDivisibility(OpTy op, const AxisInfo &lhs, const AxisInfo &rhs,
        ...
        // lhs + rhs = k * d_lhs + p * d_rhs = (k * k' + p * p') * gcd(d_lhs,
        // d_rhs)
        ...
        if constexpr (std::is_same_v<OpTy, triton::AddPtrOp>) {
            ...
            //  %0 = mul %rhs, %elemSize
            //  %ptr = add %lhs, %0
            // The result will still be contiguous in terms of elements but not
            // bytes For example: addptr [16] : !ptr<i32>, [0, 1, 2, 3] : i32 ->
            // !ptr<i32> returns: [16, 20, 24, 28] : !ptr<i32> with element
            // locations: [4, 5, 6, 7] It is "strided contiguous" with a
            // divisibility of 16 bytes
            auto rank = lhs.getRank();
            auto elemSize = std::max<int64_t>(
                1, triton::getPointeeBitWidth(op.getPtr().getType()) / 8);
            rhsDivisibility =
                multiplyDivisor(rhs.getDivisibility(dim), elemSize);
        }
  ...
  private:
    ...
    int64_t getDivisibility(OpTy op, const AxisInfo &lhs, const AxisInfo &rhs,
        ...
        if (rhs.getConstantValue().has_value() &&
            rhs.getConstantValue().value() == 1)
            return lhs.getDivisibility(dim);
        // Case 3: lhs has contiguity of 1 in this dimension and rhs is a power
        // of 2
        if (rhs.getConstantValue().has_value() &&
  ...
  private:
    ...
    int64_t getDivisibility(OpTy op, const AxisInfo &lhs, const AxisInfo &rhs,
        ...
        if (rhs.getConstancy(dim) > 1) {
            // lhs: d_lhs * k = gcd(d_lhs, d_rhs) * k' * k = gcd(d_lhs, d_rhs) *
            // k'' rhs: d_rhs * p = gcd(d_lhs, d_rhs) * p' * p = gcd(d_lhs,
            // d_rhs) * p'' lhs = gcd(d_lhs, d_rhs) * k'' = gcd(d_lhs, d_rhs) *
            // d + r r must be divisible by gcd(d_lhs, d_rhs)
  ...
  public:
    ...
    AxisInfo getAxisInfo(
        ...
        if (opInfo.getConstantValue().has_value()) {
            ...
            newDivisibility =
                highestPowOf2Divisor(opInfo.getConstantValue().value());
            ...
            for (int d = 1; d < opInfo.getRank(); ++d) {
                newDivisibility =
                    gcd(newDivisibility, opInfo.getContiguity(d) > 1
                                             ? 1
    ...
    AxisInfo getAxisInfo(
        ...
        for (short d = 0; d < rank; ++d) {
            ...
            if (lhsInfo.getConstantValue().has_value() &&
                ...
                if ((gtPredicate(getPredicate(op)) ||
                     lePredicate(getPredicate(op))) &&
                    ...
                    // lhs gt rhs: 0, 0, 0, 0
                    constHint =
                        std::max(constHint, gcd(rhsInfo.getContiguity(d),
                    ...
                    // lhs ge rhs: 1, 1, 1, 1
                    constHint =
                        std::max(constHint, gcd(lhsInfo.getContiguity(d),
                                                lhsInfo.getDivisibility(d),
                                                rhsInfo.getDivisibility(d)));
                }
            }
...
void AxisInfo::initPessimisticStateFromFunc(int argNumber, T funcOp,
    ...
    for (auto [vec, attrName] : retVecs) {
        ...
        if (auto int_attr = dyn_cast_or_null<IntegerAttr>(attr))
            *vec = DimVectorT(contiguity->size(),
    ...
    if (blockArg && blockArg.getOwner()->isEntryBlock()) {
        ...
        if (auto fun = dyn_cast<FunctionOpInterface>(op)) {
            initPessimisticStateFromFunc(blockArg.getArgNumber(), fun,
                                         ...
                                         &knownConstancy);
        } else if (isa<RegionBranchOpInterface,
                       gpu::WarpSpecializePartitionsOp>(op)) {
            // scf::ForOp, scf::IfOp, scf::WhileOp,
            // gpu::WarpSpecializePartitionsOp Control flow operations are
            // initialized with "unknown" state: the maximum possible
        ...
        // Other operations are conservatively initialized with the lowest
        // possible divisibility, contiguity, and constancy unless they have
        // specified.
        if (Attribute attr = op->getDiscardableAttr("tt.divisibility")) {
            auto vals = cast<DenseElementsAttr>(attr).getValues<int>();
...
unsigned ModuleAxisInfoAnalysis::getContiguity(Value value) {
    ...
    // Get the pointee type if we have a tensor of ptrs to compute contiguity
    // for
...
unsigned ModuleAxisInfoAnalysis::getMaskAlignment(Value mask) {
    ...
    auto alignment =
        std::max<unsigned>(axisInfo->getConstancy(maskOrder[0]), 1);
    ...
    for (auto entry : llvm::enumerate(callOp->getOperands())) {
        ...
        auto setAttrFn = [&](StringRef attrName, int64_t prevValue) {
            ...
            if (callee.getArgAttrOfType<IntegerAttr>(index, attrName)) {
                curValue = callee.getArgAttrOfType<IntegerAttr>(index, attrName)
                               .getInt();
            }
            auto attr =

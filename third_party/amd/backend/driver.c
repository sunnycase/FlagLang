...
#define HIP_SYMBOL_LIST(FOR_EACH_ERR_FN, FOR_EACH_STR_FN)                      \
    ...
    FOR_EACH_ERR_FN(hipGetDeviceProperties, hipDeviceProp_t *prop,             \
                    int deviceId)                                              \
    FOR_EACH_ERR_FN(hipModuleLoadDataEx, hipModule_t *module,                  \
...
static int checkDriverVersion(void *lib) {
    ...
    const int hipMajVersion =
        TRITON_HIP_DRIVER_EXTRACT_MAJOR_VERSION(hipVersion);
...
bool initSymbolTable() {
    ...
    // Go through the list of search paths to dlopen the first HIP driver
    // library.
        ...
        if (code != HIP_SUCCESS) {
            {
                ...
                snprintf(err, sizeof(err), "%s Code: %d, Messsage: %s", prefix,
                         code, str);
...
static PyObject *loadBinary(PyObject *self, PyObject *args) {
    ...
    hipSymbolTable.hipFuncGetAttribute(&n_regs, HIP_FUNC_ATTRIBUTE_NUM_REGS,
                                       fun);
    hipSymbolTable.hipFuncGetAttribute(
        &n_spills, HIP_FUNC_ATTRIBUTE_LOCAL_SIZE_BYTES, fun);
    hipSymbolTable.hipFuncGetAttribute(

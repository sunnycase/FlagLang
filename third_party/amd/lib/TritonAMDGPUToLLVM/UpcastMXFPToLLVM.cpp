...
static void upcast8xMxfp4(RewriterBase &rewriter, Location loc,
    ...
    if (isaFamily == AMD::ISAFamily::CDNA4) {
        ...
        for (int i = 0; i < 8; i++) {
            auto result = useFp16
                              ? mxfpScaleFp16(rewriter, loc, vf16[i], scale,
                                              op.getFastMath())
                              : mxfpScaleBf16ViaF32(rewriter, loc, vf16[i],
                                                    scale, op.getFastMath());
...
static void upcast4xMxfp8(RewriterBase &rewriter, Location loc,
    ...
    if (isaFamily == AMD::ISAFamily::CDNA4) {
        ...
        for (int i = 0; i < 4; i++) {
            auto result =
                useFp16 ? mxfpScaleFp16(rewriter, loc, xVals[idx + i], scale,
                                        fastMath)
...
class UpcastMXFPOpPattern
  ...
  public:
    ...
    matchAndRewrite(amdgpu::UpcastMXFPOp op, OpAdaptor adaptor,
        ...
        // Given that MFMA layout for the A tensor arranges thread in a
        // column-major manner, for the current tid, it's at row (tid % mDim).
        // When we set up blocked layout for the A scale tensor, we made sure
        // that it has a threadsPerWarp = [M=mDim, K=64/mDim]. So the threads
        // holding scale values for the current thread starts at ((tid % mDim) *
        // (64 / mDim)).
        Value offset = b.mul(b.urem(laneId, b.i32_val(mDim)),
                             b.i32_val(numThreads / mDim));

        if (mDim == 32) {
            // One mfma32 intrinsic processes a 32x8 A tensor slice. Due to how
            // we tile, the same warp owns the whole K dim. Inside a warp, each
            // thread only holds 4 consecutive elements along K--a 1x4 vector.
            // We need to tile the warp 4 times to cover 32 values along K. So
            // for a thread, the first 4 1x4 vectors it holds shares the first
            // scale value at row (tid % mDim). the second 4 1x4 vectors shares
            // the second scale value at row (tid % mDim); and so forth.
            std::array<Value, 2> scaleThreads = {offset,
                                                 b.add(offset, b.i32_val(1))};

            for (auto [i, scaleVal] : llvm::enumerate(scaleVals)) {
                std::array<Value, 2> si = {
                    targetInfo.shuffleIdx(rewriter, loc, scaleVal,
                                          scaleThreads[0]),
                    targetInfo.shuffleIdx(rewriter, loc, scaleVal,
                                          scaleThreads[1]),
                };
                ...
                if (isPacked) {
                    for (int j = 0; j < 16; j += 4) {
                        auto idx = 16 * i + j;
                        upcast8xMxfp4(rewriter, loc, isaFamily, op, xVals,
                                      useFp16, idx, si[j / 8], yVals);
                    }
                } else {
                    for (int j = 0; j < 32; j += 4) {
                        auto idx = 32 * i + j;
                        upcast4xMxfp8(rewriter, loc, isaFamily, xVals, useFp16,
                                      fpType, idx, si[j / 16], op.getFastMath(),
                                      yVals);
                    }
                }
            }
            ...
            assert(mDim == 16);
            // One mfma16 intrinsic processes a 16x16 A tensor slice. Similarly,
            // we need to tile the warp 2 times to cover 32 values. So for a
            // thread, the first 2 1x4 vectors shares the first scale value at
            // row (tid % mDim).
            std::array<Value, 4> scaleThreads = {
                offset, b.add(offset, b.i32_val(1)),
                b.add(offset, b.i32_val(2)), b.add(offset, b.i32_val(3))};
            ...
            for (auto [i, scaleVal] : llvm::enumerate(scaleVals)) {
                ...
                if (isPacked) {
                    for (int j = 0; j < 16; j += 4) {
                        auto idx = 16 * i + j;
                        upcast8xMxfp4(rewriter, loc, isaFamily, op, xVals,
                                      useFp16, idx, si[j / 4], yVals);
                    }
                } else {
                    for (int j = 0; j < 32; j += 4) {
                        auto idx = 32 * i + j;
                        upcast4xMxfp8(rewriter, loc, isaFamily, xVals, useFp16,
                                      fpType, idx, si[j / 8], op.getFastMath(),
                                      yVals);
                    }
                }
            }
        ...
        Value result = packLLElements(loc, getTypeConverter(), yVals, rewriter,
                                      op.getType());

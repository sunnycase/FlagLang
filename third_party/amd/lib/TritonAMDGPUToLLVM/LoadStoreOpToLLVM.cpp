...
LogicalResult emitFence(Operation *op, ConversionPatternRewriter &rewriter,
    ...
    //             A "S_WAITCNT vmcnt(0)" is needed after to ensure the
...
struct LoadStoreConversionBase {
    ...
    Value createZeroVector(OpBuilder &builder, Location loc,
        ...
        Value zeroVal =
            builder.create<LLVM::ConstantOp>(loc, vecTy, denseValue);
        return zeroVal;
    }
    ...
    SmallVector<Value>
    getMaskElemsAndUpdateVeclen(ConversionPatternRewriter &rewriter,
                                Location loc, Value llMask, Value mask,
                                unsigned &vec) const {
...
struct DirectToLdsLoadConversionBase : public LoadStoreConversionBase {
    ...
    // exceed the supported load width because splitting them would cause
    // strided (non coalesced) writes. Additionally:
    ...
    //   2) For swizzled shared encodings we need to transfer the swizzling to
    //   the
    //      source pointers. For now this is done by swizzling the pointers
    //      between the lane of a warp via permute. This only works if the
    //      swizzle pattern does not exchange elements between warps which holds
    //      for all our swizzle patterns. There is still a check performed to
    //      not silently produce wrong results if we invalidate the condition in
    //      the future
    ...
    emitSwizzledLaneOffsets(RewriterBase &rewriter, Operation *op,
        ...
        // For each load compute the difference between the flat and the
        // swizzled linear offsets into shared memory
        ...
        for (int i = 0; i < numberOfLoads; i++) {
            ...
            Value swizzledOffset =
                applyLinearLayout(loc, rewriter, regToSharedSwizzled,
                    ...
                    .second;
            Value flatOffset =
                applyLinearLayout(loc, rewriter, regToSharedFlat, indices)[0]
                    .second;
    ...
    SmallVector<Value>
    zipLoadValues(RewriterBase &rewriter, Location loc, unsigned vec,
        ...
        auto structTy = LLVM::LLVMStructType::getLiteral(
            rewriter.getContext(),
            ArrayRef<Type>{srcTy, i1_ty, otherTy, i32_ty});
        for (int i = 0; i < srcElems.size(); i++) {
            ...
            // swizzleOffset are per vec so we need to duplicate values vec
            // times
    ...
    auto unzipLoadValues(RewriterBase &rewriter, Location loc, int startIdx,
        ...
        if (hasOther) {
            for (int i = 0; i < vec; i++) {
                otherElems.push_back(
                    b.extract_val(otherTy, values[startIdx + i], 2));
            }
        }
    ...
    void applySwizzling(RewriterBase &rewriter, Location loc,
                        Value &srcOrOffset, Value &mask, Value laneId,
        ...
        if (mask) {
            mask =
                shuffleMask(rewriter, b, loc, targetInfo, swizzledLaneId, mask);
        }
    }
    ...
    void lowerDirectToLDSLoad(
        ...
        std::function<SmallVector<Value>(
            RewriterBase &, Location, ArrayRef<Value>, Value, int, VectorType)>
            lowerInst) const {
        TritonLLVMOpBuilder b(loc, rewriter);
        auto *ctx = rewriter.getContext();
        ...
        auto maskSpanAffineOffset =
            SharedMemoryObject::getMaskSpanOffsets(dstTy);
        auto [_, warpId] = getLaneAndWarpId(rewriter, loc);
        // We pass laneId==0 because GFX9 requires a scalar base pointer into
        // LDS
        lowerLdSt(
    ...
    void emitOtherStore(RewriterBase &rewriter, Location loc,
                        const LLVMTypeConverter *typeConverter,
                        VectorType vecTy, Value mask,
                        ArrayRef<Value> otherElems, Value shmemAddr,
                        Value laneId, bool hasSwizzling,
                        Value swizzleLaneOffset) const {
        TritonLLVMOpBuilder b(loc, rewriter);
        Value storeVal = packElementRangeIntoVector(rewriter, typeConverter,
                                                    loc, vecTy, otherElems, 0);
        ...
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            ...
            if (otherElems.size() != 0)
                falseVal = packElementRangeIntoVector(
                    ...
                    cast<VectorType>(vecTy), otherElems, vecStart);
...
struct BufferLoadOpConversion
    ...
    matchAndRewrite(triton::amdgpu::BufferLoadOp op, OpAdaptor adaptor,
        ...
        SmallVector<Value> offsetElems =
            unpackLLElements(loc, llOffset, rewriter);
        ...
        // Create the resource descriptor and then emit the buffer_load
        // intrinsic(s)
        Value rsrcDesc =
            bufferEmitter.createResourceDescriptor(llPtr, llStride);
        SmallVector<Value> loadedVals;
        Type vecTy = LLVM::getVectorType(valueElemTy, vec);
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            Value pred = mask ? maskElems[vecStart] : b.int_val(1, 1);
...
struct BufferLoadToLocalOpConversion
    ...
    matchAndRewrite(triton::amdgpu::BufferLoadToLocalOp op, OpAdaptor adaptor,
        ...
        //  2. The mask (if present) has "alignment" N, meaning that each group
        //  of N
        ...
        auto resElemTy =
            getTypeConverter()->convertType(dstTy.getElementType());
        ...
        auto emitBufferLoadLds =
            [this, &op, &b, &bufferEmitter, &rsrcDesc, laneId = laneId,
             threadPred, offsetTy, otherTy, hasOther, hasSwizzling](
            ...
            auto [offsetElem, maskElem, otherElems, swizzleLaneOffset] =
                unzipLoadValues(rewriter, loc, startIdx, loadVals, offsetTy,
            ...
            // If other=0.0 we remove other in canonicalizePointers and we can
            // use out of bounds to store 0 to LDS. So if we have other values
            // we need to predicate to not overwrite the other stores
            Value cond = hasOther ? b.and_(threadPred, maybeSwizzledMaskElem)
                                  : threadPred;

            auto [loadBlock, afterLoadBlock] =
                emitBranch(rewriter, loc, threadPred);
            ...
            if (hasOther) {
                emitOtherStore(rewriter, loc, this->getTypeConverter(), vecTy,
                               ...
                               hasSwizzling, swizzleLaneOffset);
            }
...
struct AsyncCopyGlobalToLocalOpConversion
    ...
    matchAndRewrite(triton::gpu::AsyncCopyGlobalToLocalOp op, OpAdaptor adaptor,
        ...
        Value threadPred = emitRedundantThreadPredicate(
            getFreeVariableMasks(srcTy), rewriter, loc, targetInfo);
        ...
        auto emitGlobalLoadLds =
            ...
            if (hasSwizzling)
                applySwizzling(rewriter, loc, srcElem, maybeSwizzledMaskElem,
                               laneId, swizzleLaneOffset);
...
struct StoreOpConversion : public ConvertOpToLLVMPattern<triton::StoreOp>,
    ...
    matchAndRewrite(triton::StoreOp op, OpAdaptor adaptor,
        ...
        Value threadPred = emitRedundantThreadPredicate(freeVarMasks, rewriter,
                                                        loc, targetInfo);
        ...
        for (size_t vecStart = 0; vecStart < elemsPerThread; vecStart += vec) {
            ...
            Value storeVal = packElementRangeIntoVector(
                rewriter, this->getTypeConverter(), loc,
                cast<VectorType>(vecTy), valueElems, vecStart);
...
struct BufferAtomicRMWOpConversion
    ...
    matchAndRewrite(triton::amdgpu::BufferAtomicRMWOp op, OpAdaptor adaptor,
        ...
        //    We set GLC=1, to return the old value. Atomics in GFX942 execute
        //    with either device (default) or system scope (controlled by the
        //    sc1 flag). This is distinct from the memory scope of the atomic
        //    (i.e, the memory fences which appear before/after the ops).
        ...
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            ...
            Value loadVal = bufferEmitter.emitAtomicRMW(
                atomicRmwAttr, vecTy, rsrcDesc, offsetElems[vecStart], storeVal,
                pred, hasUsers);
    ...
    matchAndRewrite(triton::amdgpu::BufferAtomicCASOp op, OpAdaptor adaptor,
        ...
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            ...
            Value casStoreVal = packElementRangeIntoVector(
                rewriter, this->getTypeConverter(), loc,
                cast<VectorType>(vecTy), valElems, vecStart);
            // Create the cmp val
            Value casCmpVal = packElementRangeIntoVector(
                rewriter, this->getTypeConverter(), loc,
                cast<VectorType>(vecTy), cmpElems, vecStart);

            Value loadVal = bufferEmitter.emitAtomicCAS(
                vecTy, rsrcDesc, offsetElems[vecStart], casCmpVal, casStoreVal,
                pred, hasUsers);
            // Track the last op, so we can emit a fenceop after the loop
    ...
    matchAndRewrite(triton::amdgpu::BufferStoreOp op, OpAdaptor adaptor,
        ...
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            ...
            Value storeVal = packElementRangeIntoVector(
                rewriter, this->getTypeConverter(), loc,
                cast<VectorType>(vecTy), valueElems, vecStart);
            bufferEmitter.emitStore(rsrcDesc, offsetElems[vecStart], storeVal,
                                    pred, cacheMod);
        } // end vec
...
struct AtomicCASOpConversion
    ...
    matchAndRewrite(triton::AtomicCASOp op, OpAdaptor adaptor,
        ...
        if (!atomicMemOrdering)
            return rewriter.notifyMatchFailure(
                op, "Unknown AMDGPU memory ordering");
        auto scope = op.getScope();
        auto scopeStr = getAMDGPUMemScopeStr(scope);
        if (!scopeStr)
            return rewriter.notifyMatchFailure(op,
                                               "Unknown AMDGPU memory scope");
        ...
        for (size_t i = 0; i < elemsPerThread; i += 1) {
            ...
            if (tensorTy) { // for tensor
                ...
                auto *atomicBlock =
                    rewriter.createBlock(curBlock->getParent(),
                ...
                rewriter.create<LLVM::CondBrOp>(loc, pred, atomicBlock,
                                                endBlock);
                ...
                if (!op.getResult().use_empty()) {
                    ...
                    Value atomPtr = getSharedMemoryBase(
                ...
                BuilderMemfenceLDS.create<>("s_waitcnt lgkmcnt(0)")
                    ->operator()();
        ...
        finalizeTensorAtomicResults(op, tensorTy, rewriter, resultVals,
                                    valueElemTy, b, b.true_val(), targetInfo,
                                    getTypeConverter());
        return success();
    }
};
...
struct AtomicRMWOpConversion
    ...
    matchAndRewrite(triton::AtomicRMWOp op, OpAdaptor adaptor,
        ...
        // to f16 element and reduce contention.
        ...
        if (tensorTy) {
            ...
            // Force F16 packing in the case it's not coming in as packed, but
            // the ISA can support packed atomic instructions.
            applyPackingF16 = supportsGlobalAtomicF16PackedAndDpp(
                                  targetInfo.getISAFamily()) &&
                              vec == 1 && isF16Ty &&
                              atomicRmwAttr == RMWOp::FADD &&
                              !enableIntraWaveReduce;
            numElems = tensorTy.getNumElements();

            auto threadOrder = getThreadOrder(tensorTy);
            unsigned contigWithinLanes =
                axisAnalysisPass.getAxisInfo(ptr)->getContiguity(
                    threadOrder.front());
        ...
        for (size_t i = 0; i < elemsPerThread; i += vec) {
            // TODO: in case llMask is zero we can create only one branch for
            // all elemsPerThread.
            Value rmwMask =
            ...
            if (applyPackingF16) {
                ...
                if (vec == 1) {
                    ...
                    for (size_t ii = 0; ii < vec; ++ii)
                        vecVal = b.insert_element(
                            vecTy, vecVal, valElements[i + ii], b.i32_val(ii));
                    valElement = vecVal;
                }

                // If we have a single tl.atomic_rmw that is lowered into
                // multiple llvm.atomic_rmw, and we set the ordering for each to
                // aql_rel (the default if no sem value is explicitly set in the
                // DSL level tl.atomic_add. The llvm backend will insert extra
                // buffer invalidates and L2 write backs causing a perforance
                // degration. To avoid this we set the ordering to release for
                // the first, acquire for the last, and relaxed for anything in
                // between so that only a single set of buffer_inv and
                // buffer_wbl2 instructions are inserted by the backend for any
                // "cluster" of atomic ops.
                if ((vec > 1 || elemsPerThread > 1) &&
                    op.getSem() == MemSemantic::ACQUIRE_RELEASE) {
                    if (i == 0) {
                        // First
                        emitter.setAtomicOrdering(
                ...
                Value retVal = emitter.emitAtomicRMW(
                    rewriter, ptrElements[i], valElement, rmwMask,
                ...
                if (tensorTy) {
                    for (int ii = 0; ii < vec; ++ii) {
                        resultVals[i + ii] =
                            vec == 1 ? retVal
                                     : b.extract_element(valueElemTy, retVal,
                                                         b.i32_val(ii));
                    }
                } else {

...
Value BufferEmitter::createResourceDescriptor(Value basePtr,
    ...
    if (llvm::is_contained({ISAFamily::CDNA3, ISAFamily::CDNA4},
        ...
        if (blockStride) {
            ...
            // Cache swizzle supports only upto 8k stride. Also simply swizzling
            // the largest available stride (8k) doesn't help those unsupported
            // large stride. Especially better to avoid using the stride which
            // is 2^N when N>13, e.g. by add padding to the buffer.
            Value stride16b =
                rewriter.create<LLVM::TruncOp>(loc, i16_ty, blockStride);
            Value strideSat =
                rewriter.create<LLVM::AndOp>(loc, stride16b, mask14b);

...
class ConvertLayoutOpPermlaneSwap
  ...
  public:
    ...
    matchAndRewrite(triton::gpu::ConvertLayoutOp op, OpAdaptor adaptor,
        ...
        // Following `transferWithinWarp` and
        // `getWarpLayoutConvertDecomposition`, an intra-warp layout conversion
        // can be described as a permutation of hardware index bits. The
        // `permlane_swap` instructions can be used to effect transpositions
        // (r_i l4) and (r_i l5) more cheaply than in the general pathway, where
        // `l4` and `l5` are lane index bits and `r_i` is a register index bit,
        // or 'basis vector' in the language of LinearLayouts.
        //
        // Certain layout conversions which benefit from using `permlane_swap`
        // are produced during chained matrix multiplication kernels, namely the
        // MFMA to DotOp conversion and the epilogue StoreOp vectorization
        // optimization. This was the initial motivation for the pattern, but
        // the implementation itself is entirely general.
        //
        // At the moment, we handle lane-register bit transpositions as above
        // and 3-cycles involving both `l4` and `l5` bits such as (r_i l4 l5).
        // In both cases, we require that `i >= nPack`, where `nPack` indicates
        // the number of intra-register index bits (i.e., the degree of register
        // packing), and that there are no intra-register element permutations
        // prescribed by the general decomposition algorithm.
        if (!(rBit >= nPack && t.topPreSel == 0x3210 &&
              t.topPostSel == 0x3210 && (lBit == 4 || lBit == 5))) {
            return failure();
        }
        ...
        auto permlaneSwap = [&](Value v0, Value v1, auto instr) {
            ...
            auto out = LLVM::createLLVMIntrinsicCallOp(rewriter, loc, instr,
                                                       retTy, args)
                           ->getResult(0);
        ...
        // The input values may require broadcasting so that the conversion can
        // be described as a permutation. This does not cost anything for simple
        // cases.
        ...
        if (elemsPerVec > 1) {
            ...
            if (bitwidth == 64) {
                ...
                llvm::for_each(inVals, [&](Value v) {
                    ...
                    half0.push_back(
                        b.extract_element(i32_ty, vec, b.i32_val(0)));
                    half1.push_back(
                        b.extract_element(i32_ty, vec, b.i32_val(1)));
                });
        ...
        for (int tileIdx = 0; tileIdx < numTiles; ++tileIdx) {
            ...
            for (int i = 0; i < tileSize / 2; ++i) {
                ...
                if (isThreeCycle) {
                    // E.g., we factor (r_i l5 l4) = (r_i l4)(r_i l5), read
                    // right to left.
                    const char *instr1 = lBit == 5
                                             ? "llvm.amdgcn.permlane16.swap"
                                             : "llvm.amdgcn.permlane32.swap";
                    auto swapped1 =
                        permlaneSwap(outVals[r0], outVals[r1], instr1);
        ...
        if (elemsPerVec > 1) {
            ...
            if (bitwidth == 64) {
                ...
                for (int i = 0; i < shift; ++i) {
                    ...
                    vec =
                        b.insert_element(vecTy, vec, outVals[i], b.i32_val(0));
                    vec = b.insert_element(vecTy, vec, outVals[i + shift],
                                           b.i32_val(1));
        ...
        // This needs to be removed before using `broadcastAs` to get the
        // correct broadcasting as expected by the original destination layout.
...
class ConvertLayoutForcedPadding
  ...
  public:
    ...
    collectRegsForIter(MLIRContext *ctx, const LinearLayout &layout) const {
        ...
        // The choice of iteration should be determined only by the register.
        // That is, it should be correct to split the register dimension into
        // iterations.
    ...
    SmallVector<Value> transferWithinBlockImpl(ArrayRef<Value> inVals,
        ...
        assert(shmemStoreLayout.sublayoutIsZero({kLane, kWarp, kBlock},
                                                {kIteration}));
        assert(shmemLoadLayout.sublayoutIsZero({kLane, kWarp, kBlock},
                                               {kIteration}));
        ...
        for (auto i = 0; i < rank; i++) {
            ...
            assert(scratchConfig.repShape[i] ==
                   scratchConfig.paddedRepShape[i]);
        }
        ...
        auto getVecAddr = [&](LinearLayout &layout, Value &regBase,
            ...
            if (paddedSize > 0) {
                ...
                offset = b.add(b.shl(b.lshr(offset, b.i32_val(rshiftVal)),
                                     b.i32_val(lshiftVal)),
                               offset);
            }
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            // When using `stmatrix`, we can store `inVec` elements even if they
            // are not contiguous
            auto inVec = scratchConfig.inVec;
            for (int j = 0; j < inVals.size() / iterations; j += inVec) {
                auto inRegSlice = inRegs[j];
                Value vecAddr =
                    getVecAddr(shmemStoreLayout, storeBase, inRegSlice);
            ...
            for (int j = 0; j < outSize / iterations;
                 j += scratchConfig.outVec) {
                auto outRegSlice = outRegs[j];
                auto vecAddr =
                    getVecAddr(shmemLoadLayout, loadBase, outRegSlice);
                Value valsVec =
  ...
  /// ...
    ...
    Value transferWithinBlockPadding(triton::gpu::ConvertLayoutOp op, Value src,
        ...
        // pointers to i64. This is necessary because TargetInfo::loadDShared
        // and storeDShared can't handle vectors of pointers or sub-byte
        // elements.
        ...
        auto srcLayoutWithinBlock =
            triton::gpu::getLayoutWithinBlock(srcLayout);
        auto dstLayoutWithinBlock =
            triton::gpu::getLayoutWithinBlock(dstLayout);
    ...
    matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
        ...
        auto result =
            transferWithinBlockPadding(op, adaptor.getSrc(), rewriter);
        rewriter.replaceOp(op, result);
        return success();
    }
...
void mlir::triton::AMD::populateConvertLayoutOpToLLVMPatterns(
    ...
    patterns.add<ConvertLayoutOpPermlaneSwap>(typeConverter, targetInfo,
                                              benefit);
    patterns.add<ConvertLayoutForcedPadding>(typeConverter, targetInfo,
                                             benefit);

...
Value createVectorMaskFromPredicate(RewriterBase &rewriter, Location loc,
    ...
    for (size_t s = 0; s < vecSize; ++s) {
        Value indexVal = rewriter.create<LLVM::ConstantOp>(
            loc, rewriter.getI64IntegerAttr(s));
...
static Value shuffleCommonImpl(Location loc, RewriterBase &rewriter,
    ...
    if (!valType.isInteger(32) && bits <= 32) {
        ...
        val = shuffleCommonImpl(loc, rewriter, isaFamily, val, i, strideInt,
                                mode, clamp);
    ...
    if (bits == 64) {
        ...
        val0 = shuffleCommonImpl(loc, rewriter, isaFamily, val0, i, strideInt,
                                 mode, clamp);
        val1 = shuffleCommonImpl(loc, rewriter, isaFamily, val1, i, strideInt,
                                 mode, clamp);
        vec = b.undef(vecTy);
    ...
    auto bpermute = [&](Value lane) {
        ...
        return rewriter.create<ROCDL::DsBpermuteOp>(loc, valType, permuteAddr,
                                                    val);
    };
    ...
    case ShflKind::bfly:
        if (strideInt > 16) {
            ...
            if (isRDNA(isaFamily)) {
                // Lane i in the upper 16 lanes reads the value from lane i in
                // the lower 16 lanes and vice versa.
                ...
                return rewriter.create<ROCDL::DsSwizzleOp>(loc, valType, val,
                                                           offset);
            }
        } else {
            if (!llvm::is_contained({ISAFamily::CDNA2, ISAFamily::CDNA3,
                                    ...
                                    isaFamily)) {
                // DPP is only supported for CDNA2/CDNA3/CDNA4/RDNA3 right now,
                // so we fallback to ds_swizzle for other architectures.
                //
                // This map facilates the butterfly shuffle pattern for a stride
                // less than 16. The pattern stride is the key of the map.
                DenseMap<short, unsigned int> masks{{16, 0x401F},
                                                    {8, 0x201F},
                                                    {4, 0x101F},
                                                    {2, 0x081F},
                                                    {1, 0x041F}};
            ...
            case 1: {
                // quad_perm: 1, 0, 3, 2
                uint32_t dppCtrl =
                    static_cast<uint32_t>(DppCtrl::QUAD_PERM_FIRST);
            ...
            case 4: {
                ...
                return createDppOpWithoutBoundCtrl(
                    ret, val, 4 + static_cast<uint32_t>(DppCtrl::ROW_SHL0),
                    allRows, 0x5);
            }
            case 8: {
                ...
                return createDppOpWithoutBoundCtrl(
                    ret, val, 8 + static_cast<uint32_t>(DppCtrl::ROW_SHL0),
                    allRows, 0x3);
            }
            default:
                assert(false && "bfly shfl with stride >= 16 should not be "
                                "handled by dpp.");
            }
        }
        break;
...
static Value shuffleCommon(Location loc, RewriterBase &rewriter,
    ...
    Value result = shuffleCommonImpl(loc, rewriter, isaFamily, val, i,
                                     strideInt, mode, clamp);
    if (isa<LLVM::LLVMPointerType>(valTy))
...
Value permute(Location loc, RewriterBase &rewriter, Value x, Value y,
    ...
    auto op = createLLVMIntrinsicCallOp(rewriter, loc, "llvm.amdgcn.perm",
                                        i32_ty, args);
...
unsigned getContiguity(Value ptr, Value offset,
    ...
    // To compute the contiguity of the scalar/warp-uniform ptr and offset pair
    // we need to look at the contiguity of the offsets and the alignment of the
    // ptr
    auto elemNumBits = triton::getPointeeBitWidth(tensorTy);
    auto contiguity = axisAnalysisPass.getContiguity(offset, elemNumBits);

    // To get the alignment of the scalar ptr we need to look at the
    // divisibility
...
bool canCoalesceWriteIntoSharedMemory(RewriterBase &rewriter,
    ...
    for (int inLane : llvm::seq(srcToSharedLayout.getInDimSizeLog2(kLane))) {
        ...
        if (basis != expected) {
            LDBG("detected uncoalesced layout from blocked to shared in async "
                 "copy "
                 "for lane "
...
SmallVector<Value, 4> upcast8xMxfp4_SW(RewriterBase &rewriter, Operation *op,
    ...
    // happens that llvm.amdgcn.perm supports selecting 4 bytes from 8 input
    // bytes using a 4-byte selector. So the overall idea is to use
    // llvm.amdgcn.perm to implement such a LUT; though we need to select the
    // two bytes for the resultant bf16/fp16 bit patterns separately. For the
    // byte containing S, we also need to handle the S and E bits separately.
    ...
    Value resB1LutLoNoS =
        toFp16 ? b.i32_val(0x3e3c3800) : b.i32_val(0x3f3f3f00);
    Value resB1LutHiNoS =
        toFp16 ? b.i32_val(0x46444240) : b.i32_val(0x40404040);

    Type i32Ty = rewriter.getI32Type();
    auto permU32FnTy =
        LLVM::LLVMFunctionType::get(i32Ty, {i32Ty, i32Ty, i32Ty});

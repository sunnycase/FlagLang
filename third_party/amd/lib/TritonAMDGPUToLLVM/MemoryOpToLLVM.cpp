...
class TransLocalLoadOpConversion
  ...
  public:
    ...
    matchAndRewrite(LocalLoadOpType op, OpAdaptor adaptor,
        ...
        if (isPackedLoad || canUseTransLoad(op, srcTy, dstTy)) {
            return lowerSharedToDotOperandTransLL(
  ...
  private:
    bool checkLayoutProperties(MemDescType srcTy,
                               RankedTensorType dstTy) const {
        // Verify the layout properties required for using the ds_read_tr
        // instruction. This instruction is used to load non-k contiguous
        // tensors from shared memory into a dot layout with an MFMA layout
        // parent.
        auto dotEnc =
            llvm::dyn_cast<DotOperandEncodingAttr>(dstTy.getEncoding());
        if (!dotEnc) {
            return false;
        }
    ...
    bool checkKWidth(MemDescType srcTy, RankedTensorType dstTy) const {
        ...
        return (isLargeTile &&
                llvm::is_contained({kWidthLargeTile, kWidthSmallTile},
                                   kWidth)) ||
    ...
    lowerSharedToDotOperandTransLL(LocalLoadOpType op, OpAdaptor adaptor,
        ...
        auto ldsTransLayout =
            chooseDsReadB64TrLayout(dotEnc, shape, llBitwidth);
        ...
        bool valid = emitTransferBetweenRegistersAndShared(
            ...
            laneId, warpId, [&](VectorType vecTy, Value vecAddr) {
                if constexpr (isPackedLoad) {
                    ...
                    auto dsReadOp = rewriter.create<ROCDL::ds_read_tr4_b64>(
                        loc, i32VecTy, vecAddr);
                    auto res = b.bitcast(dsReadOp.getResult(), vecTy);
                    Value vecVal = res.getResult();
                    for (int v = 0; v < vecTy.getNumElements(); v++) {
                        outVals.push_back(b.extract_element(llvmElemTy, vecVal,
                                                            b.i32_val(v)));
                    }
                } else if (bitwidth == 16) {
                    auto dsReadOp = rewriter.create<ROCDL::ds_read_tr16_b64>(
                        loc, vecTy, vecAddr);
                    if constexpr (!isPackedLoad) {
                    ...
                    auto dsReadOp = rewriter.create<ROCDL::ds_read_tr8_b64>(
                        loc, i32VecTy, vecAddr);
                    if constexpr (!isPackedLoad) {
                        AMD::addLocalLoadNoAliasScope(op, dsReadOp);
                    }
        ...
        if (bitwidth != 16) {
            ...
            for (int v = 0; v < static_cast<int>(elemsI32.size()); ++v) {
                ...
                for (int i = 0; i < numElemsPerVec; ++i)
                    outVals.push_back(
                        b.extract_element(llvmElemTy, vec, b.i32_val(i)));
            }
        ...
        Value result =
            packLLElements(loc, typeConverter, outVals, rewriter, retTy);

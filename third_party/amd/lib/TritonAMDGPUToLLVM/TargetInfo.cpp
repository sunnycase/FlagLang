...
Value TargetInfo::getClusterCTAId(RewriterBase &rewriter, Location loc) const {
    // On AMD hardware we don't have CTA clusters like NVIDIA. So this will
    // always be zero. Whoever calling into this should make sure the whole
    // program does not try to utilize CTA clusters.
...
Value TargetInfo::permute(RewriterBase &rewriter, Location loc, Value a,
    ...
    // Warning: The `a` and `b` operands are ordered to align with Nvidia's
    // `prmt` Both use little-endian ordering, but AMD puts the MSBs of the data
    // in the 0-th operand.
...
static Value permuteAndReduce(RewriterBase &rewriter, Location loc,
    ...
    Value perm = LLVM::createLLVMIntrinsicCallOp(
                     rewriter, loc, intrinsic, retType,
...
static bool warpReduceSwap16or32(RewriterBase &rewriter, Location loc,
    ...
    for (auto i = 0; i < acc.size(); i++) {
        Value redx =
            permuteAndReduce(rewriter, loc, intrinsic, acc[i], reduxOp);
...
bool TargetInfo::warpReduce(RewriterBase &rewriter, Location loc,
    ...
    if (getISAFamily() == ISAFamily::CDNA4 &&
        warpReduceSwap16or32(rewriter, loc, acc, op, numLaneToReduce,
                             interleave))
        return true;
    ...
    auto createDppReduxOpWithBoundCtrl = [&](Type valType, Value &src,
        ...
        Value dppResult = rewriter
                              .create<ROCDL::DPPUpdateOp>(
                                  loc, actualType, src, src,
        ...
        if (!valType.isF32()) {
            ...
            dppResult =
                truncAndCastFromInt(rewriter, loc, dppResult, valType, 32);
        }
    ...
    for (int i = 0; i < acc.size(); i++) {
        ...
        // into 4 rows, with each row being 16 threads. Each 16 threads are
        // arranged further into 4 banks, with each bank being 4 threads.
        // Overall it's in a (row, bank, thread) structure. When shuffling, we
        // use row/bank mask to indicate which row/bank to participate. Then
        // modifier like row_shr and row_bcast means exact data movement
        // schemes. In the following instructions, taking row 0 as an example:
        //
        ...
        //
        // Step 5: Broadcast lane 15 of each row to all the lanes of its next
        // row.
        ...
        if (isCDNA(getISAFamily())) {
            ...
            buf = createDppReduxOpWithBoundCtrl(
                valType, buf, static_cast<uint32_t>(DppCtrl::BCAST15), 0xa,
                allBanks);
            ...
            Value permlaneResult =
                rewriter
                    .create<ROCDL::PermlaneX16Op>(loc, actualType, buf, buf,
                                                  b.i32_val(-1), b.i32_val(-1),
                                                  true, false)
                    .getRes();
        ...
        Value result = rewriter.create<ROCDL::ReadlaneOp>(loc, actualType, buf,
                                                          lastLaneId);
...
void TargetInfo::printfImpl(Value formatStrStart, int formatStrByteCount,
    ...
    LLVM::LLVMFuncOp printStrFn = getOrInsertFunction(
        ...
        LLVM::LLVMFunctionType::get(
            i64_ty,
            {i64_ty, ptr_ty(ctx), /*length=*/i64_ty, /*isLast=*/i32_ty}));
    LLVM::LLVMFuncOp printArgsFn;
    if (!args.empty()) {
        printArgsFn = getOrInsertFunction(
            moduleOp, loc, rewriter, "__ockl_printf_append_args",
            LLVM::LLVMFunctionType::get(
                i64_ty, {i64_ty, /*numArgs=*/i32_ty, i64_ty, i64_ty, i64_ty,
                         i64_ty, i64_ty, i64_ty, i64_ty, /*isLast=*/i32_ty}));
    }
...
std::string TargetInfo::getMulhiFuncName(Type resultElementTy) const {
    std::string funcName = resultElementTy.isInteger(32) ? "__ockl_mul_hi_u32"
                                                         : "__ockl_mul_hi_u64";
    return funcName;
}

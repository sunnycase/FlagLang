    ...
    auto getHalfwayPointsForDstType(TypeID dstTyID) {
        ...
        if constexpr (std::is_same_v<FPType, Float32Type>) {
            if (dstTyID == TypeID::get<Float8E4M3FNType>())
                return VecType{
                    0x3a800000,  // halfway between [0/8 * 2^-6, 1/8 * 2^-6]
            ...
            if (dstTyID == TypeID::get<Float8E5M2Type>())
                return VecType{0x37000000,  // halfway between [0/4 * 2^(-14),
                                            // 1/4 * 2^(-14)]
                               0x37c00000,  // halfway between [1/4 * 2^(-14),
                                            // 2/4 * 2^(-14)]
                               0x38200000,  // halfway between [2/4 * 2^(-14),
                                            // 3/4 * 2^(-14)]
                               0x38600000}; // halfway between [3/4 * 2^(-14),
                                            // 4/4 * 2^(-14)]
            if (dstTyID == TypeID::get<Float8E4M3FNUZType>())
                // We divide the range of subnormals in 2^3 subranges.
                // Each i entry in the LUT corresponds to the midpoint of the
                // ith subrange represented in the src format (here float32)
                return VecType{
                    0x3a000000,  // halfway between [0/8 * 2^-7, 1/8 * 2^-7]
        ...
        if constexpr (std::is_same_v<FPType, Float16Type>) {
            ...
            if (dstTyID == TypeID::get<Float8E4M3FNUZType>())
                ...
                // We divide the range of subnormals in 2^3 subranges.
                // Each i entry in the LUT corresponds to the midpoint of the
                // ith subrange represented in the src format (here float16)
                return VecType{
                    0x1000,  // halfway between [0/8 * 2^-7, 1/8 * 2^-7]
        ...
        if constexpr (std::is_same_v<FPType, BFloat16Type>) {
            if (dstTyID == TypeID::get<Float8E4M3FNUZType>())
                ...
                // We divide the range of subnormals in 2^3 subranges.
                // Each i entry in the LUT corresponds to the midpoint of the
                // ith subrange represented in the src format (here bfloat16)
                return VecType{
                    0x3a00,  // halfway between [0/8 * 2^-7, 1/8 * 2^-7]
            ...
            if (dstTyID == TypeID::get<Float8E5M2FNUZType>()) {
                ...
                // 2^-18 =
                return VecType{
                    0x3680,  // halfway between [0/4 * 2^-15, 1/4 * 2^-15]
...
cvtScalePkDowncastToFp8(Location loc, ConversionPatternRewriter &rewriter,
    ...
    if constexpr (std::is_same_v<ConvertOp, ROCDL::CvtScaleF32PkFp8F32Op> ||
        ...
        v2I16Vec = rewriter.create<ConvertOp>(loc, v2I16Ty, v2I16Vec, v[0],
                                              v[1], scale,
                                              /*dstLoHiSel=*/false);
        v2I16Vec = rewriter.create<ConvertOp>(loc, v2I16Ty, v2I16Vec, v[2],
                                              v[3], scale,
                                              /*dstLoHiSel=*/true);
    } else {
        ...
        v2I16Vec =
            rewriter.create<ConvertOp>(loc, v2I16Ty, v2I16Vec, srcVec, scale,
                                       /*dstLoHiSel=*/false);
...
Fp16_to_Fp8E5M2_RTNE_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        Value bias =
            b.add(i16_ty,
                  b.lshr(i16_ty, b.and_(i16_ty, sig, b.i16_val(0x0100)),
                         b.i16_val(8)),
                  b.i16_val(0x007F));
        ...
        i16 = b.select(b.icmp_uge(sig, b.i16_val(0x7B80)), b.i16_val(0x7B00),
                       i16);
...
static Value downcastToFp8_RTNE_oneValue(Location loc,
    ...
    if constexpr (!isFp8UZ) {
        ...
        uint32_t expFullMask = ((1U << srcExponentBits) - 1U)
                               << srcMantissaBits;
    ...
    for (int i = lutSize - 1; i >= 0; i--) {
        Value cmp;
        if (i % 2 == 0) {
            cmp = b.icmp_ule(intVal,
                             srcFpInfo.toLLVMIntValue(halfwayPointsLUT[i]));
        } else {
            cmp = b.icmp_ult(intVal,
                             srcFpInfo.toLLVMIntValue(halfwayPointsLUT[i]));
        }
...
Bf16_to_Fp8E5M2_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (size_t i = 0; i < 4; ++i) {
        ...
        Value bias =
            b.add(i16_ty,
                  b.lshr(i16_ty, b.and_(i16_ty, sig, b.i16_val(0x0020)),
                         b.i16_val(5)),
                  b.i16_val(0x000F));
        i16 = b.add(i16_ty, sig, bias);
...
Bf16_to_Fp8E4M3FNUZ_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (size_t i = 0; i < 4; i++)
        result[i] =
            downcastToFp8_RTNE_oneValue<BFloat16Type, Float8E4M3FNUZType>(
                loc, rewriter, v[i]);
    return result;
}
...
Bf16_to_Fp8E5M2FNUZ_SW(Location loc, ConversionPatternRewriter &rewriter,
    ...
    for (size_t i = 0; i < 4; i++)
        result[i] =
            downcastToFp8_RTNE_oneValue<BFloat16Type, Float8E5M2FNUZType>(
                loc, rewriter, v[i]);
    return result;
}
    ...
    SmallVector<Value> createDestOps(triton::FpToFpOp op, OpAdaptor adaptor,
        ...
        if (srcElementType.isF32() &&
            ...
            assert(roundingMode.has_value() &&
                   "rounding mode must be specified for fp32->fp16/bf16 "
                   "conversion");
            if (roundingMode.value() == RoundingMode::RTNE) {
                return Fp32_to_F16_RTNE(loc, rewriter, srcElementType,
        ...
        // fp8/bf8->f32, if neither nanoo fp8/bf8 on CDNA3 nor ocp fp8/bf8 on
        // CDNA4, is done in two steps: fp8/bf8->fp16 and fp16->fp32
        ...
        inVals.resize(numElements,
                      b.undef(typeConverter->convertType(srcType)));
        SmallVector<Value> outVals;
        if (srcType != dstType) {
...
struct FMulOpConversion
    ...
    SmallVector<Value> createDestOps(arith::MulFOp op, OpAdaptor adaptor,
        ...
        if (lhsElemTy.isBF16() && rhsElemTy.isBF16()) {
            return {EmitDualBF16ElementwiseOp<LLVM::FMulOp>(loc, rewriter,
                                                            operands)};
        } else {
...
struct FAddOpConversion
    ...
    SmallVector<Value> createDestOps(arith::AddFOp op, OpAdaptor adaptor,
        ...
        auto rhsElemTy = getElementType(op.getRhs());
        if (lhsElemTy.isBF16() && rhsElemTy.isBF16()) {
            return {EmitDualBF16ElementwiseOp<LLVM::FAddOp>(loc, rewriter,
                                                            operands)};
        } else {
...
struct FSubOpConversion
    ...
    SmallVector<Value> createDestOps(arith::SubFOp op, OpAdaptor adaptor,
        ...
        auto rhsElemTy = getElementType(op.getRhs());
        if (lhsElemTy.isBF16() && rhsElemTy.isBF16()) {
            return {EmitDualBF16ElementwiseOp<LLVM::FSubOp>(loc, rewriter,
                                                            operands)};
        } else {
...
struct SIToFPOpConversion
    ...
    SmallVector<Value> createDestOps(arith::SIToFPOp op, OpAdaptor adaptor,
        ...
        if (outElemTy.isBF16() && inElemTy.isInteger(8) &&
            ...
            return {
                convertFp32ToBf16(loc, rewriter, value, RoundingMode::RTNE)};
        } else {
            return {
                rewriter.create<LLVM::SIToFPOp>(loc, elemTy, operands[0][0])};
        }
    }
};
...
struct FPToSIOpConversion
    ...
    SmallVector<Value> createDestOps(arith::FPToSIOp op, OpAdaptor adaptor,
        ...
        if (inElemTy.isBF16()) {
            ...
            return {
                rewriter.create<LLVM::FPToSIOp>(loc, elemTy, operands[0][0])};
        }
    }
};
...
struct ExtFOpConversion
    ...
    SmallVector<Value> createDestOps(arith::ExtFOp op, OpAdaptor adaptor,
        ...
        if (inElemTy.isBF16()) {
            ...
            return {
                rewriter.create<LLVM::FPExtOp>(loc, elemTy, operands[0][0])};
        }
    }
};
...
struct TruncFOpConversion
    ...
    SmallVector<Value> createDestOps(arith::TruncFOp op, OpAdaptor adaptor,
        ...
        if (inElemTy.isF32() && (outElemTy.isBF16() || outElemTy.isF16())) {
            return Fp32_to_F16_RTNE(loc, rewriter, inElemTy, outElemTy,
                                    operands, isaFamily);
        }
...
struct ExpOpConversionApprox
    ...
    SmallVector<Value> createDestOps(math::ExpOp op, OpAdaptor adaptor,
                                     ...
                                     Location loc) const {
        auto b = TritonLLVMOpBuilder(loc, rewriter);
        // For non-FP32 input, call __ocml_exp_f64 for higher-precision
        // calculation
        ...
        // flushes denorms by default, but we want to preserve denorms by
        // default for expOp.
        ...
        return {
            LLVM::createLLVMCallOp(rewriter, loc, funcOp, prod).getResult()};
    }
};
...
struct Exp2OpConversion
    ...
    SmallVector<Value> createDestOps(math::Exp2Op op, OpAdaptor adaptor,
                                     ...
                                     MultipleOperandsRange operands,
                                     Location loc) const {
        // For non-FP32 input, call __ocml_exp2_f64 for higher-precision
        // calculation
        if (elemTy.getIntOrFloatBitWidth() != 32)
            return {};

        // On AMD backend, both intrinsics are lowered to v_exp_f32 instruction,
        // which flushes input and output denorms. `llvm.amdgcn.exp2.f32`
        // provides direct access to v_exp_f32. For `llvm.exp2.f32`, the LLVM
        // backend inserts instructions to handle denorms iff
        // `allow_flush_denorm` is False.
...
struct RsqrtOpConversion
    ...
    SmallVector<Value> createDestOps(math::RsqrtOp op, OpAdaptor adaptor,
                                     ...
                                     MultipleOperandsRange operands,
                                     Location loc) const {
        // This pass only deals with FP32 input with ftz configuration. Other
        // cases are delegate to MLIR.
        //
        // For FP16/FP64 input, it's lowered to
        // __ocml_rsqrt_f16/__ocml_rsqrt_f64.
        //
        // For FP32 input with non-ftz configuration, it's lowered to
        // __ocml_rsqrt_f32, which will check the ftz/daz settings in the
        // backend dynamically to decide to preserve/flush denorms.
...
struct SqrtOpConversion
    ...
    SmallVector<Value> createDestOps(math::SqrtOp op, OpAdaptor adaptor,
                                     ...
                                     Location loc) const {
        auto b = TritonLLVMOpBuilder(loc, rewriter);
        // This function only handles FP32 inputs. Other data types are lowered
        // to LLVM::SqrtOp by MLIR.
        //
        // On the AMDGPU backend, instructions legalized from LLVM::SqrtOp are
        // designed to produce IEEE-compliant results and always preserve
        // denorms. But what we actually need is an approximated SQRT. So we
        // need to manually lower the op.
        //
        // Differences in this approach are
        // 1. Refinement iterations following llvm.amdgcn.sqrt.f32 are removed
        ...
        if (!ftz) {
            // For non-ftz cases, if the input value is below 2^{-96}, it needs
            // to be scaled up by a factor of 2^{32}, to prevent it from being
            // flushed by llvm.amdgcn.sqrt.f32.
            //
            // The result is then scaled down afterward to get the correct
            // result. Reference:
        ...
        // llvm.amdgcn.sqrt.f32 provides direct access to v_sqrt_f32, which
        // provides 1ULP accuracy and flushs denorms.
        ...
        Value intrinsicsOutput =
            LLVM::createLLVMCallOp(rewriter, loc, funcOp, operands[0])
                .getResult();

        if (!ftz) {
            // In case of non-ftz, we need to calibrate the results by scaling
            // down by a factor of 2^{-16}.
            return {scaleDownIfDenorm(rewriter, loc, intrinsicsOutput,
                                      needScale, 0x1.0p-16f)};
        } else {
...
struct PreciseSqrtOpConversion
    ...
    SmallVector<Value>
    createDestOps(triton::PreciseSqrtOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter, Type elemTy,
                  MultipleOperandsRange operands, Location loc) const {
        auto b = TritonLLVMOpBuilder(loc, rewriter);
        // If the op is neither FP32 nor denorm flushing(ftz), it's directly
        // lowered to LLVM::SqrtOp.
        if (elemTy.getIntOrFloatBitWidth() != 32 || !ftz) {
        ...
        // using `x * rsq(x)` and apply extra refinement iterations to correct
        // the result.
        StringRef funcName = "llvm.amdgcn.rsq.f32";
        ...
        Value sqrtR = LLVM::createLLVMCallOp(rewriter, loc, funcOp, operands[0])
                          .getResult();
        ...
        Value isZeroOrPosInf = rewriter.create<LLVM::IsFPClass>(
            loc, i1_ty, sqrtX, fcPosInf | fcZero);
...
void populateElementwiseOpToLLVMPatterns(
    ...
    // FP32. For other input types, ExpOpConversionApprox will return failure
    // and later pass will call __ocml_exp_f64 for higher-precision calculation
    ...
    patterns.add<Exp2OpConversion>(typeConverter, axisInfoAnalysis, ftz,
                                   benefit);
    patterns.add<RsqrtOpConversion>(typeConverter, axisInfoAnalysis, ftz,
                                    benefit);
    patterns.add<SqrtOpConversion>(typeConverter, axisInfoAnalysis, ftz,
                                   benefit);
    ...
    triton::populateClampFOpToLLVMPattern(
        typeConverter, patterns, axisInfoAnalysis, targetInfo, benefit);
}
} // namespace mlir::triton::AMD

  ...
  private:
    ...
    Operation *createFastExpf(mlir::PatternRewriter &rewriter, Location loc,
        ...
        return LLVM::createLLVMIntrinsicCallOp(rewriter, loc, intrinsic,
                                               returnType, mulOp->getResult(0));
    }

    LogicalResult
    convertToLLVMIntrinsic(LLVM::CallOp callOp,
        ...
        if (calleeName == "__triton_hip_iabs") {
            ...
            replacementOp =
                rewriter.create<LLVM::AbsOp>(loc, returnType, operands[0],
            ...
            Operation *op = rewriter.create<LLVM::RintOp>(
                loc, operands[0].getType(), operands[0]);
            replacementOp = rewriter.create<LLVM::FPToSIOp>(loc, returnType,
                                                            op->getResult(0));
        } else if (calleeName == "__triton_hip_fast_fdividef") {
            assert(operands.size() == 2);
            const char *intrinsic = "llvm.amdgcn.rcp.f32";
            auto rcpOp = LLVM::createLLVMIntrinsicCallOp(
                rewriter, loc, intrinsic, returnType, operands[1]);

            LLVM::FastmathFlagsAttr defaultFlags{};
            replacementOp = rewriter.create<LLVM::FMulOp>(
                loc, returnType, operands[0], rcpOp->getResult(0),
                defaultFlags);
            ...
            replacementOp = rewriter.create<LLVM::FDivOp>(
                loc, returnType, exp2XMinus1->getResult(0),

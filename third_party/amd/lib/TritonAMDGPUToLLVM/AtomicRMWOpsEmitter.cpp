...
Value AtomicRMWEmitter::atomicIntraWaveReduce(RewriterBase &rewriter,
    ...
    Value rightNeighbourAddr = genI32TiledOp(rewriter, generateI32DppMove,
                                             rmwPtr, 0x130, 0xF, 0xF, false);
    ...
    for (int i = 32; i != 0; i /= 2) {
        ...
        acc = b.select(b.icmp_ult(b.i32_val(i), cntRes), performOp(acc, tmp),
                       acc);
    }

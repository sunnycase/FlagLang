...
struct CoalesceAsyncCopyWrites
    ...
    LogicalResult matchAndRewrite(ttg::AsyncCopyGlobalToLocalOp copyOp,
        ...
        // Further restrict the contiguity based on the contiguity of the src to
        // dst layout e.g. if the order of the blocked and shared encoding is
        // different we can only load one element at a time or if the shared
        // encoding is swizzled we cannot exceed the vector size of the
        // swizzling pattern
        ...
        // Select the largest supported load width equal or smaller than
        // loadContig
        ...
        if (loadContig == 0) {
            return rewriter.notifyMatchFailure(
                copyOp,
                "could not find layout config to create coalesced writes");
        }

        // Do not rewrite if we already use the correct contiguity (could be
        ...
        if (blockedContig == loadContig) {
            return rewriter.notifyMatchFailure(
                copyOp, "already using the correct layout");
        }
  ...
  private:
    ...
    const DenseMap<ttg::AsyncCopyGlobalToLocalOp, unsigned>
        &asyncCopyContiguity;
};

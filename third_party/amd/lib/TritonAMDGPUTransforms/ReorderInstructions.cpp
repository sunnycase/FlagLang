...
static void moveUpTranspose(triton::FuncOp funcOp) {
    ...
    funcOp.walk(
        [&](triton::TransposeOpInterface op) { transOps.push_back(op); });
...
static void moveUpGlobalLoadInPrologue(triton::FuncOp funcOp) {
    ...
    // Avoid moving up global_load ops that don't belong to any prologue to
    // avoid extra register pressure.
    ...
    for (auto op : llvm::reverse(globalLoadOps)) {
        ...
        if (ipoint != block->end()) {
            ...
            llvm::erase_if(dfg, [&](Operation *op) {
                return !ipoint->isBeforeInBlock(op);
            });
    ...
    void runOnOperation() override {
        ...
        for (auto funcOp : m.getOps<triton::FuncOp>()) {
            ...
            if (isPureMatmulFunc(funcOp)) {
                funcOp.walk(
                    [&](scf::ForOp forOp) -> void { sinkSecondLoad(forOp); });
            } else {
                SmallVector<scf::ForOp> leafForOps =
                    triton::AMD::getLeafForOps(funcOp);

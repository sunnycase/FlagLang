...
std::pair<Value, Value> createDecomposeOffsetFromAdd(RewriterBase &rewriter,
    ...
    Value nonUniformAdd = rewriter.create<arith::AddIOp>(loc, nonUniformOffsetL,
                                                         nonUniformOffsetR);
...
std::pair<Value, Value> createDecomposeOffsetFromMul(RewriterBase &rewriter,
    ...
    Value nonUNonU = rewriter.create<arith::MulIOp>(loc, nonUniformOffsetL,
                                                    nonUniformOffsetR);
...
std::pair<Value, Value> createDecomposeOffsetFromExpr(RewriterBase &rewriter,
    ...
    // Base case 1: it is a splat. Return the scalar constant as the uniform
    // part
    if (auto scalarConst =
            maybeGetOrCreateScalarConstant(rewriter, loc, expr)) {
        auto tensorZero = createTensorZero(
            rewriter, loc, cast<RankedTensorType>(expr.getType()));
        return {*scalarConst, tensorZero};
    }

    // Base case 2: block argument. Since it is not a scalar constant, it must
    // be a tensor. Note that this means we won't be able to decompose across
    // loop boundaries (TODO: giuseros).
    ...
    auto offsets =
        llvm::TypeSwitch<Operation *, std::pair<Value, Value>>(
            ...
            .Case<arith::AddIOp>([&](Operation *op) {
                return createDecomposeOffsetFromAdd(rewriter, loc, expr,
                                                    bitness);
            })
            .Case<arith::MulIOp>([&](Operation *op) {
                return createDecomposeOffsetFromMul(rewriter, loc, expr,
                                                    bitness);
            })
...
struct FatPointers {
    ...
    const ValueT &at(const_arg_type_t<KeyT> k) const {
        // this is redundant - DenseMap will assert the same thing - but better
        // to have our own message
    ...
    if (auto arg = dyn_cast<BlockArgument>(base)) {
        ...
        for (NamedAttribute namedAttr : op->getAttrs()) {
            ...
            if (auto offsetArg = dyn_cast<BlockArgument>(offset))
                op->setAttr(llvm::formatv("{0}_arg{1}", newAttrName,
                                          offsetArg.getArgNumber())
                                .str(),
                            namedAttr.getValue());
        }
        return;
...
Value createTensorPointer(RewriterBase &rewriter, Value basePtr, Value offset,
    ...
    if (!tensorType) {
        auto addPtrOp = rewriter.create<tt::AddPtrOp>(loc, basePtr.getType(),
                                                      basePtr, offset);
    ...
    if (fatPtrAttrs.canNarrow)
        offset =
            createTruncIOffset(rewriter, loc, offset, rewriter.getI32Type());
...
struct PointerCanonicalizationPattern : ConversionPattern {
    ...
    matchAndRewrite(Operation *op, ArrayRef<ValueRange> operands,
        ...
        if (failed(matchAndRewrite_(
                sourceOp, OneToNOpAdaptor(operands, sourceOp), rewriter)))
            return failure();
    ...
    matchAndRewrite_(tt::SplatOp splatOp, OneToNOpAdaptor adaptor,
        ...
        if (!llvm::isa<IntegerType>(fatPtrOffset.getType()))
            return rewriter.notifyMatchFailure(
                splatOp, "non-integer offset unimplemented");
...
class ConvertBroadcastOp
  ...
  public:
    ...
    matchAndRewrite_(tt::BroadcastOp broadcastOp, OneToNOpAdaptor adaptor,
        ...
        if (!offsetType)
            return rewriter.notifyMatchFailure(
                broadcastOp, "non-tensor offset unimplemented");
  ...
  public:
    ...
    matchAndRewrite_(tt::AddPtrOp addPtrOp, OneToNOpAdaptor adaptor,
        ...
        if (nonRemappedOffset.size() != 1)
            return rewriter.notifyMatchFailure(
                addPtrOp,
                "expected AddPtrOp Offset to have not have been remapped");
        ...
        std::array<StringRef, 3> propagateList{"tt.divisibility",
                                               "tt.contiguity", "tt.constancy"};
        ...
        auto doSetDiscardableAttrs = [&](tt::AddPtrOp newAddPtrOp) {
            auto newPtrTy =
                llvm::dyn_cast<RankedTensorType>(newAddPtrOp.getType());
        ...
        if (llvm::isa<tt::PointerType>(addPtrOp.getPtr().getType())) {
            ...
            rewriter.replaceOpWithMultiple(addPtrOp,
                                           {{newAddPtrOp, fatPtrOffset}});
        ...
        if (auto scalarConst =
            ...
            rewriter.replaceOpWithMultiple(addPtrOp,
                                           {{newAddPtrOp, fatPtrOffset}});
            // If we are updating the tensor pointer with a constant value, we
            // can propagate the attributes of the tensor pointer to the fat
            // pointer.
            fatPtrs[{newAddPtrOp, fatPtrOffset}] =
                fatPtrs.at({fatPtrBase, fatPtrOffset});
            return success();
        ...
        auto [uniformOffset, nonUniformOffset] = createDecomposeOffsetFromExpr(
            rewriter, curLoc, origOffset, bitness);
        ...
        if (!isZeroConst(nonUniformOffset)) {
            ...
            assert(addPtrOffsetType.isIntOrIndex() &&
                   ...
                   "expected both addPtrOffsetType and fatPtrOffsetType to be "
                   "int or "
                   "index type");
            canNarrow =
                canNarrow && canNarrowOffset(fatPtrOffset, nonUniformOffset);
            // Upcast or downcast the offset accordingly
            unsigned addPtrOffsetTypeWidth =
                addPtrOffsetType.getIntOrFloatBitWidth();
            unsigned fatPtrOffsetTypeWidth =
                fatPtrOffsetType.getIntOrFloatBitWidth();
            if (addPtrOffsetTypeWidth < fatPtrOffsetTypeWidth)
                nonUniformOffset = createExtSIOffset(
...
class ConvertExtractSliceOp
  ...
  public:
    ...
    matchAndRewrite_(tt::amdgpu::ExtractSliceOp extractSliceOp,
        ...
        auto fatPtrOffsetTy =
            dyn_cast<RankedTensorType>(fatPtrOffset.getType());
        if (!fatPtrOffsetTy)
            return rewriter.notifyMatchFailure(
...
class ConvertSCFYieldOp : public PointerCanonicalizationPattern<scf::YieldOp> {
  public:
    ...
    matchAndRewrite_(scf::YieldOp yieldOp, OneToNOpAdaptor adaptor,
        ...
        // have to mutate here because otherwise scf.if, scf.for, and scf.while
        // will get confused about which yield is the "correct" yield (since
        ...
        // rewriting a parent op from a child op isn't a great idea but there's
        // no other to indicate to the parent IfOp that the result type can now
        // be rewritten and not before.
...
static void convertSimpleBlockSignature(Block *oldBlock,
    ...
    for (auto operands : remappedOperands) {
        if (operands.size() == 2) {
            assert(
                fatPtrs.contains({operands[0], operands[1]}) &&
  ...
  public:
    ...
    matchAndRewrite_(scf::WhileOp whileOp, OneToNOpAdaptor adaptor,
        ...
        convertSimpleBlockSignature(whileOp.getBeforeBody(), remappedInits,
                                    rewriter, fatPtrs);
        convertSimpleBlockSignature(whileOp.getAfterBody(), remappedInits,
                                    rewriter, fatPtrs);
        ...
        for (unsigned i = 0, offset = 0; i < valRangeLens.size(); i++) {
            ...
            ValueRange mappedValue =
                newWhileOp->getResults().slice(offset, len);
            // propagate fatPtrs
            if (mappedValue.size() == 2) {
                assert(remappedInits[i].size() == 2 &&
                       "expected corresponding inits to be a remapped fat ptr");
...
class ConvertCFCondBranch
  ...
  public:
    ...
    matchAndRewrite_(cf::CondBranchOp branchOp, OneToNOpAdaptor adaptor,
                     ConversionPatternRewriter &rewriter) const override {
        ArrayRef<ValueRange> remappedTrueOperands =
            adaptor.getTrueDestOperands();
        ArrayRef<ValueRange> remappedFalseOperands =
        ...
        rewriter.replaceOpWithNewOp<cf::CondBranchOp>(
            branchOp, branchOp.getCondition(), branchOp.getTrueDest(),
            trueOperands, branchOp.getFalseDest(), falseOperands);

        convertSimpleBlockSignature(branchOp.getTrueDest(),
                                    remappedTrueOperands, rewriter, fatPtrs);
        convertSimpleBlockSignature(branchOp.getFalseDest(),
                                    remappedFalseOperands, rewriter, fatPtrs);
  ...
  public:
    ...
    matchAndRewrite_(arith::SelectOp selectOp, OneToNOpAdaptor adaptor,
        ...
        if (adaptor.getTrueValue().size() != 2 ||
            ...
            assert(adaptor.getTrueValue().size() ==
                       adaptor.getFalseValue().size() &&
                   "expected both true and false operands to be the same size");
            return success();
        }
        // If both have been traversed, then we can rewrite select of pointers
        // as a select of base and offset
        ...
        if (!isa<RankedTensorType>(selectOp.getType())) {
            ...
            rewriter.replaceOpWithMultiple(selectOp,
                                           {{newSelectOp, fatPtrTrue[1]}});
            fatPtrs[{newSelectOp, /*fatPtrOffset*/ fatPtrTrue[1]}] =
                fatPtrs.at({fatPtrTrue[0], fatPtrTrue[1]});
            return success();
        }

        // Rewrite to select(fatBaseT, fatBaseF) and select(fatOffsetT,
        // fatOffsetF)
  ...
  public:
    ...
    matchAndRewrite_(scf::IfOp ifOp, OneToNOpAdaptor adaptor,
        ...
        if (withElseRegion) {
            ...
            if (auto thenFatPtrIndxs = ifOp.thenYield()->getDiscardableAttr(
                ...
                assert(ifOp.elseYield()->hasAttr(kSCFIfOpYieldFatPtrOffsets) &&
                       "expected then yield to report fat ptr indices");
                auto elseFatPtrIndxs = ifOp.elseYield()->getDiscardableAttr(
                    kSCFIfOpYieldFatPtrOffsets);
                assert(elseFatPtrIndxs && "expected else fat ptr indices as "
                                          "well as then fat ptr indices");
                ...
                assert(bool(thenIdxs) && bool(elseIdxs) &&
                       "expected else fat ptr index attrs to be "
                       "DenseI64ArrayAttr");
                for (auto [i, j] :
                     llvm::zip(thenIdxs.asArrayRef(), elseIdxs.asArrayRef())) {
                    assert(i == j && "expected thenFatPtrIndxs and "
                                     "elseFatPtrIndxs to agree");
                    assert(
                        i < ifOp.thenYield().getNumOperands() &&
                    ...
                    assert((fatPtrs.at({thenFatPtrBase, thenFatPtrOffset}) ==
                           ...
                           "expected then fat ptr canNarrow and else fat ptr "
                           "canNarrow "
                           "to be equal");
                }
            }
        ...
        auto newIfOp = rewriter.create<scf::IfOp>(
            ifOp.getLoc(), ifOp.thenYield().getOperandTypes(),
        ...
        // Note only the `then` yield here is considered because this whole pass
        // is effectively 1:N type conversion and thus only the types are
        // important (and for `scf.if` the types along both `then`/`else`
        // branches must be the same).
        ArrayRef<int64_t> yieldPtrOffsets =
            llvm::cast<DenseI64ArrayAttr>(
                newIfOp.thenYield()->getDiscardableAttr(
                    kSCFIfOpYieldFatPtrOffsets))
                .asArrayRef();
...
class ConvertConvertLayoutOp
  ...
  public:
    ...
    matchAndRewrite_(tt::gpu::ConvertLayoutOp cvtOp, OneToNOpAdaptor adaptor,
        ...
        auto newOffsetType = RankedTensorType::get(
            outType.getShape(), offsetTensorTy.getElementType(),
            outType.getEncoding());
        tt::gpu::ConvertLayoutOp cvtOffset =
            rewriter.create<tt::gpu::ConvertLayoutOp>(
                cvtOp.getLoc(), newOffsetType, fatPtrOffset);
        rewriter.replaceOpWithMultiple(cvtOp, {{fatPtrBase, cvtOffset}});
        fatPtrs[{fatPtrBase, cvtOffset}] =
            fatPtrs.at({fatPtrBase, fatPtrOffset});
  ...
  public:
    ...
    LogicalResult matchAndRewrite_(
        ...
        if (!llvm::isa<tt::PointerType>(
            ...
            return rewriter.notifyMatchFailure(
                op, "expected operand to be pointer-like");
    ...
    LogicalResult matchAndRewrite(tt::FuncOp newOp,
        ...
        for (auto [idx, arg] : llvm::enumerate(newOp.getArguments())) {
            ...
            if (!isa<tt::PointerType>(arg.getType()))
                continue;
            if (auto pointerRangeAttr = newOp.getArgAttrOfType<IntegerAttr>(
                    idx, "tt.pointer_range"))
                bitness = pointerRangeAttr.getInt();
            Value zeroOffset = rewriter.create<arith::ConstantIntOp>(
                newOp.getLoc(), 0, bitness);
            auto dummyCast = rewriter.create<UnrealizedConversionCastOp>(
                arg.getLoc(), TypeRange{arg.getType()},
                ValueRange{arg, zeroOffset});
            rewriter.replaceAllUsesExcept(arg, dummyCast.getResult(0),
                                          dummyCast);
            fatPtrs[{arg, zeroOffset}].canNarrow = true;
        }

        newOp->setDiscardableAttr(kInitFuncArgsRewritten,
                                  rewriter.getUnitAttr());
        return success();
    }
...
class ConvertReturnOp : public PointerCanonicalizationPattern<tt::ReturnOp> {
  public:
    ...
    matchAndRewrite_(tt::ReturnOp returnOp, OneToNOpAdaptor adaptor,
        ...
        rewriter.replaceOpWithNewOp<tt::ReturnOp>(returnOp, TypeRange{},
                                                  returns);
        return success();
    }
};
...
class ConvertFuncOpArgsUnrealizedCasts
  ...
  public:
    ...
    matchAndRewrite_(UnrealizedConversionCastOp castOp, OneToNOpAdaptor adaptor,
        ...
        // Exhaustive checking we're converting ONLY unrealized_casts inserted
        // (by the 1:N conversion) in ConvertFuncOp.
        ...
        if (!llvm::isa<BlockArgument>(fatPtrBase) ||
            !llvm::isa<tt::FuncOp>(
                fatPtrBase.getParentBlock()->getParentOp()) ||
            !llvm::isa<tt::PointerType>(fatPtrBase.getType()))
            return rewriter.notifyMatchFailure(
                castOp, "expected CastOp first operand to be tt.ptr block arg "
                        "of tt.func");
        ...
        if (auto integerAttr =
            ...
            if (integerAttr.getValue() == 0) {
                rewriter.replaceOpWithMultiple(castOp,
                                               {{fatPtrBase, fatPtrOffset}});
                return success();
            }
        }
        return rewriter.notifyMatchFailure(
...
static void getForwardSliceImpl(OpOperand *use, Operation *op,
    ...
    auto addBlockArgUses =
        [&use, &addUses](const Block::BlockArgListType &blockArgs,
                         unsigned argOffset = 0, unsigned useOffset = 0) {
            for (auto arg : blockArgs) {
                if (arg.getArgNumber() - argOffset ==
                    use->getOperandNumber() - useOffset)
                    addUses(arg.getUses());
            }
        };
    ...
    if (auto whileLoop = llvm::dyn_cast<scf::WhileOp>(op)) {
        ...
        addBlockArgUses(forLoop.getRegionIterArgs(),
                        forLoop.getNumInductionVars(),
        ...
        addBlockArgUses(condBranchOp.getTrueDest()->getArguments(),
                        /*argOffset*/ 0,
                        /*useOffset*/ 1);
...
void TritonAMDGPUCanonicalizePointersPass::runOnOperation() {
    ...
    for (auto arg : func.getArguments()) {
        if (llvm::isa<tt::PointerType>(arg.getType())) {
            // NB: reusing the same SetVector invalidates the topo order implied
            // by getForwardSlice
    ...
    auto isLegal = [&opsToRewrite](Operation *op) {
        if (auto ifOp = llvm::dyn_cast<scf::IfOp>(op)) {
            ...
            // `scf.if` will be walked over, but we do not want to rewrite it
            // yet because the `yields` in the then/else regions haven't been
            // rewritten yet (and those `yields` tell us the final result types
            // of the `scf.if`). Therefore, we check for these attributes and if
            // they're absent then the `scf.if` is legal. Once both `yields`
            // have been rewritten (the corresponding attributes have been
            // added), we report the `scf.if` as illegal, and it will be
            // rewritten (the pattern will fire).
    ...
    // "initializing" the chain of fat pointers starting from tt.func tt.ptr
    // args.
    RewritePatternSet patterns(&getContext());
    patterns.add<ConvertFuncOpArgsUnrealizedCasts, ConvertBroadcastOp,
                 ConvertSplatOp, ConvertConvertLayoutOp, ConvertAddPtrOp,
                 ConvertExtractSliceOp, MaterializeFatPointer<tt::AtomicCASOp>,
                 MaterializeFatPointer<tt::AtomicRMWOp>,
                 MaterializeFatPointer<tt::BitcastOp>,
    ...
    // Rewrite any lingering unrealized_casts that *should* only be the result
    // of unsupported ops.

...
Operation *streamPredication(RewriterBase &rewriter, Operation *op,
    ...
    // regs for the select. Conditionally executing the dot will allow the
    // backend to optimize the select away as redundant.
    if (auto dotOp = dyn_cast<tt::DotOpInterface>(op)) {
        auto loc = dotOp->getLoc();
        auto ifOp = rewriter.create<scf::IfOp>(
            loc, dotOp->getResult(0).getType(), pred, /*withElseRegion=*/true);
        auto thenB = ifOp.getThenBodyBuilder();
        auto yield = thenB.create<scf::YieldOp>(loc, dotOp->getResult(0));
        dotOp->moveBefore(yield);
        ifOp.getElseBodyBuilder().create<scf::YieldOp>(loc,
                                                       dotOp->getOperand(2));
        return ifOp;
    }
...
getSharedEncIfAllUsersAreDotEnc(Value loadedValue) {
    ...
    for (Operation *user : loadedValue.getUsers()) {
        ...
        if (auto memDesc = dyn_cast<ttg::MemDescType>(userResType)) {
            ...
            tempAttr =
                cast<ttg::SwizzledSharedEncodingAttr>(memDesc.getEncoding());
            ...
            if (rank == 3) {
                // Move the batch dimension (dim #0) to be the last so that it
                // will be the slowest varying dimension.
            ...
            auto userResEnc =
                cast<ttg::TensorOrMemDesc>(userResType).getEncoding();
            if (auto dotOpEnc =
                    dyn_cast<ttg::DotOperandEncodingAttr>(userResEnc)) {
                tempAttr = ttg::SwizzledSharedEncodingAttr::get(
                    loadedValue.getContext(), dotOpEnc, srcTy.getShape(),
                    sharedOrder, ctaLayout, bitWidth, /*needTrans=*/false);
            } else if (auto llEnc =
                           dyn_cast<ttg::LinearEncodingAttr>(userResEnc)) {
                // We use linear layout directly for scaled dot fp8 operands.
                // For such cases, we need to look further down the def-use
                // chain to find the dot op for the mfma layout to deduce
                // operand index and other information.
                unsigned opIdx;
                if (auto dotEnc = getDotEncoding(userResult, &opIdx)) {
                    unsigned vecSize =
                        llEnc.getLinearLayout().getNumConsecutiveInOut();
                    LDBG("deduced opIdx: " << opIdx
                                           << "; deduced vecSize: " << vecSize);
...
bool canBeConvertedToAsyncLoad(unsigned numBuffers, tt::LoadOp loadOp,
    ...
    // stride when loading directly to lds
...
LogicalResult scheduleLoads(const LoadToInfoMap &loadToInfo, int maxDist,
    ...
    for (auto &[loadOp, info] : loadToInfo) {
        ...
        if (!isa<tt::LoadOp>(info.use))
            schedule.insert(info.use, stages[SCHED_COMPUTE],
                            clusters[SCHED_COMPUTE]);
    }
...
scheduleLoads(std::array<tt::DotOp, 2> dotOps,
    ...
    for (auto [loadOp, info] : loadToInfo) {
        if (info.use == dotOps[0]) {
            ...
            LDBG(*loadOp
                 << " will not be pipelined because it's not used by a dot");
        }
    }
    return success();
}
...
LogicalResult scheduleOpsBetweenDots(scf::ForOp forOp,
    ...
    for (auto operand : dotOps[1]->getOperands()) {
        ...
        while (!queue.empty()) {
            ...
            // Abort path if we hit a blockarg, left the forward slice of dot0
            // or the op has already a schedule
            if (!defOp || !dot0Slice.contains(defOp) ||
                schedule.count(defOp) != 0) {
                continue;
            }
            ...
            if (numUsers > 1) {
                ...
                // dependencies will be scheduled the same by
                // scheduleDependencies
                schedule.insert(defOp, STAGE_DOT_1,
                                clusters[CLUSTER_AFTER_DOT_2]);
                // Schedule the dot2 operand to interleave with dot1. Its
                // unscheduled dependencies will be scheduled the same by
                // scheduleDependencies
    ...
    for (auto yieldOperand : yield->getOperands()) {
        ...
        schedule.insertIfAbsent(defOp, STAGE_DOT_2,
                                clusters[CLUSTER_AFTER_DOT_1]);
    }
...
void scheduleAsyncCopy(const AsyncCopyChainOps &asyncOps, tt::LoadOp loadOp,
    ...
    if (loadStage == STAGE_GLOBAL_LOAD_1) {
        schedule.insert(waitOp, STAGE_LOCAL_LOAD_1,
                        clusters[CLUSTER_ASYNC_WAIT_1]);
        if (maybeLocalLoadOp)
            scheduleLocalLoad(maybeLocalLoadOp, schedule, STAGE_LOCAL_LOAD_1,
                              clusters[CLUSTER_LOCAL_LOAD_1]);
    } else {
        schedule.insert(waitOp, STAGE_LOCAL_LOAD_2,
                        clusters[CLUSTER_ASYNC_WAIT_2]);
        if (maybeLocalLoadOp)
...
void scheduleStreamCopy(const StreamCopyChainOps &streamOps, tt::LoadOp loadOp,
    ...
    if (maybeLocalLoadOp) {
        if (auto cvt = dyn_cast<ttg::ConvertLayoutOp>(
            ...
            auto [localLoadStage, localLoadCluster] =
                schedule[maybeLocalLoadOp];
...
FailureOr<scf::ForOp> pipelineLoop(scf::ForOp forOp, int numStages,
    ...
    LoadToInfoMap loadToInfo =
        preprocessLoop(axisInfoAnalysis, forOp, numStages);
...
struct PipelinePass : impl::TritonAMDGPUStreamPipelineBase<PipelinePass> {
    ...
    void runOnOperation() override {
        ...
        for (scf::ForOp forOp : loops) {
            ...
            bool waitAtTail =
                usePingpong && (numStagesThis == 3) && useAsyncCopy;
            (void)pipelineLoop(forOp, numStagesThis, globalPrefetch,
        ...
        if (useAsyncCopy) {
            ...
            moduleOp.walk(
                [&](ttg::AsyncWaitOp waitOp) { waitOps.insert(waitOp); });

...
void refineGlobalLoadLayout(PatternRewriter &rewriter, Attribute encoding,
    ...
    rewriter.replaceOpWithNewOp<ttg::ConvertLayoutOp>(load, loadType,
                                                      newResult);
}
...
void transposeInRegsitersBeforeStoreInLocalMemory(
    ...
    auto transposedLayout = ttag::InThreadTransposeOp::deduceOutputLayout(
...
traverseRegionBranchOpForDefs(RegionBranchOpInterface regionBranch, int argIdx,
    ...
    for (auto yieldOp : yieldOps) {
        auto ops =
            traverseCFForValueDefs(yieldOp->getOperand(argIdx), visitedVals);
        if (failed(ops))
            return failure();
        foundDefs.append(ops.value());
    }
...
traverseCFForValueDefs(Value val, SetVector<Value> &visitedVals) {
    ...
    auto attachValue = [val](FailureOr<SmallVector<Value>> res)
        -> FailureOr<SmallVector<Value>> {
        if (failed(res))
            return failure();
    ...
    // if val is not a CFG op and not a block argument, it is a "normal"
    // operation
    ...
    if (auto whileOp = dyn_cast<scf::WhileOp>(parentOp))
        return attachValue(
            traverseWhileOpForDefs(whileOp, argIdx, visitedVals));
    ...
    for (auto &use : val.getUses()) {
        ...
        if (isa<scf::YieldOp>(user)) {
            ...
            if (isa<scf::ForOp, scf::IfOp>(parent)) {
                ...
                auto cfSearch =
                    traverseCFForValueUses(parentResult, visitedVals);
                if (failed(cfSearch))
                    return failure();
            ...
            if (auto forOp = dyn_cast<scf::ForOp>(parent)) {
                ...
                result.transitiveCF.append(
                    condBlockSearch.value().transitiveCF);
            } else if (auto ifOp = dyn_cast<scf::IfOp>(parent)) {
                // do nothing, there are no backward edges in scf::if
            } else {
                LDBG("    Reached unsupported CF operation in forward CF "
                     "traversal");
                return failure();
            }
            continue;
        ...
        if (isa<scf::SCFDialect>(user->getDialect())) {
            LDBG(
                "    can not traverse def-use chains, unsupported control flow "
                "operation");
            return failure();
        }
...
findReachableSMemOps(ttg::LocalLoadOp root) {
    ...
    while (!traversalStep.empty()) {
        ...
        for (auto candidate : traversalStep) {
            ...
            // Each smem operation could have at most 1 result and at most 1
            // memory operand smemOperand is a smem operand of "candidate"
            // operation smemOutput is smem output of "candidate" operation
            Value smemOperand;
            Value smemOutput;
            if (isa<ttg::LocalAllocOp>(candidate)) {
                ...
                assert(false &&
                       "  catched operation unrelated to shared memory");
                return failure();
            }
            ...
            if (smemOutput) {
                ...
                if (failed(forwardSearch))
                    return failure();
                foundNetwork.sharedMemVals.append(
                    forwardSearch.value().transitiveCF);
...
ttg::BlockedEncodingAttr getTransposableBlockedEnc(int dotOperandIdx,
    ...
    auto newKDimSize =
        std::min(maxkDimSizePerThread, dsBitWidth / elemBitwidth);
    LDBG("Choose the minimum of numIters: "
         << newKDimSize << " and numElements: " << dsBitWidth / elemBitwidth);
    ...
    LogicalResult matchAndRewrite(ttg::LocalLoadOp localLoad,
        ...
        for (auto memOp : pattern.localAllocStores)
            transposeInRegsitersBeforeStoreInLocalMemory(
                rewriter, memOp, loadShape, newBlockedEnc);

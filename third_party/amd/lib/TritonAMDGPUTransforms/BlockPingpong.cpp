...
class Pingponger {
    ...
    // entering into the dot cluster while the other warp is still busy in the
    // dot cluster. Otherwise pingpong pattern can be broken and performance
    // drops. Currently pingpong only handles two warps, we only need 0/1
    // priorities.
    int lowPriority = 0;
    int highPriority = 1;
    int32_t kWidth;
  ...
  private:
    void genOffsetConstants(Location loc, OpBuilder &builder,
    ...
    SmallVector<Operation *> genClusterBarrier(OpBuilder &builder,
                                               Location loc);
...
void Pingponger::moveOpAndPredecessorsUpSameBlock(Operation *op) {
    ...
    if (lastInsertedOp->isBeforeInBlock(op)) {
        ...
        auto hasUnsafeUser = [&checkedOp](auto &&user) {
            return user != checkedOp &&
                   user->getBlock() == checkedOp->getBlock() &&
...
SmallVector<Operation *> Pingponger::genClusterBarrier(OpBuilder &builder,
                                                       Location loc) {
    //  MembarAnalysis can recognize gpu::BarrierOp and skip inserting
    //  additional
...
void Pingponger::findClosestPredOps(Value v, DenseSet<T> &matchingOps) {
    ...
    impl = [&matchingOps, &visitedOps, &impl](Value v) {
        // If we encounter a block argument we only look at the terminators of
        // the current block
...
void Pingponger::transformOnePPClusters(OpBuilder &builder, Location loc) {
    ...
    dotOps[0]->emitRemark()
        << "Performed one ping pong cluster transformation\n";
}
...
LogicalResult Pingponger::transformTwoPPClusters(OpBuilder &builder,
    ...
    // cycles, sched.barrier prevents backend from canceling the interleaved
    // order
    updateOpInsertion(gLoadOps[1]);
    appendSlicedLoadAB(/*slice=*/0);
    appendOp(builder.create<ROCDL::SchedBarrier>(loc, 0));
    ...
    // include wait of the local_load inserted by the gpu.barrier, using
    // s.barrier instead. backend will schedule the local memory fences later in
    // the dot0 cluster.
...
LogicalResult Pingponger::transformChainedDotSchedule(OpBuilder &builder,
    ...
    if (llvm::isa<ttg::AsyncWaitOp>(memoryClusterStartOps[0])) {
        // Only append a sched barrier because membar adds a barrier after
        // asyncwait
        appendOp(builder.create<ROCDL::SchedBarrier>(loc, 0));
    } else {
...
Pingponger::transformTwoClusterWithLocalLoadAndAll(OpBuilder &builder,
    ...
    // FIXME: This can be done in the streamPipeline pass but currently there's
    // a know issue with combineRedundantWaitOps that produces incorrect IR. Can
    ...
    if (asyncWaitOps.size() > 1) {
        ...
        for (auto asyncWaitOp : asyncWaitOps) {
            asyncWaitOp.getResult().replaceAllUsesWith(
                newAsyncWaitOp.getResult());
            asyncWaitOp->erase();
        }
    }
...
void Pingponger::getDotPingponged() {
    if (numStages <= 1) {
        LDBG("Pingpong pass expect a loop transformed by pipeliner that "
             "prefetches "
    ...
    forOp->walk([&](Operation *op) {
        llvm::TypeSwitch<Operation *>(op)
            .Case<tt::LoadOp>(
                [&](auto gLoadOp) { gLoadOps.push_back(gLoadOp); })
            .Case<ttg::AsyncCopyGlobalToLocalOp>(
                [&](auto asyncCopyOp) { asyncCopyOps.push_back(asyncCopyOp); })
            .Case<ttg::LocalLoadOp>([&](auto lLoad) {
                // This scheduling doesn't help hiding intra-warp latency. So,
                // we only collect local_load ops that are software pipelined,
                // which means their source is from loop carried values
    ...
    // Currently, pingpong scheduling is known as helpful under limited
    // condition. Individual conditions are checked while collecting each
    // operation such as software pipelining and dot rank=2. Also only accept
    // the for-loop with supported combination of operations because this
    // transformation is very tightly scheduling the latencies.
    ...
    if (scaledDotOps.size() == 1 && numWarps == 8 && numStages == 2 &&
        ...
        if (scaledDotShape[0] == 256 && scaledDotShape[1] == 256 &&
            elemWidth == 8) {
            if (transformTwoClusterWithAsyncAndAll(builder,
                                                   scaledDotOps[0]->getLoc())
                    .failed()) {
    ...
    if (numWarps == 4) { // Pingpong between warps from different blocks
        ...
        // We've observed that this small tile size spent almost equivalent
        // cycle times for issuing the memory operations and issuing dot
        // operations, smaller tile sizes are not likely to get any advantage
        // from current dot centric pingpong scheduling.
        ...
        if (tileSize == mediumTile) {
            if (transformTwoPPClusters(builder, dotOps[0]->getLoc()).failed()) {
                LDBG("Encountered failure when trying to execute the two ping "
                     "pong "
                     "cluster transformation");
                return;
            }
            ...
            if (intShape[0] == 16 && intShape[1] == 16 && kWidth == 8) {
                LDBG("Reached known register spilling case, skip pingpong "
                     "scheduling");
                return;
            }
            if (transformFourPPClusters(builder, dotOps[0]->getLoc())
                    .failed()) {
                LDBG("Encountered failure when trying to execute the four ping "
                     "pong "
                     "cluster transformation");
                return;
            }
        ...
        // Let half of the warps start the loop first and the others follow
        // later but in the synchronized way. This can be accomplished by
        // calling cond_barrier for the second half before the beginning of the
        // loop so they can wait until the first half hit the first barrier in
        // the loop. Also need to call cond_barrier for the first_half after
        // exiting the loop, so all warps can converge again.
        addAsymmetricSyncToLoop(builder, loc);
    }
}

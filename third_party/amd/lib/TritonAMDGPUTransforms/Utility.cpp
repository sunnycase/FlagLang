...
int deduceMinCountOnDefChain(Value defValue, Operation *consumerOp,
    ...
    // Break recursion if we arrive at the producer updating the path based on
    // the ops between producer and consumer
    if (Operation *defOp = defValue.getDefiningOp()) {
        pathSum += deduceMinCountBetweeOps(defOp->getNextNode(), consumerOp,
                                           countFunc);
    ...
    if (auto arg = mlir::dyn_cast<BlockArgument>(defValue)) {
        ...
        pathSum +=
            deduceMinCountBetweeOps(firstOpInLoop, consumerOp, countFunc);
        ...
        int countLoopInit = deduceMinCountOnDefChain(

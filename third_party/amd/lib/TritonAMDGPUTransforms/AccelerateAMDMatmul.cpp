...
warpsPerTile(Operation *dotOp, ArrayRef<int64_t> shape, int numWarps,
    ...
    //    layout of the 2nd dot, which must match the warpsPerCTA of the 2nd
    //    dot.
    ...
    if (isTailDot) {
        ...
        ret[0] = static_cast<unsigned>(
            std::min(static_cast<int64_t>(numWarps),
                     static_cast<int64_t>(
                         llvm::divideCeil(shape[0], shapePerWarp.first))));
...
FailureOr<MfmaIntrinsic> chooseMfmaInstruction(tt::DotScaledOp dot,
    ...
    return chooseMfmaInstruction(dot.getLoc(), mfmaVersion,
                                 dot.getC().getType(), aElemType, bElemType,
                                 inputKDim, nonKDim,
...
FailureOr<MfmaIntrinsic> chooseMfmaInstruction(tt::DotScaledOp dot,
    ...
    return chooseMfmaInstruction(
        dot.getLoc(), mfmaVersion, dot.getC().getType(), elemType, elemType,
...
selectMatrixCoreOperandTypes(tt::DotOp dot,
    ...
    llvm::transform(
        dotOperands, std::back_inserter(initElemTypes), [](Value v) {
    ...
    auto calcConvertCost = [&](Type fromTy, Type toTy) -> int32_t {
        ...
        // Skip conversion between int and float. Int16/int32 cases are lowered
        // to FMA.
        ...
        if (fromTy.isIntOrIndex() && toTy.isIntOrIndex() &&
            ...
            return fromTy.isUnsignedInteger() &&
                           fromTy.getIntOrFloatBitWidth() <
    ...
    for (auto types : applicableTypes) {
        ...
        for (int i = 0; i < initElemTypes.size(); ++i) {
            accumulatedConvertCost +=
                calcConvertCost(initElemTypes[i], types[i]);
        }
...
Value convertAndCastTensor(PatternRewriter &rewriter, Value value,
    ...
    if (newElemType.isIntOrIndex()) {
        ...
        if (oldWidth == newWidth)
            ...
            castedTensor = rewriter.create<arith::TruncIOp>(loc, castedType,
                                                            convertedTensor);
        else if (oldElemType.isSignedInteger())
            castedTensor = rewriter.create<arith::ExtSIOp>(loc, castedType,
                                                           convertedTensor);
        else
            castedTensor = rewriter.create<arith::ExtUIOp>(loc, castedType,
                                                           convertedTensor);
    } else {
        if (oldElemType.isF16() && newElemType.isF32())
            castedTensor = rewriter.create<arith::ExtFOp>(loc, castedType,
                                                          convertedTensor);
        else if (oldElemType.isF32() && newElemType.isF16())
            castedTensor = rewriter.create<arith::TruncFOp>(loc, castedType,
                                                            convertedTensor);
        else
            castedTensor =
...
class BlockedToMFMA : public OpRewritePattern<tt::DotOp> {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(tt::DotOp dotOp,
        ...
        // If mfmaVersion == 4 and both inputs are of F8F6F4 types, we will try
        // to use the V_MFMA_*_F8F6F4 instructions since it has higher FLOPs per
        ...
        auto newAcc =
            convertAndCastTensor(rewriter, oldAcc, mfmaEnc, mfmaAccType);

        // Here is a brief explanation of kWidth, kBase, and kDim
        // 1. kWidth: the number of elements each thread loads from shared
        // memory in
        //    preparation for mfma instructions. In theory each thread can issue
        //    one or more load instructions to load a total of kWidth elements,
        //    since those elements are not required to be in contiguous
        //    addresses in shared memory. But in practice, we make sure the
        //    kWidth elements can be loaded from shared memory by a single
        //    ds_read instruction by setting vecSize of the sharedLayout to be
        //    kWidth.
        // 2. kDim: the k dimension size of the mfma instruction. E.g.
        // instruction
        //    mfma_32x32x16 has kDim = 16, meaning this mfma instruction can
        //    compute a matmul of operands with shape 32x16 and 16x32.
        ...
        //    5.2 kWidth = 2 * kBase, i.e. kPack = 2. In this case, each load
        //    from
        ...
        //    ds_read_b128, which is the largest vector size for shared memory
        //    load.
        auto kWidth = kBase;
        ...
        Value newDot;
        if (withScale) {
            // If a scaled mfma instruction is chosen, we will rewrite the DotOp
            // to a DotScaledOp.
            ...
            newDot = rewriter.create<triton::DotScaledOp>(
                dotOp.getLoc(), newAcc.getType(), a, b, newAcc, Value(),
            ...
            b = convertAndCastTensor(rewriter, b, newBEncoding,
                                     mfmaInstr->bElementType);
            newDot = rewriter.create<tt::DotOp>(
...
class ScaledBlockedToMFMA final : public OpRewritePattern<triton::DotScaledOp> {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(triton::DotScaledOp dotOp,
        ...
        // For mxfp4 A/B tensor, we pack every two values into one int8 value
        // there. For such cases, we have different initial kWidth for LHS and
        // RHS, which will be "fixed" later by using upcast_mxfp to convert LHS
        // to unpacked values. For such packed cases, we cannot support flexible
        // kPack choices from the developer--it just does not apply here. So
        // mandate the choice here.
        ...
        // For A/B tensor, 32 consecutive elements along K dim share the same
        // scale. We'd like to keep the scale values together with the base
        // values in the same warp to avoid cross-warp data exchange. It means
        // we want warpsPerCTA = 1 along the N/M dimension for the mxfp A/B
        // case. We achieve that by setting the M/N dimension as numWarps.
        ...
        auto upcastForMMA = [&](TensorValue v, int idx,
            ...
            if (type == ScaleDotElemType::BF16 ||
                type == ScaleDotElemType::FP16 ||
                type == ScaleDotElemType::E2M1 ||
                (mfmaVersion == 4 && (type == ScaleDotElemType::E4M3 ||
                                      type == ScaleDotElemType::E5M2)))
                return v;
            ...
            return cast<TensorValue>(
                rewriter.create<FpToFpOp>(v.getLoc(), upcastedType, v)
                    .getResult());
        };
        ...
        // tensor to make sure the scale values needed is in the same warp. So
        // we adopt the same CTA layout and warps per CTA. The warp dimensions
        // needs to match along M/N dimension too. With in a warp, we have 64
        // threads. We let each thread read in one scale value. So we need a
        // threadsPerWarp = mDim/nDim along M/N dimension. Note that For MFMA
        // intrinsics, mDim is always the same as nDim. And for scaled dot scale
        // tensor, we always have K as the innermost dimension. So we have the
        // same threadsPerWarp in the below no matter A or B scale. Similarly
        // for warpsPerCTA, the non-K dimension is always at index 0.
        assert(mDim == nDim);
        ...
        auto upcastMXFP = [&](TensorValue v, TensorValue scale,
            ...
            auto newScaleType = RankedTensorType::get(
                ...
                newScaleEncoding);
            auto convOp = rewriter.create<ttg::ConvertLayoutOp>(
                scale.getLoc(), newScaleType, scale);

            Builder b(v.getContext());
            // TODO: Emit device assert to check scale tensor range fitting into
            // fp16?
            Type outputElemType = useFp16 ? b.getF16Type() : b.getBF16Type();
            auto outputType = amdgpu::UpcastMXFPOp::deduceOutputType(
        ...
        auto newDot = rewriter.create<DotOp>(dotOp.getLoc(), newRetType,
                                             scaledA, scaledB, newAcc);
  ...
  public:
    ...
    LogicalResult matchAndRewrite(triton::DotScaledOp dotOp,
        ...
        if (mfmaVersion != 4) {
            return rewriter.notifyMatchFailure(
                dotOp,
                "F8F6F4 scaled dot is only natively supported on gfx950");
        }
        ...
        if (failed(mfmaInstr))
            return rewriter.notifyMatchFailure(
                dotOp, "cannot choose scaled mfma intrinsic");
        ...
        // along the non-K dimension. To support this, we must set tilesPerWarp
        // to {2, 2}. Failing to do so won't break correctness, but it will
        // prevent vectorized local_loads, as the data each thread needs won't
        // be contiguous due to the shuffle pattern. This requirement doesn’t
        // apply to MFMA32 instructions, since only one MFMA op spans the non-K
        // dimension at the minimal shuffling size.
        ...
        auto newRetType = RankedTensorType::get(
            oldShape, oldRetType.getElementType(), mfmaEnc);
        ...
        auto convertInputLayout = [&](TensorValue v,
            ...
            bool kPacked =
                opIdx == 0 ? dotOp.getLhsKPack() : dotOp.getRhsKPack();
            if (kPacked == false) {
                // This is FP4 with M/N packing. Create local alloc + local load
                // here so we have control of the shared layout A, M packed:
                // tensor<16x64xi8> --> 32x32 B, N packed: tensor<64x16xi8> -->
                // 32x32
                ...
                auto newVType = RankedTensorType::get(
                    newShape, vType.getElementType(), newEnc);
                ...
                if (opIdx == 0)
                    ...
                    bEncLL *= newEnc.toLinearLayout(bShape);
                auto newVType = RankedTensorType::get(
                    vType.getShape(), vType.getElementType(), newEnc);
                return rewriter.create<ttg::ConvertLayoutOp>(v.getLoc(),
                                                             newVType, v);
            }
        };
        ...
        auto convertScaleLayout = [&](TensorValue scale,
            ...
            Attribute newScaleEncoding =
                ttg::LinearEncodingAttr::get(ctx, newLL);
            // Scale's data type is always i8
            auto newScaleType =
                RankedTensorType::get(shape, i8_ty, newScaleEncoding);
            ...
            if (!scale) {
                ...
                return rewriter.create<ttg::ConvertLayoutOp>(
                    scale.getLoc(), newScaleType, scale);
            }
        };
        auto newAScale =
...
static void decomposeMixedModeDotOp(ModuleOp mod) {
    mod.walk([](triton::DotOp dotOp) -> void {
        ...
        if (isa<ttg::AMDMfmaEncodingAttr>(D.getType().getEncoding())) {
            ...
            promoteType = AElType.getIntOrFloatBitWidth() >
                                  BElType.getIntOrFloatBitWidth()
                              ? AElType
                              : BElType;
        } else {
        ...
        Value promotedA =
            promoteOperand(builder, loc, dotOp.getA(), promoteType);
        Value promotedB =
            promoteOperand(builder, loc, dotOp.getB(), promoteType);
...
class BlockedToWMMA : public OpRewritePattern<tt::DotOp> {
  ...
  public:
    ...
    LogicalResult matchAndRewrite(tt::DotOp dotOp,
        ...
        // get operand types
        auto operandTypes =
            getOperandTypesForWmmaOp(rewriter, dotOp, wmmaVersion);
        ...
        auto newRetType =
            RankedTensorType::get(retShape, operandTypes[3], wmmaEnc);
        ...
        Value castedA = convertAndCastTensor(
            rewriter, a, newAType.getEncoding(), operandTypes[0]);
        Value castedB = convertAndCastTensor(
            rewriter, b, newBType.getEncoding(), operandTypes[1]);
  ...
  public:
    ...
    Value castToElTy(PatternRewriter &rewriter, Value v, Type elTy) const {
        ...
        if (isFloat(srcElTy) && isFloat(dstElTy)) {
            auto rmode = RoundingModeAttr::get(rewriter.getContext(),
                                               RoundingMode::RTNE);
    ...
    bool isLegalFMAForm(DotOp dotOp, const DotElTypes &dotTypes) const {
        if (AMD::supportsVDot(arch)) {
            ...
            if (dotTypes.a.isF16() && dotTypes.b.isF16() &&
                dotTypes.c.isF32() && dotTypes.d.isF32() && k % 2 == 0) {
                return true;
            }

            // CDNA4 has Bf16 v_dot2
            if (AMD::deduceISAFamily(arch) == ISAFamily::CDNA4 &&
                dotTypes.a.isBF16() && dotTypes.b.isBF16() &&
                dotTypes.c.isF32() && dotTypes.d.isF32() && k % 2 == 0) {
                return true;
            }

            // TODO: enable this condition, when fp32 -> fp16 cast works
            // correctly Consider this case as non legal, despite this case is
            // covered by fp16 FMA. Because v_dot expected to give both better
            // performance and computational precision.
    ...
    LogicalResult tryAccelerateF16WithVDot(DotOp dotOp,
        ...
        if (dotTypes.a.isF16() && dotTypes.b.isF16() && dotTypes.c.isF16() &&
            ...
            auto newDot = rewriter.create<DotOp>(
                dotOp.getLoc(), newC.getType(), dotOp.getA(), dotOp.getB(),
    ...
    LogicalResult tryLegalizeFMA(DotOp dotOp, PatternRewriter &rewriter,
        ...
        SmallVector<Type> opElTy{dotTypes.a, dotTypes.b, dotTypes.c,
                                 dotTypes.d};
        ...
        // Check that type is compatible with all operands; fallback to fp32 if
        // not.
        if (commonTy.isF16()) {
            for (auto elTy : opElTy) {
    ...
    void runOnOperation() override {
        ...
        case ISAFamily::RDNA3:
            ...
            mfmaPatterns.add<::BlockedToWMMA>(
                context, getWmmaVersion(archGenerationName),

...
bool verifyNonSmallerByAssumption(
    ...
    return verifyNonSmallerByAssumption(
        expr, assumptions,
        [&](auto otherAssum) { return otherAssum == other; });
}
...
bool verifyNonNegativeExpr(
    ...
    bool nonNegative =
        llvm::TypeSwitch<Operation *, bool>(expr.getDefiningOp())
            // Various unary triton ops that don't change the sign of the
            // operand
            ...
            .Case<triton::MakeRangeOp>([&](auto makeRangeOp) {
                // See the warning in TritonOps.td: getStart/getEnd return
                // unsigned, so we need to look through get*Attr.
            ...
            .Case<arith::CeilDivUIOp, arith::DivUIOp, arith::ExtUIOp,
                  arith::FPToUIOp, arith::MaxUIOp, arith::MinUIOp,
                  arith::RemUIOp, arith::ShRUIOp>(
                // These OPs also return unsigned values.
                // TODO: We can also sniff whether a Value is unsigned by
                // looking
            ...
            .Case<scf::IfOp>([&](auto ifOp) {
                ...
                assert(it != results.end() &&
                       "expr should be the result of ifOp");
            ...
            if (auto maybeBC = addOpr.getDefiningOp<tt::BroadcastOp>()) {
                ...
                if (auto maybeMul = bcSrc.getDefiningOp<arith::MulIOp>())
                    for (auto mulOpr : maybeMul.getOperands())
                        if (auto maybeSplat =
                                mulOpr.getDefiningOp<tt::SplatOp>())
...
struct ConvertTritonAtomicCASOpToBufferAtomicCAS
    ...
    matchAndRewrite(triton::AtomicCASOp op,
        ...
        if (!isSupportedType) {
            return rewriter.notifyMatchFailure(
                op, "AtomicCAS with unsupported type");
        }
        ...
        if (auto tensorType = dyn_cast<RankedTensorType>(opValueType)) {
            ...
            opBitWidth =
                getVectorSize(basePtr, tensorOffset, axisAnalysisPass) *
                elemBitWidth;
        } else {
        ...
        Value blockStride =
            getBlockStride(op->getLoc(), tensorOffset, rewriter);
...
struct ConvertTritonAtomicRMWOpToBufferAtomicRMW
    ...
    matchAndRewrite(triton::AtomicRMWOp op,
        ...
        // 2. Check the scope. We support GPU and CTA for now (SYSTEM scope is
        // not supported yet)
        switch (scope) {
        case MemSyncScope::GPU:
        case MemSyncScope::CTA:
        ...
        if (isaFamily == ISAFamily::CDNA3 && checkType.isBF16() &&
            ...
            return rewriter.notifyMatchFailure(
                op, "RMW FADD does not support bf16");
        }
        LDBG("RMW FADD supported 16-bit type");

        auto vecSize = getVectorSize(ptr, axisAnalysisPass);
        // f16/bf16 dtypes could only be efficiently calculated using
        // instructions that pack 2 elements (e.g.
        ...
        // 6. Buffer atomics support 32 and 64-bit operations, so inputs must be
        // at
        //    least 32-bits. Otherwise, fall back to the existing path for
        //    atomics
        auto opValueType = op.getVal().getType();
        auto opBitWidth = 0;
        if (auto tensorType = dyn_cast<RankedTensorType>(opValueType)) {
            // We can't just compute the opBitWidth using the numElements *
            // elemBitWidth here. In cases such as tensor<2xf16...>, if the
            // elements are contiguous we can emit the buffer op. Otherwise, the
            // buffer ops lowering will try to emit individual (unsupported)
            // f16/bf16 ops.
            auto elemBitWidth = tensorType.getElementTypeBitWidth();
            ...
            auto bufferLoadOp = [&]() {
                if constexpr (std::is_same_v<SourceOp, triton::LoadOp>) {
                    return rewriter.create<triton::amdgpu::BufferLoadOp>(
                        op->getLoc(), op.getType(), basePtr, tensorOffset,
                    ...
                    return rewriter.create<triton::amdgpu::BufferLoadToLocalOp>(
                        op->getLoc(), op.getType(), op.getResult(), basePtr,
                        tensorOffset, maybeMask, maybeOther, blockStride,
                        op.getCache());
                } else {
                    static_assert(
                        always_false<SourceOp>::value,
...
struct TritonAMDGPUConvertToBufferOpsPass
    ...
    void runOnOperation() override {
        ...
        patterns
            .add<ConvertTritonLoadToBufferLoad<tt::LoadOp>,

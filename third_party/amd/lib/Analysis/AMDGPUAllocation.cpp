...
SmallVector<unsigned> getRepShapeForCvt(RankedTensorType srcTy,
    ...
    assert(srcLayout && dstLayout &&
           "Unexpected layout in getRepShapeForCvt()");
...
ScratchConfig getScratchConfigForCvt(RankedTensorType srcTy,
    ...
    unsigned contiguousShapeDim =
        scratchConfig.repShape[scratchConfig.order[0]];
    ...
    // Clamp the vector length to kMaxShmemVecBitLength / element bitwidth as
    // this is the max vectorisation

...
triton::AMD::TritonIntegerRangeAnalysis::maybeGetTripCount(
            ...
            if (auto attr = dyn_cast<Attribute>(*loopBound)) {
                if (auto bound = dyn_cast_or_null<IntegerAttr>(attr))
                    return bound.getValue();
                           ...
                           llvm::dyn_cast_if_present<Value>(*loopBound)) {
    ...
    APInt max = getLoopRangeInfo(upperBound, block,
                                 /*getUpper=*/true);
    // We can assume step is 1 if no range information as that gives us the
...
void inferResultRanges(tt::MakeRangeOp *op, SetIntRangeFn setResultRange) {
    ...
    assert(llvm::isa<IntegerType>(resTy.getElementType()) &&
           "expected int type");
...
std::optional<ConstantIntRanges> maybeGetAssumedRange(Operation *assumption,
    ...
    if (auto constValue = maybeConstantIntValue) {
        unsigned bitWidth =
            ConstantIntRanges::getStorageBitwidth(anchor.getType());
        ...
        default:
            emitRemark(cmpOp.getLoc(),
                       "unsupported cmp predicate for assumption");
            return {};
        }
    }
    return {};
...
bool cmpIIsStaticallyTrue(const DataFlowSolver &solver, arith::CmpIOp cmpOp) {
    if (auto inputRanges =
        ...
        if (!(*inputRanges)[0] || !(*inputRanges)[1])
            return false;
        return intrange::evaluatePred(pred, *(*inputRanges)[0],
                                      *(*inputRanges)[1])
            .value_or(false);
    }
    return false;
...
LogicalResult TritonIntegerRangeAnalysis::visitOperation(
    ...
    auto joinCallback = [&op, &resultsLattices, this](
                            Value v, const IntegerValueRange &incomingRange) {
        ...
        if (auto maybeRange = maybeGetAssumedRange(v)) {
            incomingRange_ = IntegerValueRange(
    ...
    if (llvm::isa<GetProgramIdOp, MakeRangeOp, HistogramOp, GetNumProgramsOp>(
        ...
        llvm::TypeSwitch<Operation *>(op)
            .Case<GetProgramIdOp>([&](auto getPIDOp) {
                inferResultRangesPID(getPIDOp, kDefaultMaxPrograms - 1,
                                     joinCallback);
            })
            .Case<GetNumProgramsOp>([&](auto getPIDOp) {
                inferResultRangesPID(getPIDOp, kDefaultMaxPrograms,
                                     joinCallback);
            })
    ...
    if (llvm::isa<TransOp, SplitOp, BroadcastOp, ReshapeOp,
                  gpu::ConvertLayoutOp, SplatOp, ExpandDimsOp, JoinOp, CatOp,
                  GatherOp>(op)) {
        ...
        llvm::TypeSwitch<Operation *>(op)
            .Case<TransOp, SplitOp, BroadcastOp, ExpandDimsOp, SplatOp,
                  ReshapeOp, gpu::ConvertLayoutOp>([&](auto) {
                return inferResultRangesUnaryOpForwardArgRange(
                    op, argConstIntRanges, joinCallback);
            })
            ...
            .Case<GatherOp>([&](GatherOp gatherOp) {
                return inferResultRanges(&gatherOp, argConstIntRanges,
                                         joinCallback);
            })
            .Default([&](auto) { llvm::report_fatal_error("unsupported op"); });
        return success();
    ...
    if (auto inferrable = dyn_cast<InferIntRangeInterface>(op)) {
        inferrable.inferResultRangesFromOptional(argIntValueRanges,
                                                 joinCallback);
        return success();
    }
    ...
    for (Operation *op : predecessors->getKnownPredecessors()) {
        ...
        if (inputs.size() != lattices.size()) {
            if (!point->isBlockStart()) {
                if (!inputs.empty()) {
                    firstIndex =
                        cast<OpResult>(inputs.front()).getResultNumber();
                }
                ...
                if (!inputs.empty()) {
                    firstIndex =
                        cast<BlockArgument>(inputs.front()).getArgNumber();
                }
        ...
        for (auto [oper, argLat] :
            ...
            if (loop && loopTripCounts[loop] > kDefaultMaxTripCount) {
                // If the loop's tripcount is too large, infer the maximum range
                // for the arg lattices. This will have the effect that all
                // users will also be inferred to have maximum range and end the
                // analysis will end (the maximum range is the "top" of the
                // lattice and thus no further changes/updates are possible).
            ...
            // Only increase the loop visitation count if have actually update
            // the lattice because otherwise we will over count the number of
            // visits (since not all iter_arg lattices are updated/propagated on
            // each visit).
...
struct FoldTrueCmpIOp : OpRewritePattern<arith::CmpIOp> {
    ...
    LogicalResult matchAndRewrite(arith::CmpIOp cmpOp,
        ...
        if (llvm::isa<IntegerType, IndexType>(cmpOp.getType()) &&
            ...
            if (failed(mlir::dataflow::maybeReplaceWithConstant(
                    *solver, rewriter, cmpOp.getResult()))) {

...
struct ConcatOpConversion : public ConvertOpToLLVMPattern<amdgpu::ConcatOp> {
    ...
    matchAndRewrite(amdgpu::ConcatOp op, OpAdaptor adaptor,
        ...
        for (int regId = 0; regId < dstRegNum; ++regId) {
            ...
            // The n-dim destination tensor is built by arranging n-dim source
            // tensors into a destination tensor shape. Determine which source
            // tensor contains the current CTA tile.
            ...
            for (int dim = 0; dim < rank; ++dim)
                elemCoords[dim].second -=
                    multiDimOperandIdx[dim] * srcShape[dim];

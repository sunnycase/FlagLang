...
void init_triton_xpu_passes_transform(py::module &&m) {
    ...
    m.def("add_tritonxpu_dtype_convert_pass",
          [](mlir::PassManager &self, uint32_t xpu_arch) {
              self.addPass(
                  mlir::triton::xpu::createTritonXPUDtypeConvert({xpu_arch}));
          });
    ...
    m.def("add_tritonxpu_memory_async_pass",
          [](mlir::PassManager &self, bool dump_flag) {
              self.addPass(
                  mlir::triton::xpu::createTritonXPUMemoryAsync({dump_flag}));
          });
...
void init_triton_xpu_llvm(py::module &&m) {
    ...
    m.def("amend_func", [](llvm::Module *llvm_mod, mlir::ModuleOp mlir_mod,
        ...
        llvm::DenseMap<llvm::StringRef, mlir::triton::xpu::XPUMetadata>
            XPUMetadata;
...
void init_triton_xpu(py::module &&m) {
    ...
    m.def("get_buffer_len", [](mlir::ModuleOp &mod, unsigned maxBufferSize) {
        ...
        auto _get_buffer_len = [&](mlir::Type &ptrTy, unsigned maxBufferSize,
            ...
            mlir::Type ptrdataTy;
            if (auto ptrTensorTy =
                    mlir::dyn_cast<mlir::RankedTensorType>(ptrTy)) {
                ptrdataTy = mlir::cast<mlir::triton::PointerType>(
                                ptrTensorTy.getElementType())
                                .getPointeeType();
            } else {
                ptrdataTy = mlir::cast<mlir::triton::PointerType>(ptrTy)
                                .getPointeeType();
            }

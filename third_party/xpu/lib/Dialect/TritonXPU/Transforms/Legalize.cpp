...
struct TritonXPULegalizePass
    ...
    mlir::Operation *findRootOp(mlir::Operation *op) {
        ...
        while (rootOp->getParentOp()) {
            ...
            if (rootOp->getParentOp() &&
                isa<triton::FuncOp>(rootOp->getParentOp())) {
                return rootOp;
            }
        }
        return op;
    ...
    void getGroupInfo(llvm::SetVector<Operation *>(opTree), size_t &ngroup,
        ...
        auto _getGroupInfo = [&](triton::xpu::ClusterLayoutAttr &encoding) {
            ...
            if (isFirst) {
                ...
                assert(ngroup == _ngroup &&
                       "reduction ngroup is not consistent");
                ...
                if (auto reshapeOp = dyn_cast<triton::ReshapeOp>(defOp)) {
                    if (auto reshapeResTy = dyn_cast<RankedTensorType>(
                        ...
                        if (reshapeResTy.getShape().size() == 1) {
                            ...
                            if (auto globalEncoding = mlir::dyn_cast<
                ...
                for (auto i = 0; i < rank; ++i) {
                    sizePerCluster *=
                        unrollOpt ? groupsPerCluster[i] * coresPerGroup[i]
                                  : std::min(sizePerCore[i],
                                             (unsigned)tensorShape[i]) *
    ...
    llvm::SmallVector<int64_t> getSlicedShape(const std::vector<int64_t> &shape,
        ...
        if (!isReduceMultiGroup) // no opt
            spaceSize =
                std::min(static_cast<int64_t>(spaceSize), shape[rank - 1]);
            ...
            if (auto tensorType = mlir::dyn_cast<RankedTensorType>(types[i])) {
                ...
                if (tensorShape.size() > 2) {
                    ...
                    LLVM_DEBUG(llvm::dbgs()
                               << "2D Shape[-1] = " << tensorShape[1]
                               << "); coreNum * bufferSize = "
                               << coreNum * bufferSize << "\n");
                    llvm_unreachable(
                        "2D Shape[-1] <= core_num * buffer_size Limit. "
                ...
                for (size_t j = 0; j < tensorShape.size(); ++j) {
                    iterCounts[j][i] = std::ceil(
                        static_cast<double>(tensorShape[j]) / slicedShape[j]);
                }
            }
        }
        ...
        if (auto tensorType = mlir::dyn_cast<RankedTensorType>(type)) {
            ...
            if (tensorShape.size() == iterCount.size()) {
                for (int i = 0; i < tensorShape.size(); ++i) {
                    slicedShape[i] =
                        std::max(tensorShape[i] / iterCount[i], size_t(1));
                }
            ...
            for (auto op : allOpTree) {
                if (auto rangeOp = dyn_cast<triton::MakeRangeOp>(op)) {
                    for (auto user : rangeOp.getResult().getUsers()) {
                        if (auto userOp =
                                findUserOp<triton::ExpandDimsOp>(user)) {
                            auto expandDimOp =
                                cast<triton::ExpandDimsOp>(userOp);
                ...
                if (auto expandDimOp = dyn_cast<triton::ExpandDimsOp>(op)) {
                    ...
                    if (auto srcTy =
                        ...
                        if (auto resTy = mlir::dyn_cast<RankedTensorType>(
                                result.getType())) {
                ...
                if (auto broadcastOp = dyn_cast<triton::xpu::BroadcastOp>(op)) {
                    ...
                    if (auto srcTy =
                        ...
                        if (auto resTy = mlir::dyn_cast<RankedTensorType>(
                            ...
                            if (srcShape[0] !=
                                resShape[0]) { // unequal dim 0 shape means
    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::xpu::SM2GMOp sm2gmOp) {
            ...
            if (sm2gmOp.getLen()) {
                getOpChainBwd(sm2gmPtrLenOpChain,
                              sm2gmOp.getLen().getDefiningOp());
            }
        });
        ...
        for (auto currStoreOp : endOps) {
            if (!visitedOps.contains(currStoreOp) &&
                ...
                getOpTreeBwd(opTree, visitedOps, currStoreRootOp,
                             currStoreRootBlock);
                llvm::SetVector<Operation *> sortedOpTree =
                    sortOpTreeBwd(opTree);
        ...
        for (int i = 0; i < allTensorTypes.size(); ++i) {
            ...
            //       auto cond_2 = gm2lmOp.getFixedStride() == -1 ? true :
            //       false; auto cond_3 = static_cast<mlir::OffsetState>(
            ...
            for (auto tensorTy : tensorTypes) {
                if (auto rankTensorTy =
                        mlir::dyn_cast<RankedTensorType>(tensorTy)) {
                    ...
                    if (auto rangeOp = dyn_cast<triton::MakeRangeOp>(outerOp)) {
                        ...
                        for (auto user : rangeOp->getUsers()) {
                            if (auto op =
                                    findUserOp<triton::ExpandDimsOp>(user)) {
                                auto expandDimsOp =
                                    cast<triton::ExpandDimsOp>(op);
                                if (expandDimsOp.getAxis() == 0) {
                                    // Recover MakeRangeOp
                                    OpBuilder builder(rangeOp);
                                    auto loc = builder.getUnknownLoc();
                                    auto newMakeRangeOp =
                                        builder.create<triton::MakeRangeOp>(
                                            loc, rangeOp.getType(),
                                            rangeOp.getStart(),
                                            rangeOp.getEnd());
                                    ...
                                    for (auto _it = operands.begin();
                                         _it != operands.end(); ++_it) {
                                        auto operand = *_it;
                                        if (operand == rangeOp) {
                                            user->setOperand(
                                                std::distance(operands.begin(),
                                                              _it),
                                                newMakeRangeOp);
                                        }
                                    }

                                    // Now the old common mrOp is only used by
                                    // outerChain
                                    innerChains[i].insert(newMakeRangeOp);
                                    innerChains[i].remove(rangeOp);
                                    sortedOpTrees[i].insert(newMakeRangeOp);
                                    sortedOpTrees[i] =
                                        sortOpTreeBwd(sortedOpTrees[i]);
                                }
                            }
                        }
                    }
        ...
        auto printCSV = [&](mlir::ModuleOp &mod) {
            ...
            mod.walk([&](mlir::Operation *op) {
                if (dyn_cast<triton::FuncOp>(op) ||
                    dyn_cast<mlir::ModuleOp>(op))
                    return;
            ...
            for (auto op : llvm::reverse(sortedOpTree)) {
                ...
                auto setSlicedResTy = [&](Operation *op, bool isInner = false,
                    ...
                    for (auto [i, resTy] :
                        ...
                        // LLVM_DEBUG(llvm::dbgs() << "\nOrigin Type: " <<
                        // resTy);
                        auto slicedResTy = getSlicedType(
                            resTy, iterCount, isInner, needNewEncoding);
                        // LLVM_DEBUG(llvm::dbgs() << "\nSlicedResType Type: "
                        // << slicedResTy);
                ...
                if (auto makeRangeOp = dyn_cast<triton::MakeRangeOp>(op)) {
                    ...
                    if (outerChain.count(makeRangeOp)) {
                        auto slicedResTy =
                            getSlicedType(type, iterCount, false);
                        auto newOutRangeOp =
                            builder.create<triton::xpu::OutRangeOp>(
                                loc, slicedResTy, reduceGroupSize,
                                reduceRowsPerCore, idx);
                        ...
                        auto newMakeRangeOp =
                            builder.create<triton::xpu::MakeRangeOp>(
                                loc, type, builder.getI32IntegerAttr(start),
                                builder.getI32IntegerAttr(end),
                                builder.getI32IntegerAttr(realSize), index,
                                Value());
                        setSlicedResTy(newMakeRangeOp, isInner);
                        uint32_t newEnd =
                            start +
                            product(newMakeRangeOp.getType().getShape());
                    ...
                    builder.cloneRegionBefore(reduceOp.getCombineOp(),
                    ...
                    for (auto forArg : forArgs) {
                        bool isInnerArg =
                            inOpChain(innerChain, forArg.getDefiningOp());
                        auto slicedArgType = getSlicedType(
                            forArg.getType(), iterCount, isInnerArg);
                    ...
                    for (auto &inBlockOp : forBlock) {
                        ...
                        if (auto reduceOpInFor =
                            ...
                            auto newReduceIdxOp =
                                builderInFor.create<triton::xpu::ReduceOp>(
                                    reduceOpInFor->getLoc(),
                                    reduceOpInFor->getResultTypes(),
                                    reduceOpInFor.getSrcs(),
                                    reduceOpInFor.getAxis(), idx);
                            auto &newCombineOp = newReduceIdxOp.getCombineOp();
                            builderInFor.cloneRegionBefore(
                                reduceOpInFor.getCombineOp(), newCombineOp,
                                newCombineOp.end());
                            setSlicedResTy(newReduceIdxOp, inBlockIsInner);
                            reduceOpInFor.replaceAllUsesWith(
                                newReduceIdxOp->getResults());
                            ...
                            for (auto &opInCombine : newCombineOp.getOps()) {
                                if (auto redReturnOp =
                                        dyn_cast<mlir::triton::ReduceReturnOp>(
                                            opInCombine)) {
                                    auto oldInsertionPoint =
                                        builderInFor.saveInsertionPoint();
                                    builderInFor.setInsertionPoint(redReturnOp);
                                    auto newRedReturnOp = builderInFor.create<
                                        triton::xpu::ReduceReturnOp>(
                                        redReturnOp.getLoc(),
                                        redReturnOp.getOperands());
                                    builderInFor.restoreInsertionPoint(
                                        oldInsertionPoint);
                                    redReturnOp->replaceAllUsesWith(
                                        newRedReturnOp->getResults());
                        ...
                        } else if (auto ifOp = mlir::dyn_cast<mlir::scf::IfOp>(
                                       inBlockOp)) {
                            ...
                            for (auto &inBlockOp : thenBlock) {
                                bool isInnerOp =
                                    inOpChain(innerChain, &inBlockOp);

                                auto newInBlockOp = thenBlock.begin();
                                auto dist =
                                    std::distance(thenBlock.begin(),
                                                  Block::iterator(inBlockOp));
                                std::advance(newInBlockOp, dist);

                                for (auto newInBlockOpRes :
                                     newInBlockOp->getResults()) {
                                    auto slicedOpType =
                                        getSlicedType(newInBlockOpRes.getType(),
                                                      iterCount, isInnerOp);
                            ...
                            if (!elseRegion.empty()) {
                                ...
                                for (auto &inBlockOp0 : elseBlock) {
                                    bool isInnerOp =
                                        inOpChain(innerChain, &inBlockOp0);
                    ...
                    for (auto newIfArg : newIfArgs) {
                        bool isInnerArg =
                            inOpChain(innerChain, newIfArg.getDefiningOp());
                        auto slicedArgType = getSlicedType(
                            newIfArg.getType(), iterCount, isInnerArg);
                    ...
                    if (!newIfElseRegion.empty()) {
                        ...
                        for (auto &inBlockOp : oldIfElseBlock) {
                            ...
                            auto dist =
                                std::distance(oldIfElseBlock.begin(),
                                              Block::iterator(inBlockOp));
                            std::advance(newInBlockOp, dist);
                            ...
                            for (auto newInBlockOpRes :
                               ...
                               dyn_cast<mlir::triton::ReshapeOp>(op)) {
                    if (auto reshapeResTy = dyn_cast<RankedTensorType>(
                            reshapeOp.getResult().getType())) {
                        ...
                        if (reshapeResShape.size() == 1) {
                            ...
                            slicedShape[0] = std::ceil(static_cast<double>(
                                reshapeSrcSize / iterCount[0]));
        ...
        if (!sm2gmPtrLenOpChain.empty()) {
            ...
            for (int j = sortedSm2gmPtrLenOpChain.size() - 1; j >= 0; --j) {
                ...
                for (auto res : op->getResults()) {
                    ...
                    if (auto resTensorTy =
                            mlir::dyn_cast<RankedTensorType>(resTy)) {
                        ...
                        if (auto resEncoding =
                            ...
                            auto groupsPerCluster =
                                resEncoding.getGroupsPerCluster();
                            auto order = resEncoding.getOrder();
                            auto isReduceOpt = resEncoding.getIsReduceOpt();

                            SmallVector<unsigned> newCoresPerGroup(
                                coresPerGroup.size(), 1);
                            SmallVector<unsigned> newGroupsPerCluster(
                                groupsPerCluster.size(), 1);
                            SmallVector<unsigned> newSizePerCore(
                                resShape.begin(), resShape.end());
                            ...
                            auto newResTy = RankedTensorType::get(
                                resShape, elemTy, newEncoding);
                            res.setType(newResTy);
                            if (auto constOp =
                                    dyn_cast<arith::ConstantOp>(op)) {
                                if (auto attr =
                        ...
                        } else if (auto resEncoding = mlir::dyn_cast<
                            ...
                            auto sizePerCore =
                                resGlobalEncoding.getSizePerCore();
                            auto coresPerGroup =
                                resGlobalEncoding.getCoresPerGroup();
                            auto groupsPerCluster =
                                resGlobalEncoding.getGroupsPerCluster();
                            auto order = resGlobalEncoding.getOrder();
                            auto isReduceOpt =
                                resGlobalEncoding.getIsReduceOpt();
                            ...
                            assert(0 && "Unexpected tensor encoding in SM "
                                        "Optimization");
                        }
                    }
                }
        ...
        m.walk([&](triton::MakeRangeOp mrOp) {
            ...
            auto newMakeRangeOp = builder.create<triton::xpu::MakeRangeOp>(
                ...
                builder.getI32IntegerAttr(end),
                builder.getI32IntegerAttr(realSize), Value(), Value());

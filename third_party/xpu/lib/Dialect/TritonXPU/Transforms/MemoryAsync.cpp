  ...
  public:
    ...
    void loadOpAsyncCheck(triton::xpu::LoadOp loadOp_1,
        ...
        if (loadOp_1->getBlock() == loadOp_2->getBlock() &&
            ...
            auto gm2lmOp_1 =
                cast<triton::xpu::GM2LMOp>(loadOp_1->getPrevNode());
        ...
        mod->walk([&](triton::xpu::GM2LMOp gm2lmOp_1) {
            ...
            llvm::TypeSwitch<Operation *>(*loadop_user_begin)
                .Case<XPU_VVECTORIZED_BINARY_OP, XPU_SVECTORIZED_BINARY_OP,
                    ...
                    if (lhsOp && rhsOp) {
                        triton::xpu::LoadOp loadOp_2 =
                            lhsOp == loadOp_1 ? rhsOp : lhsOp;
                        loadOpAsyncCheck(loadOp_1, loadOp_2);
                    }
                })
                .Case<mlir::arith::AddFOp, mlir::arith::SubFOp,
                      mlir::arith::MulFOp, mlir::arith::MaximumFOp,
                      mlir::arith::DivFOp>([&](auto binOp) {
                    auto lhsOp =
                        binOp.getLhs()
                            .template getDefiningOp<triton::xpu::LoadOp>();
                    auto rhsOp =
                ...
                .Default([&](auto &op) {
                    if (dumpFlag) {
                        LLVM_DEBUG({
                            op->dump();
                            llvm::dbgs() << "Unsupport Op For Memory Async "
                                            "Optimization\n";
                        });
                    }
                });

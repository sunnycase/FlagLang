    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::xpu::LoadOp loadOp) {
            ...
            if (auto other = loadOp.getOther()) {
                ...
                auto gm2lmOp =
                    loadOp.getPtr().getDefiningOp<triton::xpu::GM2LMOp>();
                auto allocaOp =
                ...
                auto sltBufLen = builder.create<arith::CmpIOp>(
                    loc, builder.getI1Type(), arith::CmpIPredicate::slt,
                    extractLen, bufLen);
                ...
                if (auto otherDef = other.getDefiningOp()) {
                    if (auto constOp = dyn_cast<arith::ConstantOp>(otherDef)) {
                        ...
                        auto storeOp = builder.create<triton::xpu::StoreOp>(
                            loc, allocaOp, newConstOp, Value(), Value(), -1,
                            false);
                        newConstOp->moveBefore(
                        ...
                        auto newVConstOp =
                            builder.create<triton::xpu::VConstOp>(
                                loc, vconstOp.getType(), vconstOp.getValue());
                        auto storeOp = builder.create<triton::xpu::StoreOp>(
                            loc, allocaOp, newVConstOp, Value(), Value(), -1,
                            false);
                        newVConstOp->moveBefore(
                            ifOp.thenBlock()->getTerminator());
                        storeOp->moveBefore(ifOp.thenBlock()->getTerminator());
                    } else {
                        auto storeOp = builder.create<triton::xpu::StoreOp>(
                            loc, allocaOp, otherDef->getResults()[0], Value(),
                            Value(), -1, false);
                        storeOp->moveBefore(ifOp.thenBlock()->getTerminator());
                    }
                } else {

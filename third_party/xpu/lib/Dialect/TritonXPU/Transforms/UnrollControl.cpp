...
struct TritonXPUUnrollControl
  ...
  public:
    ...
    template <typename T>
    static decltype(auto) createCombineVectorizedOp(T op) {
    ...
    void getUnrollTree(Operation *op, SetVector<Operation *> &opTree,
                       ...
                       Operation *rootOp, bool isTop2Bottom = true) {
        ...
        if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
            ...
            for (auto &inBlockOp : ifThenBlock) {
                getUnrollTree(&inBlockOp, opTree, visitedOps, excludeChainOps,
                              rootOp, isTop2Bottom);
            }
            // Else
            ...
            if (!ifElseRegion.empty()) {
                ...
                for (auto &inBlockOp : ifElseBlock) {
                    getUnrollTree(&inBlockOp, opTree, visitedOps,
                                  excludeChainOps, rootOp, isTop2Bottom);
                }
            }
        }
        ...
        if (isa<triton::xpu::LoadOp, arith::ConstantOp, triton::xpu::VConstOp>(
            ...
            for (auto operand : op->getOperands()) {
                ...
                getUnrollTree(defOp, opTree, visitedOps, excludeChainOps,
                              rootOp, isTop2Bottom);
            }
        }
        ...
        if (isTop2Bottom) {
            ...
            if (excludeChainOps.count(op) ||
                ...
                for (auto userOp : op->getUsers()) {
                    getUnrollTree(userOp, opTree, visitedOps, excludeChainOps,
                                  rootOp, isTop2Bottom);
                }
            }
        }
    ...
    int64_t getnumUnroll(Type type) {
        ...
        if (auto tensorTy = dyn_cast<RankedTensorType>(type)) {
            ...
            numUnroll =
                numUnrollPerCore * clusterEncoding.getCoresPerGroup().back();
        }
        return numUnroll;
    }
    ...
    createEncoding(MLIRContext *context,
    ...
    void setTensorType(MLIRContext *context, Operation *op, int64_t iterNum,
        ...
        for (auto [i, resTy] : llvm::enumerate(op->getResultTypes())) {
            if (isa<RankedTensorType>(resTy) && !isOuter) {
                ...
                if (sliceShape) {
                    shape[shape.size() - 1] =
                        ceil<int64_t>(shape.back(), iterNum);
                }
                RankedTensorType controledTensorTy;
                if (auto sliceEncoding =
                        dyn_cast<triton::gpu::SliceEncodingAttr>(
                            tensorTy.getEncoding())) {
                    auto clusterEncoding = cast<triton::xpu::ClusterLayoutAttr>(
    ...
    bool isOperandOperationInSameForBlock(mlir::Operation *op,
        ...
        if (auto blockArg = mlir::dyn_cast<mlir::BlockArgument>(operand)) {
            ...
            if (definingOp) {
                return definingOp->getBlock()->getParentOp() ==
                       forOp.getOperation();
            }
        }
        return false;
            ...
            if (auto broadcastOp = dyn_cast<triton::xpu::BroadcastOp>(op)) {
                ...
                if (auto srcTy = dyn_cast<RankedTensorType>(src.getType())) {
                    if (auto resTy =
                        ...
                        if (srcInnerNum !=
                            resInnerNum) { // unequal dim 1 shape means in
    ...
    void getDAG(Operation *op, SetVector<Operation *> &visitedOps,
        ...
        getUnrollTree(op, opTree, visitedOps, excludeChainOps, op,
                      isTop2Bottom);
        if (!opTree.empty()) {
    ...
    void createFor(OpBuilder &builder, Location &loc, int64_t start,
                   int64_t iterNum, scf::ForOp &forOp,
                   arith::IndexCastOp &idxVar, ValueRange &iterArgs) {
        ...
        for (auto op : unrollOpTree) {
            ...
            TypeSwitch<Operation *>(newOp)
                ...
                .Case<triton::xpu::StoreOp>([&](auto storeOp) {
                    if (auto tensorTy = dyn_cast<RankedTensorType>(
                            storeOp.getPtr().getType())) {
                        insertIndex(newOp, idxVar);
                    }
                })
                .Case<triton::xpu::MakeRangeOp>([&](auto makeRangeOp) {
                    if (auto tensorTy = dyn_cast<RankedTensorType>(
                            op->getResults()[0].getType())) {
                        insertIndex(newOp, idxVar);
                    }
                })
                ...
                .Case<arith::ConstantOp>([&](auto constantOp) {
                    ...
                    if (auto attr = dyn_cast<DenseElementsAttr>(value)) {
                        value = DenseElementsAttr::getFromRawBuffer(
                            cast<ShapedType>(constantOp.getType()),
                            attr.getRawData());
                    }
                ...
                .Case<scf::IfOp>([&](auto ifOp) {
                    ...
                    if (!ifElseRegion.empty()) {
                        auto &ifElseBlock = ifElseRegion.front();
                        for (auto &inBlockOp : ifElseBlock) {
                            setTensorType(context, &inBlockOp, iterNum,
                                          isOuter);
                        }
                    }
                });
            if (scf::IfOp ifOp = dyn_cast<scf::IfOp>(newOp)) {
                ...
                for (auto &inBlockOp : ifThenBlock) {
                    ...
                    if (auto yieldOp =
                            llvm::dyn_cast<scf::YieldOp>(&inBlockOp)) {
                        // 1. needExtract denotes Extraction is required if the
                        // operand of YieldOp does not match the type expected
                        // by the result of IfOp
                        // 2. isSame denotes whether the operand of YieldOp is
                        // in the same ForBlock as IfOp.
                        unsigned numyieldOpOperands = yieldOp.getNumOperands();
                        assert((numifOpResults == numyieldOpOperands) &&
                               "The number of IfOp results and YieldOp "
                               "operands must match.");
                        for (unsigned i = 0; i < numyieldOpOperands; ++i) {
                            ...
                            if (!isSame && needExtract) {
                                assert(isa<arith::ConstantOp>(
                                           inBlockOp.getOperand(i)
                                               .getDefiningOp()) &&
                                       "Unable to extract the non-constant "
                                       "operand.");
                                auto extractSliceOp = getExtractedOperand(
                                    context, builder, loc, yieldOp, i, iterNum);
                                extractSliceOp->moveBefore(ifOp);
                                inBlockOp.setOperand(
                                    i, extractSliceOp->getResult(0));
                            }
                        }
                    }
                }
    ...
    void moveAllocaAndGM2LM(scf::ForOp forOp) {
        ...
        m.walk(
            [&](triton::xpu::GM2LMOp gm2lmOp) { gm2lmOps.push_back(gm2lmOp); });
        ...
        for (int i = 0; i < unrollOpTrees.size(); ++i) {
            ...
            for (auto op : unrollOpTree) {
                ...
                if (auto storeOp = dyn_cast<triton::xpu::StoreOp>(op)) {
                    ...
                    numUnroll = numUnroll == 1
                                    ? getnumUnroll(type)
                                    : std::min(numUnroll, getNumCol(type));
                    numCol = numCol == 1 ? getNumCol(type)
                                         : std::min(numCol, getNumCol(type));
                    allStoreOps.emplace_back(storeOp);
                    //[TODO] To deal with the case that storeOps are in more
                    // than one
                    // block
                    if (insertPt &&
                        insertPt->getBlock() != storeOp->getBlock()) {
                        return;
                    }
            ...
            if (insertPt) {
                ...
                // 2.2 Set Tensor Type
                IRMapping mapping;
                createLoopBody(context, builder, loc, iterNum, unrollOpTree,
                               outerChain, idxVar, mapping);
    ...
    void unrollControlReduce(MLIRContext *context,
        ...
        if (auto reduceOp = dyn_cast<triton::xpu::ReduceOp>(insertPt)) {
            ...
            createFor(builder, loc, 1, iterNum, forOp, idxVar,
                      newIterArgsRange);
            // 2.2 Set Tensor Type
            IRMapping mapping;
            createLoopBody(context, builder, loc, iterNum, unrollOpTree,
            ...
            for (int i = 0; i < forOp.getInitArgs().size(); ++i) {
                forOp.getInitArgs()[i].replaceUsesWithIf(forArgs[i + 1],
                                                         inForOp);
            }
            ...
            for (int i = 0; i < forOp.getResults().size(); ++i) {
                reduceOp.getOperands()[i].replaceUsesWithIf(
                    forOp.getResults()[i], isReduceOp);
            }
            // 3. Erase old DAG
            eraseDAG(unrollOpTree);
        }
            ...
            TypeSwitch<const Operation *>(op)
                .Case<XPU_MEMORY_OP>([&](auto memoryOp) {
                    getOpChainBwd(excludeChainOps,
                                  memoryOp.getPtr().getDefiningOp());
                    if (memoryOp.getLen()) {
                        getOpChainBwd(excludeChainOps,
                                      memoryOp.getLen().getDefiningOp());
                    }
                })
                .Case<triton::xpu::LoadOp, triton::xpu::StoreOp>(
                    [&](auto acessOp) {
                        if (acessOp.getMask()) {
                            getOpChainBwd(excludeChainOps,
                                          acessOp.getMask().getDefiningOp());
                        }
                    });
        });
    }
    ...
    void createDiscreteOffset(ModuleOp &m) {
        m.walk([&](triton::xpu::LoadOp loadOp) {
            ...
            if (isDiscrete) {
                ...
                auto lmPtr = loadOp.getPtr();
                auto lmAddPtr = cast<triton::AddPtrOp>(
                    findDefOpBwd<triton::AddPtrOp>(lmPtr));
                ...
                auto extractOp = builder.create<triton::xpu::ExtractOp>(
                    loc, getElementTypeOrSelf(gmOffset),
                    builder.getI32IntegerAttr(0), gmOffset);
                auto splatOp = builder.create<triton::SplatOp>(
                    loc, lmOffset.getType(), extractOp);
                auto offset = builder.create<arith::SubIOp>(
                    loc, lmOffset.getType(), lmOffset, splatOp);
    ...
    void createLoadStore(scf::ForOp &forOp, scf::YieldOp &yieldOp, Value &yield,
        ...
        if (!forOp.getResults()[i].use_empty()) {
            ...
            auto inputLoadOp = builder.create<triton::xpu::LoadOp>(
                loc, yieldType, newAllocaOp, Value(), Value(), Value(), 1, -1,
                false, false, false);
            ...
            forOp.getResults()[i].replaceUsesWithIf(reduceLoadOp,
                                                    notReduceLoadOp);
        ...
        m.walk([&](triton::xpu::ReduceOp reduceOp) {
            ...
            if (numCol > numUnroll && numCol % numUnroll == 0) {
                ...
                for (auto operand : reduceOp.getOperands()) {
                    if (auto forOp =
                            dyn_cast<scf::ForOp>(operand.getDefiningOp())) {
                        if (!vistedForOps.count(forOp)) {
                            ...
                            for (auto &inForBlockOp : forBlock) {
                                if (auto ifOp =
                                        dyn_cast<scf::IfOp>(inForBlockOp)) {
                                    SmallVector<Operation *> storeOps;
                                    auto &ifBlock =
                                        ifOp.getThenRegion().front();
                                    auto yieldOp = cast<scf::YieldOp>(
                                        ifBlock.getTerminator());
                                    for (auto [i, yield] : llvm::enumerate(
                                             yieldOp.getOperands())) {
                                        createLoadStore(forOp, yieldOp, yield,
                                                        i, ifBlock, storeOps);
                                    }
                                    // Unroll control
                                    for (auto storeOp : storeOps) {
                                        SmallVector<SetVector<Operation *>>
                                            unrollOpTrees;
                                        getDAG(storeOp, visitedOps,
                                               unrollOpTrees, excludeChainOps);
                                        // Find ptr chain of discrete for moving
                                        // to loop body
                                        SmallVector<SetVector<Operation *>>
                                            newUnrollOpTrees(unrollOpTrees);
                                        findDiscretePtrChains(unrollOpTrees,
                                                              newUnrollOpTrees);
                                        unrollControl(context,
                                                      newUnrollOpTrees);
                                    }
                                    hasIf = true;
                                }
                            ...
                            if (!hasIf) {
                                SmallVector<Operation *> storeOps;
                                auto yieldOp = cast<scf::YieldOp>(
                                    forBlock.getTerminator());
                                for (auto [i, yield] :
                                     llvm::enumerate(yieldOp.getOperands())) {
                                    createLoadStore(forOp, yieldOp, yield, i,
                                                    forBlock, storeOps);
                                }
                                // Unroll control
                                for (auto storeOp : storeOps) {
                                    SmallVector<SetVector<Operation *>>
                                        unrollOpTrees;
                                    getDAG(storeOp, visitedOps, unrollOpTrees,
                                           excludeChainOps);
                                    // Find ptr chain of discrete for moving to
                                    // loop body
                                    SmallVector<SetVector<Operation *>>
                                        newUnrollOpTrees(unrollOpTrees);
                                    findDiscretePtrChains(unrollOpTrees,
        ...
        m.walk([&](triton::xpu::ReduceOp reduceOp) {
            ...
            if (numCol > numUnroll && numCol % numUnroll == 0) {
                ...
                for (int i = 0; i < reduceOperandNum; ++i) {
                    if (auto reduceDefOp =
                            reduceOp.getOperands()[i].getDefiningOp()) {
                        getDAG(reduceDefOp, visitedOps, copyOpTrees,
                               excludeChainOps, false);
                    }
                }
                ...
                rewriter.cloneRegionBefore(reduceOp.getCombineOp(),
                                           &parent.front());
                ...
                for (auto &op : newReduce) {
                    if (auto cmpfOp = dyn_cast<arith::CmpFOp>(op)) {
                        ...
                        if (tensorTy0 != tensorTy1) {
                            ...
                            cmpfOp.setOperand(operandIndexNeedModify,
                                              splatOp.getResult());
                        }
                    ...
                    for (auto [i, resTy] :
                        ...
                        auto inlineTensorTy = RankedTensorType::get(
                            shape, resTy, tensorTy.getEncoding());
                ...
                for (auto [i, returnOperand] :
                     llvm::enumerate(returnOperands)) {
                ...
                unrollControlReduce(context, newUnrollOpTree, reduceOp,
                                    iterArgsRange, returnOperands);

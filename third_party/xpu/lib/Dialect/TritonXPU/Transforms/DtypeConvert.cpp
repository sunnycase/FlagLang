        ...
        for (auto allOpTree : allOpTrees) {
            for (auto op : allOpTree) {
                ...
                if (auto constOp = dyn_cast<arith::ConstantOp>(op)) {
                    ...
                    if ((xpuArch == 2 &&
                         mlir::isa<mlir::Float16Type>(constResTy)) ||
                        (xpuArch == 3 &&
                         mlir::isa<mlir::BFloat16Type>(constResTy))) {
                        ...
                        if (mlir::isa<mlir::TensorType>(constOp.getType())) {
                            ...
                            extfOp = builder.create<arith::ExtFOp>(
                                loc, tensorType, constOp);
                        } else {
                            extfOp = builder.create<arith::ExtFOp>(
                                loc, builder.getF32Type(), constOp);
                        }
                    ...
                    for (auto res : op->getResults()) {
                        ...
                        if ((xpuArch == 2 &&
                             mlir::isa<mlir::Float16Type>(resElemTy)) ||
                            (xpuArch == 3 &&
                             mlir::isa<mlir::BFloat16Type>(resElemTy))) {

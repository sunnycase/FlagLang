  ...
  public:
    ...
    void atoNaiveMask(mlir::ModuleOp m) {
        m.walk([&](scf::IfOp ifOp) {
            ...
            auto atomicCondOp = builder.create<mlir::arith::CmpIOp>(
                loc, builder.getI1Type(), mlir::arith::CmpIPredicate::eq,
                core0Op, threadIdOp);
        ...
        *****************************************************************/
        ...
        m.walk([&](arith::CmpIOp cmpIOp) {
            if (cmpIOp.getPredicate() == arith::CmpIPredicate::eq) {
                ...
                if (auto selectOp =
                    ...
                    if (std::distance(selectop_user_begin, selectop_user_end) !=
                        1)
                        return;

                    if (auto addFOp =
                            dyn_cast<arith::AddFOp>(*selectop_user_begin)) {
                        auto trueVal = selectOp.getTrueValue();
                        auto falseVal = selectOp.getFalseValue();

                        auto trueLoadOp = dyn_cast<triton::xpu::LoadOp>(
                            trueVal.getDefiningOp());
                        auto falseConstOp = dyn_cast<arith::ConstantOp>(
                            falseVal.getDefiningOp());

                        auto trueConstOp = dyn_cast<arith::ConstantOp>(
                            trueVal.getDefiningOp());
                        auto falseLoadOp = dyn_cast<triton::xpu::LoadOp>(
                            falseVal.getDefiningOp());

                        if (trueLoadOp && falseConstOp) {
                            if (auto denseAttr =
                                    mlir::dyn_cast<DenseElementsAttr>(
                                        falseConstOp.getValue())) {
                                float constValue =
                                    *denseAttr.getValues<float>().begin();
                                if (constValue == 0.0f)
                        ...
                        auto lhsLoadOp =
                            dyn_cast<triton::xpu::LoadOp>(lhs.getDefiningOp());
                        auto rhsConstOp =
                            dyn_cast<arith::ConstantOp>(rhs.getDefiningOp());

                        auto lhsConstOp =
                            dyn_cast<arith::ConstantOp>(lhs.getDefiningOp());
                        auto rhsLoadOp =
                            dyn_cast<triton::xpu::LoadOp>(rhs.getDefiningOp());

                        if ((lhsLoadOp && rhsConstOp) ||
                            (lhsConstOp && rhsLoadOp)) {
                            rcvCmpIOp = cmpIOp;
        ...
        if (rcvCmpIOp && rcvLoadOp && atoMaskOpt &&
            ...
            if (atoMaskCond == AtomicMaskCond::PostiveCond) {
                scfIfOp =
                    builder.create<mlir::scf::IfOp>(loc, posCond,
                                                    /*withElseRegion=*/false);
            } else if (atoMaskCond == AtomicMaskCond::NegativeCond) {
                scfIfOp =
                    builder.create<mlir::scf::IfOp>(loc, negCond,
                                                    /*withElseRegion=*/false);
            }
            ...
            Operation *yieldOp =
                scfIfOp.getThenRegion().front().getTerminator();
            while (nextOp) {
    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::SplatOp splatOp) {
            if (auto splatDefOp = splatOp.getSrc().getDefiningOp()) {
                if (auto cmpiOp = dyn_cast<arith::CmpIOp>(splatDefOp)) {
                    ...
                    if (auto lhsTensorTy =
                            dyn_cast<RankedTensorType>(lhs.getType())) {
                        lhsElemTy = lhsTensorTy.getElementType();
                    }
                    auto resTy = cast<RankedTensorType>(splatOp.getType());
                    auto newSplatTy = RankedTensorType::get(
                        resTy.getShape(), lhsElemTy, resTy.getEncoding());
                    auto lhsSplatOp =
                    ...
                    auto newCmpiOp = builder.create<arith::CmpIOp>(
                        loc, resTy, cmpiOp.getPredicate(), lhsSplatOp,
                        rhsSplatOp);
            ...
            if (cmpiOp.getPredicate() == arith::CmpIPredicate::slt ||
                ...
                for (auto user : cmpiOpUsers) {
                    ...
                    if (mask) {
                        ...
                        if (auto andIOp = dyn_cast<mlir::arith::AndIOp>(
                                mask.getDefiningOp())) {
                            if (isFindUserOp(cmpiOp,
                                             andIOp.getLhs().getDefiningOp())) {
                                if (auto cmpiOpDef =
                                        findDefOpBwd<triton::ExpandDimsOp>(
                                            andIOp.getLhs())) {
                                    cmpiOps.insert(cmpiOp);
                                    auto expandDimOp =
                                        cast<triton::ExpandDimsOp>(cmpiOpDef);
                                    //  expand axis = 0 means inner dim
                                    if (expandDimOp.getAxis() == 0) {
                                        maskUsers.insert(user);
                                    }
                                }
                            } else if (isFindUserOp(
                                           cmpiOp,
        ...
        for (const auto &pair : maskUsersMap) {
            ...
            //     auto denseValues = DenseElementsAttr::get(resTensorType,
            //     values); auto rowsPerCoreValue =
            //         builder.create<arith::ConstantOp>(loc, denseValues);
            //     rhs = builder.create<mlir::arith::MulIOp>(loc, rhs,
            //     rowsPerCoreValue);
            //   }

            auto elemLenOp = builder.create<mlir::arith::SubIOp>(
                loc, rhs.getType(), rhs, lhs);
            ...
            for (auto user : users) {
                if (auto loadOp = dyn_cast<triton::xpu::GM2LMOp>(user)) {
                    loadOp.setOperand(1, elemLenOp.getResult());
                               ...
                               dyn_cast<triton::xpu::LM2GMOp>(user)) {
                    storeOp.setOperand(2, elemLenOp.getResult());
                } else if (auto storeOp =
                               dyn_cast<triton::xpu::SM2GMOp>(user)) {
        ...
        for (int i = 0; i < sortedCmpiOps.size(); ++i) {
            ...
            for (int j = sortedOpsToMoveAndErase.size() - 1; j >= 0; --j) {
                ...
                auto newBodyOp =
                    builder.clone(*bodyOp, mapping); // Clone bodyOps
                // TODO[dyq]: Open core tiling pass
                ...
                } else if (auto sm2gmOp =
                               dyn_cast<triton::xpu::SM2GMOp>(bodyOp)) {

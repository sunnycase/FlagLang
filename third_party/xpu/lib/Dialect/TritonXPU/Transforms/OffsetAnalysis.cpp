...
struct TritonXPUOffsetAnalysisPass
  ...
  public:
        ...
        std::string getToken() {
            ...
            TypeSwitch<Operation *>(mockOp)
                ...
                .Case<triton::xpu::GM2LMOp>(
                    [&](auto gm2lmOp) { prefix = "gm2lm_"; });
            return prefix + std::to_string(mockVal);
        }
    };

    template <class T,
              std::enable_if_t<is_xpu_memory_op<T>::value, bool> = true>
            ...
            if (auto memAddPtrOp = dyn_cast<triton::AddPtrOp>(ptrOp)) {
                ...
                for (auto op : ptrOpChain) {
                    if (auto addPtrOp = dyn_cast<triton::AddPtrOp>(op)) {
                        ...
                        if (addPtrDefOp) {
                            if (auto preAddPtrOp =
                                    dyn_cast<triton::AddPtrOp>(addPtrDefOp)) {
                                if (getElementTypeOrSelf(addIRes)
                                    ...
                                    auto extIntOp =
                                        builder.create<arith::ExtSIOp>(
                                            loc,
                                            preAddPtrOp.getOffset().getType(),
                                            addIRes);
                                    auto addIOp = builder.create<arith::AddIOp>(
                                        loc, preAddPtrOp.getOffset().getType(),
                                        extIntOp, preAddPtrOp.getOffset());
                                    ...
                                    auto extIntOp =
                                        builder.create<arith::ExtSIOp>(
                                            ...
                                            preAddPtrOp.getOffset());
                                    auto addIOp = builder.create<arith::AddIOp>(
                                        loc, addIRes.getType(), addIRes,
                                        extIntOp);
                                    addIRes = addIOp.getResult();
                                } else {
                                    auto addIOp = builder.create<arith::AddIOp>(
                                        loc, addPtrOp.getOffset().getType(),
                                        addIRes, preAddPtrOp.getOffset());
                                    addIRes = addIOp.getResult();
                                }
                            } else {
    ...
    void getPtrChainBwd(llvm::SetVector<Operation *> &opChain,
        ...
        if (auto addptrOp = dyn_cast<triton::AddPtrOp>(op)) {
            if (startOp.getResult().getType() ==
                addptrOp.getResult().getType()) {
                getPtrChainBwd(opChain, startOp,
                               addptrOp.getPtr().getDefiningOp());
            }
        }
        return;
    }
    ...
    void getOpChainBwdBFS(llvm::SetVector<Operation *> &opChain,
        ...
        while (!opQueue.empty()) {
            ...
            if (isa<triton::xpu::InterleaveOp>(currentOp) ||
                ...
                (isa<triton::xpu::GM2LMOp>(currentOp) &&
                 currentOp != startOp)) {
                continue;
            }
    ...
    sortByLine(llvm::SetVector<Operation *> &opChain) {
        auto compareOpsByLine = [&](mlir::Operation *op1,
                                    mlir::Operation *op2) {
    ...
    void dumpOpChain(SetVector<Operation *> &opChain) {
        for (auto it = opChain.rbegin(), eit = opChain.rend(); it != eit;
             ++it) {
            Operation *op = *it;
            if (op)
                op->dump();
    ...
    SmallVector<MockData> getMockDataItems(SetVector<Operation *> opChain) {
        ...
        auto getIndexMockVals = [](Operation *indexCastOp) {
            ...
            if (lowerBoundOp && stepOp && upperBoundOp) {
                auto lowerBoundVal =
                    mlir::cast<IntegerAttr>(lowerBoundOp.getValue())
                        .getValue()
                        .getZExtValue();
                auto stepVal = mlir::cast<IntegerAttr>(stepOp.getValue())
                                   ...
                                   .getZExtValue();
                auto upperBoundVal =
                    mlir::cast<IntegerAttr>(upperBoundOp.getValue())
                        .getValue()
                        .getZExtValue();
    ...
    constOpCalFunc(arith::ConstantOp op,
        ...
        if (auto tensorType =
                mlir::dyn_cast<RankedTensorType>(type)) { // tensor
        ...
        if (type.isF32()) {
            auto doubleVal =
                mlir::cast<FloatAttr>(op.getValue()).getValueAsDouble();
            intValue = static_cast<int>(doubleVal);
        } else {
            intValue = mlir::cast<IntegerAttr>(op.getValue())
                           .getValue()
                           .getZExtValue();
        }
    ...
    splatOpCalFunc(triton::SplatOp op,
        ...
        if (mlir::isa<BlockArgument>(operand)) {
            ...
            assert(operandOp &&
                   op2OffsetVal.find(operandOp) != op2OffsetVal.end() &&
                   "Operands must be present in op2OffsetVal map");
            prevValue = op2OffsetVal[operandOp];
        }

        assert(prevValue.size() == 1 &&
               "[splatOpCalFunc] Only support 1->N splat");
        ...
        for (size_t i = 0; i < srcShape.size(); ++i) {
            if (srcShape[i] != resShape[i]) {
                if (++broadNum > 1) { // [1x1xf32 -> NxNxf32]
                    llvm_unreachable(
                        "[broadcastOpCalFunc] Unsupported broadcast 2 dims");
                }
            }
        }
            ...
            if (srcShape[i] != resShape[i]) {
                if (srcShape[i] == 1) { // [1x1xf32 -> 1xNxf32]
                    ...
                    if (i == srcShape[srcShape.size() - 1])
                        return SmallVector<int>(numElems,
                                                op2OffsetVal[operandOp][0]);
                    else
                        return op2OffsetVal[operandOp];
                } else { // [1x2xf32 -> 1xNxf32]
                    llvm_unreachable(
                        "[broadcastOpCalFunc] Only support broadcast 1->N");
                }
            }
        }
    ...
    unaryOpCalFunc(T op,
        ...
        if (!unaryOp) {
            LLVM_DEBUG(llvm::dbgs()
                       << "Operand Must Be Defined by Operations\n");
    ...
    binOpCalFunc(T op, DenseMap<Operation *, SmallVector<int>> &op2OffsetVal) {
        ...
        if constexpr (std::is_same_v<T, arith::AddIOp>) {
            ...
            for (size_t i = 0; i < lhs.size(); ++i) {
                if (rhs[i] == 0) {
                    LLVM_DEBUG(llvm::dbgs()
                               << "Div 0, Return OffsetState::Unknown for "
                                  "Protection\n");
                    findUnsupportedOp = true;
                    return {};
                }
            ...
            for (size_t i = 0; i < lhs.size(); ++i) {
                if (rhs[i] == 0) {
                    LLVM_DEBUG(llvm::dbgs()
                               << "Rem 0, Return OffsetState::Unknown for "
                                  "Protection\n");
                    findUnsupportedOp = true;
                    return {};
                }
            ...
            if (op2OffsetVal.find(*it) == op2OffsetVal.end()) {
                TypeSwitch<Operation *>(*it)
                    .Case<arith::IndexCastOp>([&](auto indexCastOp) {
                        ...
                        op2OffsetVal[indexCastOp] = indexCastOpCalFunc(
                            indexCastOp, op2OffsetVal, mockVal);
                    })
                    .Case<mlir::gpu::ThreadIdOp>([&](auto threadIdOp) {
                        auto mockVal = op2MockVal[threadIdOp];
                        op2OffsetVal[threadIdOp] = threadIdOpCalFunc(
                            threadIdOp, op2OffsetVal, mockVal);
                    })
                    .Case<triton::GetProgramIdOp>([&](auto getProgramIdOp) {
                        auto mockVal = op2MockVal[getProgramIdOp];
                        op2OffsetVal[getProgramIdOp] = getProgramIdOpCalFunc(
                            getProgramIdOp, op2OffsetVal, mockVal);
                    })
                    .Case<triton::xpu::GM2LMOp>([&](auto xpuGm2lmOp) {
                        auto mockVal = op2MockVal[xpuGm2lmOp];
                        op2OffsetVal[xpuGm2lmOp] = xpuGm2lmOpCalFunc(
                            xpuGm2lmOp, op2OffsetVal, mockVal);
                    })
                    ...
                    .Case<arith::ConstantOp>([&](auto constOp) {
                        op2OffsetVal[constOp] =
                            constOpCalFunc(constOp, op2OffsetVal);
                    })
                    .Case<ARITH_PTR_UNARY_OP>([&](auto unaryOp) {
                        op2OffsetVal[unaryOp] =
                            unaryOpCalFunc(unaryOp, op2OffsetVal);
                    })
                    ...
                    .Case<triton::SplatOp>([&](auto splatOp) {
                        ...
                        op2OffsetVal[splatOp] =
                            splatOpCalFunc(splatOp, op2OffsetVal);
                    })
                    ...
                    .Case<triton::xpu::ConvertLayoutOp>(
                        [&](auto convertLayoutOp) {
                            op2OffsetVal[convertLayoutOp] =
                                xpuConvertLayoutOpCalFunc(convertLayoutOp,
                                                          op2OffsetVal);
                        })
                    .Default([&](auto &op) {
                        findUnsupportedOp = true;
                        LLVM_DEBUG(
                            ...
                            << ". Return OffsetState::Unknown for "
                               "Protection.\n");
                    });
            ...
            for (size_t i = 0; i < numElems; ++i) {
                if (start + i < res.size()) {
                    ...
                    if (offsets[start + i] < 0) {
                        LLVM_DEBUG(llvm::dbgs()
                                   << "[OffsetState]: The 0th Address Is Not "
                                      "the Beginning "
                                      "of the Bank.\n");
                        fixedStride = -1;
                        return OffsetState::Unknown;
    ...
    OffsetState locallyContinity(
        ...
        if (isLocallyContinuous(_allOffsets, numElems, rowLen, rowStride)) {
            LLVM_DEBUG(llvm::dbgs()
                       << "[OffsetState]: The Address is Locally Continuous, "
                          "rowLen is "
    ...
    OffsetState getOffsetState(T memoryOp) {
        ...
        if (!ptrOp) {
        ...
        } else if (isa<triton::BitcastOp>(ptrOp) ||
                   isa<triton::SplatOp>(ptrOp)) {
            ...
            LLVM_DEBUG(llvm::dbgs() << "[OffsetAnalysis]: Unsupported Offset "
                                       "Calculation Pattern\n");
            return OffsetState::Unknown;
        }
        ...
        unsigned numElems = 1;
        if (auto offsetTy =
                mlir::dyn_cast<RankedTensorType>(offset.getType())) {
    ...
    void runOnOperation() override {
        ...
        mod.walk([&](triton::xpu::GM2LMOp gm2lmOp) {
            ...
            gm2lmOp->setAttr("fixedStride",
                             builder.getSI32IntegerAttr(fixedStride));
            gm2lmOp->setAttr("rowLen",
                             builder.getIntegerAttr(
                                 builder.getIntegerType(64, true), rowLen));
            gm2lmOp->setAttr("rowStride",
                             builder.getIntegerAttr(
                                 builder.getIntegerType(64, true), rowStride));
        ...
        mod.walk([&](triton::xpu::SM2GMOp sm2gmOp) {
            // TODO: Deal with other offset states in SM2GM, especially 2D case
            // in reduction

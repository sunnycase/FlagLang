...
struct TritonXPUStoreControl
        ...
        for (int i = 0; i < types.size() - 1; ++i) {
            if (auto tensorType = dyn_cast<RankedTensorType>(types[i])) {
                ...
                if (i == 0) {
                    ...
                    assert(groupSize ==
                           product(clusterEncoding.getCoresPerGroup()));
                    assert(groupNum ==
                           product(clusterEncoding.getGroupsPerCluster()));
                }
            }
        }
    }
    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::xpu::StoreOp storeOp) {
            ...
            if (auto op =
                    findDefOpBwd<triton::xpu::ReduceOp>(storeOp.getValue())) {
                auto reduceOp = cast<triton::xpu::ReduceOp>(op);
                ReduceOpHelper help(reduceOp);
                auto srcShape = help.getSrcShape();
                if (srcShape.size() > 1 &&
                    reduceOp.getAxis() != srcShape.size() - 1) {
                    return;
                }
                if (hasBroadcast(storeOp, op) ||
                    !isSameSize(reduceOp, storeOp)) {
                    return;
                }
                ...
                auto coreId = builder.create<triton::xpu::GetCoreIdOp>(
                ...
                auto cond = builder.create<arith::AndIOp>(
                    loc, builder.getI1Type(), isCoreId0InsideGroup,
                    sltUsedCoreNum);
                auto ifOp = builder.create<scf::IfOp>(loc, cond,
                                                      /*withElseRegion=*/false);
                ifBodyMap[ifOp] = ifBodyOps;
                LLVM_DEBUG(llvm::dbgs()
                           << "[StoreControl] GroupSize: " << _groupSize

...
bool replaceAtomicOp(mlir::ModuleOp m) {
    ...
    m.walk([&](triton::AtomicRMWOp atomicRMWOp) {
        ...
        auto loadOp = builder.create<triton::xpu::LoadOp>(
            loc, dtype, ptr, mask, Value(), Value(), 1, -1, false, false,
            false);
        ...
        case RMWOp::AND: {
            arithOp =
                builder.create<arith::AndIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::OR: {
            arithOp =
                builder.create<arith::OrIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::XOR: {
            arithOp =
                builder.create<arith::XOrIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::ADD: {
            arithOp =
                builder.create<arith::AddIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::FADD: {
            arithOp =
                builder.create<arith::AddFOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::MAX: {
            arithOp =
                builder.create<arith::MaxSIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::MIN: {
            arithOp =
                builder.create<arith::MinSIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::UMAX: {
            arithOp =
                builder.create<arith::MaxUIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::UMIN: {
            arithOp =
                builder.create<arith::MinUIOp>(loc, loadOp.getResult(), val);
            break;
        }
        case RMWOp::XCHG: {
...
bool atomicSimulation(mlir::ModuleOp m) {
    ...
    if (replaceAtomicOp(m)) {
        ...
        m.walk([&](triton::ExpandDimsOp expandOp) {
            auto expandOpType =
                mlir::cast<RankedTensorType>(expandOp.getType());
            ...
            if (auto cvtOp =
                ...
                auto sliceEncoding = mlir::cast<triton::gpu::SliceEncodingAttr>(
        ...
        m.walk([&](scf::ForOp forOp) {
            ...
            for (auto forArg : forArgs) {
                if (auto argTy =
                        mlir::dyn_cast<RankedTensorType>(forArg.getType())) {
            ...
            for (auto forRes : forResults) {
                if (auto argTy =
                        mlir::dyn_cast<RankedTensorType>(forRes.getType())) {
                    auto shape = argTy.getShape();
                    auto elemTy = argTy.getElementType();
                    auto argEncoding =
                        mlir::cast<triton::xpu::ClusterLayoutAttr>(
        ...
        m.walk([&](triton::ReduceOp redOp) {
            ...
            auto srcTy =
                mlir::cast<RankedTensorType>(redOp.getOperand(0).getType());
        ...
        m.walk([&](triton::xpu::StoreOp storeOp) {
            ...
            if (xpuArch == 2) {
                if (storeVal.getDefiningOp()) {
                    if (auto truncFOp = dyn_cast<arith::TruncFOp>(
            ...
            auto lm2gmOp = builder.create<triton::xpu::LM2GMOp>(
                ...
                static_cast<int32_t>(OffsetState::Unknown), -1, -1, -1,
                hasAtomicSim);

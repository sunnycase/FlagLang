...
struct TritonXPUVectorizePass
    ...
    template <typename T>
    static decltype(auto) createUnaryVectorizedOp(T op,
    ...
    //       Calculation
    //         if (auto rhsOp =
    //                 rhs.getDefiningOp<arith::ConstantOp>()) { // Static
    //                 Rnumel
    //           auto denseAttr =
    //           rhsOp.getValue().dyn_cast<DenseElementsAttr>(); auto
    //           elemPerCore =
    ...
    bool binLikeOpVectorize(Value lhs, Value rhs, OperationTree &visited,
        ...
        bool lhsVectorized =
            lhsOp ? vectorize(lhsOp, visited, vectorizedOps)
                  : vectorize(lhsLoopInitOp, visited, vectorizedOps);
        bool rhsVectorized =
            rhsOp ? vectorize(rhsOp, visited, vectorizedOps)
    ...
    bool vectorize(Operation *op, OperationTree &visited,
        ...
        TypeSwitch<const Operation *>(op)
            .Case<triton::xpu::GM2LMOp>(
                [&](auto loadOp) { isVectorized = true; })
            .Case<triton::xpu::LM2GMOp>(
                [&](auto loadOp) { isVectorized = true; })
            ...
            .Case<arith::IndexCastOp>(
                [&](auto unaryOp) { isVectorized = true; })
            ...
            .Case<triton::xpu::StoreOp>([&](auto storeOp) {
                isVectorized = vectorize(storeOp.getValue().getDefiningOp(),
            ...
            .Case<triton::xpu::BroadcastOp>([&](auto broadCastOp) {
                ...
                auto srcTy = mlir::dyn_cast<RankedTensorType>(
                    broadCastOp.getSrc().getType());
                ...
                if (rank == 2 && resNumElems >= 16) {
                    // srcShape[0] > 32: Scalar Calculations Perform Better than
                    // Vector Calculations When The Data Size is Small.
            ...
            .Case<triton::ExpandDimsOp>([&](auto expandDimsOp) {
                isVectorized =
                    vectorize(expandDimsOp.getOperand().getDefiningOp(),
                              visited, vectorizedOps);
            })
            .Case<triton::AddPtrOp>([&](auto addPtrOp) {
                isVectorized = vectorize(addPtrOp.getPtr().getDefiningOp(),
                                         visited, vectorizedOps) &&
            ...
            .Case<arith::CmpFOp>([&](auto cmpFOp) {
                ...
                isVectorized =
                    binLikeOpVectorize(lhs, rhs, visited, vectorizedOps);
            })
            .Case<scf::IfOp>([&](auto ifOp) {
                ...
                if (auto yieldOp = dyn_cast<scf::YieldOp>(thenTerminator)) {
                    if (auto prevOp =
                            yieldOp.getOperands().front().getDefiningOp()) {
                        isVectorized =
                            vectorize(prevOp, visited, vectorizedOps);
                    }
                }
            ...
            .Case<triton::ExternElementwiseOp>([&](auto extElemwiseOp) {
                ...
                auto prevOp =
                    extElemwiseOp.getOperands().front().getDefiningOp();
                ...
                if (symbol == "_ZN3xpu5tanhfEf") {
                    ...
                    //   isVectorized =
                    //       isVectorized && vectorize(prevOp, visited,
                    //       vectorizedOps);
                    // }
        ...
        for (Operation *user : op->getUsers()) {
            ...
            // FIXME: We've omitted the `other` value of LoadOp when create
            // GM2LMOp in the past. However, `other` value comes back as we are
            // about to separate GM2LMOp and LoadOp, and it will lead to a user
            // LoadOp be in the vectorization path. Actions should be taken to
            // handle this case. Here we workaround to skip LoadOp's `other`
            // value.
    ...
    RankedTensorType getVectorType(Type tensorType, unsigned _elemWidth = 0) {
        ...
        if (numElems % vectorWidth == 0 &&
            ...
            VectorType newVectorType =
                mlir::VectorType::get(vectorWidth, elemTy);

            // Step 2. getShape
            RankedTensorType oriTensorTy =
                mlir::cast<RankedTensorType>(tensorType);
            auto oriShape = oriTensorTy.getShape();
            llvm::SmallVector<int64_t, 4> newShape(oriShape.begin(),
                                                   oriShape.end());
            auto rank = oriShape.size();
            newShape[rank - 1] /= vectorWidth;

            // Step 3. getEncoding
            auto oriEncoding = mlir::cast<triton::xpu::ClusterLayoutAttr>(
            ...
            auto newEncoding = triton::xpu::ClusterLayoutAttr::get(
                ...
                groupsPerCluster, order, isReduceOpt);
            ...
            llvm_unreachable("Only Supported vector<32xTy> or vector<16xTy> or "
                             "vector<1xTy>");
        }
        return newTensorTy;
    }
    ...
    void processOpVecTy(OperationTree &vectorizedOps, ModuleOp &mod) {
        for (auto *op : vectorizedOps) {
            TypeSwitch<Operation *>(op)
                ...
                .Case<triton::xpu::LM2GMOp>(
                    [&](auto lm2gmOp) { (void)lm2gmOp; })
                .Case<triton::xpu::StoreOp>(
                    [&](auto storeOp) { (void)storeOp; })
                .Case<ARITH_BINARY_FLOAT_OP>([&](auto binOp) {
                    auto newVectorizedTensorTy =
                        getVectorType(binOp.getResult().getType());
                    auto newBinOp =
                        createBinVectorizedOp(binOp, newVectorizedTensorTy);
                ...
                .Case<arith::ConstantOp>([&](auto constOp) {
                    ...
                    auto newConstOp = builder.create<triton::xpu::VConstOp>(
                        constOp.getLoc(), newVectorizedTensorTy,
                        constOp.getValue());
                ...
                .Case<triton::SplatOp>([&](auto splatOp) {
                    ...
                    auto newSplatOp = builder.create<triton::xpu::VSplatOp>(
                        splatOp.getLoc(), newVectorizedTensorTy,
                        splatOp.getOperand());
                ...
                .Case<scf::ForOp>([&](auto forOp) {
                    ...
                    assert(iterArgsInitValues.size() == 1 &&
                           "[Vectorization]: Only Support ForOp with One Iter "
                           "Args");
                ...
                .Case<arith::CmpFOp>([&](auto cmpFOp) {
                    ...
                    Type elemTy =
                        getElementTypeOrSelf(getElementTypeOrSelf(rhsTy));
                    auto newVectorizedTensorTy =
                    ...
                    auto newCmpFOp = builder.create<triton::xpu::VCmpFOp>(
                        cmpFOp.getLoc(), newVectorizedTensorTy,
                        cmpFOp.getPredicate(), cmpFOp.getLhs(),
                        cmpFOp.getRhs());
                ...
                .Case<triton::ExternElementwiseOp>([&](auto extElemwiseOp) {
                    ...
                    if (symbol == "_ZN3xpu5tanhfEf") {
                        auto newExtElemwiseOp = createLibdeviceOp(
                            extElemwiseOp, "_ZN3xpu6vtanhfEDv16_f",
                            newVectorizedTensorTy);
                        extElemwiseOp.replaceAllUsesWith(
                            newExtElemwiseOp.getResult());
                        extElemwiseOp.erase();
                    } else if (symbol == "_ZN3xpu3erfEf") {
                        auto newExtElemwiseOp = createLibdeviceOp(
                            extElemwiseOp, "_ZN3xpu4verfEDv16_f",
                            newVectorizedTensorTy);
                        extElemwiseOp.replaceAllUsesWith(
                            newExtElemwiseOp.getResult());
                        extElemwiseOp.erase();
                        ...
                        auto newExtElemwiseOp = createLibdeviceOp(
                            extElemwiseOp, "_ZN3xpu6visinfEDv16_f",
                            newVectorizedTensorTy);
                        extElemwiseOp.replaceAllUsesWith(
                            newExtElemwiseOp.getResult());
                        extElemwiseOp.erase();
                        ...
                        LLVM_DEBUG(llvm::dbgs()
                                   << "[Vectorization]: Can not Convert Symbol "
                                   << symbol << " to Vfunc\n");
                    }
                })
                .Default([&](auto &op) {
    ...
    void maximumFusion(arith::SelectOp selectOp) {
        if (auto orIOp =
                selectOp.getCondition().getDefiningOp<arith::OrIOp>()) {
            if (orIOp.getResult().hasOneUse()) {
                ...
                if (lhs && rhs && lhs.getResult().hasOneUse() &&
                    ...
                    if (isMax) {
                        ...
                        rhs->erase();
                        LLVM_DEBUG(llvm::dbgs()
                                   << "[Vectorization]: Apply Maximum Fusion "
                                      "Optimization "
                                      "For VVMax.\n");
                    } else if (isMin) {
                        ...
                        rhs->erase();
                        LLVM_DEBUG(llvm::dbgs()
                                   << "[Vectorization]: Apply Minimum Fusion "
                                      "Optimization "
                                      "For VVMin.\n");
                    }
                }
    ...
    bool SVOptimization_Cond(Operation *op) {
        ...
        TypeSwitch<const Operation *>(op)
            .Case<triton::xpu::LoadOp>([&](auto loadOp) {
                auto gm2lmOp =
                    cast<triton::xpu::GM2LMOp>(loadOp->getPrevNode());
            ...
            .Case<triton::xpu::BroadcastOp>([&](auto bcOp) {
                ...
                if (auto srcTy =
                        mlir::dyn_cast<RankedTensorType>(src.getType())) {
                    auto srcShape = srcTy.getShape();
                    if (srcShape.size() == 2 && srcShape[0] == 64 &&
                        srcShape[1] == 1) {
                        canSVOpt = true;
                    }
                }
            })
            .Case<triton::xpu::VConstOp>(
                [&](auto vConstOp) { canSVOpt = true; })
            .Default([&](auto &op) { canSVOpt = false; });

        return canSVOpt;
    }

    bool collectVUser(Operation *op,
            ...
            TypeSwitch<const Operation *>(user)
                .Case<XPU_VVECTORIZED_BINARY_OP>([&](auto vBinOp) {
                    ...
                    if ((opIsLhs ? lCond : rCond) && (lCond != rCond)) {
                        vBinOps[vBinOp] =
                            opIsLhs ? ElemState::SV : ElemState::VS;
                    } else {
                        canSVOpt = false;
                    }
                })
    ...
    void SVOptimization_Modify(triton::xpu::VConstOp vConstOp) {
        ...
        if (rank == 1) {
            newSrcTy = RankedTensorType::get({/*core_num=*/64}, _elemTy,
                                             vConstOpEncoding);
        } else if (rank == 2) {
        ...
        auto newConstOp = builder.create<arith::ConstantOp>(
            vConstOp.getLoc(), newSrcTy, initValue);
    ...
    template <typename T> void createSVBinOp(T vBinOp, ElemState elemStateInt) {
        if (elemStateInt == ElemState::VS) {
            ...
            if constexpr (std::is_same_v<T, triton::xpu::VvsubFOp>) {
                ...
                auto negFOp = builder.create<arith::NegFOp>(vBinOp.getLoc(),
                                                            vBinOp.getRhs());
    ...
    void VvdivToVvmul(triton::xpu::VvdivFOp vvdivOp) {
        // Only can be optimized to vvmul when the denominator is a scalar, it
        // can be further optimized to svmul
        if (auto bcOp =
            ...
            if (auto srcTy = mlir::dyn_cast<RankedTensorType>(src.getType())) {
                auto srcShape = srcTy.getShape();
                if (srcShape.size() != 2 ||
                    !(srcShape[0] == 64 && srcShape[1] == 1)) {
                    return;
                } else {
                    // Step 2. Create DivOp For Rhs
                    OpBuilder builder(bcOp);
                    SmallVector<Attribute, 4> intValues(
                        srcShape[1], builder.getF32FloatAttr(1));
                    DenseElementsAttr denseAttr =
                        DenseFPElementsAttr::get(srcTy, intValues);
                    auto ones = builder.create<arith::ConstantOp>(bcOp.getLoc(),
                                                                  denseAttr);

                    auto oneDivByRhs = builder.create<arith::DivFOp>(
                        bcOp.getLoc(), srcTy, ones, src);
                    ...
                    vvdivOp.erase();
                    LLVM_DEBUG(llvm::dbgs() << "[Vectorization]: Apply VVDIV "
                                               "-> VVMUL Optimization.\n");
                }
            } else {
                return;
    ...
    void VVMacOpFusion(triton::xpu::VvmulFOp mulOp) {
        for (auto nextOp : mulOp->getUsers()) {
            if (auto addOp = dyn_cast<triton::xpu::VvaddFOp>(nextOp)) {
                auto lDefineOp =
                    addOp.getLhs().getDefiningOp(); // getLhs define op
                OpBuilder builder(addOp);
                auto newMacOp = builder.create<triton::xpu::VMacFOp>(
                    mulOp.getLoc(), mulOp.getType(), mulOp.getLhs(),
                    mulOp.getRhs(),
    ...
    void BF16ToFP32VecOptimize(ModuleOp &mod) {
        ...
        mod.walk([&](triton::xpu::LoadOp loadOp) {
            ...
            Type ptrDataTy =
                mlir::cast<PointerType>(ptrElemTy).getPointeeType();
            ...
            if (resScalarTy.isF32() && ptrDataTy.isBF16()) {
                ...
                bf16Tofp32Unordered &=
                    (isVector && isContiguous && notCoreDealMultiRows) ||
                    isSvOpt || isDiscreteSame;
            } else {
        ...
        if (bf16Tofp32Unordered) {
            LLVM_DEBUG(llvm::dbgs()
                       << "[Vectorization]: Apply BF16ToFP32VecUnordered "
                          "Optimization.\n");
        }
    }
    ...
    void runOnOperation() override {
        ...
        if (Maximum_Fusion) {
            mod.walk(
                [&](arith::SelectOp selectOp) { maximumFusion(selectOp); });
        }
        ...
        //         selectOp->erase();
        //         LLVM_DEBUG(llvm::dbgs() << "[Vectorization]: Eliminate
        //         SelectOp For "
        ...
        if (ReduceVec) {
            ...
            mod.walk([&](triton::xpu::ReduceOp redOp) {
                for (int i = 0; i < redOp.getOperands().size() - 1; ++i) {
                    ...
                    vectorizeAndProcessOpVecTy(
                        mod, reduceOperandOp, reduceOperandTy,
                        "[Vectorization]: [Load -> "
                ...
                if (help.isVectorized()) {
                    // reduceop's correct encoding should be inferd by its input
                    // type.
                    auto srcLayout = help.getSrcLayout();
                    for (Value redRes : redOp.getResults()) {
                        if (auto resTy =
                                dyn_cast<RankedTensorType>(redRes.getType())) {
                            ...
                            auto newEncoding =
                                triton::gpu::SliceEncodingAttr::get(
                                    redOp.getContext(),
                                    resSliceEncoding.getDim(),
                                    srcClusterEncoding);
        ...
        if (cvtOp_clean) {
            mod.walk(
                [&](triton::gpu::ConvertLayoutOp cvtOp) { cvtOpclean(cvtOp); });
        }

        // Div -> Mul
        if (Div2Mul) {
            mod.walk([&](triton::xpu::VvdivFOp vvdivFOp) {
                VvdivToVvmul(vvdivFOp);
            });
        }
        ...
        if (SV_Fusion) {
            ...
            for (auto vBCOp : vectorizedBcOps) {
                SVOptimization(vBCOp, "[Vectorization]: Apply SV Optimization "
                                      "For BroadcastOp.\n");
            }
            ...
            for (auto vConstOp : vectorizedConstOps) {
                SVOptimization(
                    vConstOp,
                    "[Vectorization]: Apply SV Optimization For VConstOp.\n");
            }
        }
        ...
        if (VMAC_Fusion) {
            mod.walk(
                [&](triton::xpu::VvmulFOp vvmulFOp) { // must walk after svOpt

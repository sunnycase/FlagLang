...
struct TritonXPUCoreTilingPass
        ...
        if (auto globalEncoding =
            ...
            if (rank == 1) {
                ...
                if (rowsPerCore > 1 && shape[0] != 1) {
                    newSizePerCore = {rowsPerCore,
                                      ceil<unsigned>(shape[1], groupsize)};
                } else {
                    newSizePerCore = {1, ceil<unsigned>(shape[1], groupsize)};
                }
                if (rowsPerCore > 1) {
                    if (auto broadcastOp =
                            dyn_cast<triton::xpu::BroadcastOp>(op)) {
                        // BroadcastOp specifies the element size directly using
                        // the sizePerThread value.
                        order = {1, 0};
                    }
                }
            } else {
                llvm_unreachable(
                    "Reduce Optimization With Rank > 2 Unsupported");
            }
        ...
        mod.walk([&](arith::CmpIOp cmpiOp) {
            ...
            if (auto lhsTensorTy = dyn_cast<RankedTensorType>(lhs.getType())) {
                ...
                if (cmpiOp.getPredicate() == arith::CmpIPredicate::slt &&
                    lhsShape.size() == 2 &&
                    lhsShape[0] == 1) { // inner Cmp Calculation
    ...
    bool canBeOptimized(ModuleOp &mod) {
        ...
        auto checkGroupInfo = [&](RankedTensorType &tensorType) {
            if (auto globalEncoding = dyn_cast<triton::xpu::ClusterLayoutAttr>(
                ...
                size_t n = shape.back();
                size_t newgroupsize =
                    ceil<size_t>(n, static_cast<size_t>(bufferSize));
                newgroupsize = roundupPow2(newgroupsize);
                // min is for not using the whole 64 cores case
                size_t newngroup =
                    std::min(ceil<size_t>(ncore, newgroupsize), m);
        ...
        mod.walk([&](triton::ReduceOp reduceOp) {
            ...
            if (reduceOpTensorShape.size() == 2) {
                ...
                colSize =
                    std::max(colSize, static_cast<int>(reduceOpTensorShape[1]));
                canBeOpt = true;
                ...
                for (rowsPerCoreCal = rowsPerCoreUpper;
                    ...
                    if (reduceOpTensorShape[0] % (rowsPerCoreCal * core_num) ==
                        0)
                        break;
                }

                rowsPerCore =
                    std::min<unsigned>(rowsPerCoreUpper, rowsPerCoreCal);
                ...
                if (inOpChain(innerChain, outerOp)) { // Common MROp
                    if (auto rangeOp = dyn_cast<triton::MakeRangeOp>(outerOp)) {
                        ...
                        for (auto &use : rangeOp->getUses()) {
                            if (auto op = findUserOp<triton::ExpandDimsOp>(
                                    use.getOwner())) {
                                auto expandDimsOp =
    ...
    modifyOpEncoding(ModuleOp &mod, MLIRContext *context,
        ...
        auto getGroupInfo = [&](RankedTensorType &tensorType) {
            if (auto globalEncoding = dyn_cast<triton::xpu::ClusterLayoutAttr>(
                ...
                if (isFirst) {
                    ...
                    assert(ngroup == newngroup &&
                           "reduce ngroup is not consistent");
            ...
            if (auto reduceOp = dyn_cast<triton::ReduceOp>(op)) {
                if (auto tensorType = dyn_cast<RankedTensorType>(
                    ...
                    if (tensorType.getShape().size() == 2) {
                        ...
                        if (auto reshapeOp =
                                dyn_cast<triton::ReshapeOp>(defOp)) {
                            if (auto reshapeResTy = dyn_cast<RankedTensorType>(
                                    reshapeOp.getResult().getType())) {
                                if (reshapeResTy.getShape().size() == 1) {
        ...
        mod.walk([&](mlir::Operation *op) {
            ...
            for (auto opResult : opResults) {
                if (auto resTy =
                    ...
                    if (auto globalEncoding =
                            dyn_cast<triton::xpu::ClusterLayoutAttr>(
                                encoding)) {
                        ...
                        if (auto parentEncoding =
                            ...
                            llvm_unreachable(
                                "Unsupported SliceEncoding's Parent Attribute");
                        }
                    } else {
                    ...
                    auto newResTy =
                        RankedTensorType::get(shape, elemTy, newEncoding);
        ...
        mod.walk([&](arith::ConstantOp constOp) {
            ...
            if (auto attr =
                ...
                newValue = DenseElementsAttr::getFromRawBuffer(
                    mlir::cast<ShapedType>(constOp.getType()),
                    attr.getRawData());
            }
        ...
        mod.walk([&](triton::ExpandDimsOp expandOp) {
            ...
            auto globalEncoding = cast<triton::xpu::ClusterLayoutAttr>(
                expandOpType.getEncoding());

            if (auto cvtOp =
                    expandOp.getSrc()
                        .getDefiningOp<triton::xpu::ConvertLayoutOp>()) {
                auto cvtOpType = cast<RankedTensorType>(cvtOp.getType());
                auto sliceEncoding = cast<triton::gpu::SliceEncodingAttr>(
                    cvtOpType.getEncoding());
                ...
                auto newResTy = RankedTensorType::get(
                    cvtOpType.getShape(), cvtOpType.getElementType(),
                    newSliceEncoding);
        ...
        mod.walk([&](scf::ForOp forOp) {
            ...
            for (auto forArg : forArgs) {
                if (auto argTy = dyn_cast<RankedTensorType>(forArg.getType())) {
                    ...
                    auto argEncoding = cast<triton::xpu::ClusterLayoutAttr>(
                        argTy.getEncoding());

                    auto newArgEncoding = getOptimizedGEncoding(
                        context, argTy, innerChains, forOp, ngroup, groupsize);
                    auto newArgTy =
                        RankedTensorType::get(shape, elemTy, newArgEncoding);
        ...
        mod.walk([&](triton::ReshapeOp reshapeOp) {
            if (auto reshapeResTy = dyn_cast<RankedTensorType>(
                ...
                if (reshapeResShape.size() == 1) {
                    ...
                    unsigned isReduceOpt = 1;
                    Attribute newReshapeResEncoding =
    ...
    void addCvtForBCOp(ModuleOp &mod, MLIRContext *context) {
        mod.walk([&](triton::xpu::BroadcastOp bcOp) {
            ...
            auto finEncoding = triton::xpu::ClusterLayoutAttr::get(
                context, resEncoding.getSizePerCore(),
                resEncoding.getCoresPerGroup(),
            ...
            auto cvt = builder.create<triton::xpu::ConvertLayoutOp>(
                bcOp->getLoc(), finTy, newBCOp);
        ...
        mod.walk([&](triton::xpu::GM2LMOp gm2lmOp) {
            ...
            if (resShape.size() == 2 && resShape[0] > core_num) {
                ...
                gm2lmOp->setAttr("tensorColSize",
                                 builder.getSI32IntegerAttr(std::min(
                                     (unsigned)resShape[1], rawColSize)));
            }
        });

...
struct TritonXPULoopGrid
    ...
    void runOnOperation() override {
        ...
        m.walk([&](triton::FuncOp func) {
            ...
            auto argIdx =
                func.getNumArguments() - TRITON_PROGRAM_INFO_ARG_COUNT;
            auto idxCluster = b.create<triton::xpu::GetClusterIdOp>(loc, i32Ty);
            auto numCluster =
                b.create<triton::xpu::GetNumClusterOp>(loc, i32Ty);
            ...
            Value index = b.create<arith::IndexCastOp>(
                loc, i32Ty, loopGrid.getInductionVar());
            ...
            func.walk([&](triton::GetNumProgramsOp op) {
                op.replaceAllUsesWith(
                    func.getArgument(argIdx + op.getAxisAsInt()));
            });
        });
    }
};

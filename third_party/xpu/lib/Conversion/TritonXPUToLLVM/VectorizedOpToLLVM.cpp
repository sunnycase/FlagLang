...
struct XPUVectorizedOpsConversionBase {
    ...
    unsigned getVectorSize(Type type) const {
        ...
        if (shape[0] !=
            16) { // return vecSize = numElems for vector<numElemsxf32>
            return shape[0];
        }
...
struct VVBinOpsConversion : public ConvertOpToLLVMPattern<SrcOp>,
    ...
    matchAndRewrite(SrcOp op, OpAdaptor adaptor,
        ...
        Value resultStruct =
            packLLElements(loc, getTypeConverter(), calculatedVals, rewriter,
                           llvmResultStructTy);
        ...
        for (unsigned i = 0; i < vecElems; ++i) {
            ...
            if (elemTy.isF32()) {
                Value devCall = mlir::LLVM::XPU::createDeviceCall(
                    VLibOp<SrcOp>::value, rewriter, op, vecTy,
                ...
                default:
                    llvm_unreachable(
                        "Failed to create device call with unsupported xpu "
                        "architecture.");
                }
                resultVals[i] = devCall;
            } else {
                llvm_unreachable(
                    "Only FP16 and FP32 are supported in LibDevice!");
            }
            if (!bool(resultVals[i]))
                return failure();
        }
        Value view = packLLElements(loc, getTypeConverter(), resultVals,
                                    rewriter, structTy);
...
struct VSplatOpConversion : public ConvertOpToLLVMPattern<SrcOp>,
    ...
    LogicalResult matchAndRewrite(SrcOp op, OpAdaptor adaptor,
        ...
        auto llStruct = convertSplatLikeOp(op.getType(), llsrc,
                                           llvmResultStructTy, rewriter, loc);
        ...
        for (size_t elemIter = 0; elemIter < numElems; ++elemIter) {
            ...
            if (elemTy.isF32()) {
                ...
                auto zerosIAsmOp = rewriter.create<LLVM::InlineAsmOp>(
                    loc, resElemTy, xor_operands, xor_asm_string,
                    xor_constraints,
                    /*has_side_effects=*/true,
                    /*is_align_stack=*/false,
                    LLVM::AsmDialectAttr::get(ctx, LLVM::AsmDialect::AD_ATT),
                ...
                Value vvorFOp = rewriter.create<mlir::LLVM::XPU::VVOR_HF_MHOp>(
                    loc, resElemTy, maskV, zerosFAsmOp.getRes(),
                    trueValElems[elemIter], falseValElems[elemIter]);
                resVals.push_back(vvorFOp);
            } else {
                llvm_unreachable(
                    "Only FP16 and FP32 are supported in VSelect!");
            }
        }

        // Wrap data into a struct
        auto llvmResultStructTy = getTypeConverter()->convertType(resTy);
        auto llStruct = packLLElements(loc, getTypeConverter(), resVals,
...
struct VExtFOpConversion : public ConvertOpToLLVMPattern<triton::xpu::VExtFOp>,
    ...
    Value convertFp16ToFp32(Location loc, ConversionPatternRewriter &rewriter,
                            Value val, Value res, Type valElemTy,
                            Type resElemTy, Value llVal,
                            Type llvmResultStructTy) const {
        ...
        Value resultStruct = packLLElements(
            loc, getTypeConverter(), fp32x16Vecs, rewriter, llvmResultStructTy);
        return resultStruct;
    }
    ...
    Value convertBf16ToFp32(Location loc, ConversionPatternRewriter &rewriter,
        ...
        for (size_t elemIdx = 0; elemIdx < 32; ++elemIdx) {
            padVec =
                insert_element(vecFp16Ty, padVec, f16_val(0), i16_val(elemIdx));
        }
    ...
    matchAndRewrite(triton::xpu::VExtFOp op, OpAdaptor adaptor,
        ...
        Type llvmResultStructTy = getTypeConverter()->convertType(resTy);
        assert(_resElemTy.isF32() &&
               "Only support F32 as target dtype inVExtF!");
        if (_valElemTy.isF16()) {
            auto result =
                convertFp16ToFp32(loc, rewriter, val, res, valElemTy, resElemTy,
                                  llVal, llvmResultStructTy);
            rewriter.replaceOp(op, {result});
        } else if (_valElemTy.isBF16()) {
            auto result =
                convertBf16ToFp32(loc, rewriter, val, res, valElemTy, resElemTy,
                                  llVal, llvmResultStructTy);
            rewriter.replaceOp(op, {result});
        } else {
...
struct VTruncFOpConversion
    ...
    using ConvertOpToLLVMPattern<
        triton::xpu::VTruncFOp>::ConvertOpToLLVMPattern;
    ...
    Value convertFp32ToFp16(Location loc, ConversionPatternRewriter &rewriter,
        ...
        for (int i = 0; i < numElems; i += 2) {
            auto asmlh = rewriter.create<LLVM::InlineAsmOp>(
                loc, resElemTy,
                ValueRange{llVals[i], llVals[i + 1]}, // operands
        ...
        Value resultStruct = packLLElements(
            loc, getTypeConverter(), fp16x32Vecs, rewriter, llvmResultStructTy);
        return resultStruct;
    }
    ...
    matchAndRewrite(triton::xpu::VTruncFOp op, OpAdaptor adaptor,
        ...
        if (_resElemTy.isF16()) {
            auto result =
                convertFp32ToFp16(loc, rewriter, val, res, valElemTy, resElemTy,
                                  llVal, llvmResultStructTy);
            rewriter.replaceOp(op, {result});
        } else {
...
void mlir::triton::xpu::populateTTXPUVectorizedOpToLLVMConversionPatterns(
    ...
    patterns.add<VConstOpConversion<triton::xpu::VConstOp, LLVM::ConstantOp>>(
        typeConverter, benefit, targetInfo);
    patterns.add<VSplatOpConversion<triton::xpu::VSplatOp>>(
        typeConverter, benefit, targetInfo);
    patterns.add<VSelectOpConversion<triton::xpu::VSelectOp>>(
        typeConverter, benefit, targetInfo);
    patterns.add<VMacFOpConversion<triton::xpu::VMacFOp>>(typeConverter,
                                                          benefit, targetInfo);

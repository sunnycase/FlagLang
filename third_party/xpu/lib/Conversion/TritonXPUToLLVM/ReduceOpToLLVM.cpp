...
struct XPUReduceOpConversion
    ...
    getSmemBases(triton::xpu::ReduceOp op, unsigned elems,
        ...
        indexToBase[indices[0]] = gep(
            ptr_ty(rewriter.getContext(), 2), getElementType(op, indices[0]),
            indexToBase[indices[0]], i32_val(prevSMOffset));

        offsets.push_back(
            (getElementType(op, indices[0]).getIntOrFloatBitWidth() * elems) /
            8);
        for (unsigned i = 1; i < (op.getNumOperands() - 1);
             ++i) { // skip loopIndex
            ...
            offsets.push_back(
                (getElementType(op, indices[i - 1]).getIntOrFloatBitWidth() *
                 elems) /
                8);
        }
        ...
        // loopResCacheSmemBases[k] is the base pointer for the k-th operand
        // which is the prev loop reduce result
        SmallVector<Value> loopResCacheSmemBases(op.getNumOperands() -
                                                 1); // skip loopIndex
        std::map<unsigned, Value> indexToBaseForLoopCache;
        indexToBaseForLoopCache[indices[0]] =
            gep(ptr_ty(rewriter.getContext(), 2),
                getElementType(op, indices.back()), smemBases.back(),
                i32_val(elems));
        offsets.push_back(
            (getElementType(op, indices[0]).getIntOrFloatBitWidth() * 1) / 8);
        for (unsigned i = 1; i < (op.getNumOperands() - 1);
             ++i) { // skip loopIndex
            indexToBaseForLoopCache[indices[i]] =
                gep(ptr_ty(rewriter.getContext(), 2),
                    getElementType(op, indices[i - 1]),
                    indexToBaseForLoopCache[indices[i - 1]], i32_val(1));
            offsets.push_back(
                (getElementType(op, indices[0]).getIntOrFloatBitWidth() * 1) /
                8);
        }
  ...
  private:
    ...
    SmallVector<Value> initVals(triton::xpu::ReduceOp &op,
        ...
        auto naiveInit = [&](Type elemTy, int value) {
            ...
            if (elemTy.isInteger(1)) {
                ...
                llvm_unreachable(
                    "[Reduce Init]: Unsupported ElemTy in Naive Init");
            }
            return val;
        };
        ...
        auto maxInit = [&](Type elemTy) {
            ...
            if (elemTy.isInteger(1)) {
                ...
                LLVM_DEBUG(elemTy.dump());
                llvm_unreachable(
                    "[Reduce Init]: Unsupported ElemTy in Max Init");
            }
            return val;
        };
        ...
        auto minInit = [&](Type elemTy) {
            ...
            if (elemTy.isInteger(1)) {
                ...
                LLVM_DEBUG(elemTy.dump());
                llvm_unreachable(
                    "[Reduce Init]: Unsupported ElemTy in Min Init");
            }
            return val;
        };
        ...
        for (int i = 0; i < blockArgDefOps.size(); ++i) {
            ...
            TypeSwitch<Operation *>(blockArgDefOp)
                .Case<arith::AndIOp>(
                    [&](auto andIOp) { val = naiveInit(elemTy, 1); })
                .Case<arith::OrIOp>(
                    [&](auto orIOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::AddFOp>(
                    [&](auto addFOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::AddIOp>(
                    [&](auto addIOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::SubFOp>(
                    [&](auto subFOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::SubIOp>(
                    [&](auto subIOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::MulFOp>(
                    [&](auto mulFOp) { val = naiveInit(elemTy, 1); })
                .Case<arith::MulIOp>(
                    [&](auto mulIOp) { val = naiveInit(elemTy, 1); })
                .Case<arith::DivFOp>(
                    [&](auto divFOp) { val = naiveInit(elemTy, 1); })
                .Case<arith::DivSIOp>(
                ...
                .Case<arith::MaxSIOp>(
                    [&](auto maxSIOp) { val = minInit(elemTy); })
                .Case<arith::MaxUIOp>(
                    [&](auto maxUIOp) { val = naiveInit(elemTy, 0); })
                .Case<arith::MinNumFOp>(
                    [&](auto minNumFOp) { val = maxInit(elemTy); })
                .Case<arith::MinSIOp>(
                    [&](auto minSIOp) { val = maxInit(elemTy); })
                .Case<arith::MinUIOp>(
                    [&](auto minUIOp) { val = maxInit(elemTy); })
                ...
                .Case<arith::CmpFOp>([&](auto cmpFOp) {
                    if (cmpFOp.getPredicate() == arith::CmpFPredicate::OGT ||
                        ...
                        llvm_unreachable("[Reduce Init]: Unsupported "
                                         "CmpFPredicate in CmpFOp");
                    }
                })
                .Case<arith::CmpIOp>([&](auto cmpIOp) {
                    if (cmpIOp.getPredicate() == arith::CmpIPredicate::sgt ||
                        ...
                        llvm_unreachable("[Reduce Init]: Unsupported "
                                         "CmpFPredicate in CmpIOp");
                    }
                })
                .Default([&](auto defaultOp) {
                    LLVM_DEBUG(defaultOp->dump());
                    llvm_unreachable("[Reduce Init]: Unsupported Operation in "
                                     "Reduce Output");
                });
    ...
    void initSharedMemory(ReduceOpHelper &helper,
        ...
        for (unsigned i = 0; i < (op.getNumOperands() - 1);
            ...
            Value initPtr = gep(ptr_ty(rewriter.getContext(), 2), elemTy,
                                smemBases[i], coreId);
    ...
    void accumulate(ConversionPatternRewriter &rewriter, Region &combineOp,
                    SmallVector<Value> &acc, ValueRange cur,
                    bool isFirst) const {
        if (isFirst) {
    ...
    void accmulateNaive(ConversionPatternRewriter &rewriter,
                        const Location &loc, SmallVector<Operation *> &ops,
                        SmallVector<Value> &accs, SmallVector<Value> &curs,
                        bool isFirst) const {
    ...
    void reduceWithinThreads(
        ...
        // indices in `offsets` that hold unique values, and only accumualte
        // over those.
        ...
        auto srcIndices =
            emitIndices(op.getLoc(), rewriter, targetInfo,
                        helper.getSrcLayout(), operandType, op.getLoopIndex());
        ...
        if (helper.isVectorized()) {
            ...
            for (auto &it : accs) {
                ...
                for (unsigned i = 0; i < returnDefOps.size(); ++i) {
                    accmulateWithinVector(rewriter, op.getLoc(),
                                          returnDefOps[i], accVecs[i]);
                }
            }
        }
    }
    ...
    void storeCoreReduceToSharedMemory(
        ...
        for (auto it : accs) {
            ...
            for (unsigned i = 0; i < (op.getNumOperands() - 1);
                 ++i) { // skip loopIndex
                auto elemTy = getElementType(op, i);
                Value writePtr = gep(ptr_ty(rewriter.getContext(), 2), elemTy,
                                     smemBases[i], coreId);
    ...
    accumulatePartialReductions(ReduceOpHelper &helper,
        ...
        for (unsigned i = 0; i < operandNum; ++i) { // skip loopIndex
            ...
            for (unsigned readOffset = 0; readOffset < groupSizeInt;
                 ++readOffset) {
        ...
        for (unsigned i = 0; i < operandNum; ++i) { // skip loopIndex
            ...
            writePtrs[i] =
                gep(ptr_ty(rewriter.getContext(), 2), elemTy, smemBases[i],
        ...
        if (isNeedLoopCacheResult(op)) {
            ...
            for (unsigned i = 0; i < (operandNum); ++i) { // skip loopIndex
                ...
                loopResCacheSmemValues[i] =
                    load_sm(elemTy, loopResCacheSmemBases[i]);
            }

            SmallVector<Operation *> returnDefOps = helper.getReturnDefOps();
            if (helper.isVectorized()) {
                accmulateNaive(rewriter, loc, returnDefOps,
                               loopResCacheSmemValues, loopResCacheSmemValues,
                               false);
            } else {
    ...
    void loadReductionAndPackResult(ReduceOpHelper &helper,
        ...
        for (unsigned i = 0; i < (op.getNumOperands() - 1);
            ...
            if (auto resultTy =
                ...
                // nd-tensor where n >= 1
                auto resultLayout =
                    cast<SliceEncodingAttr>(resultTy.getEncoding());
                unsigned resultElems = getTotalElemsPerThread(resultTy);
                auto resultIndices =
                    emitIndices(loc, rewriter, targetInfo, resultLayout,
                                resultTy, op.getLoopIndex());
                auto resultShape = resultTy.getShape();
                auto resultCTATile =
                    getShapePerCTATile(resultLayout, resultShape);
                assert(resultIndices.size() == resultElems);

                SmallVector<Value> resultVals(resultElems);
                for (size_t j = 0; j < resultElems; ++j) {
                    SmallVector<Value> readIdx = resultIndices[j];
                    //   readIdx.insert(readIdx.begin() + op.getAxis(),
                    //   i32_val(0)); for (size_t resultIdx = 0, resultDim =
                    //   resultShape.size();
                    //        resultIdx < resultDim; ++resultIdx) {
                    //     auto smemIdx = resultIdx < op.getAxis() ? resultIdx :
                    //     resultIdx
                    //     + 1; if (resultCTATile[resultIdx] >
                    //     smemShape[smemIdx] ||
                    //         resultShape[resultIdx] > smemShape[smemIdx]) {
                    //       // When srcShape smaller then src sizePerThread,
                    //       only srcShape
                    //       // elements is accumulated in smem. Modulo
                    //       smemShape effectively
                    //       // replicates srcShape elements to src
                    //       sizePerThread. readIdx[smemIdx] =
                    //           urem(readIdx[smemIdx],
                    //           i32_val(smemShape[smemIdx]));
                    //     }
                    //   }

                    Value readOffset = groupSkip;
                    //   Value readOffset = linearize(rewriter, loc, readIdx,
                    //   smemShape, smemOrder);
                    Value readPtr = gep(ptr_ty(rewriter.getContext(), 2),

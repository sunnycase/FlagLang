...
TritonXPUTypeConverter::TritonXPUTypeConverter(MLIRContext *context,
    ...
    addArgumentMaterialization(
        [&](OpBuilder &builder, RankedTensorType tensorType, ValueRange inputs,
            Location loc) -> std::optional<Value> {
            llvm_unreachable(
                "Argument rematerialization should not happen in Triton "
    ...
    addSourceMaterialization([&](OpBuilder &builder,
                                 ...
                                 Location loc) -> std::optional<Value> {
        llvm_unreachable(
            "Source rematerialization should not happen in Triton -> "
            "TritonXPU Conversion");
        return std::nullopt;
    });
    ...
    addTargetMaterialization([&](OpBuilder &builder,
                                 RankedTensorType tensorType, ValueRange inputs,
                                 Location loc) {
        auto cast = builder.create<triton::xpu::ConvertLayoutOp>(
...
TritonXPUConversionTarget::TritonXPUConversionTarget(
    ...
    // TODO[dyq]: XPUSDNN-CHECK check addDynamicallyLegalDialect for
    // triton::DotOp
}

...
def compile(src, target=None, options=None):
    ...
    def post_stage(module):
        ...
        if (fn_override_manager is not None and fn_override_manager.has_file(ir_filename)):
            ...
            module = parse(full_name, ext, context)
        ...
        if use_ttgir_loc and ext == "ttgir":
            ...
            module.create_location_snapshot(ttgir_full_name)
    ...
    if target.backend == "xpu":
        ...
        if first_stage > make_ttxir_stage_index:
            for ext, compile_ir in list(stages.items())[first_stage:]:
                ...
                post_stage(next_module)
                module = next_module
        else:
            index = 0
            while True:
                for ext, compile_ir in list(stages.items())[first_stage:make_elf_stage_index + 1]:
                    next_module = compile_ir(module, metadata)
                    post_stage(next_module)
                    module = next_module
                if backend.is_elf_stack_size_oob(module):
                    index = index + 1
                    if metadata["buffer_size_limit"] == 16:
                        raise RuntimeError("Failed to tune buffer size.")
                    module = src.make_ir(options, codegen_fns, context)
                    buffer_size_limit_tuned = metadata["buffer_size_limit"] // 2
                    tune_log = ('[Buffer Size Limit Tunning] '
                                f'Tune buffer_size_limit from {metadata["buffer_size_limit"]}'
                                f' to {buffer_size_limit_tuned}')
                    # print(tune_log)
                    metadata["buffer_size_limit"] = buffer_size_limit_tuned
                else:
                    break

            for ext, compile_ir in list(stages.items())[make_elf_stage_index + 1:]:
                next_module = compile_ir(module, metadata)
                post_stage(next_module)
                module = next_module
    else:
...
class CompiledKernel:
    ...
    def __init__(self, src, metadata_group, hash):
        ...
        self.asm = {
            file.suffix[1:]:
            file.read_bytes() if file.suffix[1:] == binary_ext or file.suffix[1:] == 'elf' else
            file.read_text()  # ELF Check For Triton XPU

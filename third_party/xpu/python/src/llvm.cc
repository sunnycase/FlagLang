...
initObjectFileByMemory(unsigned char *ObjArray, uint64_t ObjLen) {
    ...
    ErrorOr<std::unique_ptr<MemoryBuffer>> FileOrErr =
        MemoryBuffer::getMemBuffer(StringRef((char *)ObjArray, ObjLen),
                                   ".debug", false);
    if (!FileOrErr) {
...
bool isElfStackSizeOOB(std::string ElfObject) {
    ...
    if (!StackSizeLimitStr.empty()) {
        ...
        if (StackSizeLimitSRf.getAsInteger(10, StackSizeLimit)) {
            llvm::report_fatal_error(
                "Invalid value for TRITON_TUNE_BUFFER_LM_SIZE: " +
                StackSizeLimitSRf);
        }
    }
    ...
    for (std::pair<object::SymbolRef, uint64_t> &SymbolSize : SymbolSizes) {
        ...
        Expected<StringRef> ContentsOrErr =
            SymSectionOrErr.get()->getContents();
        if (!ContentsOrErr) {
           ...
           const std::string triple) {
...

            {
                ...
                if (!LMSizeLimitStr.empty()) {
                    ...
                    if (LMSizeLimitSRf.getAsInteger(10, LMSizeLimit)) {
                        llvm::report_fatal_error(
                            "Invalid value for LLVM_ERROR_LM_SIZE: " +
                            LMSizeLimitSRf);
                    }
                }
                llvm::StringMap<llvm::cl::Option *> optMap =

...
void CircularLayoutParser::parseSegment(
    ...
    std::unordered_map<int, CircularLayoutParserResult::ProfileEvent>
        activeEvent;
    ...
    for (auto &bt : result->blockTraces) {
        for (auto &trace : bt.traces) {
            for (auto &event : trace.profileEvents) {
                ...
                // Adjust the cycle for tiny events below the profiling
                // precision

...
void StreamChromeTraceWriter::write(std::ostream &outfile) {
    if (streamTrace.empty()) {
        std::cerr << "Failed to write the trace file: empty trace!"
                  << std::endl;
        return;
    }
            ...
            for (auto &trace : bt->traces) {
                ...
                for (auto &event : trace.profileEvents) {
                    ...
                    if (!scopeColor.count(scopeId)) {
                        ...
                        curColorIndex =
                            (curColorIndex + 1) % kChromeColor.size();
                    }
                    const std::string &color =
                        kChromeColor[scopeColor[scopeId]];
                    pid = metadata->kernelName + " Core" +
                          std::to_string(procId) + " CTA" +
                          std::to_string(ctaId) +
                    ...
                    args["Unit"] = "GPU cycle";
                    args["Kernel Gap"] =
                        std::to_string(kKernelTimeGap) + "cycle(ns)";
                    element["args"] = args;

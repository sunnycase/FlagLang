    ...
    createDefaultResult(int numBlocks, int numTraces, int numEvents) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            ...
            for (int j = 0; j < numTraces; j++) {
                result->blockTraces[i].traces[j].profileEvents.resize(
                    numEvents);
...
TEST_F(ChromeTraceWriterTest, MultiBlockMultiWarp) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            result->blockTraces[j].traces[i].profileEvents[0].first->cycle =
                122;
            result->blockTraces[j].traces[i].profileEvents[0].second->cycle =
                162;
            result->blockTraces[j].traces[i].profileEvents[0].first->scopeId =
                1;
            result->blockTraces[j].traces[i].profileEvents[0].second->scopeId =
                1;
            result->blockTraces[j].traces[i].profileEvents[1].first->cycle =
                142;
            result->blockTraces[j].traces[i].profileEvents[1].second->cycle =
                182;
            result->blockTraces[j].traces[i].profileEvents[1].first->scopeId =
                2;
            result->blockTraces[j].traces[i].profileEvents[1].second->scopeId =
                2;
            result->blockTraces[j].traces[i].profileEvents[2].first->cycle =
                172;
            result->blockTraces[j].traces[i].profileEvents[2].second->cycle =
                200;
            result->blockTraces[j].traces[i].profileEvents[2].first->scopeId =
                3;
            result->blockTraces[j].traces[i].profileEvents[2].second->scopeId =
                3;
            result->blockTraces[j].traces[i].profileEvents[3].first->cycle =
                183;
            result->blockTraces[j].traces[i].profileEvents[3].second->cycle =
                210;
            result->blockTraces[j].traces[i].profileEvents[3].first->scopeId =
                4;
            result->blockTraces[j].traces[i].profileEvents[3].second->scopeId =
                4;
        }
    }
    std::vector<KernelTrace> kerneltrace = {std::make_pair(result, metadata)};
...
TEST_F(ChromeTraceWriterTest, MultiKernel) {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 1; j++) {
            ...
            result1->blockTraces[j].traces[i].profileEvents[0].first->cycle =
                1220000;
            result1->blockTraces[j].traces[i].profileEvents[0].second->cycle =
                1620000;
            result1->blockTraces[j].traces[i].profileEvents[0].first->scopeId =
                1;
            result1->blockTraces[j].traces[i].profileEvents[0].second->scopeId =
                1;
        }
    }
    ...
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            result2->blockTraces[j].traces[i].profileEvents[0].first->scopeId =
                1;
            result2->blockTraces[j].traces[i].profileEvents[0].second->scopeId =
                1;
        }
    }

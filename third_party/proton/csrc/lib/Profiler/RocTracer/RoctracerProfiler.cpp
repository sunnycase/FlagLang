...
void processActivity(RoctracerProfiler::CorrIdToExternIdMap &corrIdToExternId,
    ...
    case kHipVdiCommandTask:
    case kHipVdiCommandKernel: {
        processActivityKernel(corrIdToExternId, externId, dataSet, record,
                              isAPI, isGraph);
        break;
    }
    default:
...
struct RoctracerProfiler::RoctracerProfilerPimpl
    ...
    ThreadSafeMap<hipGraph_t, uint32_t,
                  std::unordered_map<hipGraph_t, uint32_t>>
        GraphToNumInstances;
    ...
    if (domain == ACTIVITY_DOMAIN_HIP_API) {
        ...
        if (data->phase == ACTIVITY_API_PHASE_ENTER) {
            ...
            if (cid == HIP_API_ID_hipGraphLaunch) {
                ...
                if (!findGraph)
                    std::cerr << "[PROTON] Cannot find graph and it may cause "
                                 "a memory leak."
                                 "To avoid this problem, please start "
                                 "profiling before the "
                                 "graph is created."
                              << std::endl;
            }
            ...
            case HIP_API_ID_hipModuleLaunchCooperativeKernel: {
                hipStream_t Stream =
                    data->args.hipModuleLaunchCooperativeKernel.stream;
                if (pImpl->StreamToCapture.contain(Stream))
                    pImpl->StreamToCaptureCount[Stream]++;
                break;
            }
...
void RoctracerProfiler::RoctracerProfilerPimpl::activityCallback(
    ...
    while (record != endRecord) {
        ...
        bool isGraph =
            pImpl->CorrIdToIsHipGraph.contain(record->correlation_id);

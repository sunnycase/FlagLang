...
void CuptiProfiler::CuptiProfilerPimpl::completeBuffer(CUcontext ctx,
    ...
    do {
        status =
            cupti::activityGetNextRecord<false>(buffer, validSize, &activity);
        if (status == CUPTI_SUCCESS) {
            ...
            throw std::runtime_error(
                "[PROTON] cupti::activityGetNextRecord failed");
        }
    } while (true);
...
void CuptiProfiler::CuptiProfilerPimpl::callbackFn(void *userData,
    ...
    if (domain == CUPTI_CB_DOMAIN_RESOURCE) {
        ...
        if (cbId == CUPTI_CBID_RESOURCE_MODULE_LOADED) {
            ...
            auto *graphData = static_cast<CUpti_GraphData *>(
                resourceData->resourceDescriptor);
            uint32_t graphId = 0;
        ...
        if (callbackData->callbackSite == CUPTI_API_ENTER) {
            ...
            if (cbId == CUPTI_DRIVER_TRACE_CBID_cuGraphLaunch ||
                ...
                if (!findGraph)
                    std::cerr
                        << "[PROTON] Cannot find graph for graphExecId: "
                        << graphExecId
                        << ", and t may cause memory leak. To avoid this "
                           "problem, "
...
void CuptiProfiler::CuptiProfilerPimpl::doStart() {
    ...
    if (profiler.isPCSamplingEnabled()) {
        ...
        // Continuous PC sampling is not compatible with concurrent kernel
        // profiling
...
void CuptiProfiler::CuptiProfilerPimpl::doFlush() {
    ...
    // contexts/streams.
    // This is a blocking call but it doesn’t issue any CUDA synchronization
    // calls implicitly thus it’s not guaranteed that all activities are
    // completed on the underlying devices. We do an "opportunistic"
    // synchronization here to try to ensure that all activities are completed
    // on the current context. If the current context is not set, we don't do
    // any synchronization.

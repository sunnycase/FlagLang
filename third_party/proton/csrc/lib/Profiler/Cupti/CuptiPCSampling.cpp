...
getSassToSourceCorrelation(const char *functionName, uint64_t pcOffset,
    ...
    // Get source can fail if the line mapping is not available in the cubin so
    ...
    auto fileNameStr = sassToSourceParams.fileName
                           ...
                           : "";
    auto dirNameStr = sassToSourceParams.dirName
                          ? std::string(sassToSourceParams.dirName)
                          : "";
...
size_t matchStallReasonsToIndices(
    ...
    for (size_t i = 0; i < numStallReasons; i++) {
        ...
        for (size_t j = 0; j < PCSamplingMetric::PCSamplingMetricKind::Count;
             j++) {
...
CUpti_PCSamplingData allocPCSamplingData(size_t collectNumPCs,
    ...
    if ((libVersion < CUPTI_CUDA12_4_VERSION &&
        ...
        throw std::runtime_error(
            ...
            " TRITON_CUPTI_INCLUDE_PATH and TRITON_CUPTI_LIB_PATH to resolve "
            "the "
            "problem.");
    }
        ...
        for (size_t i = 0; i < pcSamplingData->totalNumPcs; ++i) {
            ...
            if (cubinData->lineInfo.find(key) == cubinData->lineInfo.end()) {
                auto [lineNumber, fileName, dirName] =
                    getSassToSourceCorrelation(
                        pcData->functionName, pcData->pcOffset,
                        cubinData->cubin, cubinData->cubinSize);
                cubinData->lineInfo.try_emplace(
                    key, lineNumber, std::string(pcData->functionName), dirName,
                    fileName);
            }
            ...
            for (size_t j = 0; j < pcData->stallReasonCount; ++j) {
                ...
                if (!configureData->stallReasonIndexToMetricIndex.count(
                    ...
                    throw std::runtime_error(
                        "[PROTON] Invalid stall reason index");
                for (auto *data : dataSet) {
                    ...
                    if (lineInfo.fileName.size())
                        scopeId = data->addOp(
                            scopeId, lineInfo.dirName + "/" +
                                         lineInfo.fileName + ":" +
                                         std::to_string(lineInfo.lineNumber) +
                                         "@" + lineInfo.functionName);
                    auto metricKind =
                        static_cast<PCSamplingMetric::PCSamplingMetricKind>(
                    ...
                    auto stalledSamples =
                        configureData->notIssuedStallReasonIndices.count(
                            ...
                            ? 0
                            : samples;
                    auto metric = std::make_shared<PCSamplingMetric>(

    ...
    if (scopeIdIt == scopeIdToContextId.end()) {
        ...
        if (!currentContexts.empty())
            std::merge(currentContexts.begin(), currentContexts.end(),
                       ...
                       currentContexts.begin());
        scopeIdToContextId[scopeId] = tree->addNode(currentContexts);
    } else {
        // Add a new context under it and update the context
        scopeId = Scope::getNewScopeId();
        scopeIdToContextId[scopeId] =
            tree->addNode(contexts, scopeIdIt->second);
    }
    return scopeId;
}
...
void TreeData::addMetrics(
    ...
    for (auto [metricName, metricValue] : metrics) {
        if (node.flexibleMetrics.find(metricName) ==
            node.flexibleMetrics.end()) {
...
void TreeData::dumpHatchet(std::ostream &os) const {
    ...
    this->tree->template walk<Tree::WalkPolicy::PreOrder>(
        [&](Tree::TreeNode &treeNode) {
            ...
            (*jsonNode)["frame"] = {{"name", contextName},
                                    {"type", "function"}};
            ...
            for (auto [metricKind, metric] : treeNode.metrics) {
                if (metricKind == MetricKind::Kernel) {
                    ...
                    std::string deviceTypeName = getDeviceTypeString(
                        static_cast<DeviceType>(deviceType));
                    (*jsonNode)["metrics"][kernelMetric->getValueName(
                        KernelMetric::Duration)] = duration;
                    (*jsonNode)["metrics"][kernelMetric->getValueName(
                        KernelMetric::Invocations)] = invocations;
                    (*jsonNode)["metrics"][kernelMetric->getValueName(
                        KernelMetric::DeviceId)] = std::to_string(deviceId);
                    (*jsonNode)["metrics"][kernelMetric->getValueName(
                        KernelMetric::DeviceType)] = deviceTypeName;
                    ...
                    auto cycleMetric =
                        std::dynamic_pointer_cast<CycleMetric>(metric);
                    uint64_t duration = std::get<uint64_t>(
                    ...
                    (*jsonNode)["metrics"][cycleMetric->getValueName(
                        CycleMetric::Duration)] = duration;
                    (*jsonNode)["metrics"][cycleMetric->getValueName(
                        CycleMetric::NormalizedDuration)] = normalizedDuration;
                    (*jsonNode)["metrics"][cycleMetric->getValueName(
                        CycleMetric::DeviceId)] = std::to_string(deviceId);
                    (*jsonNode)["metrics"][cycleMetric->getValueName(
                        CycleMetric::DeviceType)] = std::to_string(deviceType);
                    deviceIds.insert({deviceType, {deviceId}});
    ...
    for (auto [deviceType, deviceIds] : deviceIds) {
        ...
        for (auto deviceId : deviceIds) {
            Device device =
                getDevice(static_cast<DeviceType>(deviceType), deviceId);

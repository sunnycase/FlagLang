...
class GPUProfiler : public Profiler,
  ...
  protected:
    ...
    struct Correlation {
        ...
        void pushExternId(size_t externId) {
            externIdQueue.push_back(externId);
        }
    ...
    // Use the pimpl idiom to hide the implementation details. This lets us
    // avoid including the cupti header from this header. The cupti header and
    // the equivalent header from AMD define conflicting macros, so we want to
    // use those headers only within cpp files.

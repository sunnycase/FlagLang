  ...
  private:
    Session(size_t id, const std::string &path, Profiler *profiler,
        ...
        : id(id),
          path(path),
          profiler(profiler),
          contextSource(std::move(contextSource)),
          data(std::move(data)) {}

    template <typename T> std::vector<T *> getInterfaces() {
        std::vector<T *> interfaces;
        // There's an implicit order between contextSource and profiler/data.
        // The latter two rely on the contextSource to obtain the context, so we
        // need to add the contextSource first.
  ...
  private:
    ...
    void updateInterfaceCount(size_t sessionId, Counter &interfaceCounts) {
        ...
        for (auto *interface : interfaces) {
            auto it =
                std::find_if(interfaceCounts.begin(), interfaceCounts.end(),
                             [interface](const auto &pair) {
    ...
    void registerInterface(size_t sessionId, Counter &interfaceCounts) {
        updateInterfaceCount<Interface, Counter, true>(sessionId,
                                                       interfaceCounts);
    }

    template <typename Interface, typename Counter>
    void unregisterInterface(size_t sessionId, Counter &interfaceCounts) {
        updateInterfaceCount<Interface, Counter, false>(sessionId,
                                                        interfaceCounts);
    }
    ...
    void executeInterface(Counter &interfaceCounts, FnT &&fn,
        ...
        if (isReversed) {
            for (auto it = interfaceCounts.rbegin();
                 it != interfaceCounts.rend(); ++it) {
                process(*it);
            }
        } else {

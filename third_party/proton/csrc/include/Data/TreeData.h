...
class TreeData : public Data {
  ...
  private:
    ...
    // `tree` and `scopeIdToContextId` can be accessed by both the user thread
    // and the background threads concurrently, so methods that access them
    // should be protected by a (shared) mutex.
    class Tree;

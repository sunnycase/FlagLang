...
template <typename ExternLib> class Dispatch {
  public:
        ...
        if (*lib == nullptr) {
            ...
            if (dir.length() > 0) {
                ...
                *lib = dlopen(fullPath.c_str(), RTLD_LOCAL | RTLD_LAZY);
                ...
                // Only if the default path is not set, we try to load it from
                // the system. First reuse the existing handle
    ...
    static void check(typename ExternLib::RetType ret,
    ...
    exec(FnT &handler, const char *functionName, Args... args) {
        ...
        if (handler == nullptr) {
            handler =
                reinterpret_cast<FnT>(dlsym(ExternLib::lib, functionName));
            if (handler == nullptr) {

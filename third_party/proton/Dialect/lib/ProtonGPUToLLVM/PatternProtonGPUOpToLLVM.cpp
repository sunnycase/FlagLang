...
struct ReadCounterOpConversion
    ...
    explicit ReadCounterOpConversion(
        ...
        : mlir::ConvertOpToLLVMPattern<
              mlir::triton::proton::gpu::ReadCounterOp>(typeConverter, benefit),
...
struct FinalizeOpConversion
    ...
    matchAndRewrite(mlir::triton::proton::gpu::FinalizeOp op, OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        auto loc = op.getLoc();
        auto segmentObj = LLVM::SegmentObject::fromStruct(
            loc, adaptor.getSegment(), rewriter);
        ...
        const int circularHeaderWordSize =
            proton::gpu::getCircularHeaderSize() / 4;

        // Header: preamble (1 word), threadblock id (1 word), SM id (1 word),
        // buffer size (1 word)

        // Circular strategy memory layout (total: allocprofileScratchSize
        // bytes)
        //  +-----------------------------------------------+
        ...
        auto scratchPtrTy =
            mlir::cast<LLVM::LLVMPointerType>(scratchPtr.getType());

        // Add the `warp_index` section.
        Value warpIndexOffset =
            b.add(warpId, b.i32_val(circularHeaderWordSize));
        ...
        auto copyWord = [&](Value bufOffset, Value gmemOffset,
                            Attribute memSpace) {
            // Load the value from buffer
            Value ptr =
                b.gep(bufferBaseType, i32_ty, segmentObj.base, bufOffset);
            Value load;
        ...
        Value gmemPidPtr =
            b.gep(scratchPtrTy, i32_ty, scratchPtr, gmemPidOffset);
        ...
        Value gmemHwidPtr =
            b.gep(scratchPtrTy, i32_ty, scratchPtr, gmemHwidOffset);
        ...
        rewriter.create<cf::CondBranchOp>(loc, isFirstThread, ifBlock,
                                          thenBlock);
...
struct SegmentAllocOpConversion
    ...
    explicit SegmentAllocOpConversion(
        ...
        const proton::gpu::TargetInfoBase &targetInfo, PatternBenefit benefit)
        : mlir::ConvertOpToLLVMPattern<
              mlir::triton::proton::gpu::SegmentAllocOp>(typeConverter,
                                                         benefit),
          targetInfo(targetInfo) {}
    ...
    matchAndRewrite(mlir::triton::proton::gpu::SegmentAllocOp op,
        ...
        // Specialize the segment base address calculation might bring a few
        // cycles saving per record measurement overhead.
        Value segmentBase;
        if (isAllIds) {
            if (granularity == proton::gpu::Granularity::WARP)
                segmentBase = allWarpSegmentAlloc(b, curWarpId, numWarps,
                                                  bufferSizeInBytes);
            else
                llvm::report_fatal_error(
        ...
        auto segmentObj =
            LLVM::SegmentObject(bufferBase, segmentBase, indexPtr);
...
struct GlobalScratchAllocOpConversion
    ...
    matchAndRewrite(proton::gpu::GlobalScratchAllocOp op, OpAdaptor adaptor,
        ...
        auto gmemBase = funcOp.getArgument(funcOp.getNumArguments() +
                                           kProfileScratchBufferOffset);
        auto allocSizeAttr =
            mod.getOperation()->getAttrOfType<mlir::IntegerAttr>(
...
struct InitCtxOpConversion
    ...
    matchAndRewrite(mlir::triton::proton::gpu::InitCtxOp op, OpAdaptor adaptor,
        ...
        // InitCtxOp can only be called in the master warps, so using
        // `getThreadId` is fine.
        Value threadId = getThreadId(rewriter, loc);
        Value isFirstThread = b.icmp_eq(threadId, b.i32_val(0));
        const int circularHeaderWordSize =
...
struct RestoreCtxOpConversion
    ...
    explicit RestoreCtxOpConversion(
        LLVMTypeConverter &typeConverter,
        const proton::gpu::TargetInfoBase &targetInfo, PatternBenefit benefit)
    ...
    LogicalResult
    matchAndRewrite(mlir::triton::proton::gpu::RestoreCtxOp op,
                    OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        auto loc = op.getLoc();
        auto segmentObj = LLVM::SegmentObject::fromStruct(
        ...
        // We need to use the absolute warp id in case warp specialization is
        // used.
...
struct SaveCtxOpConversion
    ...
    matchAndRewrite(mlir::triton::proton::gpu::SaveCtxOp op, OpAdaptor adaptor,
        ...
        Block *thenBlock = rewriter.splitBlock(ifBlock, op->getIterator());
        rewriter.setInsertionPointToEnd(prevBlock);
        rewriter.create<cf::CondBranchOp>(loc, isWarpMaster, ifBlock,
                                          thenBlock);
        rewriter.setInsertionPointToEnd(ifBlock);
        rewriter.create<cf::BranchOp>(loc, thenBlock);
...
Type convertProtonGPUSegmentType(SegmentType type,
    ...
    return LLVM::SegmentObject::getStructType(
        type.getContext(), memorySpace, targetInfo.getIndexPtrAddrSpace());
}

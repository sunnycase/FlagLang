...
LogicalResult replaceProtonRecordOp(OpBuilder &builder, FuncOp func,
    ...
    func->walk([&](triton::gpu::WarpSpecializePartitionsOp partitions) {
        ...
        for (auto &partition : partitions.getPartitionRegions()) {
            if (hasOperator<Region, proton::RecordOp>(&partition)) {
                ...
                builder.create<gpu::RestoreCtxOp>(loc, newSegment,
                                                  profileMemArg);
                ...
                partition.walk([&](proton::RecordOp record) {
                    ...
                    Value counter = builder.create<gpu::ReadCounterOp>(
                        loc, clkType, metricType);
                    int scopeId = scopeInfo.getOpScopeId(record);
                    builder.create<gpu::CircularStoreOp>(
                        loc, newSegment, counter, record.getIsStart(), scopeId);
                    record.erase();
                });
                ...
                partition.walk([&](triton::gpu::WarpReturnOp ret) {
                    ...
                    builder.create<gpu::SaveCtxOp>(loc, newSegment,
                                                   profileMemArg);
                });
            }
        }
    ...
    // Replace all proton::RecordOp in the master warps. For the master warps,
    // we don't need to restore warp-level context and we save the context in
    // the end of kernel (right before FinalizeOp).
...
int getAllocSharedMemSize(int maxSharedMemSize, int sharedMemUsed,
    ...
    if (sharedMemUsed >= maxSharedMemSize) {
        // We just assume there's enough shared memory and error out if not
        // during execution.
...
class ConvertProtonToProtonGPUPass
  ...
  public:
    ConvertProtonToProtonGPUPass(
        ...
        int32_t bufferSize, int32_t maxSharedMemSize,
    ...
    LogicalResult circularRecordStrategyLowering(FuncOp func) {
        ...
        if (!samplingOptions.empty() &&
            ...
            if (segmentNum && granularity != gpu::Granularity::WARP) {
                mlir::emitError(
                    loc,
                    "only warp granularity supports selective ids for now.");
                return failure();
            }
        }
        ...
        if (bufferType == gpu::BufferType::SHARED) {
            if (bufferSize > 0)
                allocBufferSize =
                    std::min(allocSharedMemSize, bufferSize.getValue());
            else
        ...
        if (allocBufferSize <= 0) {
            mlir::emitError(loc,
                            "profiling buffer size should be greater than 0");
            return failure();
        }
        ...
        if (bufferType == gpu::BufferType::SHARED) {
            ...
            buffer = builder.create<triton::gpu::LocalAllocOp>(
                loc, sharedBufferType);
        } else if (bufferType == gpu::BufferType::GLOBAL) {
        ...
        ModuleScopeIdAllocation &scopeInfo =
            getAnalysis<ModuleScopeIdAllocation>();
    ...
    void runOnOperation() override {
        ...
        if (metricType != MetricType::CYCLE) {
            mlir::emitError(loc,
                            "only CYCLE metric type is supported currently");
            signalPassFailure();
            return;
        }
        ...
        if (bufferStrategy == gpu::BufferStrategy::CIRCULAR) {
            if (failed(circularRecordStrategyLowering(func))) {
                // No need to call signalPassFailure() here as it's already
                // called in circularRecordStrategyLowering
                signalPassFailure();
            }
        } else {
            mlir::emitError(

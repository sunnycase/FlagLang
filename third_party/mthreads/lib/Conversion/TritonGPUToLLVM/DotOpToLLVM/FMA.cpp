            ...
            for (unsigned n = 0; n < N; n += nShapePerCTATile)
                for (unsigned mm = 0; mm < mSizePerThread; ++mm)
                    for (unsigned nn = 0; nn < nSizePerThread; ++nn) {
                        ...
                        int z =
                            isCRow
                                ? mIdx * N / nShapePerCTATile * mSizePerThread +
                                      nIdx
                                : nIdx * M / mShapePerCTATile * nSizePerThread +
                                      mIdx;
                        // FIXME: ph1 support fp16 and bf16 fma, no need to
                        // convert
                        Value a =
                            extendfp16Andbf16(loc, has[{m + mm, k}], rewriter);
                        Value b =
                            extendfp16Andbf16(loc, hbs[{n + nn, k}], rewriter);

...
struct ReduceOpConversion
  ...
  private:
    ...
    void accumulate(ConversionPatternRewriter &rewriter, Region &combineOp,
        ...
        auto returnOp =
            dyn_cast<triton::ReduceReturnOp>(newReduce.getTerminator());
    ...
    getMultiDimWarpId(ReduceOpHelper &helper, Value &warpId, Location &loc,
        ...
        if (auto sliceLayout = mlir::dyn_cast<SliceEncodingAttr>(srcLayout)) {
            ...
            multiDimWarpId = delinearize(rewriter, loc, warpId,
                                         parentWarpsPerCTA, parentOrder);
            multiDimWarpId.erase(multiDimWarpId.begin() + sliceLayout.getDim());
        } else {
            SmallVector<unsigned> warpsPerCTA =
                triton::gpu::getWarpsPerCTA(srcLayout);
            warpsPerCTA[helper.getAxis()] =
    ...
    accumulatePartialReductions(ReduceOpHelper &helper,
        ...
        for (unsigned round = 0; round < elemsPerThread; ++round) {
            ...
            for (unsigned i = 0; i < op.getNumOperands(); ++i) {
                ...
                acc[i] =
                    targetInfo.loadShared(rewriter, loc, getTypeConverter(),
            ...
            Value laneIdModSizeInterWarps =
                urem(laneId, i32_val(sizeInterWarps));
                ...
                for (size_t j = 0; j < resultElems; ++j) {
                    ...
                    for (size_t resultIdx = 0, resultDim = resultShape.size();
                        ...
                        if (resultCTATile[resultIdx] > smemShape[smemIdx] ||
                            ...
                            readIdx[smemIdx] = urem(
                                readIdx[smemIdx], i32_val(smemShape[smemIdx]));
                        }
                    }
                    Value readOffset =

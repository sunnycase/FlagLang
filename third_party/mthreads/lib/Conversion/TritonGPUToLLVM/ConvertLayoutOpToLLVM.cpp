  ...
  public:
    ...
    matchAndRewrite(triton::gpu::LocalLoadOp op, OpAdaptor adaptor,
        ...
        if (isa<DotOperandEncodingAttr>(dstLayout) &&
            ...
            return lowerSharedToDotOpFMA(op, adaptor, getTypeConverter(),
                                         rewriter);
        }
        return failure();
    }
...
struct ConvertLayoutOpConversion
  ...
  private:
    bool isSupported(Attribute srcLayout, Attribute dstLayout) const {
        return isaDistributedLayout(srcLayout) &&
               isaDistributedLayout(dstLayout) && !isLayoutMmaV1(srcLayout) &&
               !isLayoutMmaV1(dstLayout);
    }
    ...
    void processReplica(Location loc, ConversionPatternRewriter &rewriter,
        ...
        for (unsigned ctaId = 0; ctaId < accumNumCTAsEachRep; ++ctaId) {
            ...
            for (const auto &it : llvm::enumerate(multiDimCTAInRepId)) {
                auto d = it.index();
                multiDimCTAId[d] =
                    multiDimRepId[d] * numCTAsEachRep[d] + it.value();
            }
            ...
            for (unsigned elemId = 0; elemId < accumSizePerThread;
                 elemId += vec) {
                SmallVector<Value> multiDimOffset = getMultiDimOffset(
                    layout, loc, rewriter, targetInfo, elemId, type,
                    multiDimCTAInRepId, shapePerCTATile);
                SmallVector<Value> multiDimOffsetWrapped =
                    getWrappedMultiDimOffset(rewriter, loc, multiDimOffset,
                ...
                if (stNotRd) {
                    ...
                    for (unsigned v = 0; v < vec; ++v) {
                        ...
                        valVec =
                            insert_element(vecTy, valVec, currVal, i32_val(v));
                    }
                    store(valVec, ptr);
                } else {
                    ...
                    for (unsigned v = 0; v < vec; ++v) {
                        Value currVal =
                            extract_element(llvmElemTy, valVec, i32_val(v));
                        if (isInt1)
                            currVal = icmp_ne(
                                currVal,
    ...
    lowerDistributedToDistributed(triton::gpu::ConvertLayoutOp op,
        ...
        if (product(srcTy.getShape()) == 1) {
            ...
            SmallVector<Value> outVals(getTotalElemsPerThread(dstTy),
                                       inVals[0]);
            Value result =
                packLLElements(loc, typeConverter, outVals, rewriter, dstTy);
            rewriter.replaceOp(op, result);
        ...
        for (unsigned repId = 0; repId < accumNumReplicates; ++repId) {
            ...
            if (!successful) {
                processReplica(loc, rewriter, /*stNotRd*/ true, srcTy,
                               ...
                               paddedRepShape, origRepShape, outOrd, vals,
                               smemBase);
            }
            barrier();

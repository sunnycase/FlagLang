...
LinearLayout identityND(StringAttr inDimName, ArrayRef<unsigned> shape,
    ...
    for (int i = 0; i < shape.size(); i++) {
        ...
        int dim = order[i];
        ret *=
            LinearLayout::identity1D(shape[dim], inDimName, outDimNames[dim]);
    }
    return ret;
}
...
LinearLayout shrinkCodomain(const LinearLayout &layout, StringAttr inDimName,
    ...
    // out-dims.  We can easily calculate what happens when we remove the
    // relevant bases from O.  Call this new function O'.
    //
    // Because of linearity, removing the bases from L is equivalent to
    // composing L with O'.  So that's what we do below.
...
LinearLayout ensureLayoutNotLargerThan(
    ...
    // This is just a consequence of how legacy layouts work: We only put the
    // same tensor element into two different blocks as a last resort, only
    // after all the registers in all the lanes in all the warps in a block
    // already have the same tensor element.
    ...
    for (auto outDimName : layout.getOutDimNames()) {
        ...
        // TODO: We claim this is invariant to the order of dims, so can we get
        // rid of llvm::reverse?
...
LinearLayout ampereMmaToLinearLayout(ArrayRef<int64_t> shape,
    ...
    assert(
        (rank == 2 && mma.getInstrShape() == ArrayRef<unsigned>({16, 8})) ||
...
LinearLayout hopperMmaToLinearLayout(ArrayRef<int64_t> shape,
    ...
    ctaLayout *= LinearLayout::identity1D(
        n / ctaLayout.getOutDimSize(S("dim1")), S("register"), S("dim1"));

    // Expand the `warp` dimension according to warpsPerCTA.
    //
    // It's weird that this is order [0,1] when MMAv2's warpsPerCTA is [1,0],
    // but this really does seem to be correct.
...
std::optional<LinearLayout> toLinearLayout(ArrayRef<int64_t> shape,
    ...
    //
    // TODO(jlebar): Once getTotalElemsPerThread uses LLs instead of the
    // existing legacy code, I think we can remove this.

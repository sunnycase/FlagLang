             ...
             [](FuncOp &self) -> void {
                 ...
                 self.walk([&](Operation *op) {
                     if (isa<AdvanceOp>(op) && op->getResult(0).use_empty())
                         outputWarning(op->getLoc(),
                                       "The result of tl.advance is not "
                ...
                std::vector<int64_t> &shape) -> Value {
                 if (auto argType = dyn_cast<RankedTensorType>(arg.getType()))
                     return self.createOrFold<BroadcastOp>(
                         RankedTensorType::get(shape, argType.getElementType()),
                         arg);
                 throw std::invalid_argument(
                 ...
                 if (auto srcTensorType =
                     ...
                     Type dstElemType =
                         cast<PointerType>(srcTensorType.getElementType())
                             .getPointeeType();
                     dstType = RankedTensorType::get(srcTensorType.getShape(),
                                                     dstElemType);
                 } else {
                     auto ptrType =
        ...
        .def("create_assert",
             [](TritonOpBuilder &self, Value &condition,
                ...
                const std::string &funcName, unsigned lineNo) -> void {
                 ...
                 auto fileNameAttr = StringAttr::get(
                     self.getBuilder().getContext(), llvm::StringRef(fileName));
                 auto funcNameAttr = StringAttr::get(
            ...
            if (auto debugOnly =
                ...
                llvm::transform(split, std::back_inserter(debugTypes),
                                [&storage](StringRef str) {
                                    // StringRefs are not always
                                    // null-terminated. The purpose for this
                                    // storage pattern is to produce a
                                    // collection of C-strings that are.
                                    storage.push_back(str.str());
                                    return storage.back().c_str();
                                });

                ::llvm::DebugFlag = true;
                ::llvm::setCurrentDebugTypes(debugTypes.data(),
                                             debugTypes.size());
            }

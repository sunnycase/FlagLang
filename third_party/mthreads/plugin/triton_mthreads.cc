...
PLUGIN_EXPORT void init_triton_mthreads_passes_ttgpuir(py::module &&m) {
    ...
    m.def("add_to_llvmir", [](mlir::PassManager &pm, int32_t capability) {
        pm.addPass(
            mlir::triton::createConvertTritonMTGPUToLLVMPass(capability));
    });
...
PLUGIN_EXPORT void init_triton_mthreads(py::module &&m) {
    ...
    m.def(
        ...
        [](const std::string llvmIR, const std::string opt_option,
           int capability,
           int version) -> std::tuple<std::string, std::string> {
            ...
            auto mubinCode = triton::translateLLVMIRToMUBIN(
                *module, opt_option, capability, version);
            return mubinCode;
        },
        ret::take_ownership);

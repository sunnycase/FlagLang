...
static void convertOpTypes(Operation *op, const TypeConverter &typeConverter) {
    ...
    for (Value &operand : operands) {
        ...
        if (type != operand.getType()) {
            operand = b.create<UnrealizedConversionCastOp>(type, operand)
                          .getResult(0);
        }
    }
    ...
    for (Region &region : op->getRegions()) {
        ...
        for (BlockArgument arg : llvm::to_vector(region.getArguments())) {
            ...
            auto cast =
                b.create<UnrealizedConversionCastOp>(arg.getType(), newArg);
    ...
    for (unsigned i = 0; i != worklist.size(); ++i) {
        ...
        if (auto arg = dyn_cast<BlockArgument>(capture)) {
            if (arg.getOwner() == &func.getBody().front())
                continue;
            // Otherwise, this is some other block argument that cannot be
            // elided.
            return failure();
        }
...
static void elideTrivialCaptures(LLVM::LLVMFuncOp func,
    ...
    // The goal is to completely eliminate captures by hoisting or
    // rematerializing computations. We could minimize captures by
    // rematerializing subcomputations, but that is much more complicated.
    // Prefer rematerializing because that reduces liveranges. If subgraphs are
    // duplicated more than once, we will rely on CSE to clean them up.
    ...
    for (WarpSpecializeOp wsOp : wsOps) {
        ...
        for (auto [i, capture] : llvm::enumerate(wsOp.getExplicitCaptures())) {
            ...
            for (Region *region : wsOp.getPartitionRegions()) {
                ...
                if (!subgraph.empty()) {
                    unsigned resultIdx =
                        cast<OpResult>(capture).getResultNumber();
                    remat =
                        mapping.lookup(subgraph.back())->getResult(resultIdx);
                }
...
static LogicalResult rewriteWarpGroupBarriers(LLVM::LLVMFuncOp func,
    ...
    for (WarpSpecializeOp op : wsOps) {
        for (auto [idx, partition] :
             llvm::enumerate(op.getPartitionRegions())) {
            ...
            unsigned warpGroupSize =
                threadsPerWarp * op.getPartitionNumWarps()[idx];
    ...
    for (Region *partition : ws.getPartitionRegions()) {
        ...
        if (partition->getNumArguments()) {
            ...
            for (auto [i, arg] :
                ...
                Value ptr = b.gep(ptrTy, captureType, capturePtr,
        ...
        partition->walk([&](WarpReturnOp op) {
            ...
            if (auto actRegs = ws.getActualRegisters()) {
                createRegRealloc(
                    b, (*actRegs)[partition->getRegionNumber() + 1], lowRegs);
            }
...
static void disableLICM(LLVM::BrOp latchBr) {
    ...
    auto loopMD = LLVM::LoopAnnotationAttr::get(b.getContext(), {}, {}, {}, {},
                                                {}, licmMD, {}, {}, {}, {}, {},
                                                {}, {}, {}, {});
...
static LogicalResult lowerWarpSpecialize(LLVM::LLVMFuncOp func,
    ...
    if (!totalNumWarpsAttr) {
        return mlir::emitError(
            module.getLoc(), "module missing 'ttg.total-num-warps' attribute");
    }
    ...
    // begin execution.
    auto maxnreg =
        func->getParentOfType<ModuleOp>()->getAttrOfType<IntegerAttr>(
    ...
    if (maxnreg) {
        ...
        // First determine how many extra registers the default warp group can
        // get if the workers surrender the maximum number of registers.
        lowRegs = 24;
        int extraRegs =
            (startRegs - lowRegs) * numWorkerWarps / defaultNumWarps;
        defRegs = (startRegs + extraRegs) / 8 * 8;

        // If the default warp group goes over 256 registers, the workers don't
        // need to give up this much.
        if (defRegs > 256) {
            defRegs = 256;
            int giveRegs =
                (defRegs - startRegs) * defaultNumWarps / numWorkerWarps;
            lowRegs = (startRegs - giveRegs) / 8 * 8;
        }
    }

    // Attempt to elide captures of trivial computations by hoisting them into
    // the header or rematerializing them into each partition.
    ...
    for (auto [ws, stateMap] : llvm::zip(wsOps, warpToState)) {
        ...
        if (ws.getNumOperands()) {
            ...
            Value capturePtr =
                LLVM::getSharedMemoryBase(b.getLoc(), b, targetInfo, ws);
            for (auto [i, arg] :
                 llvm::zip(llvm::seq<int32_t>(ws.getNumOperands()),
                           ws.getOperands())) {
                Value ptr = b.gep(ptrTy, captureType, capturePtr,
                                  ArrayRef<LLVM::GEPArg>{0, i});
        ...
        // First barrier releases the waiting warpgroups. The second barrier
        // ensures they have read the captures before the memory is released
        // upon entry.
        createAllBarrier(b, kSwitchLoopBarrierIdx);
        if (auto actRegs = ws.getActualRegisters())
        ...
        after->getParent()->getBlocks().splice(
            after->getIterator(), ws.getDefaultRegion().getBlocks());

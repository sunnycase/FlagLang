...
struct TensormapFenceproxyAcquireOpConversion
    ...
    matchAndRewrite(ttng::TensormapFenceproxyAcquireOp op, OpAdaptor adaptor,
        ...
        // prepare asm operands
        auto *descAddrOpr =
            ptxBuilder.newAddrOperand(adaptor.getDescPtr(), "l");
        auto *sizeOpr = ptxBuilder.newConstantOperand(TMA_SIZE_BYTES);
        ...
        // We run the fence on a single warp, then use a barrier to synchronize
        // the rest. This ends up being faster than running the fence on each
        // warp.
...
struct TensormapCreateOpConversion
    ...
    matchAndRewrite(ttng::TensormapCreateOp op, OpAdaptor adaptor,
        ...
        tensormap_replace_elemtype(loc, ctx, rewriter, smemBase,
                                   op.getElemType());
        ...
        tensormap_replace_swizzle_mode(loc, ctx, rewriter, smemBase,
                                       op.getSwizzleMode());
        tensormap_replace_fill_mode(loc, ctx, rewriter, smemBase,
                                    op.getFillMode());
        tensormap_cp_fenceproxy(loc, ctx, rewriter, adaptor.getDescPtr(),
                                smemBase);
        rewriter.eraseOp(op);
        return success();
    }
...
struct ReinterpretTensorDescOpConversion
    ...
    matchAndRewrite(ttng::ReinterpretTensorDescOp op, OpAdaptor adaptor,
        ...
        rewriter.replaceOpWithNewOp<LLVM::AddrSpaceCastOp>(
            op, resultType, adaptor.getRawDesc());
        return success();
    }
};
...
void mlir::triton::NVIDIA::populateTMAToLLVMPatterns(
    ...
    patterns.add<TensormapCreateOpConversion>(typeConverter, targetInfo,
                                              benefit);

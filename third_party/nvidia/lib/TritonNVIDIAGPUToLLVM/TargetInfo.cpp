...
void TargetInfo::storeDShared(RewriterBase &rewriter, Location loc, Value ptr,
    ...
    // load/store ops only support v2 and v4.  If the vector width is larger
    // than 4, we have two strategies for dealing with it.
    ...
    if (vec * elemBitwidth > 128) {
        ...
        for (int i = 0; i < vec / maxVec; i++) {
            auto newPtr =
                b.gep(ptr.getType(), elemTy, ptr, b.i32_val(i * maxVec),
                      LLVM::GEPNoWrapFlags::inbounds);
            storeDShared(rewriter, loc, newPtr, ctaId,
                         packLLVector(loc,
                                      ArrayRef(vals).slice(i * maxVec, maxVec),
                                      rewriter),
                         pred);
        }
        return;
    ...
    if (isConstantTruePred(pred)) {
        ...
        if (vec > 1) {
            ...
            for (int i = 0; i < vec; i++) {
                vecVals.push_back(
                    {b.extract_element(val, b.i32_val(i)), constraint});
            }
...
Value TargetInfo::loadDShared(RewriterBase &rewriter, Location loc, Value ptr,
    ...
    if (!isa<VectorType>(loadTy)) {
        SmallVector<Value> values = unpackLLVector(
            loc,
            loadDShared(rewriter, loc, ptr, ctaId, vec_ty(loadTy, 1), pred),
            rewriter);
    ...
    if (elemBitwidth < 8) {
        ...
        SmallVector<Value> vals = unpackLLVector(
            loc, loadDShared(rewriter, loc, ptr, ctaId, int_ty(8), pred),
            rewriter);
        assert(vals.size() == 1);
    ...
    if (!elemTy.isInteger()) {
        ...
        SmallVector<Value> vals = unpackLLVector(
            loc, loadDShared(rewriter, loc, ptr, ctaId, newLoadTy, pred),
            rewriter);
        for (Value &v : vals) {
    ...
    if (isConstantTruePred(pred)) {
        ...
        if (vec > 1) {
            Type structTy =
                struct_ty(SmallVector<Type>(vec, int_ty(elemBitwidth)));
            Value structValue = b.undef(structTy);
            for (int i = 0; i < vec; i++) {
                structValue =
                    b.insert_val(structTy, structValue,
        ...
        std::string elemConstraint =
            "=" + getConstraintForBitwidth(elemBitwidth);
...
bool TargetInfo::warpReduce(RewriterBase &rewriter, Location loc,
    ...
    if (auto kind = matchReduxKind(op, computeCapability, useNanQualifier)) {
        ...
        if (numLaneToReduce == 32) {
            ...
            // Even though we currently don't use redux for partitioned
            // reduction the code below supports it in case we want to tweak the
            // heuristic.
            if (numLaneToReduce < 32) {
                // For partitioned reduction we need to calculate the mask so
                // that each group of numLaneToReduce threads has the correct
                // mask.
            ...
            for (unsigned i = 0; i < acc.size(); ++i) {
                ...
                acc[i] = rewriter.create<NVVM::ReduxOp>(
                    loc, acc[i].getType(), acc[0], *kind, mask, /*abs=*/false,
                    /*nan=*/useNanQualifier);
                if (acc[i].getType().isInteger()) {
                    if (bitwidth < 32)
...
void TargetInfo::printf(RewriterBase &rewriter, Value formatStrStart,
    ...
    if (args.size() >= 1) {
        ...
        for (const auto &entry : llvm::enumerate(newArgs)) {
            ...
            auto fieldPtr = b.gep(ptr_ty(ctx), structTy, allocated,

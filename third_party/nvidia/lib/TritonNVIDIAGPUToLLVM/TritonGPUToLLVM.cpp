  ...
  public:
    explicit TleLLVMConversionTarget(MLIRContext &ctx,
        ...
        addLegalDialect<arith::ArithDialect, LLVM::LLVMDialect,
                        math::MathDialect, NVVM::NVVMDialect>();
        ...
        addDynamicallyLegalOp<tle::DSLRegionOp, tle::YieldOp>(
            [&](Operation *op) -> bool {
                ...
                for (auto &region : op->getRegions()) {
                    hasLegalRegions =
                        hasLegalRegions && typeConverter.isLegal(&region);
                }
...
struct ConvertTritonGPUToLLVM
    ...
    void runOnOperation() override {
        ...
        ModuleAllocation allocation(
            mod,
            mlir::triton::nvidia_gpu::getNvidiaAllocationAnalysisScratchSizeFn(
                targetInfo));
        ...
        // flagtree tle raw
        {
            ...
            mlir::triton::tle::populatePackOpToLLVMPatterns(typeConverter,
                                                            patterns, benefit);
            if (failed(
                    applyPartialConversion(mod, target, std::move(patterns)))) {
                return signalPassFailure();
            }
        }
        ...
        mlir::triton::NVIDIA::populateSPMDOpToLLVMPattern(typeConverter,
                                                          patterns, benefit);
        mlir::triton::populateSPMDOpToLLVMPattern(typeConverter, patterns,
                                                  targetInfo, benefit);
        // TODO(thomas): this should probably be done in a separate step to not
        // interfere with our own lowering of arith ops. Add arith/math's
        // patterns to help convert scalar expression to LLVM.
        ...
        mlir::triton::NVIDIA::populateTCGen5MMAOpToLLVMPattern(
            typeConverter, patterns, benefit);
        mlir::triton::NVIDIA::populateFp4ToFpToLLVMPatterns(typeConverter,
                                                            patterns, benefit);
  ...
  private:
    void initSharedMemory(LLVMTypeConverter &typeConverter) {
        ...
        // shared allocation to allow a larger shared memory size for each
        // kernel.
        //
...
bool NVIDIA::canSkipBarSync(Operation *before, Operation *after) {
    // Multiple init barriers on the same allocation would usually not happen
    // but that allows us to avoid barriers between multiple subslice of an
    // array of mbarriers. This is still correct even if the inits happen on the
    // same allocation.
    ...
    // AsyncTMACopyGlobalToLocalOp read and write to the mbarrier allocation it
    // is valid for them to happen in different order on different threads,
    // therefore we don't need a barrier between those operations.

    ...
    ConverterT converter =
        ...
        for (size_t i = 0; i < numElements; i++)
            inPacked[i / inVecWidth] =
                b.insert_element(inVecTy, inPacked[i / inVecWidth], v[i],
                                 b.i32_val(i % inVecWidth));
...
struct OpToExternCallConversion
    ...
    using Base =
        ElementwiseOpConversionBase<TritonOp,
                                    OpToExternCallConversion<TritonOp>>;
    using Base::Base;
    using Adaptor = typename Base::OpAdaptor;

...
ValueTableV2 getValuesFromDotOperandLayoutStruct(
    ...
    if (largeK) {
        // For layouts with a large K dimension, the original register layout
        // needs to be divided into multiple MMAs, where each MMA has contiguous
        // 32 bits along the K dimension per thread. Using kWidth = 8 and
        // bitwidth = 2 as an example, we split the MMA into 4 sub-MMAs, each
        // with a stride 4 x 32-bit along the K dimension.
        ...
        if (dot.getOpIdx() == 0) {
            ...
            //
            // To derive four independent MMA operations, a stride of 4 is
            // applied to the original register layout:
            //
            ...
            if (kIters <= repK) {
                for (size_t kRep = 0; kRep < kWidth / numElemsPerVec; ++kRep)
                    for (size_t tile = 0; tile < 4; ++tile)
                        for (size_t e = 0; e < numElemsPerVec; ++e) {
                            si.push_back(kRep * numElemsPerVec + tile * kWidth +
                                         e);
                        }
            } else {
                // Suppose kWidth=4 and type=fp32, so numElemsPerVec=1.
                // Each tile of the dot operand layout has a size of 16x32.
                // However, if the triton tensor size is 16x16, elements along
                // the k dimension are duplicated. Within each tile, each
                // register contains 2x8 elements arranged as follows:
                //
                //       tile0/0           tile0/1
                //   |<--kWidth=4-->|   |<--kWidth-->|
                //   |<-mmaWidth=2->|
                ...
                //   [4,  5,  6,  7]    [4,  5,  6,  7]
                //
                // tile0/1 replicates the elements in tile0/0 along the k
                // dimension. For a tensor size of 32x32, the next tile on the m
                ...
                //   4th MMA: [10, 14, 11, 15]
                //
                // Additionally, we should reorder the elements by moving the
                // duplicated elements to the end.  In the example above, we
                // convert the order from tile0/0, tile0/1, tile1/0, tile1/1 to
                // tile0/0, tile1/0, tile0/1, tile1/1, so that only the first
                // two tiles will be used in the computation.
                ...
                for (size_t rep = 0; rep < repMma; ++rep)
                    for (size_t tile = 0; tile < elems.size() / elemsPerTile;
                         ++tile)
                        for (size_t mmaKWidth = 0; mmaKWidth < mmaWidth;
                             ++mmaKWidth)
                            for (size_t kTile = 0; kTile < 2; ++kTile)
                                for (size_t mTile = 0; mTile < 2; ++mTile)
                                    for (size_t e = 0; e < numElemsPerVec;
                                         ++e) {
                                        si.push_back(
                                            rep * mmaWidth * elemsPerMma +
            ...
            //
            // A stride of 4 is applied to derive four independent MMA
            // operations:
            //
            //  1st MMA: [[0, 1], [8, 9]]
            //  2nd MMA: [[2, 3], [10, 11]]
            ...
            if (kIters <= repK) {
                ...
                //   [0, 1, 2, 3]^T, [0, 1, 2, 3]^T
                //
                // Similar to the opIdx=0 situation, we should reorder the
                // elements by moving the duplicated elements to the end.
...
static void callMmaAmpereFp64(PTXBuilder &builder, int b, int m, int n, int k,
    ...
    for (int i = 0; i < numMmaRets / 2; ++i) {
        cArgs1->listAppend(
            builder.newOperand(fc[(m * colsPerThread + 4 * n) / numCPackedElem +
                                  i + batchOffset * b],
                               std::to_string(i)));
        // reuse the output registers
    }
...
static void callMmaV2(PTXBuilder &builder, int b, int m, int n, int k,
    ...
    for (int vk = 0; vk < numVecK; ++vk) {
        ...
        aArgs->listAppend(
            builder.newOperand(ha[{b, m + 1, k + vk}], constraintAB));
    }
    ...
    auto callMma = [&](unsigned b, unsigned m, unsigned n, unsigned k) {
        ...
        if (isTuring) {
            ...
            if (isIntMMA) // Turing int8
                callMmaTuringInt8(builder, b, m, n, k, mma, numMmaRets,
                                  colsPerThread, numCPackedElem, ha, hb, fc);
            else // Turing fp16
                callMmaTuringFp16(builder, b, m, n, k, mma, numMmaRets,
                                  colsPerThread, numCPackedElem, ha, hb, fc,
                                  isAccF16);
        } else { // Ampere and later
            if (isFp64MMA) {
                if (!isHopperF64) {
                    callMmaAmpereFp64(builder, b, m, n, k, mma, numMmaRets,
                                      colsPerThread, numCPackedElem,
                                      batchOffset, ha, hb, fc);
                } else {
                    callMmaV2(builder, b, m, n, k, mma, numMmaRets,
                              colsPerThread, numCPackedElem, batchOffset, ha,
                              hb, fc, "=d", "d", 4);
                }
            } else {
        ...
        Value mmaOut = builder.launch(rewriter, loc,
                                      getMmaRetType(mmaType, op.getContext()));
    ...
    for (int i = 0; i < fc.size(); ++i) {
        for (int j = 0; j < numCPackedElem; ++j) {
            results[i * numCPackedElem + j] =
                numCPackedElem > 1
                    ? tb.bitcast(tb.extract_element(fc[i], tb.i32_val(j)),
                                 resElemTy)
...
LogicalResult convertMMA(triton::DotOp op, triton::DotOp::Adaptor adaptor,
                         ...
                         bool isHopperF64) {
    assert(
        mlir::isa<DotOperandEncodingAttr>(op.getA().getType().getEncoding()) &&
    ...
    return convertDot(typeConverter, rewriter, op.getLoc(), op.getA(),
                      op.getB(), op.getC(), op.getD(), adaptor.getA(),
                      adaptor.getB(), loadedC, op, adaptor, isTuring,
                      isHopperF64);
}

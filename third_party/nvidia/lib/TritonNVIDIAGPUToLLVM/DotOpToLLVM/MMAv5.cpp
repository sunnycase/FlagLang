...
static void createMMACommit(ConversionPatternRewriter &rewriter, Location loc,
    ...
    std::string opcode;
    if (twoCTAs) {
        // .multicast::cluster and mask 0x3 means the completion of UTCMMA.2CTA
        // will be broadcasted into CTAid 0 and 1
        ...
        opcode =
            "@$0 tcgen05.commit.cta_group::1.mbarrier::arrive::one.b64 [$1];";
    }
...
void convertDotImpl(const LLVMTypeConverter &typeConverter,
    ...
    auto *mmaBlock = rewriter.createBlock(
    ...
    if (aInTmem) {
        aLoader = std::make_unique<DotOpMmaV5TmemLoader>(
            a, baseA, aOperandShape, interleaved, transA);
    } else {
    ...
    for (int m = 0; m < numRepM; m++) {
        for (int n = 0; n < numRepN; n++) {
            ...
            for (int k = 0; k < numRepK; k++) {
                ...
                op.createMMAInst(rewriter, loc, accAddress, a, b, elect,
    ...
    for (auto [barrier, barrierPred] : llvm::zip(barriers, barrierPreds)) {
        ...
        auto smemObj = LLVM::getSharedMemoryObjectFromStruct(loc, barrier,
                                                             i64_ty, rewriter);
...
void convertDot(const LLVMTypeConverter &typeConverter,
    ...
    dot.createMMAInst = [&](ConversionPatternRewriter &rewriter, Location loc,
                            MemDescOperand accAddress, MemDescOperand a,
                            Value b, Value pred, Value useInitAcc,
...
void convertScaledDot(const LLVMTypeConverter &typeConverter,
    ...
    dot.createMMAInst = [&](ConversionPatternRewriter &rewriter, Location loc,
        ...
        Value instDescriptor = createScaleInstDescriptor(
            rewriter, op, desc.mmaSizeM, desc.mmaSizeN, desc.transA,
            desc.transB, subWordIdx, subWordIdx, mxfpInstKind);
...
struct TCGen5MMAScaledOpConversion
    ...
    matchAndRewrite(ttng::TCGen5MMAScaledOp op, OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        convertScaledDot(*getTypeConverter(), rewriter, op.getLoc(), op,
                         adaptor);
        rewriter.eraseOp(op);
        return success();
    }

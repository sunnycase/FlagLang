...
LogicalResult lowerLdStMatrix(
    ...
    if (!removeBroadcast.isIdentity()) {
        if (isStore) {
            ...
            return lowerLdStMatrix(loc, newRegLayout, memDescType, vals,
...
struct LocalLoadOpConversion
  ...
  public:
    ...
    matchAndRewrite(triton::gpu::LocalLoadOp op, OpAdaptor adaptor,
        ...
        auto result =
            lowerLdStMatrix(op.getLoc(), regLayout, memDescType, values,
                            smemObj, rewriter, targetInfo, getTypeConverter());
        if (failed(result)) {
            return failure();
        }
        auto structTy = LLVM::LLVMStructType::getLiteral(
            op.getLoc().getContext(),
            SmallVector<Type>(values.size(), llvmElemTy));
        auto value = packLLElements(op.getLoc(), typeConverter, values,
                                    rewriter, structTy);

...
std::optional<LinearLayout> getReps(const LinearLayout &cvt,
    ...
    // inB bases (per in-dim) and keeping the remainder bases unchanged from
    // cvt.
    ...
    for (StringAttr id : cvt.getInDimNames()) {
        ...
        assert(inB <= inA &&
               "tile has more in-bits than cvt for a given in-dim");
        ...
        // 2) Validate no overlap: the remaining cvt bases must have zeros in
        // all
        ...
        // 3) Emit reps bases: first inB as all-zeros; remainder copied from
        // cvt.
        for (int i = 0; i < inB; ++i) {
...
getVec(const LinearLayout &cvt, const LinearLayout &tile, int maxnreg,
    ...
    ColumnAction perm;
    // Heuristic:
    // Do not use more than half the registers as otherwise it's prone to
    // spilling
...
LinearLayout getTileLayout(MLIRContext *ctx, TMemAccessAtom atom, int bitwidth,
    ...
    // Set the output order to be kRow, kCol and the input order to be kReg
    // first
    ...
    // For unpacked, the tile above is for 32-bit elements, so we have to
    // multiply by identity1D(32 / bitwidth, kReg, kCol) to get the correct tile
    // to allow us to divide the cvt layout by it
    if (unpacked) {
...
void createTensorMemoryStore(Location loc, Value address, int colOffset,
    ...
    unsigned numRepeats =
        srcs.size() / (atom.rowsPerThread * atom.colsPerThread);
...
int getContextualMaxNReg(Operation *op) {
    ...
    auto getFromParent = [](Operation *op) -> std::optional<int> {
        ...
        if (auto partitions = dyn_cast<WarpSpecializePartitionsOp>(parent)) {
            ...
            if (auto actRegisters =
                    partitions.getParentOp().getActualRegisters())
    ...
    // instructions based on the static number of registers set on the module,
    // not the dynamic allocation. This just means the register limit used for
    // the purpose of subtiling TMEM messages cannot be higher than the
    // module's.
...
Value createTensorMemoryLoad(Location loc, MLIRContext *ctx, Value address,
    ...
    std::string opcode = "tcgen05.ld.sync.aligned." +
                         std::string(atom.opShape) + ".x" +
                         std::to_string(numRepeats) + packedStr + ".b32 {";
...
lowerTMemLdSt(Location loc, MLIRContext *ctx,
    ...
    if (llvmElemTy.getIntOrFloatBitWidth() < 32) {
        ...
        auto quot =
            LinearLayout(bases, reps.getOutDims(), /*isSurjective=*/false);
    ...
    if (reps.getInDimSize(kWarp) > 4) {
        ...
        tmemBase = b.add(
            tmemBase, b.or_(b.shl(row, b.i32_val(16)), col, /*disjoint*/ true));
    }
    ...
    for (int i = 0; i < reps.getInDimSize(kReg); i += valsPerMessage) {
        ...
        if (isStore) {
            ...
            createTensorMemoryStore(loc, tmemBase, /*colOffset=*/staticOffset,
                                    chunk,
            ...
            resultVals.append(unpackResults(outVals, llvmElemTy, valsPerMessage,
                                            loc, rewriter));
        }
    }
...
lowerTMemLdSt(Location loc, MLIRContext *ctx,
    ...
    if (!removeBroadcastSrc.isIdentity()) {
        ...
        auto outValsOr =
            lowerTMemLdSt(loc, ctx, rewriter, prmtCvt, inVals, llvmElemTy,
    ...
    // Note that it can match one and only one of the layouts, even after
    // register reordering, as the layouts yield predetermined positions for the
    // lanes We store the instruction, the resulting reps layout, the
    // permutation and the number of registers per message
    ...
    if (!msgInfo) {
        ...
        if (maybeReps) {
            ...
            basis[kLane][4] = {0, 0};
            reps =
                LinearLayout(basis, reps.getOutDims(), /*isSurjective=*/false);
...
static FailureOr<SmallVector<Value>> lowerTMemLdStFromTypes(
    ...
    // tmemBase already encodes CTA/block offsets so we just remove them from
    // the cvt
    ...
    auto resultValsOr =
        lowerTMemLdSt(loc, ctx, rewriter, packedLayout, inVals,
                      packedLlvmElemTy, tmemBase, maxnreg, unpacked, pred);
...
struct TensorMemoryLoadOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::TMEMLoadOp op, OpAdaptor adaptor,
        ...
        Value resultStruct = packLLElements(loc, getTypeConverter(),
                                            *resultValsOr, rewriter, structTy);
...
struct TensorMemoryAllocOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::TMEMAllocOp op, OpAdaptor adaptor,
        ...
        int colOffset =
            cast<IntegerAttr>(op->getAttr("tensor_memory_col_offset"))
                .getValue()
                .getZExtValue();
        int rowOffset =
            cast<IntegerAttr>(op->getAttr("tensor_memory_row_offset"))
                .getValue()
                .getZExtValue();
        Value allocAddress =
            b.add(baseInt, b.i32_val(colOffset | rowOffset << 16));
        ...
        if (op.getSrc()) {
            ...
            auto lowered = lowerTMemLdStFromTypes(
                loc, ctx, rewriter, regTy, memTy, ptr, maxnreg, b.i1_val(true),
            ...
            // Emit a barrier to ensure all threads have finished writing to
            // tensor memory before any use of the tensor memory.
            b.barrier();
        }
    ...
    auto createCopy = [&](int repMorN, int repK) {
        for (int i = 0; i < repMorN; ++i) {
            for (int j = 0; j < repK; ++j) {
                // Multiple copies of 32x128b blocks are laid out along M/N
                // first then
                // K
                auto colOffset = b.int_val(32, (j * repMorN + i) * 4);
                auto tmemAddr = b.add(b.ptrtoint(i32_ty, baseDst), colOffset);
                auto blockSize =
                    (32 * 128) / llvmElementTy.getIntOrFloatBitWidth();
                auto linearIdx = (i * repK + j) * blockSize;
                auto smemOffset =
                    applyLinearLayout(loc, rewriter, invLayout,
                                      {{kDim, b.i32_val(linearIdx)}})[0]
                        .second;
                auto smemAddr =
                    b.gep(elemPtrTy, llvmElementTy, baseSrc, smemOffset);
                smemDesc =
    ...
    auto createCopy = [&](int repM, int repN) {
        ...
        for (int m = 0; m < repM; m++) {
            for (int n = 0; n < repN; n++) {
                int colIndx = (n % repNPerBlock) * instShape[1] +
                              m * repNPerBlock * instShape[1] +
                              (n / repNPerBlock) *
                                  (srcTy.getDimSize(0) / instShape[0]) * blockN;
...
struct TensorMemoryCopyOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::TMEMCopyOp op, OpAdaptor adaptor,
        ...
        if (isa<TensorMemoryScalesEncodingAttr>(
            ...
            // Special case for copy of scales as they behave differently from
            // other copies. This can be unified once we fix the smem layout
...
struct TMEMSubSliceOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::TMEMSubSliceOp op, OpAdaptor adaptor,
        ...
        if (blockM == 64) {
            // The layout interleaves blocks along the N dimension with the
            // rows, such that the odd numbered blocks are in lanes [16, 32),
            // below the previous even-numbered block.

...
struct LoadOpConversion : public ConvertOpToLLVMPattern<triton::LoadOp>,
    ...
    matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
        ...
        if (llMask) {
            LLVM_DEBUG(DBGS() << "vec = " << vec << " mask_alignment = "
        ...
        if (vec == 1 && numElems > 1) {
            ...
            op->emitRemark()
                << "Warning: vectorization fails vec = " << vec
        ...
        for (size_t vecStart = 0; vecStart < numElems; vecStart += vec) {
            ...
            if (other) {
                for (size_t ii = 0; ii < nWords; ++ii) {
                    ...
                    PTXInstr &mov = ptxBuilder.create<>("mov")->o(
                        "u" + std::to_string(movWidth));
            ...
            SmallVector<Type> retTys(nWords,
            ...
            for (unsigned int ii = 0; ii < nWords; ++ii) {
                ...
                if (isa<LLVM::LLVMStructType>(retTy)) {
                    curr = b.extract_val(IntegerType::get(getContext(), width),
                                         ret, ii);
                } else {
                    curr = ret;
                }
                curr = b.bitcast(
                    curr,
                    LLVM::getVectorType(valueElemTy, width / valueElemNBits));
                rets.push_back(curr);
            }
            ...
            for (size_t ii = 0; ii < vec; ++ii) {
                ...
                Value loaded =
                    b.extract_element(valueElemTy, rets[ii / tmp], vecIdx);
        ...
        for (size_t vecStart = 0; vecStart < elemsPerThread; vecStart += vec) {
            ...
            for (size_t wordIdx = 0; wordIdx < nWords; ++wordIdx) {
                ...
                for (size_t elemIdx = 0; elemIdx < wordNElems; ++elemIdx) {
                    const size_t elemOffset =
                        vecStart + wordIdx * wordNElems + elemIdx;
                    ...
                    llWord = b.insert_element(wordTy, llWord, elem,
                                              b.i32_val(elemIdx));
                }
    ...
    matchAndRewrite(triton::AtomicCASOp op, OpAdaptor adaptor,
        ...
        for (size_t i = 0; i < elemsPerThread; i += 1) {
            ...
            if (tensorTy) {
                ...
                auto old =
                    ptxBuilderAtomicCAS.launch(rewriter, loc, valueElemTy);
                if (op.getResult().use_empty()) {
                    rewriter.eraseOp(op);
                    return success();
                }
                Value atomPtr = LLVM::getSharedMemoryBase(
                    loc, rewriter, targetInfo, op.getOperation());
                atomPtr = b.bitcast(atomPtr, ptr_ty(ctx, 3));
                // Only threads with mask = True store the result
                PTXBuilder ptxBuilderStore;
                auto *dstOprStore =
                    ptxBuilderStore.newAddrOperand(atomPtr, "r");
    ...
    matchAndRewrite(triton::AtomicRMWOp op, OpAdaptor adaptor,
        ...
        if (tensorTy) {
            ...
            if (!supportsVectorized(atomicRmwAttr, valTy.getElementType())) {
                packed = std::min<unsigned>(
                    vecOrig, valTy.getElementType().isF16() ? 2 : 1);
                vec = 1;
            }
        ...
        for (size_t i = 0; i < elemsPerThread; i += vec * packed) {
            if (auto canonicalStart = getCanonicalIndex(i, regMask);
                ...
                for (auto iVecPack = 0; iVecPack < vec * packed; ++iVecPack) {
                    resultVals[i + iVecPack] =
                        resultVals[canonicalStart + iVecPack];
                }
                continue;
            }
            ...
            Value pred =
                llMask ? maybeAnd(rewriter, loc, threadPred, maskElements[i])
                       : threadPred;

            if (doPTXLDPromotion) {
                Type convertedValueTy = getTypeConverter()->convertType(
                    getElementTypeOrSelf(op.getType()));
                auto loadAcquireOp =
                    rewriter.create<triton::nvgpu::LoadAcquireOp>(
                ...
                targetInfo.storeShared(rewriter, loc, atomPtr, loadAcquireOp,
                                       pred);
                createBarrier(rewriter, loc, numCTAs);
                Value ret = b.load(valueElemTy, atomPtr);
                rewriter.replaceOp(op, {ret});
            ...
            // Let LLVM handle compare+swap loop; branch-based pred should be
            // fine
            if (valueElemTy.isBF16() &&
                getNVIDIAComputeCapability(moduleOp) < 90) {
                ...
                if (packed == 2 && valueElemNBits == 16) {
                    ...
                    auto tmp = b.insert_element(vecTy, b.undef(vecTy), atom,
                                                b.i32_val(0));
                    atom = b.insert_element(vecTy, tmp, atom2, b.i32_val(1))
                               .getResult();
                }
                ...
                if (tensorTy) {
                    ...
                    if (vec > 1) {
                        for (unsigned ii = 0; ii < vec; ++ii) {
                            resultVals[i + ii] =
                                b.extract_val(valueElemTy, ret, ii);
                        }
                    } else if (packed > 1) {
                        for (unsigned ii = 0; ii < packed; ++ii) {
                            resultVals[i + ii] = b.extract_element(
                                valueElemTy, ret, b.i32_val(ii));
                        }
                    } else {
                        resultVals[i] = ret;
                    }
                    ...
                    if (!doesAtomicNeedMEM) {
                        rewriter.create<LLVM::BrOp>(loc, atom, endBlock);
                        rewriter.eraseOp(op);
                        // if type isn't a tensor and there is no need to write
                        // to SMEM then we are done here
                        return success();
                    }

                    // Commit values from predicated block to SMEM and return
                    // from predicate block Note: there is no need to use the
                    // BlockArgument here because
                    //       the value is recovered from SMEM in the !tensorTy
                    //       case
            ...
            std::string tyId = getRegisterSizeCode(valueElemNBits * packed,
                                                   /*is_float=*/false);
            ...
            if (vec > 1) {
                ...
                for (unsigned ii = 0; ii < vec; ++ii) {
                    dstOpr->listAppend(ptxBuilderAtomicRMW.newOperand(
                        "=" + tyId, /*init=*/true));
                }
            } else {
                dstOpr =
            ...
            if (vec > 1) {
                ...
                for (unsigned ii = 0; ii < vec; ++ii) {
                    valOpr->listAppend(ptxBuilderAtomicRMW.newOperand(
                        valElements[i + ii], tyId));
                }
            } else if (packed > 1) {
                Value rmwVal = b.undef(packedTy);
                for (int ii = 0; ii < packed; ++ii) {
                    rmwVal = b.insert_element(
                        packedTy, rmwVal, valElements[i + ii], b.i32_val(ii));
                }
            ...
            auto scope = stringifyMemSyncScope(op.getScope()).str();
            auto &atom =
                ptxBuilderAtomicRMW.create<>("atom")->global().o(scope);
            ...
            if (tensorTy) {
                ...
                atom(dstOpr, ptrOpr, valOpr).maybePredicate(pred);
                auto old =
                    ptxBuilderAtomicRMW.launch(rewriter, loc, valueElemTy);
                if (op.getResult().use_empty()) {
                    rewriter.eraseOp(op);
                    return success();
                }
    ...
    matchAndRewrite(triton::gpu::AsyncCopyGlobalToLocalOp op, OpAdaptor adaptor,
        ...
        if (vecBytes < 4) {
            return emitError(loc,
                             "cp.async does not support transfers smaller than "
        ...
        // NOTE(@peterbell10): We load redundant data on different CTAs, so the
        // data is available in each CTAs respective shared memory. Otherwise,
        // we would need an additional broadcast step to copy the data between
        // CTAs.
        ...
        auto emitCpAsync = [&b, threadPred, ptrTy, hasMask = bool(llMask)](
            ...
            auto nBytes =
                vecTy.getNumElements() * elemTy.getIntOrFloatBitWidth() / 8;
            ...
            if (hasMask) {
                ...
                // op.getMask() and redundantDataMask() into the same predicate,
                // the way it is done for LoadOp.
                auto selectOp =
                    b.select(maskElem, b.i32_val(nBytes), b.i32_val(0));
        ...
        if (!cvt.isTrivialOver({str_attr("block")})) {
            return emitError(
                loc, "cp.async does not support non-trivial block dimension");
        }
        cvt = cvt.sublayout(
        ...
        lowerLdSt(
            ...
            laneId, warpId, rewriter, targetInfo, maxVec, emitCpAsync);
...
struct AsyncTMACopyGlobalToLocalOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::AsyncTMACopyGlobalToLocalOp op,
        ...
        // Select just one thread for the TMA copy. This also helps the compiler
        // to figure out that the op is uniform.
        ...
        auto msgToOffset =
            getMsgToUnpackedOffsetLayout(msgToPackedOffset, smemTy);
        ...
        for (int copyIdx = 0; copyIdx < numCopies; copyIdx += numWarps) {
            ...
            Value boxPred = b.and_(
                pred, b.icmp_ult(id, b.i32_val(numWarpsToCopy * warpSize)));
            ...
            std::string tmaInst = "@$0 cp.async.bulk.tensor." +
                                  std::to_string(rank) +
                                  "d.shared::cluster.global.mbarrier::complete_"
                                  "tx::bytes [$1], [$2, {";

            auto offsets =
                applyLinearLayout(loc, rewriter, msgToOffset,
            ...
            for (int i = 0; i < rank; i++) {
                ...
                if (i < offsets.size())
                    coord =
                        b.add(coord, offsets[offsets.size() - i - 1].second);
...
struct AsyncTMACopyLocalToGlobalOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::AsyncTMACopyLocalToGlobalOp op,
        ...
        return convertTMAStoreLikeOp(op, typeConverter, rewriter,
                                     adaptor.getDesc(), op.getSrc().getType(),
...
struct AsyncTMAReduceOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::AsyncTMAReduceOp op, OpAdaptor adaptor,
        ...
        tmaInst << "d.global.shared::cta." << kind.str()
                << ".bulk_group [$1, {";
        int operandIdx = 2;
        for (int i = 0; i < rank; i++) {
            tmaInst << "$" << operandIdx++;
...
static LogicalResult iterateGatherScatterIndices(
    ...
    // Each warp can issue a distinct `gather4` instruction that loads 4 rows
    // into consecutive shared memory. Thus, the layout of the x offsets must be
    // such that 4 consecutive elements are broadcasted to a warp.
    ...
    // each other in shared memory, which lines up with how `gather4` loads
    // data.
    ...
    // swizzle tile size, e.g. [4, 32] vs. [8, 32], then, for example, the
    // address of the 0th element of row 4 will not be at the start of the
    // segment.
    ...
    // thread might also have redundant indices if there is register
    // broadcasting.
    ...
    SmallVector<Value> xOffsets =
        unpackLLElements(loc, xOffsetsValue, rewriter);
    ...
    for (auto regId : seq<unsigned>(0, xOffsets.size(), 4)) {
        ...
        for (auto msgId : llvm::seq(numMessagesPerRow)) {
            ...
            // Because we checked that the memdesc's allocshape and shape match,
            // we can ignore the strides and directly index into the shmem
            // object.
            Value shMemPtr =
                b.gep(elemPtrTy, llvmElemTy, smemBase, shMemOffset);
            Value yOffset = b.add(yOffsetValue, b.i32_val(msgId * msgSize));

            callback(pred, shMemPtr, yOffset,
...
LogicalResult AsyncTMAScatterOpConversion::matchAndRewrite(
    ...
    auto callback = [&](Value pred, Value shMemPtr, Value yOffset,
                        ArrayRef<Value> xOffsets) {
        std::string tmaInst =
            "@$0 cp.async.bulk.tensor.2d.tile::scatter4.global"
...
void mlir::triton::NVIDIA::populateLoadStoreOpToLLVMPatterns(
    ...
    patterns.add<LoadOpConversion, StoreOpConversion>(
        typeConverter, targetInfo, computeCapability, axisInfoAnalysis,
        benefit);

...
Value permute(Location loc, RewriterBase &rewriter, Value a, Value b,
    ...
    auto op = createLLVMIntrinsicCallOp(rewriter, loc, "llvm.nvvm.prmt", i32_ty,
                                        args);
...
LogicalResult lowerLdStMatrix(
    ...
    // We must have at least 32-bits worth of registers to use these
    // instructions
    if (transpose &&
        cvt.getInDimSizeLog2(kReg) < llvm::Log2_32(32 / bitwidth)) {
        return failure();
    }
    ...
    if (!transpose) {
        ...
        // Division does not quite work here. To define this properly, we would
        // need to define a different multiplication that does: A *' B = [[0,
        // A], [B, 0]] and define leftDivision for it We do it ad-hoc for now,
        // as I beleive there's not much demand for this op outside of this
        // lowering.

        // We implement leftDivision as above for B = identity1D(8, kLane,
        // kOffset) Divisibility in the sense above is the same as regular
        // divisibility You need to see that the tile A is a sublayout of the
        // matrix, and that it has zeros above it and to its right.
        ...
        // translated it to checking that the matrix to the left of A is all
        // zeros
        ...
        // Hack: We are not going to use in the rest of the function
        // reps[kLane][2:] so we don't need to zero them out
    ...
    LinearLayout addrLayout = LinearLayout(
        {{kLane, laneBases}, {kWarp, reps.getBases().lookup(kWarp)}},
    ...
    for (int i = 0; i < cvt.getInDimSize(kReg); i += nAdditive) {
        ...
        for (int i2 = 0; i2 < nAdditive; i2 += elemsPerVec * nVecs) {
            ...
            if (isStore) {
                ...
                for (int j = 0; j < nVecs; j++) {
                    ...
                    for (int k = 0; k < elemsPerVec; k++) {
                        input = b.insert_element(
                            vecTy, input, vals[i + i2 + j * elemsPerVec + k],
                            b.i32_val(k));
                    }
                ...
                for (int j = 0; j < nVecs; j++) {
                    Value output =
                        nVecs == 1 ? res : b.extract_val(i32_ty, res, j);
                    output = b.bitcast(output, vec_ty(llvmElemTy, elemsPerVec));
                    for (int k = 0; k < elemsPerVec; k++) {
                        vals.push_back(b.extract_element(llvmElemTy, output,
                                                         b.i32_val(k)));
                    }
                }
            }

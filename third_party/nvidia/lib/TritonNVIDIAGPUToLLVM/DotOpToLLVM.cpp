    ...
    matchAndRewrite(triton::DotOp op, OpAdaptor adaptor,
        ...
        if (!isOuter && mmaLayout &&
            ...
            if (mmaLayout.getVersionMajor() == 2) {
                bool isHopperF64 = computeCapability == 90 &&
                                   cast<RankedTensorType>(A.getType())
                                       .getElementType()
                                       .isF64();
...
struct WarpGroupDotOpConversion
    ...
    matchAndRewrite(triton::nvidia_gpu::WarpGroupDotOp op, OpAdaptor adaptor,
        ...
        return op.emitError("Unsupported WarpGroupDotOp found when converting "
                            "TritonGPU to LLVM.");
    }
};
...
struct WarpGroupDotWaitOpConversion
    ...
    LogicalResult
    matchAndRewrite(triton::nvidia_gpu::WarpGroupDotWaitOp op,
                    OpAdaptor adaptor,
                    ConversionPatternRewriter &rewriter) const override {
        ...
        for (Value input : inputs) {
            for (auto [i, type] : llvm::enumerate(
                ...
                Value value =
                    rewriter.create<LLVM::ExtractValueOp>(loc, input, i);
                packed = rewriter.create<LLVM::InsertValueOp>(
                    loc, packedType, packed, value, outputStructIndex++);
            }
        }
        Value packedOutput = rewriter.create<triton::nvgpu::WGMMAWaitGroupOp>(
            loc, packed, pendings);
        ...
        for (Type type : inputs.getTypes()) {
            ...
            for (auto [i, type] : llvm::enumerate(structType.getBody())) {
                ...
                unpacked = rewriter.create<LLVM::InsertValueOp>(
                    loc, structType, unpacked, value, i);
            }

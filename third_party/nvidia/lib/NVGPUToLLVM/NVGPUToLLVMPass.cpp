...
unpackOperands(const OperandsAndConstraints &operandsAndConstraints,
    ...
    for (const auto &[operand, constraint] : operandsAndConstraints) {
        auto llvmStruct =
            llvm::dyn_cast<LLVM::LLVMStructType>(operand.getType());
        // if a constraint is a number, then we are doing input/output tying
        // if the operand is a struct, then we need to unpack it, and
        // add the constraint to each of the unpacked operands uses the
        // constraint as an offset
        auto isConstraintNumber = isNumber(constraint);
        if (llvmStruct) {
...
getPtxOperands(const OperandsAndConstraints &operandsAndConstraints,
    ...
    for (auto &[operand, constraint] : unpackedOperandsAndConstraints) {
        auto convertedOperand =
            convertToType(operand, constraint, loc, rewriter);
...
class WarpIdOpPattern : public OpRewritePattern<ttn::WarpIdOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ttn::WarpIdOp op,
        ...
        // If this is inside a warp specialize op, compute the relative thread
        // ID within the warp group.
...
class MatrixOpPattern : public OpRewritePattern<MatrixOpType> {
  ...
  protected:
    ...
    virtual std::string getOperands(MatrixOpType op,
  ...
  public:
    ...
    LogicalResult matchAndRewrite(ttn::LoadAcquireOp op,
        ...
        const unsigned valueNBits =
            std::max(8u, valueTy.getIntOrFloatBitWidth());
        ...
        bool init = true;
        auto *dstOpr =
            ptxBuilder.newOperand(writeConstraint, init); // =r operation
        auto *addrOpr =
        ...
        auto &ld =
            ptxBuilder.create<>("ld")
                ...
                .o("acquire",
                   op.getSem() == triton::nvgpu::MemSemantic::ACQUIRE)
                .o("relaxed",
                   op.getSem() == triton::nvgpu::MemSemantic::RELAXED)
                .b(width);
...
class WGMMAOpPattern : public OpRewritePattern<ttn::WGMMAOp> {
  public:
    ...
    std::string getPtxAsm(ttn::WGMMAOp op) const {
        ...
        // Below instructions do support transposing, must pass `trans`
        // arguments
        supported |=
            (eltTypeA == WGMMAEltType::f16) &&
            (eltTypeB == WGMMAEltType::f16) &&
        ...
        if (!supported && !transA && !transB) {
            supported |= (eltTypeA == WGMMAEltType::tf32) &&
                         ...
                         (eltTypeC == WGMMAEltType::f32) &&
                         (m == 64 && 8 <= n && n <= 256 && k == 8);
            supported |= (eltTypeA == WGMMAEltType::e4m3 ||
                          eltTypeA == WGMMAEltType::e5m2) &&
                         (eltTypeB == WGMMAEltType::e4m3 ||
                          eltTypeB == WGMMAEltType::e5m2) &&
                         (eltTypeC == WGMMAEltType::f16 ||
                          eltTypeC == WGMMAEltType::f32) &&
                         (m == 64 && 8 <= n && n <= 256 && k == 32);
            floatTypeWGMMA = supported;
        ...
        // `imm-scale-a`, and `imm-scale-b` are 1 by default only for
        // float-based WGMMA
        if (floatTypeWGMMA)
            args += ", 1, 1";
...
static Value createTMAlloc(IRRewriter &rewriter, LLVM::LLVMFuncOp func,
    ...
    allocOp({ptxBuilder.newOperand(pred, "b"),
             ptxBuilder.newOperand(sharedMem, "r")},
...
void freeTMAlloc(LLVM::LLVMFuncOp func, Value alloc, size_t size, Value pred,
    ...
    func.walk([&](LLVM::ReturnOp ret) {
        ...
        // liveranges.
        std::string ptxString = "@$0 tcgen05.dealloc.cta_group::" +
                                std::to_string(twoCTAs ? 2 : 1) +
                                ".sync.aligned.b32 $1, " +
                                std::to_string(size) + ";";
        auto &dealloc = *ptxBuilder.create<>(ptxString);
        dealloc({ptxBuilder.newOperand(pred, "b"),
                 ptxBuilder.newOperand(alloc, "r")},
                /*onlyAttachMLIRArgs=*/true);
...
static Value initTensorMemory(LLVM::LLVMFuncOp func) {
    ...
    auto b = TritonLLVMOpBuilder(loc, rewriter);
    // A proper error will be raised by the frontend, but to allow compilation
    // to continue we emit a trap.
    if (size > 512) {
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        patterns.add<ClusterCTAIdOpPattern, LoadMatrixOpPattern, WGMMAOpPattern,
                     LoadAcquireOpPattern, WGMMAWaitGroupOpPattern,

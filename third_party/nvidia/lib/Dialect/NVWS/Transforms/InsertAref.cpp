...
SmallVector<ProducedValueInfo> getProducedValues(Operation *op, Block *loopBody,
    ...
    auto partition =
        schedule.getPartition(loopBody->findAncestorOpInBlock(*op));
...
ArefCreateOp createAref(OpBuilder &builder, ProducedValueInfo &producedValue) {
    ...
    auto getSmemDescType = [](Value tensorResult) {
        ...
        if (auto load =
            ...
            memDescType = MemDescType::get(tensorType.getShape(),
    ...
    return createArefCreateOp(builder, {arefBufType}, {alloc->getResult(0)},
                              loc);
}
...
SmallVector<Operation *> createArefPut(PartitionBuilder &builder,
    ...
    if (auto opt = isDescLoadAndAlloc<LocalAllocOp>(result)) {
        ...
        if (auto descOp =
                result.getDefiningOp<triton::DescriptorOpInterface>()) {
            createNVWSDescriptorLoadOp(builder, descOp, dataBuf,
                                       producerPartition, schedule, loc);
            producerKind = AsyncOp::TMALoad;
            staleOps.push_back(descOp);
        } else if (auto loadOp = result.getDefiningOp<triton::LoadOp>()) {
            llvm_unreachable("cpasync not supported yet");
        } else {
            // Create LocalStore of result into dataBuf. This is a value aref,
...
void createArefGet(PartitionBuilder &builder, scf::ForOp loop,
    ...
    auto getEnterOp = builder.createInto<ArefGetEnterOp>(
        *consumerPartition, stageClusterEnter, SmallVector{bufferType},
    ...
    auto consumers =
        getTransitiveConsumers(results, consumerPartition, schedule);
    ...
    auto replaceUsesWithLocalLoad = [&](Value result,
        ...
        if (consumers.size() == 1) {
            // If there is only one consumer and we hit this code path, the
            // empty barrier can be released after local load.
    ...
    for (auto result : results) {
        if (auto localAlloc = result.getDefiningOp<LocalAllocOp>()) {
            ...
            std::string msg =
                "createArefGet: unsupported produced value type: " +
                mlir::debugString(result.getType());
            llvm::report_fatal_error(msg.c_str());
        }
    }
...
bool insertArefs(PartitionBuilder &builder, scf::ForOp loop,
    ...
    if (auto opt = isDescLoadAndAlloc<LocalAllocOp>(producedValue.result)) {
        ...
        processResultUses(alloc.getSrc());
                   ...
                   isDescLoadAndAlloc<TMEMAllocOp>(producedValue.result)) {
        auto alloc = opt->first;
        processResultUses(alloc.getSrc());
    }
  ...
  public:
    void runOnOperation() override {
        ...
        for (scf::ForOp loop : loops) {
            ...
            // To handle cases where desc_load result in registers is used as is
            // in addition to being consumed by local_alloc op, we process
            // local_alloc(desc_load()) first, followed by remaining register
            ...
            for (auto allowDescLoadRegUse : {false, true}) {
                ...
                for (auto op : ops) {
                    ...
                    for (auto producedValue : producedValues) {
                        ...
                        if (insertArefs(builder, loop, *schedule, producedValue,
                                        arefTag))
                            arefTag++;
                    }
                }

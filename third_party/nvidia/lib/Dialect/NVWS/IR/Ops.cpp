...
LogicalResult ArefCreateOp::verify() {
    ...
    for (auto operand : getOperands()) {
        SmallVector<Operation *> users(operand.user_begin(),
                                       operand.user_end());
        if (!llvm::all_of(users, [](Operation *op) {
                return isa<ArefCreateOp, gpu::LocalDeallocOp>(op);
            }))
            return emitError(
                "Aref buffer is used elsewhere, Aref cannot guarantee "
                "async safety");
...
LogicalResult WarpGroupOp::verify() {
    ...
    if (getResults().size() > 0) {
        if (regions.size() == 0) {
            return emitError(
                "Must have at least one region when there are results.");
        }
        if (!isa<nvws::WarpGroupYieldOp>(
                regions.front()->front().getTerminator())) {
            return emitError(
                "When nvws.warp_group op has results, the first region "
                "should be terminated by nvws.warp_group.yield op.");
        }
        auto yieldOp = cast<nvws::WarpGroupYieldOp>(
            regions.front()->front().getTerminator());
        if (getResults().size() != yieldOp.getNumOperands()) {
            return emitError(
                "Mismatch in the number of results returned by nvws.warp_group "
                "op "
...
ParseResult WarpGroupOp::parse(OpAsmParser &p, OperationState &result) {
    ...
    while (succeeded(p.parseOptionalKeyword(
        ...
        if (p.parseKeyword("num_warps") || p.parseLParen() ||
            ...
            p.parseRParen() || p.parseRegion(*result.addRegion()))
            return failure();
    }

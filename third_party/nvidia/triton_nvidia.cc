...
void init_triton_nvidia(py::module &&m) {
    ...
    // TODO: could be done in python if we had a generic interface to set
    // metadata
    m.def("set_nvvm_reflect_ftz", [](llvm::Module *mod) {
        // please check
        // https://llvm.org/docs/NVPTXUsage.html#reflection-parameters this will
        // enable fast math path in libdevice for example, when enable
        // nvvm-reflect-ftz, sqrt.approx.f32 will change to sqrt.approx.ftz.f32
        ...
        .def("matmul",
             [](CublasLtInstance &self, py::object &A, py::object &B,
                py::object &C) {
                 ...
                 checkMatmulConstraints(A_dtype, B_dtype, C_dtype, A_shape,
                                        B_shape, C_shape);
                 ...
                 if (dtype_str == "float8_e4m3fn") {
                     ...
                     // Use FP32 inputs with TF32 compute in cublasLt (set in
                     // compute type)
                     dtype = CUDA_R_32F;

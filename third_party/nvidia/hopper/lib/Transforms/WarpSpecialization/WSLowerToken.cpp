...
Value getMBarrierPhaseBit(OpBuilder &builder, Operation *op,
                          bool emptyBarrier) {
    auto loc = op->getLoc();
    assert(isa<ttnvws::ProducerAcquireOp>(op) ||
           isa<ttnvws::ConsumerWaitOp>(op));
    Value curPhase;
...
void processProducerCommitOp(OpBuilder &builder, ttnvws::ProducerCommitOp op,
    ...
    if (loadType == ttnvws::TokenLoadType::TMALoadOp) {
        // Get the count from the barriers: trace the local_alloc for the
        // barrier then find the count from init_barrier
        arriveOp =
            builder.create<ttng::ArriveBarrierOp>(loc, bufferFull, fullCnt);
    } else {
        assert(false);
    }
...
void lowerTokenOperations(Operation *parentOp, int numCTAs,
    ...
    parentOp->walk([&](ttnvws::CreateTokenOp createTokenOp) {
        ...
        Value bufferEmptyArray =
            builder.create<mlir::triton::gpu::LocalAllocOp>(
                loc, barrierMemDescType, Value());
        ...
        auto extractBufferFull = [&](Location loc, Value idx) -> Value {
            return builder.create<ttg::MemDescIndexOp>(
                loc, singleBarrierMemDescType, bufferFullArray, idx);
        };
        ...
        auto handleOneUser = [&](Operation *user) -> bool {
            ...
            if (auto op = dyn_cast<ttnvws::ProducerAcquireOp>(user)) {
                ...
                assert(user->hasAttr("async_task_id"));
                setAsyncTaskIds(bufferFull.getDefiningOp(),
                                getAsyncTaskIds(user));
        ...
        // Process token users: ProducerAcquireOp, ProducerCommitOp,
        // ConsumerWaitOp, and ConsumerReleaseOp.
        for (OpOperand &use : createTokenOp.getResult().getUses()) {
            ...
            if (auto wsOp = dyn_cast<ttg::WarpSpecializeOp>(user)) {
                ...
                // Handle the regions. Trace uses of the argument corresponding
    ...
    for (auto op : deprecatedTokenOps) {
        ...
        if (auto tokenOp = dyn_cast<ttnvws::CreateTokenOp>(op)) {
            ...
            // eraseArgument.
            for (OpOperand &use :
                 llvm::make_early_inc_range(tokenOp->getUses())) {
                Operation *user = use.getOwner();
                if (auto wsOp = dyn_cast<ttg::WarpSpecializeOp>(user)) {
                    unsigned opndNum = use.getOperandNumber();
                    LDBG("wsOp user numOperands: " << wsOp->getNumOperands()
                                                   << " idx " << opndNum);
                    ...
                    for (Region *region : wsOp.getPartitionRegions()) {
                        ...
                        BlockArgument arg2 = region->addArgument(

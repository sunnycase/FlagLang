...
scf::IfOp rewriteIfOp(scf::IfOp ifOp, SmallVector<Operation *> &taskTopOps,
    ...
    // Create else block since we need to generate accumulated count for then
    // and else.
    ...
    // Go through region ops in the thenBlock. updateAccumLoopCount takes
    // current accumCnt value and returns the value at the end of the thenBlock.
    Value endAccum = updateAccumLoopCount(opList, taskTopOps,
    ...
    if (parentForOp) {
        ...
        getAccumCntsPreOrder(parentForOp.getOperation(), regionsWithChannels,
                             preOrderOpsOfParent);
        parentTCnts =
            getAccumCnts(parentForOp.getOperation(), regionsWithChannels);
    }
    ...
    if (hasDirectChannel) {
        ...
        if (parentForOp) {
            ...
            LDBG("rewrite ifOp: ifOp itself parentArg " << parentArgSize << " "
                                                        << accumArgId);
            // All the accumCnts are at the end of argument list. When
            // accumArgId is parentTCnts - 1, the corresponding accumCnt will be
            // the last argument.
            Value arg = parentForOp.getBody()->getArgument(
                parentArgSize - parentTCnts + accumArgId);
            // Either parent[accumCnt] + 1 or parent[accumCnt].
            Value one = ifBuilder.createWithAsyncTaskIds<arith::ConstantIntOp>(
                loc, 1, 64);
            endAccum =
                ifBuilder.createWithAsyncTaskIds<arith::AddIOp>(loc, arg, one);
            endAccumElse = arg;
        } else {
            endAccum = ifBuilder.createWithAsyncTaskIds<arith::ConstantIntOp>(
                loc, 1, 64);
            endAccumElse =
                elseBuilder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, 0,
                                                                         64);
        }
    ...
    for (auto *op : opList) {
        ...
        // Set up value for thenYield and elseYield for accumCnts nested under
        // "op". Each accumCnt nested under "op", it will have a corresponding
        // argument in this "IfOp". If "op" has tCnts, this "IfOp" will have the
        // same number of corresponding accumCnts, in the same order.
        for (unsigned i = 0; i < tCnts; ++i) {
            ...
            if (parentForOp) {
                ...
                LDBG("rewrite ifOp: elseYield parentArg "
                     << parentArgSize << " " << accumArgId << " " << i);
            } else
                elseVal =
                    elseBuilder.createWithAsyncTaskIds<arith::ConstantIntOp>(
                        loc, 0, 64);
    ...
    for (Operation *op : opList) {
        if (auto forOp = dyn_cast<scf::ForOp>(op)) {
            ...
            if (enclosingAChannel(ifOp.getOperation(), regionsWithChannels)) {
                ...
                prevAccum = newIfOp.getResult(
                    numRes - 1); // accumCnt is the last result.
            } else {
                ...
                for (auto innerFor : innerForOps) {
                    auto newFor = createNewLoopWrapper(
                        innerFor, taskTopOps, regionsWithChannels, prevAccum);
...
scf::ForOp createNewLoopWrapper(scf::ForOp origForOp,
    ...
    for (unsigned i = 0; i < tCnts; ++i) {
        ...
        Value startAccum;
        if (parentForOp)
            startAccum = parentForOp.getBody()->getArgument(pArgSize - pCnts +
                                                            accumArgId + i);
        else {
            ...
            startAccum = builder.createWithAsyncTaskIds<arith::ConstantIntOp>(
                loc, 0, 64);
        }
    ...
    // origForOp is erased in createNewLoop. Make sure taskTopOps is updated
    // with the newForOp.
    ...
    for (auto *op : regionsWithChannels) {
        if (newForOp.getOperation() == op) {
            ...
            // In createNewLoop, yieldOp yields the argument value directly, it
            // is fixed here.
    ...
    for (auto *op : opList) {
        ...
        LDBG("-- numRes, tCnts, accumArgId " << numRes << " " << tCnts << " "
                                             << accumArgId);
        // Each accumCnt nested under "op", it will have a corresponding
        // argument in this "ForOp". If "op" has tCnts, this "ForOp" will have
        // the same number of corresponding accumCnts, in the same order.
        for (unsigned i = 0; i < tCnts; ++i) {
            Value arg = newForOp.getBody()->getArgument(accumArgId);
            Value endAccum = op->getResult(numRes - tCnts + i);
            LLVM_DEBUG({
            ...
            // In createNewLoop, yieldOp yields the argument value directly, it
            // is fixed here. Now, it will yield the accumCnt from the "op".
            yieldOp->replaceUsesOfWith(arg, endAccum);
...
void appendAccumCntsForOps(SmallVector<Operation *> &taskTopOps,
    ...
    // Go through all the regions in opList and correctly add accumCnt.
    // taskTopOps will be updated if it is replaced in the process.

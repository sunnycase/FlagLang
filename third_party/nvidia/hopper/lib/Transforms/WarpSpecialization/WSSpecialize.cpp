            ...
            for (auto user : arg.getUsers()) {
                ...
                if (isa<scf::YieldOp>(user)) {
                    if (auto ifOp = dyn_cast<scf::IfOp>(user->getParentOp())) {
                        // For block arguments, we need to check the initial
                        // value as well.
                        if (auto blockArg = dyn_cast<BlockArgument>(arg)) {
                            auto initArg =
                                nestedForOp
                                    .getInitArgs()[blockArg.getArgNumber() - 1];
                            if (Operation *def = initArg.getDefiningOp()) {
                                ...
                                llvm_unreachable(
                                    "Initial value should have a defining op");
                            }
                        }
                    }
                ...
                if (auto userFor = dyn_cast<scf::ForOp>(user)) {
                    // For block arguments, we need to check the initial value
                    // as well.
                    if (auto blockArg = dyn_cast<BlockArgument>(arg)) {
                        auto initArg =
                            nestedForOp
                        ...
                        if (Operation *def = initArg.getDefiningOp()) {
                            ...
                            // Recursive search the nested loop for the real
                            // users. find corresponding arg of userFor
                            Value userArg;
                            for (auto item :
                                 llvm::enumerate(userFor.getInitArgs())) {
                                if (item.value() == arg) {
                                    userArg =
                                        userFor.getRegionIterArg(item.index());
                                    break;
                                }
                            }
                            if (userArg) {
...
Operation *SpecializeIfOp(scf::IfOp ifOp, IRMapping &mapping,
        ...
        for (Value yieldV : ifOp.thenYield().getOperands()) {
            ...
            if (Operation *def = yieldV.getDefiningOp()) {
                ...
                if (auto forOp = dyn_cast<scf::ForOp>(bbAargOwner)) {
                    // track initial value
                    auto initArg =
                        forOp.getInitArgs()[bbArg.getArgNumber() - 1];
                    if (Operation *def = initArg.getDefiningOp()) {
                        if (hasAsyncTaskId(def, asyncTaskId))
                            keptResultVec.push_back(resultIdx);
                    } else {
    ...
    auto newIfOp = builder.createWithAsyncTaskIds<scf::IfOp>(
        ifOp.getLoc(), newResultTypes, mapping.lookup(ifOp.getCondition()),
        true, ifOp.elseBlock());
    ...
    auto updateYield = [&](scf::YieldOp yield, SmallVector<Value> &operands) {
        ...
        ifBuilder.createWithAsyncTaskIds<scf::YieldOp>(yield.getLoc(),
                                                       operands);
        yield.erase();
    };
    ...
    if (ifOp.elseBlock()) {
        ...
        if (keptResultVec.size() < ifOp->getResultTypes().size()) {
            ...
            for (auto idx : keptResultVec) {
                elseYieldOperands.push_back(
                    newIfOp.elseYield().getOperand(idx));
            }
...
Operation *SpecializeForOp(scf::ForOp forOp, IRMapping &mapping,
    ...
    if (forOp->getAttr("tt.loop_schedule"))
        newForOp->setAttr("tt.loop_schedule",
                          forOp->getAttr("tt.loop_schedule"));
...
Operation *SpecializeOp(Operation *op, IRMapping &mapping,
    ...
    // yieldOp are sometimes implict, meaning they do not necessarily have a
    // task id, but they should be shared by all async tasks.
    ...
    if (op->getNumRegions() == 0) {
        ...
        if (auto ifOp = dyn_cast<scf::IfOp>(op)) {
            ...
            newOp->walk([&](Operation *childOp) {
                setAsyncTaskIds(childOp, asyncTaskId);
            });
            for (unsigned i = 0; i < op->getNumResults(); ++i)
                mapping.map(op->getResult(i), newOp->getResult(i));
            return newOp;
...
void specializeRegion(triton::FuncOp funcOp, unsigned requestedRegisters) {
    ...
    auto wsOp = impB.create<ttg::WarpSpecializeOp>(
        dummyTypes, partitionNumWarps, nTaskIds.size() - 1);

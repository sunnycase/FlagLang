            ...
            for (Value operand : op->getOperands()) {
                ...
                if (!containsAll(defTaskIds, asyncTaskIds)) {
                    ...
                    if (isa<scf::YieldOp, scf::ForOp, scf::IfOp>(op))
                        continue;
                    // Only propagate backward to arithmetic ops (e.g.
                    // constants). Const ops with same value but different task
                    // ids can be folded.
                ...
                if (operand.hasOneUse() &&
                    !containsAll(asyncTaskIds, defTaskIds)) {
                    // YieldOp may lose task attribute during MLIR
                    // canonicalization.
    ...
    void dump() const {
        ...
        for (auto &op : ops) {
            ...
            if (dotPartitionOperand.contains(op)) {
                operand =
                    "operand " + std::to_string(dotPartitionOperand.at(op));
            }
...
static bool rematerializeOp(Operation *op, DataPartitionScheme &partitionScheme,
    ...
    if (isa<LocalAllocOp, arith::ConstantOp>(op)) {
        ...
        assert(existingDim != currentDim &&
               "op has no conflicting partition dim");
    ...
    if (auto op = v.getDefiningOp()) {
        ...
        if (op->hasTrait<OpTrait::Elementwise>() ||
            isa<arith::ConstantOp, arith::ExtSIOp, arith::ExtUIOp,
                ...
                ConvertLayoutOp, triton::gpu::LocalAllocOp, LoadOp, TransOp,
                MemDescTransOp, AtomicRMWOp, triton::AddPtrOp, DescriptorLoadOp,
                nvidia_gpu::TMEMAllocOp, nvidia_gpu::TMEMLoadOp, FpToFpOp>(
                op)) {
            for (Value operand : op->getOperands())
        ...
        if (auto forOp = dyn_cast<scf::ForOp>(bbAargOwner)) {
            ...
            if (!getBackwardSliceToPartition(initArg, partitionScheme,
                                             currentDim))
                return false;
            // track yield value
            auto yieldArg = forOp.getYieldedValues()[bbArg.getArgNumber() - 1];
            if (!getBackwardSliceToPartition(yieldArg, partitionScheme,
                                             currentDim))
                return false;
        }
    }
    ...
    for (Operation *depOp : v.getUsers()) {
        ...
        if (auto dotOp = dyn_cast<nvidia_gpu::WarpGroupDotOp>(depOp)) {
            if ((currentDim == 0 && v == dotOp.getB()) ||
                ...
                // It is fine to continue the partition if the dot output is
                // immediately stored out via an atomic add, as the dot computes
                // a partial result.
                if (onlyUsedByAtomicStore(dotOp.getD())) {
                    partitionScheme.dotPartitionOperand[dotOp] =
                        v == dotOp.getA() ? 0 : 1;
                    // Duplicate the users of the dot output since the shape of
                    // the output will not be changed
                    currentDim = DataPartitionScheme::noOpPartitionDim;
                } else {
                    LLVM_DEBUG({
                        auto opnd = (v == dotOp.getA()) ? "A" : "B";
                        LDBG("skip partitioning along K of " << opnd
                                                             << " of dot\n");
                        dotOp.dump();
                    });
                    return false;
...
static bool getSliceToPartition(Value root,
    ...
    DenseSet<Value> seen;
    bool forwardSuccess = getForwardSliceToPartition(
        root, forwardPartitionScheme, currentDim, seen);
    ...
    for (auto op : forwardPartitionScheme.ops) {
        ...
        } else if (isa<nvidia_gpu::WarpGroupDotOp, nvidia_gpu::TCGen5MMAOp>(
                       op)) {
...
static bool computePartitionScheme(triton::FuncOp &funcOp,
    ...
    for (auto op : dots) {
        if (partitionScheme.isPartitioned(op) ||
            partitionScheme.isSkipped(op)) {
            continue;
        }
        ...
        if (partitionDim.empty()) {
            LDBG("Partition not available: " << sliceSizeM << " "
                                             << sliceSizeN);
            return false;
        }
        ...
        if (partitionScheme.numPartitions == 0) {
            ...
            if (partitionScheme.numPartitions != asyncTaskIds.size()) {
                LDBG("partition not possible, in conflict with previous "
                     "partition\n");
                return false;
            }
        }
        ...
        for (unsigned i = 1; i < opDim.second.size(); i++) {
            ...
            if (auto allocOp = dyn_cast<LocalAllocOp>(oldOp)) {
                ...
                auto slicedMemdescType = MemDescType::get(
                    shape, memdescType.getElementType(),
                ...
                auto viewOp = builder.createWithAsyncTaskIds<MemDescSubsliceOp>(
                    allocOp.getLoc(), slicedMemdescType, allocOp.getResult(),
                    offsets);
                newOp = viewOp;
            ...
            auto dimMatches = [&](OpOperand &operand) {
                ...
                if (isa<TransOp, MemDescTransOp>(user)) {
                    ...
                    userDim = partitionScheme.flipPartitionDim(userDim);
                               ...
                               dyn_cast<nvidia_gpu::WarpGroupDotOp>(user)) {
                    // infer userDim for dot
                    assert(partitionScheme.dotPartitionOperand.contains(user) &&
                           "no operand info");
                    unsigned opndIndx =
                        partitionScheme.dotPartitionOperand[user];
            ...
            oldOp->getResult(0).replaceUsesWithIf(newOp->getResult(0),
                                                  dimMatches);
        }
    }
}
...
static Operation *sliceOp(Operation *op, int offset, IRMapping &mappings,
        ...
        if (!op->getResults().empty()) {
            ...
            if (needRetype) {
                if (auto type = dyn_cast<MemDescType>(v.getType())) {
                    ...
                    shape[dim] = sliceSize;
                    // change encoding for ttng.tensor_memory_encoding to match
                    // gen5.
                    if (auto tmem =
                            dyn_cast<nvidia_gpu::TensorMemoryEncodingAttr>(
                                type.getEncoding())) {
                        Attribute accEncoding =
                            triton::nvidia_gpu::TensorMemoryEncodingAttr::get(
                                builder.getContext(),
                                dim == 0 ? tmem.getBlockM() / 2
                                         : tmem.getBlockM(),
                                dim == 1 ? tmem.getBlockN() / 2
                                         : tmem.getBlockN(),
                                tmem.getUnpacked(), tmem.getCTASplitM(),
                                tmem.getCTASplitN());
                        auto newType = MemDescType::get(
                            shape, type.getElementType(), accEncoding,
    ...
    if ((dim == DataPartitionScheme::noOpPartitionDim) ||
        ...
        for (Value operand : op->getOperands())
            sliceOp(operand, offset, mappings, reverseMappings,
                    partitionScheme);
        ...
        auto tmem =
            cast<nvidia_gpu::TensorMemoryEncodingAttr>(type.getEncoding());
        ...
        builder.setInsertionPoint(op);
        // The source op is already sliced at this point, so srcTy, type, tmem
        // is sliced. We use getTmemCompatibleLayout to get a block layout that
        // is for the sliced tmem here.
        ...
        auto newAccType = RankedTensorType::get(
            shape, oldRetType.getElementType(), newDistributedEncoding);
        auto ld =
        ...
        if (tmemAllocOp.getSrc()) {
            ...
            int sliceSize = shape[dim] / numOfPartitions;
            shape[dim] = sliceSize;
            auto tmem = cast<nvidia_gpu::TensorMemoryEncodingAttr>(
                retType.getEncoding());
            auto accEncoding =
                triton::nvidia_gpu::TensorMemoryEncodingAttr::get(
                    builder.getContext(),
                    ...
                    tmem.getCTASplitN());
            auto newType = MemDescType::get(
                shape, retType.getElementType(), accEncoding,
                retType.getMemorySpace(), retType.getMutableMemory());

            Attribute newDistributedEncoding =
                nvidia_gpu::getTmemCompatibleLayout(accEncoding.getBlockM(),
                                                    accEncoding.getBlockN(),
                                                    srcTy, numWarps);
            auto newAccType =
        ...
        newOp = builder.createWithAsyncTaskIds<arith::ConstantOp>(op->getLoc(),
                                                                  newValAttr);
        // Do not drop original task id as constant folding may lose one
        // constant.
        ...
        newOp = cloneAndSetResultType(op);
    } else if (auto tensorDescOp =
                   dyn_cast<ttng::ReinterpretTensorDescOp>(op)) {
        newOp = cloneAndSetResultType(op);
        ...
        if (dim == 0 && opndIndx == 1 || dim == 1 && opndIndx == 0) {
            ...
            sliceOp(op->getOperand(otherOpndIndx), offset, mappings,
        ...
        sliceOp(accumulator, offset, mappings, reverseMappings,
                partitionScheme);
        newOp = cloneAndSetResultType(op);
        ...
        for (unsigned i = 0; i < forOp.getInitArgs().size(); i++) {
            ...
            Value newInitArg;
            auto newInitArgOp = sliceOp(initArg, offset, mappings,
                                        reverseMappings, partitionScheme);
            ...
            if (newInitArg) {
                ...
                forOp.getBody()->insertArgument(
                    forOp.getBody()->getNumArguments(), newInitArg.getType(),
                    forOp.getLoc());
        ...
        auto newThenYieldOp = sliceOp(thenYieldOp, offset, mappings,
                                      reverseMappings, partitionScheme);
        sliceOp(elseYieldOp, offset, mappings, reverseMappings,
                partitionScheme);
        ...
        newOp->walk([&](Operation *childOp) {
            setAsyncTaskIds(childOp, sliceTaskIds);
        });
    } else {
...
static bool doDeepCleanup(triton::FuncOp &funcOp,
    ...
    do {
        ...
        scf::IfOp::getCanonicalizationPatterns(cleanUpPatterns,
                                               funcOp.getContext());
        if (applyPatternsGreedily(funcOp, std::move(cleanUpPatterns))
                .failed()) {
            return false;
        }

...
std::pair<Value, Value> getOutOfScopeBufferIdxAndPhase(
    ...
    // The accumulation count is one past the last iteration. Subtract one to
    // get the last valid iteration index.
    auto loc = bbAargOwner->getLoc();
    Value one =
        builder.createWithAsyncTaskIds<arith::ConstantIntOp>(loc, 1, 64);
    accumCnt =
        builder.createWithAsyncTaskIds<arith::SubIOp>(loc, accumCnt, one);
    ...
    for (Operation *userOp : root.getUsers()) {
        if (auto yieldOp = dyn_cast<scf::YieldOp>(userOp)) {
            for (OpOperand &operand : yieldOp->getOpOperands()) {
                if (operand.get() == root) {
                    auto result = yieldOp->getParentOp()->getResult(
        ...
        for (auto user : users) {
            ...
            if (consumerTaskIds.size() > 0) {
                if (auto dotOp = dyn_cast<ttng::TCGen5MMAOp>(op)) {
                    // When traversing Gen5MMA, we create channel for the
                    // accumulator.
                    if (auto tmemAllocOp =
                            dyn_cast<ttng::TMEMAllocOp>(producerOp)) {
                        // Always use two buffers for TMEM channels.
                        channels.push_back(
                            std::make_unique<ttng::TmemDataChannel>(
                                producerTaskId, consumerTaskIds, tmemAllocOp,
                                dotOp, userOp, user.second, NUM_TMEM_BUFFERS));
                    }
                } else {
    ...
    funcOp.walk([&](Operation *op) {
        ...
        if (isa<tt::LoadOp, tt::DescriptorLoadOp>(op) ||
            ...
            if (producerTaskIds.empty() || producerTaskIds.size() > 1) {
                LLVM_DEBUG({
                    LDBG(" ignoring load ops without async task id or with "
                         "multiple task "
                         "ids: ");
                    op->dump();
                });
            ...
            if (auto dotOp = dyn_cast<ttng::TCGen5MMAOp>(op)) {
                ...
                createChannel(producerOp, op, dom, channels, false,
                              producerNumBuffers);
    ...
    if (isa<ttg::LocalAllocOp>(consumerOp)) {
        ...
        for (auto user : consumerOp->getUsers()) {
            if (isa<tt::TransOp, ttg::MemDescTransOp>(user)) {
                // TransOp is not a real consumer. It caculates the shared
                // memory address for the real consumer. Continue to find its
                // transitive users recursively.
...
void groupChannels(
    ...
    for (auto &item : producerChannels) {
        ...
        for (auto c : channels) {
            assert(c->numBuffers == numBuffers &&
                   "Unmatched number of buffers");
        }
    }
#endif
    ...
    auto channelCanBeMerged = [](Channel *c1, Channel *c2) -> bool {
        ...
        if (dst1 == dst2)
            return true;
        // We only have one CommChannel for channels in
        // channelsGroupedByConsumers. A CommChannel can have multiple tokens,
        // one for each consumer taskId. Consider the case where channel v is
        // between producer task 0 and consumer task 1, while channel p is
        // between producer task 2 and consumer task 1, but in createToken, we
        // only consider the first channel in the group.
    ...
    for (auto *c0 : channels) {
        ...
        for (auto &kv : consumerChannels) {
            if (kv.second.size() > 0 &&
                channelCanBeMerged(c0, kv.second.front())) {
...
void reorderProducerOps(SmallVector<Channel *> &channels) {
    ...
    for (auto &group : groupedProducerOps) {
        std::sort(group.second.begin(), group.second.end(),
                  [&](Channel *a, Channel *b) {
                      return a->relation.second.size() <
                             b->relation.second.size();
                  });
    }
...
void createToken(
    ...
    for (auto *key : orderedChannels) {
        ...
        for (auto consumerAsyncTaskId : channel->relation.second) {
            ...
            // For channels associated with acc of gen5, consumerOp is not the
            // gen5, it is usually tmem_load.
            bool useGen5Barrier =
                isa<ttng::TCGen5MMAOp>(consumerOp) &&
            ...
            if (useGen5Barrier) {
                ...
                // If the gen5 barrier for this mmaOp is already used for
                // another channel, do not use it for this channel.
                if (gen5Barriers.count(mmaOp) &&
                    gen5Barriers[mmaOp] != channel) {
    ...
    LLVM_DEBUG({
        ...
        for (auto &item : tokenMap) {
            ...
            if (item.second.producerBarrier)
                llvm::dbgs()
                    << "producer barrier: " << *item.second.producerBarrier
                    << "\n";
            for (auto &kv : item.second.consumerBarriers)
                llvm::dbgs() << "consumer barrier: " << kv.first << " "
                             << kv.second << "\n";
        }
    });
}
    ...
    for (auto *channelInOrder : orderedChannels) {
        ...
        if (channel->channelKind == DataChannelKind::TMEM) {
            ...
            if (requireMMASharedEncoding) {
                ...
                sharedLayout = ttg::SwizzledSharedEncodingAttr::get(
                    context, 1, 1, 1, order, CTALayout);
            }

            // Get shape, layout and type of the complete buffer
            SmallVector<int64_t> bufferShape(sliceShape.begin(),
                                             sliceShape.end());
            ...
            Type memdescType = ttg::MemDescType::get(
                bufferShape, elemType, sharedLayout, sharedMemorySpace,
                /*mutableMemory*/ true);
            buffer =
...
desyncTCGen5MMAOp(OpBuilderWithAsyncTaskIds &builder, ttng::TCGen5MMAOp mmaOp,
    ...
    Value phase = builder.createWithAsyncTaskIds<mlir::arith::XOrIOp>(
        loc, inPhase, _1_1b);
    ...
    for (auto item : users) {
        ...
        if (user->getBlock() != mmaOp->getBlock()) {
            ...
            // mmaOp can be in a different task from headProducer. Even if user
            // and mma are in the same block and they share the same barrier,
            // but the phases should be offset by 1.
...
void insertAsyncComm(
    ...
    auto getSameLevelOp = [](Operation *p, Operation *c) -> Operation * {
        ...
        llvm_unreachable(
            "Failed to find consumer's same level Op with producer");
    };
    ...
    for (auto kv : orderedChannelsGroupedByConsumers) {
        ...
        if (auto funcOp =
                dyn_cast<triton::FuncOp>(headProducer->getParentOp())) {
            builder.setInsertionPointToStart(&(funcOp.getBody().front()));
        } else {
        ...
        // Lower TMA loads and TCGen5MMAOp first before inserting
        // synchronization primitives to avoid displacement.
        ...
        for (auto &consumerTaskId : masterChannel->relation.second) {
            ...
            // acquire.
            auto mmaOp = dyn_cast<ttng::TCGen5MMAOp>(getUniqueActualConsumer(
                masterChannel->getDstOp(), consumerTaskId));
        ...
        for (const auto &token : commChannel.tokens) {
            if (commChannel.consumerBarriers.empty()) {
                ...
                auto producerAcquirePoint =
                    getSameLevelOp(headConsumer, headProducer);
            ...
            // Insert ProducerCommitOp if producer is not TMA. For TMA, TMA
            // lowering will handle the ProducerCommit.
            if (!commChannel.producerBarrier) {
                ...
                if (masterChannel->channelKind == DataChannelKind::TMEM) {
                    ...
                    producerCommitPoint =
                        tmemWaitBarriers[tmemChannel->tmemMmaOp];
                } else {
                    producerCommitPoint =
        ...
        for (const auto &token : commChannel.tokens) {
            ...
            // TCGen5MMAOp, TCGen5MMAOp lowering will handle the
            // ConsumerReleaseOp.
            if (commChannel.consumerBarriers.empty()) {
                auto consumerReleasePoint = consumerReleaseHeuristic(
                    tailProducer, tailConsumer, token.first);
        ...
        // Optimize TMA loads.
        if (tmaLoads.size() > 0) {
            optimizeTMALoads(builder, tmaLoads, buffers,
                             *commChannel.producerBarrier, bufferIdx, bufferIdx,
                             phase, headProducer, headConsumer);
        }
    }
}
...
void foldLocalLoads(triton::FuncOp funcOp) {
    ...
    funcOp.walk([&](ttg::LocalAllocOp localAlloc) {
        if (auto src = localAlloc.getSrc()) {
            if (auto localLoad =
                    dyn_cast<ttg::LocalLoadOp>(src.getDefiningOp())) {

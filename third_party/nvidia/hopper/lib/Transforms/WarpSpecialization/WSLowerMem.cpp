...
Operation *optimizeTMALoads(OpBuilderWithAsyncTaskIds &builder,
    ...
    for (auto [tmaLoad, buffer] : zip(tmaLoads, buffers)) {
        ...
        auto pipelineBuffer = getBufferForPipelineStage(
            builder, tmaLoad.getType(), buffer, bufferIdx, true);
    ...
    for (auto kv : channelsGroupedByProducers) {
        ...
        for (auto &c : kv.second) {
            ...
            while (it != mutuallyNonDominatingChannels.end()) {
                ...
                } else if (dom.properlyDominates(channel->getDstOp(),
                                                 c->getDstOp())) {
                    break;
                } else {
                    ++it;
        ...
        for (auto channel : mutuallyNonDominatingChannels) {
            ...
            // to feed into both tmem_store and users of tmem_alloc. There are
            // cases where a TMEM channel has srcOp in task 2, dstOp in task 2,
            // while mmaOp is in task 1.
        ...
        std::pair<Operation *, Operation *> producerConsumerOps{nullptr,
                                                                nullptr};
        ...
        if (isa<tt::DescriptorLoadOp>(srcOp)) {
            ...
            SmallVector<AsyncTaskId> asyncTasksPC = getAsyncTaskIds(srcOp);
            asyncTasksPC.append(
                getAsyncTaskIds(domininatingChannel->getDstOp()));
            // After createAsyncCopy, c->getSrcOp()/headProducer are no longer
            // valid.
            producerConsumerOps = createAsyncCopy(
                bufferMap, domininatingChannel, domininatingChannel->getSrcOp(),
            ...
            producerConsumerOps = createLocalCopy(
                bufferMap, domininatingChannel, bufferIdx, bufferIdx);
        }

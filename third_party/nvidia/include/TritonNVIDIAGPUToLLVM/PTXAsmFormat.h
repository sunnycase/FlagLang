...
struct PTXBuilder {
    ...
    Operand *
    newListOperand(ArrayRef<std::pair<mlir::Value, std::string>> items) {
    ...
    // @formatter: extra format to represent this operand in ASM code, default
    // is
    //             "%{0}".format(operand.idx).
    Operand *
    newOperand(mlir::Value value, StringRef constraint,
    ...
    // asm would be permitted to write to an input register. Instead of
    // providing constraint code for this operand, the constraint code of the
    // tied operand is used.
  ...
  private:
    ...
    void reorderArgArchive(ArrayRef<Operand *> order) {
        ...
        // The order in argArchive is unnecessary when onlyAttachMLIRArgs=false,
        // but it does necessary when onlyAttachMLIRArgs is true for the $0,
        // $1... are determined by PTX code snippet passed from external.
...
struct PTXInstrCommon {
  ...
  protected:
    ...
    // \param onlyAttachMLIRArgs Indicate that it simply attach the MLIR
    // Arguments to the inline Asm without generating the operand ids(such as
    // $0, $1) in PTX code.
...
template <class ConcreteT> struct PTXInstrBase : public PTXInstrCommon {
    ...
    // A predicate is used to tell whether to apply the suffix, so that no
    // if-else code needed. e.g. `PTXInstr("add").o("s32", isS32).o("u32",
    // !isS32);` will get a `add.s32` if isS32 is true.
...
struct PTXInstrExecution {
    ...
    PTXInstrExecution &predicate(mlir::Value value,

...
static void boundaryCheckToMaskDim(OpBuilder &builder, Location loc,
    ...
    for (auto i : llvm::seq<uint32_t>(0, ptrState.getRank())) {
        ...
        auto maskedShape =
            builder.create<arith::SubIOp>(loc, upperBound, start);
...
LogicalResult PtrState::addState(const PtrState &lhsState,
    ...
    if (lhsState.scalar && rhsState.scalar) {
        auto addOp = builder.create<arith::AddIOp>(loc, lhsState.scalar,
                                                   rhsState.scalar);
    ...
    for (uint64_t i = 0; i < lhs->getRank(); i++) {
        if (!lhs->dimHasModulo(i)) {
            ...
            op->emitWarning("PtrAnalysis: allowing adding pointer state with "
                            "modulo in dim 0 to "
                            "another pointer state with offset in dim "
                            "0.\nPlease verify the "
                            "operand that contains a scalar is meant to "
                            "increment pointers in "
                            "dim1. If that is not the case it WILL LEAD TO "
                            "WRONG COMPILATION "
                            "RESULTS.\n\nTo avoid this warning, use "
                            "expand_dims (instead of "
                            "splat) to explicitly specify which dimension "
                            "contains the scalar.");
            break;
        } else {
            op->emitRemark(
...
LogicalResult PtrState::mulState(const PtrState &lhsState,
    ...
    if (lhsState.scalar && rhsState.scalar) {
        scalar = builder.create<arith::MulIOp>(loc, lhsState.scalar,
                                               rhsState.scalar);
    }
    ...
    if (isa<triton::PointerType>(operand.getType())) {
        ...
        if (auto op = operand.getDefiningOp()) {
            if (auto addPtrOp = dyn_cast<triton::AddPtrOp>(op)) {
                return visitOperandAddptr(cast<triton::AddPtrOp>(op), state,
                                          loc, builder);
                ...
                llvm_unreachable(
                    "Unexpected operand defining operation tts.make_tptr");
...
LogicalResult PtrAnalysis::rewriteAddptrOp(triton::AddPtrOp op) {
    ...
    if (isa<RankedTensorType>(op.getPtr().getType())) {
        ...
        // record the ptr as we have visited and built up the state for this
        // scalar pointer, which may be used by rewriteForOp later.
...
LogicalResult PtrAnalysis::rewriteBitcastOp(triton::BitcastOp op) {
    ...
    if (isa<RankedTensorType>(op.getOperand().getType())) {
        ...
        auto newBitcast = builder.create<triton::BitcastOp>(
            op->getLoc(), resType, state.source);
...
FailureOr<PtrState> PtrAnalysis::getLoopInitArgPtrState(scf::ForOp forOp,
  ...
  //  }
    ...
    // If the pointer isn't defined by tts.get_structured_state nor another
    // loop, it means the current pointer is an iterarg of the outer loop. In
    // such cases, the outer loops would have already set up the PtrState for us
    // already.
    //
...
LogicalResult PtrAnalysis::rewriteForOp(scf::ForOp op) {
    for (auto [i, arg] : llvm::enumerate(op.getRegionIterArgs())) {
        ...
        if (failed(state)) {
            ...
            // considered structured by PtrAnalysis, failing to retrieve the
            // PtrState should not fail the rewrite process. We emit an error
            // for diagnostics and debugging purposes.
            op->emitWarning("Rewrite for-op failed. Could not find PtrState "
                            "for iter-arg index " +
                            std::to_string(i));
            continue;
        }
        ...
        // For tensors of pointers, create a tts.make_tptr at the beginning of
        // the loop body that correspond to this region iter arg. In case it is
        // used by tt.load/tt.store in the loop body before pointer updates,
        ...
        // original pointer.
        // Note that there can be tensor of indices in iter-arg, so we only
        // create the make_tensor_ptr op when the arg is of pointer type.
...
void PtrAnalysis::initializeMaybeStructuredArgs(Operation *op) {
    ...
    while (!q.empty()) {
        ...
        for (auto user : v.getUsers()) {
            ...
            // `tts.get_structured_state` op, its corresponding iter-arg and
            // loop result will also be considered "maybeStructured".
        ...
        return TypeSwitch<Operation *, WalkResult>(op)
            .Case<triton::AddPtrOp>([&](auto addptr) {
                if (rewriteAddptrOp(addptr).failed()) {
                    addptr->emitRemark(
                        "PtrAnalysis: Failed to rewrite AddPtrOp");
                }
                return WalkResult::advance();
            })
            ...
            .Case<scf::ForOp>([&](auto forOp) {
                ...
                // whether the rewrite succeeds or not, we need to return "skip"
                // so that the the walk does not visit the for-op's child
                // operations the second time.
            ...
            .Case<tts::GetStructuredStateOp>(
                [&](tts::GetStructuredStateOp getStateOp) {
                    ...
                    // arithmetic sequence, we need to manually populate the
                    // state of none already exists. This process is necessary
                    // because unlike triton pointers in a loop which always
                    // have a `tt.addptr` that triggers the rewrite process
                    // which includes generating the ops for updating offsets
                    // and strides, tensor of indices only have a simple
                    // `arith.addi` (or other arith ops). Without visiting these
                    // ops manually, the ops to update the offsets and strides

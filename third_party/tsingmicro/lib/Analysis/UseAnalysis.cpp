    ...
    funcOp.walk([&](Operation *op) {
        ...
        if (useType == UseType::Undefined) {
            ...
            assert(op->getNumResults() == 1 &&
                   "Ops used for meta computation are expected to have one "
                   "result");
            ...
            for (auto user : result.getUsers()) {
                TypeSwitch<Operation *>(user)
                    ...
                    .Case<triton::DotOp>([&](auto dot) {
                        ...
                        if (opc)
                            splat =
                                opc.template getDefiningOp<triton::SplatOp>();
                    ...
                    .Default([&](Operation *op) {
                        // if all output of user are used as meta data, user is
                        // a meta user. This condition account for addptr, or an
                        // addi whose output only feeds into addptr
                        bool allMeta = true;

...
void PtrState::addState(const PtrState &lhsState, const PtrState &rhsState,
    ...
    if (lhsState.scalar && rhsState.scalar) {
        auto addOp = rewriter.create<arith::AddIOp>(loc, lhsState.scalar,
                                                    rhsState.scalar);
    ...
    for (uint64_t i = 0; i < lhsState.sizes.size(); i++) {
        ...
        assert(!lhsState.hasModulo() ||
               !rhsState.hasModulo() &&
                   "AddPtr where both lhs and rhs containing "
                   "modulo operators not supported");

        modulos.push_back(lhsState.modulos[i].has_value()
                              ? lhsState.modulos[i]
                              : rhsState.modulos[i]);
    }
}
...
PtrState::createSideBySideCastOps(ArrayRef<int64_t> resultShape,
    ...
    Value modN = rewriter.create<arith::IndexCastOp>(
        loc, rewriter.getIndexType(), modulos[1]->size);
...
PtrState::createCastOp(ArrayRef<int64_t> resultShape, const Location loc,
    ...
    auto resultType = getResultMemrefType(rewriter.getContext(),
                                          staticOffset[0], resultShape);
...
void PtrAnalysis::visitOperandAdd(
    ...
    if ((lhsState.getRank() == 1 && lhsState.hasModulo()) ||
        ...
        assert(0 &&
               "Current do not support this pattern: a + arange(0, K) % M");
    }
...
void PtrAnalysis::visitOperandExpandDims(
    ...
    // operands in-place and turn them into memrefs in loops, so we have to
    // bypass the cast by using getSrcMutable. These are temporary fix only
    // since we will be moving over to StructuredPtrAnalysis soon which separate
    // out the memref conversion.
...
void PtrAnalysis::visitOperandMakeTensorPtr(
    ...
    if (auto castOp =
            remappedValue.getDefiningOp<memref::ReinterpretCastOp>()) {
        visitOperandReintCast(castOp, state, loc, rewriter, knownPtrs);
    } else {
        llvm_unreachable(
            "Expect value to me mapped to a memref.reinterpret_cast");
    }
}
...
void PtrAnalysis::visitOperandAddptr(
    ...
    // scalar pointer but replaced with a memref. In this case, ptrState will
    // have rank 1 and offsetState will have rank 0.
    // TODO:
    ...
    if (isa<triton::PointerType>(operand.getType())) {
        ...
        if (auto op = operand.getDefiningOp()) {
            if (auto addPtrOp = dyn_cast<triton::AddPtrOp>(op)) {
                visitOperandAddptr(cast<triton::AddPtrOp>(op), state, loc,
                                   rewriter, knownPtrs);
            } else if (auto makeTensorOp =
...
void PtrAnalysis::rewriteAddptrOp(
    ...
    if (llvm::any_of(state.modulos, [](auto mod) { return mod.has_value(); })) {
        ...
        if (!state.modulos[0].has_value() && state.modulos[1].has_value()) {
            casts = state.createSideBySideCastOps(resultShape, op.getLoc(),
                                                  rewriter);
            type = ModuloState::WraparoundSideBySide;
        } else if (state.modulos[0].has_value() &&
                   !state.modulos[1].has_value()) {
            casts =
                state.createStackedCastOps(resultShape, op.getLoc(), rewriter);
    ...
    for (auto [i, v] : llvm::enumerate(adaptor.getOperands())) {
        if (auto mappedV = rewriter.getRemappedValue(v)) {
            ...
            if (v.getDefiningOp<triton::AddPtrOp>() ||
                ...
                } else if (auto castOp = mappedV.getDefiningOp<
                                         memref::ReinterpretCastOp>()) {
                    v = castOp;
                } else {
                    llvm_unreachable(
                        "mapped value defined by an unexpected op");
                }
            } else {
                ...
                if (isa<TensorType>(mappedV.getType()) &&
                    isa<triton::PointerType>(
                        dyn_cast<TensorType>(mappedV.getType())
                            .getElementType()))
                    llvm_unreachable(
                        "unsupported scenario where a value is a tensor of "
        ...
        assert(reintCastOp ||
               ...
               (isa<TensorType>(v.getType()) &&
                isa<IndexType>(
                    dyn_cast<TensorType>(v.getType()).getElementType())));
        ...
        if (reintCastOp) {
            ...
            visitOperandUnrealizedCast(unrealizedCastOp, state, op.getLoc(),
                                       rewriter, knownPtrs);
        } else {
...
void PtrAnalysis::visitOperandUnrealizedCast(
    ...
    if (knownPtrs.contains(origPtr)) {
        ...
        visitOperandAddptr(origPtr.getDefiningOp<triton::AddPtrOp>(), state,
                           loc, rewriter, knownPtrs);
    }
}
...
void PtrAnalysis::rewriteForOp(
    ...
    // If we have a load op that uses a modulo pointer, we need to insert both
    // of the memref chunks to the init args. We reuse the sizes from the
    // original memrefs. This data structure keeps track of where these
    // additional init args should be inserted.
    //
    // As an example, if we have a 2D memrefs being split, we first put the
    // first chunk in the order as it appears. Then, once all of the original
    // init args are processed, we insert their offsets and strides, and finally
    // the second chunk.
    ...
    // reinterpret_casts whose return types need to be rewritten to match what
    // the for loop is yielding.
    ...
    for (auto [i, arg] : llvm::enumerate(op.getInitArgs())) {
        ...
        if (mappedV) {
            ...
            //  supported.
            assert(
                !(dyn_cast<BlockArgument>(mappedV) &&
        ...
        auto indexTensor =
            ...
            isa<IndexType>(
                dyn_cast<TensorType>(arg.getType()).getElementType());
    ...
    auto newOp = rewriter.create<scf::ForOp>(
        ...
        [&](OpBuilder &b, Location loc, Value iv, ValueRange args) {
            ...
            // memrefs into a single one using unrealized_cast_op. This way,
            // when lowering the load, the pattern can detect if additional
            // copies are inserted. When we are in a loop, it is more
            // complicated because we have to insert a new unrealized_cast_op
            // that combines the two memrefs in the init arg list. In addition,
            // because init args hold no offset and size information, we have to
            // manually insert two additional reinterpret_cast ops as input to
            // this unrealized_cast_op so that the load have enough information
            ...
            for (auto &[unrealizedCastOp, chunkData, state] : moduloStates) {
                ...
                auto combinedCast = b.create<UnrealizedConversionCastOp>(
                    loc, unrealizedCastOp.getResult(0).getType(),
    ...
    // Update the loop body. Manually invoke the rewrite logic on addptr and
    // yield in the loop body, so we can take advantage of the states we built
    // up
    ...
    if (op.getNumRegionIterArgs()) {
        ...
        rewriteYieldOp(yieldOp, rewriter, levelToBlockArgIndex, level,
                       knownPtrs);
    }
...
Value PtrAnalysis::getScalarMemRef(Value ptr, Value memRef, const Location loc,
                                   ConversionPatternRewriter &rewriter) {
    assert(cast<triton::PointerType>(ptr.getType()) &&
           "expected scalar pointer");

    // If the pointer is generated by tt.addptr, we will have already inserted
    // an ReinterpretCastOp to cast its type from tt.ptr to unranked memref.
    // Return the result.

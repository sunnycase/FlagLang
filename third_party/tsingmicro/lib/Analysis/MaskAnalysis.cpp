...
LogicalResult MaskState::parseCmp(arith::CmpIOp cmpOp, const Location loc,
    ...
    for (int32_t i = 0; i < lhsState.getRank(); i++) {
        ...
        if (!dimIntAttr || dimIntAttr.value() != 1) {
            if (cmpDim != -1) {
                InFlightDiagnostic diag =
                    emitError(loc) << "Unsupported cmpi with more than one "
    ...
    if (lhsState.scalar) {
        ...
        // If both lhs and rhs are scalars, we can't just derive the dimension
        // of the mask as the minimum value: lhs/rhs could be 0 and then we
        // don't load/store anything.
        //
        ...
        newDim = compareOFRs(lhsState.scalar, rhsState.scalar,
                             cmpOp.getPredicate(), lhsState.dims[cmpDim],
                             builder.getIndexAttr(0), loc, builder);
    } else if (cmpOp.getPredicate() == arith::CmpIPredicate::slt ||
               cmpOp.getPredicate() == arith::CmpIPredicate::ult) {
        // Important:
        // In the case where the values we are loading are entirely masked off
        // like the following:
        //
        // ---|-------|-----------|
        //    ^       ^           ^
        ...
        // Now scalar < start, so simply doing dim = newEnd - start is
        // incorrect.
        //
        // The correct formula is to optionally move `newDim` back to `start`
        // using max(newEnd, start).
...
LogicalResult MaskState::parseLoopIterArg(Value v, const Location loc,
    ...
    if (auto getStateOp = initArg.getDefiningOp<tts::GetStructuredStateOp>()) {
        ...
        if (llvm::isa_and_nonnull<arith::ConstantOp>(
                tritonValue.getDefiningOp())) {
        ...
        if (failed(this->addStateScalar(lhsState,
                                        forOp.getRegionIterArgs()[argIndex + 1],
                                        loc, builder))) {
            return failure();
        }

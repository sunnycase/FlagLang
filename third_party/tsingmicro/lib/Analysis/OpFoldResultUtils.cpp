...
OpFoldResult addOFRs(const OpFoldResult lhs, const OpFoldResult rhs,
    ...
    if (lhsIntAttr) {
        auto lhsOp = b.create<arith::ConstantOp>(
            loc, b.getIndexAttr(lhsIntAttr.value()));
    ...
    if (rhsIntAttr) {
        auto rhsOp = b.create<arith::ConstantOp>(
            loc, b.getIndexAttr(rhsIntAttr.value()));
...
OpFoldResult mulOFRValue(const OpFoldResult lhs, const Value rhs,
    ...
    if (lhsIntAttr && !rhsIsConst) {
        auto lhsConstOp = b.create<arith::ConstantOp>(
            loc, b.getIndexAttr(lhsIntAttr.value()));
...
OpFoldResult compareOFRs(const OpFoldResult lhs, const OpFoldResult rhs,
    ...
    auto selectOp =
        b.create<arith::SelectOp>(loc, cmpOp, trueValue, falseValue);

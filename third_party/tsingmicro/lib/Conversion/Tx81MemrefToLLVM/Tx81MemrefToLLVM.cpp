...
struct TsmMemRefAllocOpLowering : public AllocLikeOpLLVMLowering {
    ...
    allocateBufferFromSPM(ConversionPatternRewriter &rewriter, Location loc,
        ...
        spmAddr = rewriter.create<LLVM::PtrToIntOp>(
            op->getLoc(), rewriter.getI64Type(), spmAddr);
        spmAddr = rewriter.create<LLVM::AddOp>(
            op->getLoc(), rewriter.getI64Type(), spmAddr, spmOffsetOp);

        spmAddr = rewriter.create<LLVM::IntToPtrOp>(op->getLoc(),
...
struct MemrefLoadOrStoreOpLowering : public ConvertOpToLLVMPattern<MemrefOp> {
    ...
    matchAndRewrite(MemrefOp op, OpAdaptor adaptor,
        ...
        Type indexType =
            ConvertToLLVMPattern::getTypeConverter()->getIndexType();
        ...
        if (isMaskEle) {
            ...
            Value ShrAmtC =
                rewriter.create<LLVM::ConstantOp>(loc, indexType, 3);
            Offset = rewriter.create<LLVM::AndOp>(loc, index, MaskC);
            Offset = rewriter.create<LLVM::TruncOp>(loc, rewriter.getI8Type(),
                                                    Offset);
        ...
        int isSpm = cast<IntegerAttr>(opBase->getAttr("isSpm"))
                        .getValue()
                        .getSExtValue();
        ...
        if (isSpm) {
            ...
            auto i8PtrTy = LLVM::LLVMPointerType::get(
                rewriter.getContext(), *ConvertToLLVMPattern::getTypeConverter()
        ...
        if constexpr (std::is_same<MemrefOp, memref::LoadOp>()) {
            if (isMaskEle) {
                Value newVal = rewriter.create<LLVM::LoadOp>(
                    loc, rewriter.getI8Type(), adjustedPtr, 0, false,
                    op.getNontemporal());
                Value Zero = rewriter.create<LLVM::ConstantOp>(
                    loc, rewriter.getI8Type(), 0);
                Value MaskC = rewriter.create<LLVM::ConstantOp>(
                    loc, rewriter.getI8Type(), 1);
                MaskC = rewriter.create<LLVM::ShlOp>(loc, MaskC, Offset);
                newVal = rewriter.create<LLVM::AndOp>(loc, newVal, MaskC);
                newVal = rewriter.create<LLVM::ICmpOp>(
                    loc, LLVM::ICmpPredicate::ne, newVal, Zero);
                rewriter.replaceOp(op, ValueRange{newVal});
            } else {
                rewriter.replaceOpWithNewOp<LLVM::LoadOp>(op, op.getType(),
                                                          adjustedPtr, 0, false,
                                                          op.getNontemporal());
            }
        } else {
            ...
            if (isMaskEle) {
                Value srcVal = rewriter.create<LLVM::LoadOp>(
                    loc, rewriter.getI8Type(), adjustedPtr, 0, false,
                    op.getNontemporal());
                Value MaskC = rewriter.create<LLVM::ConstantOp>(
                    loc, rewriter.getI8Type(), 1);
                ...
                Value FalseVal =
                    rewriter.create<LLVM::XOrOp>(loc, TrueVal, MaskC);
                StoreVal = rewriter.create<LLVM::SelectOp>(loc, StoreVal,
                                                           TrueVal, FalseVal);
            }

            rewriter.replaceOpWithNewOp<LLVM::StoreOp>(
                op, StoreVal, adjustedPtr, 0, false, op.getNontemporal());
        }
...
struct MemRefReinterpretCastOpLowering
  ...
  private:
    /// Extracts allocated, aligned pointers and offset from a ranked or
    /// unranked memref type. In unranked case, the fields are extracted from
    /// the underlying ranked descriptor.
    void extractPointersAndOffset(Location loc,
        ...
        // Extract pointer to the underlying ranked memref descriptor and cast
        // it to ElemType**.
    ...
    LogicalResult convertSourceMemRefToDescriptor(
        ...
        for (unsigned i = 0, e = targetMemRefType.getRank(); i < e; ++i) {
            ...
            if (castOp.isDynamicStride(i))
                ...
                desc.setConstantStride(rewriter, loc, i,
  ...
  public:
    ...
    matchAndRewrite(memref::ExtractStridedMetadataOp extractStridedMetadataOp,
        ...
        MemRefDescriptor dstMemRef = MemRefDescriptor::fromStaticShape(
            ...
            cast<MemRefType>(
                extractStridedMetadataOp.getBaseBuffer().getType()),
...
struct MemRefCastOpLowering : public ConvertOpToLLVMPattern<memref::CastOp> {
  ...
  public:
    LogicalResult match(memref::CastOp memRefCastOp) const override {
        ...
        // used for type erasure. For now they must preserve underlying element
        // type and require source and result type to have the same rank.
        // Therefore, perform a sanity check that the underlying structs are the
        // same. Once op semantics are relaxed we can revisit.
    ...
    void rewrite(memref::CastOp memRefCastOp, OpAdaptor adaptor,
        ...
        auto targetStructType =
            typeConverter->convertType(memRefCastOp.getType());
        ...
        if (isa<MemRefType>(srcType) && isa<UnrankedMemRefType>(dstType)) {
            ...
            UnrankedMemRefDescriptor memRefDesc =
                UnrankedMemRefDescriptor::poison(rewriter, loc,
                                                 targetStructType);
        ...
        } else if (isa<UnrankedMemRefType>(srcType) &&
                   isa<MemRefType>(dstType)) {
            // Casting from unranked type to ranked.
            // The operation is assumed to be doing a correct cast. If the
            // destination type mismatches the unranked the type, it is
            // undefined behavior.

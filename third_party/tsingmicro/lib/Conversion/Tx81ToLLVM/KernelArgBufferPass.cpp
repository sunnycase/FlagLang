...
class KernelArgBufferPass
  ...
  private:
    ...
    Value insertKernelArgLoad(OpBuilder &builder, Location loc,
...
Value KernelArgBufferPass::insertKernelArgLoad(OpBuilder &builder, Location loc,
    ...
    // NOTE: GEPOp need distinguish the scalar and ptr type. So here ptr +
    // offset
    Value elementPtr =
...
void KernelArgBufferPass::runOnOperation() {
    ...
    for (auto func : kernelFuncs) {
        ...
        for (auto argIndex : llvm::seq<unsigned>(0, numArguments)) {
            ...
            Value loadedArg = insertKernelArgLoad(
                builder, func.getLoc(), argsBuffer, argType, currentOffset);

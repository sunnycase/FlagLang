...
static Value createInt32ValueArray(ConversionPatternRewriter &rewriter,
    ...
    //       rewriter.getI32IntegerAttr(spmPointer));
    //   auto elementPtrType =
    //   LLVM::LLVMPointerType::get(rewriter.getContext()); Value spmAddr =
    //   rewriter.create<LLVM::ZeroOp>(loc, elementPtrType); spmAddr =
    ...
    for (size_t i = 0; i < array.size(); i++) {
        ...
        Value elemPtr = rewriter.create<LLVM::GEPOp>(
            loc, i32PtrTy, i32Ty, allocaOp, ArrayRef<Value>{idx});
    ...
    matchAndRewrite(arith::IndexCastOp op, OpAdaptor adaptor,
        ...
        if (isa<IntegerType>(srcType) && isa<IntegerType>(dstType)) {
            ...
            if (srcWidth < dstWidth) {
                ...
                rewriter.replaceOpWithNewOp<LLVM::ZExtOp>(op, dstType,
                                                          adaptor.getIn());
...
struct BarrierConversion : public OpConversionPattern<tx::BarrierOp> {
    ...
    matchAndRewrite(tx::BarrierOp op, OpAdaptor adaptor,
        ...
        */
        ...
        auto call =
            rewriter.create<LLVM::CallOp>(op.getLoc(), TypeRange{i8PtrTy},
                                          "__Barrier", // funcPtr,
                                          ValueRange{});
...
struct RdmaWdmaOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        auto call = rewriter.create<LLVM::CallOp>(
            ...
            ValueRange{src, target, srcShapeArray, srcStridesArray,
                       dstShapeArray, dstStridesArray, rank, elemBytes, fmt});
...
struct MaskMoveOpConversion : public OpConversionPattern<tx::MaskMoveOp> {
    ...
    matchAndRewrite(tx::MaskMoveOp op, OpAdaptor adaptor,
        ...
        // Need to bitcast src to i8*
        target =
            rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, target);
...
struct GatherScatterOpConversion
    ...
    matchAndRewrite(tx::GatherScatter op, OpAdaptor adaptor,
        ...
        */
        ...
        // Get strides
        auto srcStrideN = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcStrideN());
        auto srcStrideH = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcStrideH());
        auto srcStrideW = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcStrideW());
        auto dstStrideN = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstStrideN());
        auto dstStrideH = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstStrideH());
        auto dstStrideW = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstStrideW());

        // Get iterator
        auto srcIterN = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcIterN());
        auto srcIterH = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcIterH());
        auto srcIterW = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getSrcIterW());
        auto dstIterN = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstIterN());
        auto dstIterH = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstIterH());
        auto dstIterW = rewriter.create<LLVM::ConstantOp>(
            loc, i32Ty, adaptor.getDstIterW());
...
struct ArgMinMaxOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        // Types for function declaration
        SmallVector<Type, 17> argTypes = {i8PtrTy, i8PtrTy, i8PtrTy, i32Ty,
                                          i16Ty};
...
struct ReduceOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        // Declare the runtime function if not already declared
        // Signature:
        // __ReduceSum(uint64_t *src, uint64_t *dst, uint32_t dim, uint16_t
        // src_n, uint16_t src_h, uint16_t src_w, uint16_t src_c, uint16_t fmt)
        auto i8PtrTy = LLVM::LLVMPointerType::get(rewriter.getContext());
        auto i32Ty = rewriter.getI32Type();
        auto i16Ty = rewriter.getI16Type();
        ...
        // Convert shape attribute to Value
        Value shape_n = rewriter.create<LLVM::ConstantOp>(op.getLoc(), i16Ty,
                                                          op.getShape()[0]);
        Value shape_h = rewriter.create<LLVM::ConstantOp>(op.getLoc(), i16Ty,
                                                          op.getShape()[1]);
        Value shape_w = rewriter.create<LLVM::ConstantOp>(op.getLoc(), i16Ty,
                                                          op.getShape()[2]);
        Value shape_c = rewriter.create<LLVM::ConstantOp>(op.getLoc(), i16Ty,
                                                          op.getShape()[3]);
...
struct ElementWiseOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        // Declare the runtime function if not already declared
        // Signature: void* __Add(void* a, void* b, void* out, uint32_t
        // elem_count, uint32_t rnd_mode, uint32_t fmt);
        auto i8PtrTy = LLVM::LLVMPointerType::get(rewriter.getContext());
        auto i32Ty = rewriter.getI32Type();
...
struct UnaryBoolLogicVOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        // Declare the runtime function if not already declared
        // Signature: void* __BoolNotV(void* src, void* dst, uint32_t
        // elem_count);
        auto i8PtrTy = LLVM::LLVMPointerType::get(rewriter.getContext());
        auto i32Ty = rewriter.getI32Type();
...
struct RelationVSOpConversion : public OpConversionPattern<Tx81Op> {
    ...
    matchAndRewrite(Tx81Op op, OpAdaptor adaptor,
        ...
        // Types for function declaration
        SmallVector<Type, 17> argTypes = {i8PtrTy, i32Ty, i8PtrTy, i32Ty,
                                          i32Ty};
...
struct ZeroPointConvertOpConversion
    ...
    matchAndRewrite(ZeroPointConvertOp op, OpAdaptor adaptor,
        ...
        input = rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, input);
        output =
            rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, output);
...
struct MXFPScaleBF16OpConversion
    ...
    matchAndRewrite(tx::MXFPScaleBF16Op op, OpAdaptor adaptor,
        ...
        // Declare the function
        Value funcPtr = triton::utils::declareTx81Function(
            module, rewriter, op.getLoc(), "__mxfpScaleBf16", i8PtrTy,
            argTypes);
...
struct ChannelNormOpConversion : public OpConversionPattern<tx::ChannelNormOp> {
    ...
    matchAndRewrite(tx::ChannelNormOp op, OpAdaptor adaptor,
        ...
        auto call = rewriter.create<LLVM::CallOp>(
            ...
            ArrayRef<Value>{src, dst, shape_n, shape_h, shape_w, shape_c,
                            c0Align, dtypeSize});
...
struct DechannelNormOpConversion
    ...
    matchAndRewrite(tx::DechannelNormOp op, OpAdaptor adaptor,
        ...
        Value funcPtr = triton::utils::declareTx81Function(
            module, rewriter, op.getLoc(), "__DechannelNorm", i8PtrTy,
            argTypes);
...
struct GemmOpConversion : public OpConversionPattern<tx::GemmOp> {
    ...
    matchAndRewrite(tx::GemmOp op, OpAdaptor adaptor,
        ...
        // Signature: void __Gemm(int64_t* srcA, int64_t *srcB, int64_t *
        // srcBias, int64_t *dst, int32_t *dims, bool enPsum, int64_t *psum,
        // bool enTransA, bool enTransB, int64_t batchSizeA, int64_t batchSizeB,
        ...
        srcB = rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, srcB);
        srcBias =
            rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, srcBias);
        dst = rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, dst);
        psumAddr =
            rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy, psumAddr);
        srcNegScale = rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy,
                                                        srcNegScale);
        srcPosScale = rewriter.create<LLVM::IntToPtrOp>(op.getLoc(), i8PtrTy,
                                                        srcPosScale);
        ...
        for (size_t i = 0; i < dimsValues.size(); i++) {
            ...
            // Create GEP to get pointer to array element
            Value elemPtr = rewriter.create<LLVM::GEPOp>(op.getLoc(), i64PtrTy,
                                                         i32Ty, dimsArrayI32Ptr,
                                                         ArrayRef<Value>{idx});
...
struct MemsetOpConversion : public OpConversionPattern<tx::MemsetOp> {
    ...
    matchAndRewrite(tx::MemsetOp op, OpAdaptor adaptor,
        ...
        //                    int32_t* strides, int32_t* iterations, uint16_t
        //                    fmt);
        auto i8PtrTy = LLVM::LLVMPointerType::get(rewriter.getContext());
        auto i32Ty = rewriter.getI32Type();
        auto i32PtrTy = LLVM::LLVMPointerType::get(rewriter.getContext());
        ...
        auto call = rewriter.create<LLVM::CallOp>(
            ...
            ArrayRef<Value>{dst, value, dstShapeArray, dstStridesArray, rank,
                            fmt});

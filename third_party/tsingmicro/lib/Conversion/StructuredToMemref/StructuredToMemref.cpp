...
struct MakeTensorPtrConverter
  ...
  private:
    ...
    static MemRefType getResultMemrefType(tts::MakeTensorPtrOp op,
                                          int64_t offset,
    ...
    createSideBySideCastOps(tts::MakeTensorPtrOp op, OpAdaptor adaptor,
        ...
        auto targetOffset = ofrToIndexValue(
            accumulateTargetOffset(op, rewriter), loc, rewriter);

        ////////////////////////////////////////////////////////////////////////////
        //
        // Handling side-by-side wraparound
        //
        // Note: We do not support cases where the target has already overflown
        // the number of columns! This is because in PtrAnalysis, the offset has
        // already been collapsed into a single dimension, so it is ambiguous to
        // determine whether the offset actually overflows or just refers to an
        // element on the subsequent rows.
        //
    ...
    createStackedCastOps(tts::MakeTensorPtrOp op, OpAdaptor adaptor,
        ...
        //
        // We do not support cases where the target offset has already overflown
        // the number of rows. See side-by-side wraparound for details.
        //
        ////////////////////////////////////////////////////////////////////////////
        ...
        Value strideRow =
            ofrToIndexValue(op.getMixedStrides()[0], loc, rewriter);
        Value strideCol =
            ofrToIndexValue(op.getMixedStrides()[1], loc, rewriter);
        ...
        Value d1 =
            rewriter.create<arith::SubIOp>(loc, clampedOff, targetOffset);
    ...
    LogicalResult rewriteSplitPtr(tts::MakeTensorPtrOp op, OpAdaptor adaptor,
        ...
        if (parentShape[0] == ShapedType::kDynamic) {
            ...
            auto [cast1, cast2] =
                createSideBySideCastOps(op, adaptor, rewriter);
            casts = {cast1.getResult(), cast2.getResult()};
    ...
    LogicalResult rewriteBlockPtr(tts::MakeTensorPtrOp op, OpAdaptor adaptor,
                                  ConversionPatternRewriter &rewriter) const {
        // Block pointers are basically the same as structured pointers except
        // that the return types are !tt.ptr<tensor<AxBxCxbf16>> instead of
  ...
  public:
    ...
    matchAndRewrite(tts::MakeTensorPtrOp op, OpAdaptor adaptor,
        ...
        // TODO: Order is a compiler hint. We can optimize data load/store
        // according the order attribute.
        if (op.isBlockPtr()) {
...
struct LoadConverter : public OpConversionPattern<tts::LoadOp> {
  private:
    ...
    memref::SubViewOp createSubview(Value src, ArrayRef<OpFoldResult> offsets,
        ...
        auto dstType = memref::SubViewOp::inferResultType(srcType, offsets,
                                                          sizes, strides);
        return rewriter.create<memref::SubViewOp>(
            loc, cast<MemRefType>(dstType), src, offsets, sizes, strides);
    }

    std::pair<memref::SubViewOp, memref::SubViewOp>
    getSideBySideSubviews(ArrayRef<OpFoldResult> dims, Value block1,
                          Value block2, Location loc,
                          ConversionPatternRewriter &rewriter) const {
        ...
        SmallVector<OpFoldResult> strides(dims.size(),
    ...
    LogicalResult rewriteMaskedLoad(tts::LoadOp op, OpAdaptor adaptor,
        ...
        if (!other) {
            ...
            op->emitRemark("Masked load without other value, using zero "
                           "padding instead\n");
        ...
        rewriter.create<scf::IfOp>(
            loc, accBase, [&](OpBuilder &b, Location loc) {
                b.create<linalg::FillOp>(loc, ValueRange{other},
                                         ValueRange{alloc});
        ...
        if (auto unrealizedCast =
            ...
            if (unrealizedCast->hasAttr(WRAP_SIDE_BY_SIDE)) {
                auto [subview1, subview2] = getSideBySideSubviews(
                    mixedDims, block1, block2, loc, rewriter);
                createSideBySideCopies(subview1, subview2, alloc, loc,
                                       rewriter);
            } else if (unrealizedCast->hasAttr(WRAP_STACKED)) {
                auto [subview1, subview2] = getStackedSubviews(
                    mixedDims, block1, block2, loc, rewriter);
            ...
            memref::SubViewOp dstSubview = getSubview(
                tensorType.getRank(), mixedDims, alloc, loc, rewriter);
  ...
  private:
    ...
    getExtractSlice(int rank, ArrayRef<OpFoldResult> dims, Value source,
        ...
        auto dstType = tensor::ExtractSliceOp::inferResultType(
            sourceType, offsets, dims, strides);

...
struct ScalarLoadConverter : public OpConversionPattern<tts::GatherOp> {
    ...
    ScalarLoadConverter(const TypeConverter &typeConverter,
                        MLIRContext *context)
...
struct ScalarStoreConverter : public OpConversionPattern<tts::ScatterOp> {
    ...
    ScalarStoreConverter(const TypeConverter &typeConverter,
                         MLIRContext *context)
...
struct GatherConverter : public OpConversionPattern<tts::GatherOp> {
    ...
    matchAndRewrite(tts::GatherOp gatherOp, OpAdaptor adaptor,
        ...
        auto genericOp = rewriter.create<linalg::GenericOp>(
            loc, TypeRange{resultType}, inputs, ValueRange{emptyTensor},
            affineMaps, iteratorTypes,
            [&](OpBuilder &b, Location loc, ValueRange args) {
                auto getValueAtIndex = [baseTensor](OpBuilder &b, Location loc,
                                                    Value index) -> Value {
                    Value index0 = b.create<arith::IndexCastOp>(
                        loc, b.getIndexType(), index);
                ...
                if (!gatherOp.getMask()) {
                    // If there is no mask, simply extract the current element
                    // from the base tensor and use it as the yield value.
                    auto loadValue = getValueAtIndex(b, loc, offset);
                    b.create<linalg::YieldOp>(loc, loadValue);
                } else {
                    // If the mask value is truthy, the current element is
                    // loaded from the base tensor using its offset. Otherwise,
                    // if `other` is present, yield `other`. If `other` is not
                    // present, a default value of 0 is used.
                    auto mask = args[1];
                    auto ifOp = b.create<scf::IfOp>(
                        ...
                        [&](OpBuilder &b, Location loc) {
                            // Falsy case, yield `other` or 0 as the default
                            // value.
...
struct ScatterConverter : public OpConversionPattern<tts::ScatterOp> {
    ...
    matchAndRewrite(tts::ScatterOp scatterOp, OpAdaptor adaptor,
        ...
        auto genericOp = rewriter.create<linalg::GenericOp>(
            ...
            [&](OpBuilder &b, Location loc, ValueRange args) {
                auto storeValueAtIndex = [baseMemref](OpBuilder &b,
                ...
                if (!scatterOp.getMask()) {
                    // If there is no mask, simply insert the current value to
                    // the base memref using its offset.
                    storeValueAtIndex(b, loc, offset, value);
                } else {
                    // If the mask value is truthy, insert the current value to
                    // the the base memref using its offset. Otherwise, noop.
                    auto mask = args[2];
                    auto ifOp = b.create<scf::IfOp>(
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        patterns.add<GatherConverter, ScatterConverter, ScalarLoadConverter,
                     ScalarStoreConverter>(typeConverter,
                                           patterns.getContext());

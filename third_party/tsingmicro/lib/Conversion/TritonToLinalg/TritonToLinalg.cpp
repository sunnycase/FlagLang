...
void mlir::triton::populateTritonToLinalgConversionPatterns(
    ...
    // MetaOpConverter has PatternBenefit == 10 which should take precedence
    // over these linalg patterns, but to be safe, add these patterns last so
    // that they will be tried last. Incorrect ordering or having
    // MetaOpConverter has lower PatternBenefit will result in element-wise meta
    // ops being converted to linalg.generic ops.

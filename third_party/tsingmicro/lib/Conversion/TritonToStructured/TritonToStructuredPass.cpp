...
class TritonToStructuredPass
  ...
  public:
    ...
    LogicalResult convertToPointerTupleWithOffsetsAndStrides() {
        ...
        converter.addConversion(
            [context](RankedTensorType tensorType, SmallVectorImpl<Type> &types)
                ...
                // Important note:
                // We only care about tensor of index / int (in addition to
                // pointer type) because only values of int and index type can
                // potentially be part of a pointer arithmetic sequence.
                // FIXME: Integer type may not always involved in pointer
                // arithmetic
                if (!isa<triton::PointerType>(tensorType.getElementType()) &&
                    !tensorType.getElementType().isIntOrIndex()) {
                    // There's a subtle difference between returning failure()
                    // and std::nullopt. From the documentation:
                    //
                    // If std::nullopt is returned, the converter is allowed to
                    // try another conversion function to perform the
                    // conversion.
                    //
                    // Say we have type tensor<4x256xbf16> which is a
                    // RankedTensorType. Even though this RankedTensorType
                    // matches the converter that handles the tuple conversion,
                    // we want to keep this type as is because the inner type
                    // isn't a pointer.
                    //
                    // By returning failure(), the TypeConverters will stop
                    // trying the remaining converters. In our case, the last
                    // type converter which simply returns the same type is
                    // skipped. And because the conversion for this type has
                    // failed, the whole conversion process is also skipped.
                    //
                ...
                types = SmallVector<Type>{
                    getStructuredStateTupleType(context, tensorType)};
                return success();
            });
        ...
        converter.addConversion(
            [context](triton::PointerType ptrType, SmallVectorImpl<Type> &types)
                -> std::optional<LogicalResult> {
                types = SmallVector<Type>{
                    getStructuredStateTupleType(context, ptrType)};
                return success();
            });

        // Hooks to compute the correct materialization, "argument" and "source"
        // materialization are used when we need to convert the tuple type back
        // to the original triton pointer type. These are used when there are
        // ops that still need to use the original pointer type. For instance,
        // we convert the result of tt.addptr from tt.ptr type to a tuple, but
        // the original ptr result is still being used by another tt.load or
        // tt.store.
        ...
        // type, convert that value to a tuple type.
    ...
    LogicalResult decomposePointerTuple() {
        ...
        // Hooks to compute the correct materialization, "argument" and "source"
        // materialization are used when we need to convert a series of
        // {pointer, offset_0, offset_1,..., stride_0, stride_1,...} type back
        // to the "pointer tuple type".
        //
        // Because we actually want to get rid of the tuple type, return
        // `inputs[0]` which corresponds to a "triton pointer type". This
        // approach will work as intended because the ops that currently take
        // "pointer tuple type" are `unrealized_conversion_cast` ops which will
        // get removed below during reconcile-unrealized-conversion-casts.
        auto materialize = [](OpBuilder &builder, Type resultType,
                              ValueRange inputs,
        ...
        // sequence of {pointer, offset_0, offset_1,..., stride_0,
        // stride_1,...}, create a `tts.get_structured_state` op that serves as
        // a placeholder. The return values for this op will be used as the
        // init-args for scf.for. At the end of pointer analysis, we will use
        // the PtrState to create the correct offsets, strides, and remove these
        // ops.
    ...
    // Prepass that inserts `tts.get_structured_state` ops. These ops are used
    // as placeholders to make passing structured pointer state into scf.for
    // loop's init args easier, especially with multiple levels of loops.
    //
    // Background:
    //
    // PtrAnalysis computes a PtrState for every operand (or triton value)
    // involved in a sequence of pointer arithmetic; some examples include:
    // triton pointer, offsets (which could be a tensor of indices or just a
    // simple index value).
    //
    ...
    // `tts.get_structured_state`. The return values of these ops, which include
    // a triton value with its original result type and its corresponding
    // offsets and strides, will be used as "placeholders" into the scf.for's
    // init-args. We leverage standard MLIR infrastructure 1->N conversion to
    // perform this rewrite, which helps simplify the logic significantly.
    //
    ...
    // then take {triton_value, offset_0, offset_1, ..., stride_0,
    // stride_1,...}.
    //
    ...
    // that gets converted to a {triton_value, offset_0, offset_1, ...,
    // stride_0, stride_1,...} will recursively trigger other conversions.
    //
    ...
    //
    // Note that even though the type conversion happens for every integer
    // tensor appearing in loops' iter-args, this conversion is reversible. If
    // the integer tensor isn't used in a pointer arithmetic sequence,
    ...
    void runOnOperation() override {
        ...
        // Now that all the PtrStates have been populated, we can wire up the
        // states with the tts.get_structured_state ops inserted in the prepass.

...
struct AddrTypeConverter : public ::mlir::LLVMTypeConverter {
    AddrTypeConverter(MLIRContext *ctx, const LowerToLLVMOptions &options,
        ...
        addConversion([&](AddressType type) {
            unsigned as = 0;
            if (auto attr =
                    dyn_cast_or_null<IntegerAttr>(type.getAddressSpace()))
                as = attr.getUInt();
...
LogicalResult ConstantOpConversion::matchAndRewrite(
    ...
    auto cst = rewriter.create<LLVM::ConstantOp>(
        op.getLoc(), rewriter.getIntegerAttr(typeConverter->convertType(
                                                 op.getValueAttr().getType()),
                                             operands.getValue()));
...
void AddrToLLVM::runOnOperation() {
    ...
    AddrTypeConverter typeConverter(&getContext(), options,
                                    &dataLayoutAnalysis);
    ...
    patterns
        .insert<ConstantOpConversion, TypeOffsetOpConversion, CastOpConversion,
                CastIntOpConversion, FromMemRefOpConversion,

...
extern "C" void memrefCopy(int64_t elemSize, UnrankedMemRefType<char> *srcArg,
    ...
    int64_t *srcStrides =
        static_cast<int64_t *>(alloca(sizeof(int64_t) * rank));
    int64_t *dstStrides =
        static_cast<int64_t *>(alloca(sizeof(int64_t) * rank));
...
#define IMPL_STDSORT(VNAME, V)                                                 \
    extern "C" void _mlir_ciface_stdSort##VNAME(                               \
        uint64_t n, StridedMemRefType<V, 1> *vref) {                           \
        assert(vref);                                                          \

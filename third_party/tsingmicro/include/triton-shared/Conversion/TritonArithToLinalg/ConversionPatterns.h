        ...
        for (auto op = ops.rbegin(); op != ops.rend(); ++op) {
            src = TypeSwitch<Operation *, Value>(*op)
                      .Case<arith::SIToFPOp>([&](Operation *op) {
                          ...
                          return rewriter.create<arith::SIToFPOp>(loc, resType,
                                                                  src);
                      })
                      .Case<arith::TruncFOp>([&](Operation *op) {
                          ...
                          return rewriter.create<arith::TruncFOp>(loc, resType,
                                                                  src);
                      })
...
struct MakeTensorPtrConverter
    ...
    matchAndRewrite(triton::MakeTensorPtrOp op, OpAdaptor adaptor,
        ...
        populateVectorAsIndex(pointerState.offsets, op.getOffsets(), rewriter,
                              loc);
        populateVectorAsIndex(pointerState.strides, op.getStrides(), rewriter,
                              loc);
        ...
        for (auto [offset, stride] :
            ...
            auto mulOp = rewriter.create<arith::MulIOp>(
                loc, cast<Value>(offset), cast<Value>(stride));
        ...
        if (auto shapedType =
                dyn_cast<ShapedType>(pointerType.getPointeeType())) {
            resultShape = shapedType.getShape();
            for (auto dim_size : resultShape) {
                pointerState.sizes.push_back(IntegerAttr::get(
                    IntegerType::get(op.getContext(), 64), dim_size));
            }
        } else {
...
struct LoadConverter : public OpConversionPattern<triton::LoadOp> {
  private:
    ...
    void createStackedCopies(Value block1, Value block2, Value dst,
                             Location loc,
                             ConversionPatternRewriter &rewriter) const {
  ...
  public:
    ...
    matchAndRewrite(triton::LoadOp op, OpAdaptor adaptor,
        ...
        if (!isa<ShapedType>(op.getResult().getType())) {
            auto sMemRef = PtrAnalysis::getScalarMemRef(
        ...
        if (!type) {
            ...
            // operations. The workaround is to broadcast the pointers early in
            // the address calculation. A proper fix is complicated, but at
            // least we can provide a better error message.
        ...
        if (!mask) {
            ...
            if (auto unrealizedCast =
                ...
                if (auto wrapType = unrealizedCast->getAttrOfType<StringAttr>(
                    ...
                    if (wrapType.getValue() ==
                        ModuloState::WraparoundSideBySide) {
                        createSideBySideCopies(block1, block2, alloc, loc,
                                               rewriter);
                    } else if (wrapType.getValue() ==
                               ModuloState::WraparoundStacked) {
                        createStackedCopies(block1, block2, alloc, loc,
                                            rewriter);
                    } else {
            ...
            Value tensor = rewriter.create<bufferization::ToTensorOp>(
                loc, tensorType, alloc, true /* restrict */,
                true /* writable */);
        ...
        if (other) {
            ...
            // For each dimension check if mstate.dims[i] < shape[i],
            // or-accumulate the result
            auto shape = type.getShape();
            auto accBase =
                rewriter
                    .create<arith::ConstantOp>(loc, rewriter.getBoolAttr(false))
                    .getResult();
            for (size_t i = 0; i < type.getShape().size(); i++) {
                ...
                accBase =
                    rewriter
                        .create<arith::OrIOp>(loc, accBase, cmpOp.getResult())
                        .getResult();
            }
        ...
        if (auto unrealizedCast =
            ...
            if (auto wrapType = unrealizedCast->getAttrOfType<StringAttr>(
                ...
                if (wrapType.getValue() == ModuloState::WraparoundSideBySide) {
                    auto [subview1, subview2] = mstate.getSideBySideSubviews(
                        block1, block2, loc, rewriter);

                    createSideBySideCopies(subview1, subview2, alloc, loc,
                                           rewriter);
                } else if (wrapType.getValue() ==
                           ModuloState::WraparoundStacked) {
                    auto [subview1, subview2] = mstate.getStackedSubviews(
                        block1, block2, loc, rewriter);

                    createStackedCopies(subview1, subview2, alloc, loc,
                                        rewriter);
                } else {
                    llvm_unreachable("unexpected wraparound type");
                }
            ...
            memref::SubViewOp srcSubview =
                mstate.getSubview(ptr, loc, rewriter);
            memref::SubViewOp dstSubview =
                mstate.getSubview(alloc, loc, rewriter);
...
struct LoopConverter : public OpConversionPattern<scf::ForOp> {
    ...
    matchAndRewrite(scf::ForOp op, OpAdaptor adaptor,
        ...
        PtrAnalysis::IndexMapSet
            levelToBlockArgIndex; // level -> set of block arg index to be
                                  // replaced

        PtrAnalysis::rewriteForOp(op, rewriter, levelToBlockArgIndex, 0,
                                  knownPtrs);
        return success();
    }
};
  ...
  public:
    ...
    LogicalResult matchAndRewrite(Operation *op,
        ...
        if (isa<triton::AddPtrOp>(op)) {
            return rewriter.notifyMatchFailure(
                op, "AddPtrOp will be handled separately");
        }
...
struct MakeRangeConverter : public OpConversionPattern<triton::MakeRangeOp> {
    ...
    matchAndRewrite(triton::MakeRangeOp op, OpAdaptor adaptor,
        ...
        SmallVector<AffineMap> indexingMaps{AffineMap::get(
            ...
            SmallVector<AffineExpr>{mlir::getAffineDimExpr(0, context)},
            context)};
...
struct AssertConverter : public OpConversionPattern<triton::AssertOp> {
    ...
    matchAndRewrite(triton::AssertOp op, OpAdaptor adaptor,
        ...
        if (isa<mlir::TensorType>(condVal.getType())) {
            auto scalarVal =
                getScalarValue(op.getCondition(), op.getLoc(), rewriter);
...
struct PrintOpConverter : public OpConversionPattern<triton::PrintOp> {
    ...
    matchAndRewrite(triton::PrintOp op, OpAdaptor adaptor,
        ...
        for (size_t i = 0; i < op.getNumOperands(); i++) {
            ...
            // If the operand is not a ranked tensor, we should create a new
            // tensor. See
            // mlir/lib/Interfaces/DestinationStyleOpInterface.cpp#L39
            if (!isa<RankedTensorType>(operand.getType())) {
                auto operandTensor = rewriter.create<tensor::FromElementsOp>(
                    loc, RankedTensorType::get({}, operand.getType()), operand);
                rewriter.create<mk::PrintOp>(
                    loc, operandTensor.getType(), op.getPrefix(), op.getHex(),
            ...
            if (operandType.getRank() != 1) {
                SmallVector<int64_t> flatten_shape = {
                    operandType.getNumElements()};
                auto targetType = RankedTensorType::get(
                    flatten_shape, operandType.getElementType());
    ...
    matchAndRewrite(triton::CallOp op, OpAdaptor adaptor,
        ...
        if (FuncOp parentFunc = op->getParentOfType<triton::FuncOp>()) {
            ...
            if (ModuleOp module = op->getParentOfType<ModuleOp>()) {
                if (FuncOp calleeFunc =
                        module.lookupSymbol<FuncOp>(calleeName)) {
                    size_t argsNeed =
                        calleeFunc.getFunctionType().getInputs().size();
...
struct SplitConverter : public OpConversionPattern<triton::SplitOp> {
    ...
    matchAndRewrite(triton::SplitOp op, OpAdaptor adaptor,
        ...
        SmallVector<OpFoldResult> offsets(shape.size(),
    ...
    matchAndRewrite(triton::JoinOp op, OpAdaptor adaptor,
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            result = rewriter.create<tensor::InsertSliceOp>(
                loc, inputs[i], result, offsets, sizes, strides);
        }
...
struct DotScaledConverter : public OpConversionPattern<triton::DotScaledOp> {
    ...
    matchAndRewrite(triton::DotScaledOp op, OpAdaptor adaptor,
        ...
        Value res = rewriter
                        .create<mk::DotScaledOp>(loc, TypeRange{op.getType()},
                                                 a, aScale, b, bScale, zeroes,
  ...
  private:
    ...
    bool isReductionOpSupported(Operation *redOp) const {
        return isa<arith::AddFOp, arith::AddIOp, arith::MulIOp, arith::MulFOp,
                   ...
                   arith::MaxSIOp, arith::MaxUIOp, arith::XOrIOp, arith::AndIOp,
                   arith::OrIOp>(redOp);
    }
        ...
        return llvm::TypeSwitch<Operation *, Value>(redOp)
            .Case<arith::AddFOp, arith::MulFOp>([&](auto redOp) {
                if (convertLhsToF32Precision) {
                    lhs = b.create<arith::ExtFOp>(
                        loc, Float32Type::get(b.getContext()), lhs);
                }
                return b.create<decltype(redOp)>(loc, lhs, rhs);
            })
            .Case<arith::AddIOp, arith::MulIOp, arith::MaximumFOp,
    ...
    convertToLinalgReduce(triton::ReduceOp op,
        ...
        // Reduction of arbitrary operations isn't supported because using the
        // first element across the reduction dimension requires us to iterate
        // over a subview that skips over each first element.
        ...
        if (isVectorReduce) {
            // The affine vectorizer cannot vectorize affine loops generated
            // from linalg.reduce for the vector reduce case, so we must rewrite
            // the linalg.reduce to affine loops manually. Here we lower to
            // AllocTensor directly instead of EmptyOp so that the subsequent
            // pass can recognize the patterns (EmptyOp is susceptible to being
            // CSE'd away, making it harder to match the patterns correctly).
            initTensor = rewriter.create<bufferization::AllocTensorOp>(
                loc, RankedTensorType::get({}, constantType), ValueRange{});
            initTensor = rewriter.create<tensor::InsertOp>(
                loc, accBaseConstOp, initTensor, ValueRange{});
        } else {
            ...
            rewriter
                .create<linalg::ReduceOp>(
                    ...
                    [&](OpBuilder &opBuilder, Location loc, ValueRange inputs) {
                        ...
                        Value result =
                            getRedElement(inputs[0], inputs[1], loc, rop,
        ...
        if (sourceType.getRank() == 1) {
            finalResult = rewriter.create<tensor::ExtractOp>(loc, constantType,
                                                             finalResult);
        }

        if (convertToF32Precision) {
            finalResult =
                rewriter.create<arith::TruncFOp>(loc, resType, finalResult);
        }
...
class ArgMinMaxBaseConverter : public OpConversionPattern<triton::ReduceOp> {
  ...
  public:
    ArgMinMaxBaseConverter(MLIRContext *context)
    ...
    LogicalResult match(ReduceOp op) const override final {
        ...
        if (failed(matchArgMinMax(currValue, currIndex, reduceValue,
                                  reduceIndex, opsIt, indexSelectOp,
    ...
    void rewrite(ReduceOp op, OpAdaptor adaptor,
        ...
        // Set the initial value of the rank-0 tensor containing the index of
        // the min or max value to -1
        ...
        // Get the shape of the resulting tensors (both for values and indices).
        // If we are reducing to a single scalar, then the result's type is a
        // tensor of rank-0, otherwise we can reuse the original result shape
        ...
        SmallVector<Value> outputs{getInitTensor(rewriter, reductionResultShape,
                                                 valuesAccBaseVal, loc),
                                   getInitTensor(rewriter, reductionResultShape,
                                                 indicesAccBaseVal, loc)};
...
struct MinMaxConverter : public OpRewritePattern<CmpOp> {
    ...
    void rewriteOpWithMinMax(PatternRewriter &rewriter, arith::CmpFOp cmpOp,
        ...
        case arith::CmpFPredicate::OGE:
            rewriter.replaceOpWithNewOp<arith::MaximumFOp>(
                selectOp, cmpOp.getLhs(), cmpOp.getRhs());
            break;
    ...
    void rewriteOpWithMinMax(PatternRewriter &rewriter, arith::CmpIOp cmpOp,
        ...
        case arith::CmpIPredicate::sgt:
            rewriter.replaceOpWithNewOp<arith::MaxSIOp>(
                selectOp, cmpOp.getLhs(), cmpOp.getRhs());
            break;
        ...
        case arith::CmpIPredicate::slt:
            rewriter.replaceOpWithNewOp<arith::MinSIOp>(
                selectOp, cmpOp.getLhs(), cmpOp.getRhs());
            break;
        case arith::CmpIPredicate::ult:
            rewriter.replaceOpWithNewOp<arith::MinUIOp>(
                selectOp, cmpOp.getLhs(), cmpOp.getRhs());
            break;
        default:
            llvm_unreachable("Unhandled predicate");
...
class CumSumConverter : public OpConversionPattern<triton::ScanOp> {
    ...
    bool isCumSum(triton::ScanOp op) const {
        ...
        if (isa<arith::AddFOp, arith::AddIOp>(addOp)) {
            if (addOp->getResult(0) !=
                scanBlock->getTerminator()->getOperand(0)) {
                return false;
            }

            auto blockArgs = llvm::map_range(
                scanBlock->getArguments(),
...
struct GatherConverter : public OpConversionPattern<triton::GatherOp> {
    ...
    matchAndRewrite(triton::GatherOp op, OpAdaptor adaptor,
        ...
        auto gatherOp = rewriter.create<mk::GatherOp>(
            op.getLoc(), op.getType(), op.getSrc(), op.getIndices(), dstInit,
            op.getAxis());
...
class ExternElementwiseFiniteOpConverter
  ...
  public:
    ...
    matchAndRewrite(triton::ExternElementwiseOp op, OpAdaptor adaptor,
        ...
        Value notNaN = rewriter.create<arith::CmpFOp>(
            loc, arith::CmpFPredicate::OEQ, x, x);
        ...
        auto constTensor = rewriter.create<tensor::EmptyOp>(
            loc, shapedType.getShape(), elemType);
...
class ExternElementwiseFmodOpConverter
  ...
  public:
    ...
    matchAndRewrite(triton::ExternElementwiseOp op, OpAdaptor adaptor,
        ...
        auto mulResult =
            rewriter.create<arith::MulFOp>(loc, truncResult, input2);
...
static void populateExternElementwiseOpToMLIROps(RewritePatternSet &patterns) {
    patterns.add<
        ...
        ExternElementwiseBinaryOpConverter, ExternElementwiseUnaryOpConverter>(
        patterns.getContext());
}
...
struct HistogramOpConversion : public OpConversionPattern<triton::HistogramOp> {
    ...
    matchAndRewrite(triton::HistogramOp op, OpAdaptor adaptor,
        ...
        // NOTE: Triton only support rank1. But flatten the tensor to 1D can
        // always implement the histogram operation.
        if (srcTy.getRank() != 1) {
        ...
        Value one = rewriter.create<arith::ConstantOp>(
            loc, resTy, rewriter.getOneAttr(resTy));
        ...
        auto linalgOp = rewriter.create<linalg::GenericOp>(
            ...
            ValueRange{allocatedRangeVec}, indexingMaps,
        ...
        auto forOp = rewriter.create<scf::ForOp>(
            ...
            [&](OpBuilder &builder, Location loc, Value iv,
                ...
                Value elemVec =
                    builder.create<tensor::FromElementsOp>(loc, elems);

                // Compare with range vector
                Value mask = builder.create<arith::CmpIOp>(
                    loc, arith::CmpIPredicate::eq, elemVec,
                    linalgOp.getResult(0));
                // Select based on mask
                Value delta = builder.create<arith::SelectOp>(loc, resTy, mask,
                                                              one, zero);
                // Add to running result
                Value newRes =
                    builder.create<arith::AddIOp>(loc, currentRes, delta);
    ...
    TypedAttr makeRangeAttr(RankedTensorType resTy,
        ...
        if (elemTy.isInteger(32)) {
            ...
            llvm_unreachable("unsupported src elem type for histogram "
                             "(expected i32 or i64)");
        }
    }
};
...
struct ScanOpConverter : public OpConversionPattern<triton::ScanOp> {
  private:
    ...
    SmallVector<Value> lowering(ValueRange inputs, triton::ScanOp op,
        ...
        std::transform(inputs.begin(), inputs.end(), res.begin(),
                       [&](auto val) {
        ...
        std::function<void(unsigned)> buildLoops = [&](unsigned d) {
            ...
            Value lowerBound = rewriter.create<arith::ConstantIndexOp>(loc, 0);
            Value upperBound =
                rewriter.create<arith::ConstantIndexOp>(loc, shape[d]);
            Value step = rewriter.create<arith::ConstantIndexOp>(loc, 1);

            SmallVector<Value> curRes =
                d == 0 ? res
                       : loops[d - 1].getInitArgs().take_front(res.size());
            auto loop = rewriter.create<scf::ForOp>(loc, lowerBound, upperBound,
                                                    step, ValueRange{curRes});
            ...
            if (d == axis - 1) {
                ...
                dynamicIndices.insert(
                    dynamicIndices.end(), shape.size() - axis,
                ...
                SmallVector<int64_t> reshape_shape(shape.begin() + axis,
                                                   shape.end());

                SmallVector<Value> subInputs(inputs.size());
                std::transform(
                    inputs.begin(), inputs.end(), subInputs.begin(),
                    [&](auto val) {
                        auto extract_tensor =
                            rewriter.create<tensor::ExtractSliceOp>(
                                loc,
                                RankedTensorType::get(
                                    extract_shape, inputType.getElementType()),
                        ...
                        auto shapeAttr =
                            rewriter.getI64TensorAttr(reshape_shape);
                        auto shapeConst =
                            rewriter.create<arith::ConstantOp>(loc, shapeAttr);
    ...
    lowerTrailingDimension(triton::ScanOp op,
        ...
        // TODO: Optimization: The last two dimensions' data can be read with
        // column stride and computed in parallel.
    ...
    Value lookupMappedValue(IRMapping &localMap, Value val,
        ...
        if (!res) {
            ...
            // region. We check if it has been already translated and add a
            // splat operation if it hasn't.
            res = invariantsMap.lookupOrNull(val);
            if (!res) {
                auto ip = rewriter.saveInsertionPoint();
                rewriter.setInsertionPointAfterValue(val);
                res = rewriter.create<tensor::SplatOp>(
                    val.getLoc(), RankedTensorType::get(shape, val.getType()),
                    val);
    ...
    SmallVector<Value> lower1DInput(ValueRange inputs, ScanOp op,
        ...
        std::transform(
            inputs.begin(), inputs.end(), acc.begin(), [&](auto val) {
        ...
        auto forOp = rewriter.create<scf::ForOp>(
            ...
            [&](OpBuilder &b, Location loc, Value iv, ValueRange iterArgs) {
                ...
                if (reverse) {
                    auto actualIdx =
                        b.create<arith::SubIOp>(loc, upperBound, iv);
                ...
                std::transform(inputs.begin(), inputs.end(), inputsElem.begin(),
                               [&](auto val) {
                ...
                for (int i = 0; i < res.size(); ++i) {
                    curRes[i] = rewriter.create<tensor::InsertOp>(
                        loc, currAcc[i], curRes[i], idxIndex);
                }
    ...
    lowerLeadingDimension(ValueRange inputs, ScanOp op,
        ...
        SmallVector<Value> acc(inputs.size());
        std::transform(inputs.begin(), inputs.end(), acc.begin(),
                       [&](auto val) {
                           return rewriter.create<tensor::EmptyOp>(
                               loc, accShape, inputType.getElementType());
                       });
                ...
                std::transform(
                    ...
                    [&](auto val) {
                        return b.create<tensor::ExtractSliceOp>(
                            loc,
                            RankedTensorType::get(
                                sizeVal, cast<RankedTensorType>(resTypes[0])
                                             .getElementType()),

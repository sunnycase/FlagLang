...
struct MaskState {
    ...
    bool isMask() const {
        return !start && !end && !scalar && dims.size() != 0;
    }
  ...
  private:
    ...
    LogicalResult addStates(const MaskState &lhsState,
    ...
    LogicalResult minStates(const MaskState &lhsState,
                            const MaskState &rhsState, Location loc,
                            OpBuilder &builder);
    // -------
    ...
    //  1. Constant comparison with both left and right-hand sides being
    //  scalars.
    ...
    LogicalResult parseMakeRange(triton::MakeRangeOp rangeOp,

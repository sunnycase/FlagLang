    ...
    for (const auto &[inDim, inDimBases] : bases) {
        for (const auto &basis : inDimBases) {
            if (basis.size() != outDimNames.size()) {
                llvm::report_fatal_error(
                    "Invalid bases passed to LinearLayout.  Expect all bases "
                    "to have "
...
int getMatrixRank(const LinearLayout &layout, StringAttr outDim) {
    ...
    //   L(2,0) = 0b11
    //
    // We will create one column per key value.  The max bit width of these
    // values is 2, so our matrix will have 2 rows.  The final matrix will be
    //
    //  |    ↑      ↑      ↑      ↑   |   | 0b0111 |
    ...
    // Note that `new int[n]()` is zero-initialized, whereas `new int[n]` is
    // not.
    ...
    for (const auto &outDim : layout.getOutDimNames()) {
        ...
        if (rank != expectedRank) {
            llvm::report_fatal_error(
                ...
                "surjective, i.e. all possible output coordinates can be "
                "reached "
                "by some input coordinates.  But this failed for output "
                "dimension " +
                Twine(outDim) + ", where we got rank " + Twine(rank) +
                " instead of expected rank " + Twine(expectedRank) +
                ".  Full list of bases:\n" +
                Twine(stringifyBases(layout.getBases(),
                                     layout.getOutDimNames())));
        }
    }
}
...
LinearLayout operator*(LinearLayout inner, LinearLayout outer) {
    ...
    auto checkCommonDims = [&](auto outerDimsRange, auto innerDimsRange) {
        ...
        if (outerCommonDims != innerCommonDims) {
            llvm::report_fatal_error(
                "Cannot multiply layouts.  All in/out dimensions common to "
                "both "

...
scanThreadContiguousElements(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        accs[accIndex] = accumulate(rewriter, helper.getCombineOp(),
                                    accs[accIndex], srcValues[srcIndex]);
...
static void storeWarpAccumulator(SmallVector<SmallVector<Value>> &srcValues,
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        Value index =
            add(parallelLaneId, mul(warpId, i32_val(numParallelLane)));
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        for (unsigned i = 0; i < axisNumWarps; ++i) {
            ...
            for (unsigned j = 0; j < helper.getNumOperands(); ++j) {
                ...
                Value ptr = gep(ptr_ty(rewriter.getContext(), 3), elemTy,
                                smemBases[j], index);
        ...
        for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
            ...
            lastElement[i] =
                select(maskFirstLane, accumulator.maskedAcc[i], elem);
        }
        for (unsigned i = 1; i < scanElementsPerThreads; ++i) {
            auto laneValue = srcValues[srcIndex - i * elementStride];
            laneValue = accumulate(rewriter, helper.getCombineOp(), lastElement,
                                   laneValue);
            if (axisBlockId == 0) {
                // For the first warp and first chunk we don't have anything to
                // accumulate.
    ...
    for (unsigned srcIndex = 0; srcIndex < srcValues.size(); srcIndex++) {
        ...
        if (scanDim > 1) {
            for (unsigned i = 0; i < helper.getNumOperands(); ++i) {
                ...
                lastElement[i] =
                    select(maskFirstLane, accumulator[i], lastElement[i]);
                if (numScanBlocks > 1)
...
ScanOpConversion::emitFastScan(triton::ScanOp op, triton::ScanOpAdaptor adaptor,
    ...
    if (axisNumWarps > 1) {
        ...
        auto threadsPerWarp =
            triton::gpu::getThreadsPerWarp(helper.getEncoding());
        auto laneIdLast =
            linearize(rewriter, loc, multiDimLaneId, threadsPerWarp,

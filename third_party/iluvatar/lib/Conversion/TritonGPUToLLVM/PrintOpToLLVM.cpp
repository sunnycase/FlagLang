...
struct PrintOpConversion : public ConvertOpToLLVMPattern<triton::PrintOp> {
    ...
    matchAndRewrite(triton::PrintOp op, OpAdaptor adaptor,
        ...
        for (size_t i = 0; i < op.getNumOperands(); i++) {
            ...
            auto elems =
                unpackLLElements(loc, adaptor.getOperands()[i], rewriter);

            // Get the indices of `elems` within the tensor.  Note that if
            // `elems` has an "interesting" layout, then these will not be in
            // any particularly nice order.

            // Extract the shape of the tensor being printed and use it to
            // figure out how many digits we need for each of the dimensions.
            ...
            if (auto rankedTy =
                ...
                for (int64_t dim : rankedTy.getShape()) {
                    if (dim > 0) {
                        dimWidths.push_back(
                            static_cast<int>(std::ceil(std::log10(dim))));
                    } else {
            ...
            if (!elems.empty()) {
                printTensor(op.getPrefix(), /*operand=*/i,
                            /*numOperands=*/op.getNumOperands(), elems, pid,
                            indices, dimWidths, op.getHex(), rewriter);
            }
        }
        rewriter.eraseOp(op);
        return success();
    ...
    void printTensor(StringRef prefixStr, size_t operand, size_t numOperands,
        ...
        for (int i = 0; i < elems.size(); i++) {
            ...
            // nvptx printf can only accept 32 args; if we pass more than that,
            // it will print garbage for the trailing args.
            constexpr int kMaxPrintfOperands = 32;
            SmallVector<Value, kMaxPrintfOperands> printfOperands;

            // TODO(jlebar): We really should pad the pid, but because the max
            // pid is not known at compile-time, this would require nontrivial
            // device-side work.
            os << "pid (";
            ...
            // (Subtract 2 because we're going to add two operands after the
            // index.)
    ...
    std::string getFormatSubstr(Value value, bool hex = false,
        ...
        if (width.has_value()) {
            ...
            prefix += "0";
            prefix +=
                std::to_string(value.getType().getIntOrFloatBitWidth() / 4);
        }
    ...
    // Returns a Value for the format string, which you can reuse. Writes the
    // byte count for the string to |formatStrByteCount| if not null.

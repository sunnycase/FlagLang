...
SmallVector<Value> reorderValues(const SmallVector<Value> &values, Type inType,
    ...
    if (!inTensorTy || !ouTensorTy)
        return values;
    auto inEncoding =
        dyn_cast<DotOperandEncodingAttr>(inTensorTy.getEncoding());
    auto ouEncoding =
        dyn_cast<DotOperandEncodingAttr>(ouTensorTy.getEncoding());
    ...
    // reorder elements
    auto parentEncoding =
        dyn_cast<NvidiaMmaEncodingAttr>(ouEncoding.getParent());
    if (!parentEncoding)
        return values;
...
int getNumElementsPerThreads(Type type,
    ...
    assert(eltType.getIntOrFloatBitWidth() <= 32 &&
           ...
           "layout");
    // dot operand data are packed into i32 elements so use the following
    // formula to get the number of elements per thread.
    ...
    matchAndRewrite(AddPtrOp op, OpAdaptor adaptor,
        ...
        if (resultTensorTy) {
            ...
            Type elemTy = typeConverter->convertType(
                cast<PointerType>(resultTensorTy.getElementType())
                    .getPointeeType());
            Type ptrTy =
                typeConverter->convertType(resultTensorTy.getElementType());
            auto ptrs = unpackLLElements(loc, adaptor.getPtr(), rewriter);
            ...
            Value view = packLLElements(loc, typeConverter, resultVals,
                                        rewriter, resultTy);
            ...
            Value result = gep(resultPtrTy, resultElemTy, adaptor.getPtr(),
                               adaptor.getOffset());
...
struct ElementwiseInlineAsmOpConversion
    ...
    SmallVector<Value> packOperands(ElementwiseInlineAsmOp op,
        ...
        for (int i = 0, e = op.getNumOperands(); i < e; i++) {
            ...
            for (int j = 0; j < numPackedElements; j += numElementPerReg) {
                ...
                Type t = vec_ty(getTypeConverter()->convertType(elemTy),
                                numElementPerReg);
                Value packed = undef(t);
                for (int k = 0; k < numElementPerReg; k++) {
                    packed =
                        insert_element(packed, operands[j + k][i], i32_val(k));
                }
    ...
    createDestOps(ElementwiseInlineAsmOp op, OpAdaptor adaptor,
        ...
        if (operands.size() % op.getPackedElement() != 0)
            llvm::report_fatal_error(
                "Inline asm op has more packed elements than "
        ...
        // Types returned by the LLVM asm op.  If there's more than one, they'll
        ...
        for (auto result : op.getResult()) {
            ...
            unsigned bitWidth =
                ty.isIntOrFloat() ? ty.getIntOrFloatBitWidth() : 64;
            unsigned numElemsPerReg = std::min(
                bitWidth < 32 ? 32 / bitWidth : 1, op.getPackedElement());
    ...
    matchAndRewrite(ElementwiseInlineAsmOp op, OpAdaptor adaptor,
        ...
        for (auto operand : adaptor.getOperands()) {
            ...
            unpackedOperands.push_back(unpackI32(subOperands, argTy, rewriter,
                                                 loc, getTypeConverter()));
        }

        int numElemsPerThread = getNumElementsPerThreads(
            op->getResult(0).getType(), getTypeConverter());
        ...
        if (numElemsPerThread % op.getPackedElement() != 0) {
            ...
            int numPaddedValue = op.getPackedElement() -
                                 numElemsPerThread % op.getPackedElement();
        ...
        for (unsigned i = 0; i < numElemsPerThread;
             i += op.getPackedElement()) {
            // Block of elements to process with one call to the inline asm.
            // This is ordered opposite `unpackedResults`: The outer dim is
            ...
            for (unsigned j = 0; j < cur.size(); j++) {
                unpackedResults[j].insert(unpackedResults[j].end(),
        ...
        for (int i = 0; i < unpackedResults.size(); i++) {
            ...
            auto packed =
                packI32(unpackedResults[i], op->getResult(i).getType(),
                        rewriter, loc, getTypeConverter());
            outs.push_back(packLLElements(loc, getTypeConverter(), packed,
                                          rewriter,
    ...
    SmallVector<Value> createDestOps(arith::IndexCastOp op, OpAdaptor adaptor,
        ...
        if (targetBits < sourceBits)
            return {rewriter.replaceOpWithNewOp<LLVM::TruncOp>(op, elemTy,
                                                               operands[0][0])};
        return {rewriter.replaceOpWithNewOp<LLVM::SExtOp>(op, elemTy,
                                                          operands[0][0])};
    }
};
...
struct SelectOpConversion
    ...
    SmallVector<Value> createDestOps(arith::SelectOp op, OpAdaptor adaptor,
        ...
        if (operands[0].size() == 2) {
            ...
            llvmOperands = {adaptor.getCondition(), operands[0][0],
                            operands[0][1]};
        } else {
...
struct MinMaxFOpConversion
    ...
    SmallVector<Value> createDestOps(OpTy op, Adaptor adaptor,
        ...
        auto lhsIsNan = rewriter.create<LLVM::FCmpOp>(
            loc, LLVM::FCmpPredicate::une, lhs, lhs);
        auto rhsIsNan = rewriter.create<LLVM::FCmpOp>(
            loc, LLVM::FCmpPredicate::une, rhs, rhs);
        auto isNan = rewriter.create<LLVM::OrOp>(loc, lhsIsNan, rhsIsNan);
        auto nonNanRes =
            rewriter.create<DestOpNoNanProp>(loc, elemTy, lhs, rhs);
    ...
    SmallVector<Value> createDestOps(ClampFOp op, OpAdaptor adaptor,
        ...
        if (op.getPropagateNan() == PropagateNan::ALL) {
            if (targetInfo.supportMaximumMinimum()) {
                auto v = rewriter.create<LLVM::MaximumOp>(
                    loc, elemTy, operands[0][0], operands[0][1]);
                return {
                    rewriter.create<LLVM::MinimumOp>(loc, v, operands[0][2])};
            }
            // On pre-80 compute capability, we need to handle NaN propagation
            // manually. We need to check only the first operand for clamp.
            auto lhs = operands[0][0];
            auto isNan = rewriter.create<LLVM::FCmpOp>(
                loc, LLVM::FCmpPredicate::une, lhs, lhs);
            auto v = rewriter.create<LLVM::MaxNumOp>(
                loc, elemTy, operands[0][0], operands[0][1]);
            auto nonNanRes =
                rewriter.create<LLVM::MinNumOp>(loc, v, operands[0][2]);
            auto nan = LLVM::createNaNConstant(loc, rewriter, elemTy);
            // Select the result based on the isNan flag.
            return {
                rewriter.create<LLVM::SelectOp>(loc, isNan, nan, nonNanRes)};
        }

        ...
        for (int i = 0; i < nBits; i++) {
            ...
            for (auto &[outDimName, outIdx] : outIndices) {
                ...
                if (basis == 0)
                    continue;
                outIdx =
                    xor_(outIdx, select(bit_is_zero, zero, i32_val(basis)));
            }
        }
    }
...
SmallVector<Value> getMultiDimOffset(Attribute layout, Location loc,
    ...
    if (auto mmaLayout = mlir::dyn_cast<NvidiaMmaEncodingAttr>(layout)) {
        ...
        if (mmaLayout.isHopper()) {
            ...
            multiDimOffset[1] =
                elemIdRem4 % 2 == 0 ? mmaColIdx[0] : mmaColIdx[1];
            ...
            multiDimOffset[rank - 2] = add(
                multiDimOffset[rank - 2], i32_val(multiDimCTAInRepId[rank - 2] *
                                                  shapePerCTATile[rank - 2]));
            multiDimOffset[rank - 1] = add(
                multiDimOffset[rank - 1], i32_val(multiDimCTAInRepId[rank - 1] *
    ...
    if (auto mmaLayout = mlir::dyn_cast<IluvatarMmaEncodingAttr>(layout)) {
        ...
        if (mmaLayout.isVolta()) {
            ...
            auto coords =
                func(threadId, loc, rewriter, mmaLayout.getWarpsPerCTA(),
    ...
    if (isa<AMDMfmaEncodingAttr, AMDWmmaEncodingAttr>(layout)) {
        auto multiDimBase = emitBaseIndexForLayout(loc, rewriter, targetInfo,
                                                   layout, type, false);
...
SmallVector<Value> getWrappedMultiDimOffset(
    ...
    for (unsigned d = 0; d < rank; ++d) {
        if (shapePerCTATile[d] > shapePerCTA[d])
            multiDimOffsetWrapped[d] =
                urem(multiDimOffset[d], i32_val(shape[d]));
        else

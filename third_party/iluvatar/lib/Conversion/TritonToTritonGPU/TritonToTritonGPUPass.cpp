    ...
    matchAndRewrite(arith::ConstantOp op, OpAdaptor adaptor,
        ...
        if (dyn_cast<RankedTensorType>(retShapedType)) {
            ...
            if (value.getElementType().isInteger(1) && value.isSplat())
                // Workaround until https://reviews.llvm.org/D133743 is
                // included.
                value = DenseElementsAttr::get(retShapedType,
                                               value.getSplatValue<bool>());
            else
    ...
    matchAndRewrite(triton::ExpandDimsOp op, OpAdaptor adaptor,
        ...
        triton::gpu::BlockedEncodingAttr retEncoding =
            triton::gpu::BlockedEncodingAttr::get(
                ...
                retWarpsPerCTA, retOrder, retCTALayout, false, smeCTA);
...
struct TritonDotPattern : public OpConversionPattern<triton::DotOp> {
    ...
    matchAndRewrite(triton::DotOp op, OpAdaptor adaptor,
        ...
        RankedTensorType retType = RankedTensorType::get(
            origShape, origType.getElementType(), dEncoding);
...
struct TritonCatPattern : public OpConversionPattern<triton::CatOp> {
    ...
    matchAndRewrite(triton::CatOp op, OpAdaptor adaptor,
        ...
        auto newRetType = RankedTensorType::get(
            retShape, retType.getElementType(), newRetEncoding);
...
struct TritonSplitOpPattern : public OpConversionPattern<triton::SplitOp> {
    ...
    LogicalResult matchAndRewrite(SplitOp op, OpAdaptor adaptor,
        ...
        if (!srcEnc || srcEnc.getSizePerThread().back() != 2 ||
            ...
            srcTy = RankedTensorType::get(srcTy.getShape(),
                                          srcTy.getElementType(), srcEnc);
    ...
    matchAndRewrite(BroadcastOp op, OpAdaptor adaptor,
        ...
        Type retType =
            RankedTensorType::get(op.getType().getShape(),
                                  op.getType().getElementType(), srcEncoding);
  ...
  public:
    ...
    void runOnOperation() override {
        ...
        mod->setAttr(AttrNumThreadsPerWarp,
                     IntegerAttr::get(
                         i32_ty, llvm::APInt(32, threadsPerWarp.getValue())));
        ...
        if (this->target.getValue().empty()) {
            mod.emitError(
                "expected target specification to attach to the module op");

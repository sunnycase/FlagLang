...
getThreadsPerWarpWithUniqueData(Attribute layout,
    ...
    for (unsigned i = 0; i < threadsPerWarp.size(); i++) {
        threadsPerWarp[i] =
            std::min<unsigned>(threadsPerWarp[i], tensorShape[i]);
    }
...
SmallVector<int64_t> getShapePerCTA(Attribute layout, ArrayRef<int64_t> shape) {
    if (auto sharedLayout = mlir::dyn_cast<SharedEncodingAttr>(layout)) {
        // Special logic for pipeline pass, where shape is 3D and CTALayout is
        // 2D. The first dim of shape is numStages. This is a work around,
        // otherwise too many places would have to be modified in pipeline pass.
        // Maybe we need to refactor this logic in the future.
...
bool isMmaConvertLayout(Operation *op) {
    ...
    if (auto convertOp = dyn_cast<ConvertLayoutOp>(op)) {
        auto srcType =
            convertOp.getOperand().getType().cast<RankedTensorType>();
        auto dstType = convertOp.getResult().getType().cast<RankedTensorType>();
        if (!srcType || !dstType)
            return false;
        auto srcMmaEnc =
            dyn_cast<IluvatarMmaEncodingAttr>(srcType.getEncoding());
        auto dstMmaEnc =
            dyn_cast<IluvatarMmaEncodingAttr>(dstType.getEncoding());
        if (!srcMmaEnc || !dstMmaEnc)
            return false;
        return srcMmaEnc.getVersionMinor() == 0 &&
               dstMmaEnc.getVersionMinor() > 0;
    }
    return false;
}
...
LogicalResult BlockedEncodingAttr::verify(
    ...
    if (!isPermutationOfIota(order)) {
        return emitError()
               << "order must be a permutation of 0..(rank-1), but was ["
               << order << "]";
    }
    return success();
}
...
static LogicalResult parseBoolAttrValue(AsmParser &parser,
    ...
    if (!boolAttr) {
        parser.emitError(parser.getNameLoc(), "expected an bool type in ")
            << desc;
        return failure();
    }
...
SmallVector<unsigned> SliceEncodingAttr::getThreadsPerWarp() const {
    ...
    int32_t nextDim =
        getDim() < threadsPerWarp.size() ? getDim() : getDim() - 1;
...
SliceEncodingAttr::getShapePerCTATile(ArrayRef<int64_t> tensorShape) const {
    SmallVector<unsigned> shape =
        ::getShapePerCTATile(getParent(), tensorShape);
...
AMDMfmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
    ...
    if (getIsTransposed()) {
        unsigned elemsCol =
            ceil<unsigned>(shape[rank - 1],
                           nonKDim * getWarpsPerCTA()[rank - 1]) *
            elemsPerThreadPerTile;
        unsigned elemsRow = ceil<unsigned>(
            shape[rank - 2], nonKDim * getWarpsPerCTA()[rank - 2]);
        elemsPerThread[rank - 2] = elemsRow;
        elemsPerThread[rank - 1] = elemsCol;
    } else {
        unsigned elemsCol = ceil<unsigned>(
            shape[rank - 1], nonKDim * getWarpsPerCTA()[rank - 1]);
        unsigned elemsRow =
            ceil<unsigned>(shape[rank - 2],
...
NvidiaMmaEncodingAttr::getElemsPerThread(ArrayRef<int64_t> shape,
    ...
    if (isVolta()) {
        ...
        unsigned resN =
            2 * repN * std::max<int>(1, shapePerCTA[1] / (spwN * wptN));
        elemsPerThread[0] = resM;
        elemsPerThread[1] = resN;
    } else if (isAmpere()) {
        unsigned elemsRow = ceil<unsigned>(shapePerCTA[rank - 2],
                                           16 * getWarpsPerCTA()[rank - 2]) *
                            2;
        unsigned elemsCol = ceil<unsigned>(shapePerCTA[rank - 1],
                                           8 * getWarpsPerCTA()[rank - 1]) *
                            2;
        if (rank == 3)
            elemsPerThread[0] =
                ceil<unsigned>(shapePerCTA[0], getWarpsPerCTA()[0]);
        elemsPerThread[rank - 2] = elemsRow;
        elemsPerThread[rank - 1] = elemsCol;
    } else if (isHopper()) {
...
unsigned DotOperandEncodingAttr::getTotalElemsPerThread(ArrayRef<int64_t> shape,
    ...
    if (auto mmaParent = mlir::dyn_cast<MmaEncodingTrait>(getParent())) {
        return mmaParent.getTotalElemsPerThreadForOperands(
    ...
    if (auto blockedLayout = mlir::dyn_cast<BlockedEncodingAttr>(getParent())) {
        ...
        int mShapePerCTATile = order[0] == 1 ? shapePerCTATile[order[1]]
                                             : shapePerCTATile[order[0]];
        int nShapePerCTATile = order[0] == 0 ? shapePerCTATile[order[1]]
                                             : shapePerCTATile[order[0]];
        int shapePerCTAMNTile = isM ? mShapePerCTATile : nShapePerCTATile;

        return K * std::max<int>(otherDim / shapePerCTAMNTile, 1) *
               sizePerThreadMN;
    }
...
LogicalResult DotOperandEncodingAttr::verify(
    ...
    if (!parent) {
        return emitError()
               << "triton_gpu.dot_op parent paramenter cannot be null";
    }
    if (auto parentAttr = mlir::dyn_cast<NvidiaMmaEncodingAttr>(parent)) {
        if (kWidth != 0 && !parentAttr.isAmpere())
            return emitError()
                   << "triton_gpu.dot_op kWidth parameter can only be "
...
static std::optional<CTALayoutAttr> getCTALayoutOrError(
    ...
    if (CTAsPerCGA && CTASplitNum && CTAOrder) {
        return CTALayoutAttr::get(parser.getContext(), *CTAsPerCGA,
...
Attribute BlockedEncodingAttr::parse(AsmParser &parser, Type type) {
    ...
    for (const NamedAttribute &attr : dict) {
        if (attr.getName() == "sizePerThread") {
            ...
            if (parseIntArrayAttr(parser, attr, CTAsPerCGA.emplace(),
                                  "CTAsPerCGA")
                    .failed())
                return {};
            ...
            if (parseIntArrayAttr(parser, attr, smeWarpsPerCTA,
                                  "smeWarpsPerCTA")
                    .failed())
                return {};
        } else {
    ...
    std::optional<CTALayoutAttr> CTALayout =
        getCTALayoutOrError(parser, CTAsPerCGA, CTASplitNum, CTAOrder,
...
Attribute NvidiaMmaEncodingAttr::parse(AsmParser &parser, Type type) {
    ...
    for (const NamedAttribute &attr : dict) {
        ...
        if (attr.getName() == "instrShape") {
            if (parseIntArrayAttr(parser, attr, instrShape, "instrShape")
                    .failed()) {
                return {};
            }
        }
...
Attribute SliceEncodingAttr::parse(AsmParser &parser, Type type) {
    ...
    return parser.getChecked<SliceEncodingAttr>(parser.getContext(), dim,
                                                parent, noWarpReduce);
}
...
AMDMfmaEncodingAttr::getShapePerCTATileForDotOperands(ArrayRef<int64_t> shape,
    ...
    if (opIdx == 0) {
        ...
        if (rank == 2)
            ...
            return {parentShapePerCTATile[0], 32,
...
SmallVector<unsigned> NvidiaMmaEncodingAttr::getSizePerThread() const {
    ...
    if (isHopper()) {
        ...
        // unique thread ids (32 in a warp group) per column. It is 1 warp wide
        // with 4 unique thread ids in the row. So the size per thread is the
        // instruction size divided by the number of unique thread ids.
...
SmallVector<int64_t> NvidiaMmaEncodingAttr::getMMAv2Rep(ArrayRef<int64_t> shape,
    ...
    if (opIdx == 0)
        return {numRepBatch,
                std::max<int64_t>(1, shape[rank - 2] / (shapePerWarp[1] *
                                                        warpsPerCTA[rank - 2])),
...
unsigned NvidiaMmaEncodingAttr::getTotalElemsPerThreadForOperands(
    ...
    if (isAmpere()) {
        auto rep =
            getMMAv2Rep(shapePerCTA, eltTy.getIntOrFloatBitWidth(), opIdx);
        if (opIdx == 0)
...
ParseResult AsyncCopyGlobalToLocalOp::parse(OpAsmParser &parser,
    ...
    if (parser.parseOperandList(allOperands) ||
        ...
        parser.parseColon() || parser.parseCustomTypeWithFallback(srcType))
        return failure();
...
void AsyncCopyGlobalToLocalOp::print(OpAsmPrinter &printer) {
    ...
    printer << " : ";
    if (getInputStride()) {
        printer.printStrippedAttrOrType(llvm::ArrayRef<Type>{
            getSrc().getType(), getInputStride().getType()});
    } else
    ...
    AliasResult getAlias(Attribute attr, raw_ostream &os) const override {
        if (auto mmaAttr = mlir::dyn_cast<MmaEncodingTrait>(attr)) {
            ...
            return AliasResult::FinalAlias;
                       ...
                       mlir::dyn_cast<BlockedEncodingAttr>(attr)) {
            os << "blocked";
            return AliasResult::FinalAlias;
...
struct TritonGPUInferLayoutInterface
    ...
    inferReduceOpEncoding(Attribute operandEncoding, unsigned axis,
        ...
        resultEncoding = SliceEncodingAttr::get(
            getDialect()->getContext(), axis, operandEncoding, noWarpReduce);
        return success();
    }
    ...
    // elements, they're just "renamed" -- what was element [i,j] of the tensor
    // is now element [j,i], but that element is held by the same GPU thread.
    //
    ...
    //
    // This works because prop[i] tells you something about the i'th dimension
    // of the tensor. (For example, sizePerThread[2] == 4 means that one GPU
    // thread contains 4 elements along dim 2 of the tensor.) The transpose
    // reorders the dimensions according to the perm trans.order, so we achieve
    // our goal of having a "nop" transpose by reordering the values in the prop
    // the same way.
    //
    ...
    //
    // But!  If we invert enc.order, it *does* meet this criterion.  For
    // example, if enc.order = [2,0,1], inverse(enc.order) = [1,2,0].  If you
    // stare at it, you'll see that inverse(enc.order)[i] == j means that
    // dimension i is the j'th most minor.  Therefore we can safely permute
    // *this* by trans.order.
    //
    // Thus we have
    //
    ...
    inferTransOpEncoding(Attribute operandEncoding,
                         ...
                         Attribute &resultEncoding) const override {
        // Note: inferFooOpEncoding should not crash if given invalid inputs,
        // which happens when someone creates invalid IR.  If we return
        // failure() on error, then MLIR will generate a helpful error message.

        auto invOrder = inversePermutation(order);
        SmallVector<unsigned> invOrderUnsigned(invOrder.begin(),
                                               invOrder.end());
        ...
        if (auto enc = mlir::dyn_cast<SharedEncodingAttr>(operandEncoding)) {
            ...
            resultEncoding = SharedEncodingAttr::get(
                ...
                enc.getHasLeadingOffset(), enc.getUseTcu());
            return success();
        }
    ...
    inferDotOpEncoding(Attribute operandEncoding, unsigned opIdx,
                       ...
                       std::optional<Location> location) const override {
        auto mmaRetEncoding =
            mlir::dyn_cast<NvidiaMmaEncodingAttr>(retEncoding);
        if (mmaRetEncoding && mmaRetEncoding.isHopper()) {
            ...
            if (!mlir::isa<SharedEncodingAttr>(operandEncoding) &&
                ...
                return emitOptionalError(
                    location,
                    "unexpected operand layout for NvidiaMmaEncodingAttr v3");
            }
        } else if (auto dotOpEnc = mlir::dyn_cast<DotOperandEncodingAttr>(
                       operandEncoding)) {
    ...
    LogicalResult verifyDotOpEncodingCompatibility(
        Operation *op, Attribute operandEncodingA,
        Attribute operandEncodingB) const override {
        auto aEncoding = mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(
            operandEncodingA);
        auto bEncoding = mlir::dyn_cast<triton::gpu::DotOperandEncodingAttr>(
            operandEncodingB);
        if (!aEncoding && !bEncoding)
            return mlir::success();
        auto mmaAEncoding = mlir::dyn_cast_or_null<NvidiaMmaEncodingAttr>(
            aEncoding.getParent());
        ...
        if (!aEncoding || !bEncoding)
            return op->emitError(
                "mismatching encoding between A and B operands");
        if (aEncoding.getKWidth() != bEncoding.getKWidth())
            return op->emitError("mismatching kWidth between A and B operands");
        return success();
    }

    // Given a src shape + encoding and a dst shape, our goal is to compute a
    // dst encoding that makes the reshape a "nop".  That is, if GPU thread
    // [x,y,z] contains elements [a,b,c,d] before the reshape, it contains those
    // same elements after the reshape, they're just "renamed".
    //
    // A dst encoding that satisfies this property does not exist for all
    // inputs. Here are some positive and negative examples.
    //
    //   - NOT OK: 4x4 order=[0,1] -> 16.  Reshape merges elements so
    //     dim 1 is the fastest-changing in the dst, but the src has the
    //     opposite order.
    ...
    //     contains elements [(0,0), (0,1), (1,0), and (1,1)].  We cannot
    //     express this with an encoding based on the dst shape.
    //   - OK: 32x4 sizePerThread=[4,4] -> 128.  dst with sizePerThread=[16]
    //   will
    ...
    // srcEnc.
    LogicalResult inferReshapeOpNoReorderEncoding(
        ArrayRef<int64_t> srcShape, Attribute srcEnc,
        ...
        // should be like the other fields in blocked encoding, but I'm not sure
        // how to handle CTASplitNum.
        if (!all_of(src.getCTAsPerCGA(), [](int32_t x) { return x == 1; }) ||
            !all_of(src.getCTASplitNum(), [](int32_t x) { return x == 1; })) {
            return emitOptionalError(
                loc,
                "Non-reordering reshape does not currently support multi-CTA "
                "layouts other than the default layout.");
        }

        // Cowardly refuse to handle encodings where shape[dim] is not divisible
        // by sizePerThread[dim], threadsPerWarp[dim], and warpsPerCTA[dim]. (We
        // make an exception if the block is larger than the shape.)
        auto checkDivisibility = [&](StringRef name,
                                     ArrayRef<unsigned> subblock) {
            for (int dim = 0; dim < srcShape.size(); dim++) {
                if (srcShape[dim] >= subblock[dim] &&
                    ...
                    return emitOptionalError(
                        ...
                        dim, " (", srcShape[dim], ")", " is not divisible by ",
                        name, "[", dim, "]", " = ", subblock[dim]);
                }
            }
            return success();
        };
        if (!succeeded(
            ...
            !succeeded(
                checkDivisibility("warpsPerCTA", src.getWarpsPerCTA()))) {
            return failure();
        }

        SmallVector<std::pair<SmallVector<int64_t>, SmallVector<int64_t>>>
            decomp = getReshapeDecomposition(srcShape, dstShape);

        // enc.order[i] == j means that dimension j is the enc.order[i]'th most
        // minor. But what we usually want is the inverse: inverse(enc.order)[i]
        // = j means that dimension i is the j'th most minor (larger means more
        // major).
        auto srcInvOrder = inversePermutation(src.getOrder());

        // If src dims [a,b,c] are to be merged, then they must be consecutive
        // in physical order, with `a` being the most major.
        for (const auto &[srcDims, dstDims] : decomp) {
            if (!isConsecutive(
                    to_vector(reverse(gather(srcInvOrder, srcDims))))) {
                return emitOptionalError(
                    loc,
                    "Cannot do a non-reordering reshape given "
                    "this src encoding order.  Dimensions [",
                    join(srcDims), "] must be physically consecutive.");
            }
        }

        // If src dims [a,b,c] are to be merged, then `c` must fill up
        // sizePerThread / threadsPerWarp / blocksPerCTA before `b` can have any
        // non-1 values. Examples:
        //
        ...
        //    Dim 2's sizePerThread covers its whole size, so dim 1 is allowed
        //    to have non-1 sizePerThread.
        //
        //  - NOT OK: shape=[4,4,4], sizePerThread=[2,1,4].
        //    Dim 1's sizePerThread does not cover its whole size, so dim 0 is
        //    not allowed to have non-1 sizePerThread.
        //
        //  - NOT OK: shape=[4,4,4], sizePerThread=[1,1,2],
        //            threadsPerWarp=[1,2,1].
        //    Dim 2 has 2 elems per thread and 1 thread per warp.  2*1 is less
        //    than dim 2's size.  Therefore dim 1 must have threadsPerWarp=1.
        //
        // In addition, the encoding's block can be larger than the shape, but
        // only in the most-major dimension of each decomposed chunk, and only
        // after we've "used up" the more minor dims.  Examples:
        //
        //  - OK: shape=[4,4,4], sizePerThread=[1,2,4], threadsPerWarp=[16,2,1],
        //        warpsPerCTA=[4,1,1].
        //    The whole size of dims 0 and 1 are covered by sizePerThread *
        //    threadsPerWarp.  Therefore dim 2 is allowed to have threadsPerWarp
        //    and warpsPerCTA larger than its size.
        for (const auto &[srcDims, dstDims] : decomp) {
            auto shapeRemaining = gather(srcShape, srcDims);
            auto checkSubblock = [&, srcDims =
                                         srcDims](ArrayRef<unsigned> subblock) {
                ...
                for (int i = srcDims.size() - 1; i >= 0; i--) {
                    ...
                    for (int j = i + 1; j < srcDims.size(); j++) {
                        if (shapeRemaining[j] != 1) {
                            return emitOptionalError(
                                loc, "Invalid src encoding for non-reordering "
                                     "reshape.  Must use "
                                     "up sizePerThread / threadsPerWarp / "
                                     "warpsPerCTA for "
                                     "more-minor dimensions before more "
                                     "major-dims can use them.");
                        }
                    }

                    if (shapeRemaining[i] >= subblock[dim]) {
                        assert(shapeRemaining[i] % subblock[dim] ==
                               0); // checked earlier
                    ...
                    // Is the block larger than the shape in this dimension?
                    // This is OK only if we're the most-major dimension of the
                    // chunk and in all future chunks, only this most-major dim
                    // has a non-1 size.
                    if (shapeRemaining[i] == 0 && i != 0) {
                        return emitOptionalError(
                            loc,
                            "Invalid src encoding for non-reordering reshape.  "
                            "Block "
                            "size in dimension ",
                            dim,
                            " is larger than the shape that dimension, but "
                            "this is only "
                            "allowed for the most-major dimension of a reshape "
                            "chunk");
                    }
                }
                return success();
        ...
        // dst.getSizePerThread().  This should be called for each of
        // sizePerThread, threadsPerWarp, and warpsPerCTA, in that order.
        ...
        auto computeSubblockSize = [&](ArrayRef<unsigned> srcSubblock,
            ...
            // The dst subblock is "filled up" greedily starting with the most
            // minor dim.  When we're done, we are left with a smaller shape, of
            // size dstShape / dstSubblock, which we store in dstShapeRemaining
            // and use for the next call to computeSubblockSize.
            dstSubblock.resize(dstShape.size());
            for (const auto &[srcDims, dstDims] : decomp) {
                int64_t subblockRemaining =
                    product(gather(srcSubblock, srcDims));
                ...
                // If there are any elems remaining in the subblock, it must be
                // because the block is larger than the shape.  This excess goes
                // into the most-major dim of the subblock.
        ...
        if (!succeeded(computeSubblockSize(
                src.getSizePerThread(), dstSizePerThread, "sizePerThread")) ||
        ...
        llvm::sort(decomp, [&](const auto &a, const auto &b) {
            ...
            return srcInvOrder[srcDimsA.front()] <
                   srcInvOrder[srcDimsB.front()];
        });

        // Compute the dst order.  Make the dimensions appear in the same order
        // as their corresponding src dimensions.
        ...
        // CTALayout can be all 1's because we bailed on multi-CTA layouts
        // above.
        ...
        dstEnc = BlockedEncodingAttr::get(
            src.getContext(), dstSizePerThread, dstThreadsPerWarp,
            dstWarpsPerCTA, dstOrder, CTALayout, loadType, smeWarpsPerCTA);
    ...
    inferJoinOpEncoding(Attribute srcEnc, Attribute &dstEnc,
        ...
        if (!enc) {
            return emitOptionalError(
                loc, "JoinOp can only operate on BlockedEncoding");
        }

        // JoinOp takes two tensors of shape AxBxC and generates a tensor of
        // shape AxBxCx2.  The encoding is the same as the input, but with 2
        // elems per thread in the new dimension.  The new dimension is
        // most-minor.
    ...
    inferSplitOpEncoding(Attribute srcEnc, Attribute &dstEnc,
        ...
        if (!enc) {
            return emitOptionalError(
                loc, "SplitOp can only operate on BlockedEncoding");
        }
        ...
        if (enc.getOrder().front() != enc.getOrder().size() - 1) {
            return emitOptionalError(loc, "SplitOp requires the last dimension "
                                          "to be most-minor in order");
        }
        if (enc.getCTALayout().getCTAsPerCGA().back() != 1) {
            return emitOptionalError(loc, "SplitOp requires the last dimension "
                                          "to be most-minor in CTAOrder");
        }
...
struct CanonicalizeConvertFromConvert
    ...
    matchAndRewrite(ConvertLayoutOp op,
        ...
        if (auto reshape = dyn_cast<ReshapeOp>(arg)) {
            ...
            // In TritonGPUToLLVM phase, ViewOp is converted to unpacking and
            // packing operations, which requires the element type to match
            // between unpacking and packing. However, part of values with dot
            // operand encoding will be packed/unpacked as i32 elements instead
            // of the underlying element type. To avoid errors, skip this
            // folding when either the operand or result of view has a dot
            // operand encoding.
        ...
        if (auto histogram = dyn_cast<HistogramOp>(arg)) {
            // For histogram ops the input and output layouts are independent,
            // so we can always fold convert into the histogram op.
            rewriter.replaceOpWithNewOp<HistogramOp>(
                op, op->getResult(0).getType(), histogram.getSrc());
            return success();
        }

        // cvt(local_load) -> local_load.
        if (auto sharedLoad = dyn_cast<LocalLoadOp>(arg)) {
            // Shared_load can load to any layout so we can always fold convert
            // into it.
            rewriter.replaceOpWithNewOp<LocalLoadOp>(
        ...
        if (auto splat = dyn_cast<triton::SplatOp>(arg)) {
            rewriter.replaceOpWithNewOp<triton::SplatOp>(
                op, op->getResultTypes(), splat.getSrc());
            return success();
        }
...
void LocalAllocOp::getEffects(
    ...
    // After the memory offset is computed, we attach the true side effect to
    // the op.
...
LogicalResult MemDescSubviewOp::verify() {
    ...
    if (!!srcEnc != !!dstEnc) {
        return emitError(
            "src and result must both have or not have an encoding");
    }
    ...
    // verifier for them.  In particular, we use the same encoding for the src
    // and dst of a subview op, when the subview removes a dimension.  That
    // generates an illegal shared encoding (because the size of `order` doesn't
    // match the rank of the tensor), but it's not checked anywhere, and we
    // believe the resulting code ultimately works.

...
class SwizzleShmemConvert : public OpRewritePattern<ConvertLayoutOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ConvertLayoutOp cvtOp,
        ...
        // TODO(Qingyi): need to check whether the CTALayout of innerCvtEnc
        // should be used here. For tests where numCTAs = 1, this is not a
        // problem since all CTALayouts are the same.
        //
        ...
        // The type inference of TransOp simply swap the order but doesn't fix
...
class HoistLayoutConversion : public OpRewritePattern<ConvertLayoutOp> {
  public:
    ...
    LogicalResult matchAndRewrite(ConvertLayoutOp cvt,
        ...
        // Check that the conversion is transitively dependent on a load, and
        // all operations between the load and the conversion are layout
        // preserving.
        //
        // TODO(jlebar): This is accidentally quadratic; we iterate over the
        // whole slice but then at the end we only modify one op!
        SetVector<Operation *> slice;
        BackwardSliceOptions opt;
        opt.omitBlockArguments = true;
        // TODO(jlebar): Is this filter redundant with omitBlockArguments ==
        // true? That is, is it possible to get into a different region without
        // going through a block argument?
        opt.filter = [&](Operation *op) {
            return op->getParentRegion() == cvt->getParentRegion();
        };
        getBackwardSlice(cvt.getOperation(), &slice, opt);

        // TODO(jlebar): This is too conservative when there are multiple loads
        // in the chain (e.g. cvt(load(x) + load(y))).  The intent is to check
        // that all of the ops between the loads and the convert are
        // elementwise.  But actually we set foundLoad = true once we see the
        // first load, and so we will reject the chain if the *second* load we
        // encounter uses a non-elementwise op to calculate its pointers.
        ...
        for (auto operand : src->getOperands()) {
            ...
            Type newCvtTy = RankedTensorType::get(srcTy.getShape(),
                                                  operandTy.getElementType(),
                                                  cvtTy.getEncoding());
            newOperands.push_back(rewriter.create<ConvertLayoutOp>(
                cvt.getLoc(), newCvtTy, operand));
        }
...
class FuseTransHopper : public OpRewritePattern<LocalAllocOp> {
  public:
    ...
    LogicalResult matchAndRewrite(LocalAllocOp allocOp,
        ...
        MemDescType innerTy = MemDescType::get(
            srcTy.getShape(), srcTy.getElementType(), newInnerEnc);
...
struct MMAV3UseRegOperand : public OpRewritePattern<DotOp> {
    ...
    LogicalResult matchAndRewrite(DotOp dotOp,
        ...
        if (!isa<SharedEncodingAttr>(getEncoding(dotOp.getOperand(0))))
            return failure();
        auto srcEnc =
            dyn_cast<NvidiaMmaEncodingAttr>(getEncoding(alloc.getSrc()));
        auto dstEnc =

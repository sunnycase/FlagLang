...
scf::ForOp Prefetcher::createNewForOp() {
    ...
    for (Operation &op : forOp.getBody()->without_terminator()) {
        ...
        if (dot && dots.contains(dot)) {
            ...
            while (kRem != 0) {
                ...
                Attribute dotOperandEncodingA =
                    dot.getA().getType().getEncoding();
                Value aRem = generatePrefetch(
                    mapping.lookup(dot2aLoopArg[dot]), 0, false, dotEncoding,
                    builder, dotOperandEncodingA, kOff, kShape);
                cloneElementwiseOps(aRem, dot2aVals[dot], builder);
                Attribute dotOperandEncodingB =
                    dot.getB().getType().getEncoding();
                Value bRem = generatePrefetch(
                    mapping.lookup(dot2bLoopArg[dot]), 1, false, dotEncoding,
                    builder, dotOperandEncodingB, kOff, kShape);
    ...
    for (triton::DotOp dot : dots) {
        ...
        Attribute dotOperandEncodingA = dot.getA().getType().getEncoding();
        Value aToYield =
            generatePrefetch(mapping.lookup(dot2aYield[dot]), 0, true,
        ...
        Attribute dotOperandEncodingB = dot.getB().getType().getEncoding();
        Value bToYield =
            generatePrefetch(mapping.lookup(dot2bYield[dot]), 1, true,

...
class BlockedToMMA : public mlir::RewritePattern {
    ...
    // arithmetic operations, bitcasting to fp16, and finally computing in fp16.
        ...
        for (auto op : slice) {
            if (Value arg = op->getOperand(0))
                if (auto argTy = dyn_cast<RankedTensorType>(arg.getType())) {
                    auto argBitWidth =
                        argTy.getElementType().getIntOrFloatBitWidth();
  ...
  public:
    ...
    static Value getMMAv3Operand(Value v, mlir::PatternRewriter &rewriter,
        ...
        auto newLayout = SharedEncodingAttr::get(
    ...
    matchAndRewrite(mlir::Operation *op,
        ...
        if (versionMajor == 1) {
            ...
            auto getCvtArgOrder = [](Operation *op) {
                return mlir::cast<BlockedEncodingAttr>(cast<ConvertLayoutOp>(op)
                                                           .getSrc()
                                                           .getType()
                                                           .getEncoding())
                    .getOrder();
            ...
            mmaEnc = NvidiaMmaEncodingAttr::get(
                ...
                instrShape, oldAType.getShape(), oldBType.getShape(),
                retShapePerCTA, isARow, isBRow, mmaV1Counter++);
        } else if (versionMajor == 2 || versionMajor == 3) {
            int versionMinor = computeCapability == 75 ? 1 : 0;
            auto warpsPerTile = getWarpsPerTile(
                dotOp, retShapePerCTA, versionMajor, numWarps, instrShape);
            mmaEnc = NvidiaMmaEncodingAttr::get(
                oldRetType.getContext(), versionMajor, versionMinor,
...
static void decomposeMixedModeDotOp(ModuleOp mod, int computeCapability) {
    mod.walk([=](DotOp dotOp) -> void {
        ...
        if (mmaLayout) {
            ...
            // promote operands for sm < 89 since fp8 mma is not natively
            // supported promote operands for sm >= 90 when mma is not v3

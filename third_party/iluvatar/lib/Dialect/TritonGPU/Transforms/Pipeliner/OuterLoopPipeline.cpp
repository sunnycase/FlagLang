...
static bool preCondition(scf::ForOp forOp) {
    // Check if there is a dependency from the loop to the async copy op. In
    // this case we cannot pipeline the async copy.
    SmallVector<Operation *> insertOps;
    int numForOps = 0;
    for (Operation &op : forOp.getBody()->without_terminator()) {
        if (isa<ttg::AsyncCopyGlobalToLocalOp, ttg::AsyncCommitGroupOp>(op))
    ...
    // If there is a recurrence containing both the async and the for op we
    // cannot pipeline.

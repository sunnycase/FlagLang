...
class CoarseSchedule {
  public:
    class ClusterList {
      ...
      public:
        ...
        iterator newBefore(iterator cluster) {
            ...
            for (auto &clusterId :
                 llvm::make_range(cluster, orderClusters.end())) {
                clusterId++;
            }
            return ret;
        }
    ...
    void insertDepsOfOp(Operation *op, int stage,
                        CoarseSchedule::Cluster cluster, bool includeArg) {
    ...
    createFinalSchedule(scf::ForOp forOp) {
        ...
        for (auto [op, stage, cluster] : opsInOrder) {
            LDBG("Adding op to schedule at stage " << stage << " cluster "
                                                   << *cluster << ":" << *op);
...
static void createAsyncCopy(scf::ForOp &forOp, tt::LoadOp loadOp, Value alloc,
    ...
    if (!isExpensiveLoadOrStore(loadOp) && loadToInfo[loadOp].blockedEncoding) {
        ...
        auto convertBlockLayout = [&](Value src) {
            ...
            auto newTy = RankedTensorType::get(ty.getShape(),
                                               ty.getElementType(), encoding);
            auto cvt = builder.create<ttg::ConvertLayoutOp>(loadOp->getLoc(),
                                                            newTy, src);
    ...
    auto view = builder.create<ttg::MemDescSubviewOp>(loc, subviewTy, alloc,
                                                      copyOffsets);
...
getSharedEncIfAllUsersAreDotEnc(Value val) {
    ...
    for (Operation *user : val.getUsers()) {
        ...
        if (auto memDesc =
            ...
            // First time we find a shared encoding in the chain, save it and
            // try to use it if it is compatible with the other users.
            tempAttr = cast<ttg::SharedEncodingAttr>(memDesc.getEncoding());
            if (!getSharedEncIfAllUsersAreDotEnc(user->getResult(0))
                     .has_value())
                return std::nullopt;
        } else {
            ...
            auto dotOpEnc = dyn_cast<ttg::DotOperandEncodingAttr>(
                cast<TensorOrMemDesc>(user->getResult(0).getType())
                    .getEncoding());
            if (!dotOpEnc)
                return std::nullopt;
...
getSharedEncoding(Operation *loadOp, bool isMMAV3) {
    ...
    if (isMMAV3) {
        return ttg::SharedEncodingAttr::get(ty.getContext(), ty.getShape(),
                                            order, ctaLayout,
                                            ty.getElementType());
    }

    // If the load is used by a LocalAllocOp, use the same encoding as the
    // allocs. If the allocs don't all have the same encoding, bail.
...
assignMemoryLayouts(llvm::SmallVector<std::tuple<Operation *, int, Operation *>>
    ...
    for (auto &[op, dist, use] : loadOpToIndLevelAndUse) {
        if (loadToInfo.count(op))
            // TODO pawel: err, we'd need to verify that the distance is the
            // same
            continue;
        LoadInfo loadInfo;
        ...
        if (auto loadOp = dyn_cast<tt::LoadOp>(op)) {
            ...
            if (auto mask = loadOp.getMask())
                vec = std::min<unsigned>(
                    vec, axisInfoAnalysis.getMaskAlignment(mask));

            auto tensorTy = dyn_cast<RankedTensorType>(ptr.getType());
            if (!tensorTy)
                continue;
            auto ty = cast<tt::PointerType>(tensorTy.getElementType())
                          .getPointeeType();
            ...
            // performance
            //    improvement and may even hurt performance by increasing
            //    register pressure.
        ...
        if (auto dot = dyn_cast<tt::DotOp>(use)) {
            ...
            if (loadIsMMAv3(op)) {
                ...
                loadInfo.sharedEncoding =
                    getSharedEncoding(op, /*loadIsMMAv3=*/true)
                        .value_or(nullptr);
                ...
                // HACK: Triton LLVM codegen has a bug where local_loads from
                // #shared to #mma layout can lead to invalid code if the loaded
                // shape is smaller than the mma tile (e.g. loading a 128x1
                // tensor for an MMAv2 dot with tile {16,8} is bad because 1 <
                // 8).  To work around this, don't pipeline such loads.
                //
                // The codegen bug is caught by an assertion, so if you think
                // you've fixed it, feel free to delete this code and see if the
                // assert still fails.  :)
                if (!loadInfo.sharedEncoding) {
                    if (auto dotEnc = dyn_cast<ttg::NvidiaMmaEncodingAttr>(
                        ...
                        for (int i = 0; i < mmaInstrShape.size(); i++) {
                            if (loadTy.getShape()[loadTy.getRank() -
                                                  mmaInstrShape.size() + i] <
                                mmaInstrShape[i]) {
                                ok = false;
                                break;
                            }
                        }
                        // If this load might trigger the bug, don't do the
                        // fallback logic below, which might allow the load to
                        // be pipelined.
                        if (!ok)
                            continue;
                    }
            ...
            // the loadsToPipeline already, it means that the use is not valid
            // for pipelining for some reason. We should skip this loadOp, too.
            // Note that we have an assumption that distAndUse.second (i.e. the
            // use of this loadOp) has already be processed in a previous loop
            // iteration. This assumption is held by how
            // loadOpsToIndirectionLevelAndUse recursively collects
...
scheduleLoads(scf::ForOp forOp, CoarseSchedule &schedule,
    ...
    LLVM_DEBUG({
        LDBG("Found " << loadOpToIndLevelAndUse.size()
                      << " loads to pipeline:");
...
schedulePrologueAndEpilogue(scf::ForOp forOp, CoarseSchedule &schedule,
    ...
    // Look for the IfOp that is in the forward slice of the root users and put
    ...
    for (auto rootUser : rootUsers) {
        ...
        for (auto op : forwardSlice) {
            ...
            if (ifOp == nullptr) {
                // check if the op is in the body of an if op that's part of the
                // loop
    ...
    auto getNestedOperands = [](Operation *op) -> SmallVector<Value> {
        ...
        op->walk([&](Operation *nestedOp) {
            for (Value operand : nestedOp->getOperands()) {
                if (operand.getParentBlock()->getParentOp()->isAncestor(
                        nestedOp))
                ...
                if (arg.getArgNumber() > 0 && arg.getOwner() == op.getBlock()) {
                    ...
                    if (defOp && schedule.count(defOp) == 0) {
                        if (isa<tt::LoadOp>(defOp)) {
                            // Exception: Schedule loads with a distance of 1
                            // together with the current op.
                            ...
                            schedule.insertIfAbsent(defOp, stage + 1,
                                                    dist1Cluster[&cluster]);
                            schedule.insertDepsOfOp(
                                defOp, stage + 1, dist1Cluster[&cluster], true);
                        }
                    }
                }
            }
        ...
        auto addToGroup = [&](AsyncLoad *loadInfo) {
            ...
            for (Operation *user : loadInfo->loadOp->getUsers()) {
                ...
                if (it != loadToInfo.end()) {
                    // Special case for MMAv3 loads, we can ignore the alloc and
                    // only consider uses of the alloc op since it will be
                    // removed.
                    if (it->second.loadIsMMAV3) {
                        auto alloc = cast<ttg::LocalAllocOp>(
                            (*loadInfo->loadOp->getUsers().begin()));
                        if (alloc->getBlock() == loadBlock) {
                            users.insert(alloc->getUsers().begin(),
                                         alloc->getUsers().end());
                            continue;
                        }
                    }
                ...
                Operation *userInBlock =
                    loadBlock->findAncestorOpInBlock(*user);
                if (userInBlock)
    ...
    for (SmallVector<AsyncLoad *> &group : loadGroups) {
        ...
        for (AsyncLoad *asyncLoad : group) {
            auto tensorTy = cast<RankedTensorType>(
                asyncLoad->loadOp->getResult(0).getType());
            int loadSize = product(tensorTy.getShape());
            sizeInBytes += loadSize *
...
createAsyncOps(scf::ForOp &forOp, CoarseSchedule &schedule,
    ...
    int numBuffers =
        ...
        std::max_element(
            ...
            ->distToUse;
    bool hasMMAV3 = llvm::any_of(
        loadToInfo, [](auto &kv) { return kv.second.loadIsMMAV3; });
    if (hasMMAV3) {
    ...
    for (auto &[loadOp, info] : loadToInfo) {
        ...
        Value alloc =
            createAlloc(forOp, loadOp, info.sharedEncoding, numBuffers);
...
static void invalidateBarriers(OpBuilder &builder,
    ...
    for (Value barrier : barriers) {
        int numBarriers =
            barrier.getType().cast<tt::MemDescType>().getShape()[0];
        for (int i = 0; i < numBarriers; i++) {
            Value idx =
                builder.create<arith::ConstantIntOp>(barrier.getLoc(), i, 32);
            tt::MemDescType barrierTy = tt::MemDescType::get(
                {1}, builder.getI64Type(),
...
bool mlir::triton::preProcessLoopAndGetSchedule(
    ...
    CoarseSchedule::Cluster afterPrologue = schedulePrologueAndEpilogue(
        forOp, coarseSchedule, rootUsers, numStages);
    LLVM_DEBUG({
...
static int minNumInterleavedCommitOps(Operation *waitOp) {
    ...
    // we calculate the number of async_commit. Then we select the minimum
    // number of async_commit ops among all the paths.
    std::function<int(Value, Operation *, int)> minOverHistories =
        ...
        if (auto arg = mlir::dyn_cast<BlockArgument>(val)) {
            ...
            // get the value value assigned to the argument coming from outside
            // the loop
            Value incomingVal = forOp.getInitArgs()[arg.getArgNumber() - 1];
            int min1 = minOverHistories(incomingVal, forOp, thisHistorySum);

            // get the value value assigned to the argument coming from the
            // previous iteration
...
combineRedundantWaitOps(llvm::SmallSetVector<ttg::AsyncWaitOp, 8> &waitOps) {
    ...
    for (auto waitOp : waitOps) {
        ...
        auto newWaitOp = builder.create<ttg::AsyncWaitOp>(
            waitOp.getLoc(), depTokens, minWaitNumber);
...
static std::optional<int> dotCanBeProperlyAsync(ttng::DotAsyncOp dotOp,
    ...
    auto checkOperand = [&](Value operand) {
        ...
        // If it's a shmem operand, it must either be defined outside the loop,
        // or come from an MemDescSubview op.  Only ConvertLayout and Trans ops
    ...
    assert(
        isa<ttg::NvidiaMmaEncodingAttr>(dotOp.getC().getType().getEncoding()));
    if (!checkOperand(dotOp.getA()) || !checkOperand(dotOp.getB())) {
        LDBG("Can't make dot async because shmem operands aren't "
             "multi-buffered");
        return std::nullopt;
    }
    ...
    while (!queue.empty()) {
        ...
        if (user->getParentOp() == forOp) {
            if (isa<scf::YieldOp>(user)) {
                if (iterArg) {
                    // The dot is used by the loop's yield, but we can't have
                    // any other uses.
                    LDBG("Can't make dot async because dot is used by multiple "
                         "ops in "
                         "the loop.");
                    return std::nullopt;
                }
            ...
            LDBG("Can't make dot async because dot is unconditionally used in "
                 "the "
                 "loop.");
            return std::nullopt;
        }
...
static void insertAsyncDotWaitInLoop(
    ...
    // IOW we wait until there are most `asyncDots.size()` dots in flight.
    //
    ...
    // async dot and the end of the loop, and that could clobber memory being
    // used by a dot.)
    IRRewriter builder(forOp.getContext());
...
void triton::asyncLaunchDots(scf::ForOp forOp) {
    ...
    //
    // We call those dots that don't need to be followed immediately by a `wait
    // 0` "properly async", or sometimes just "async".
    IRRewriter builder(forOp.getContext());
    ...
    // For each dot, determine whether it can be properly async, or if it needs
    // a sync immediately after.  If it can be properly async, we know its only
    // use is in the loop's `yield` statement; asyncDots maps the op to its
    // index in the yield op.
    ...
    for (auto dotOp : forOp.getBody()->getOps<ttng::DotAsyncOp>()) {
        if (auto iterArgIdx = dotCanBeProperlyAsync(dotOp, forOp)) {
            ...
            auto wait = builder.create<ttng::DotWaitOp>(dotOp.getLoc(),
                                                        ArrayRef<Value>{},
                                                        /*pendings=*/0);
    ...
    // third iteration's set of asynchronous dots (and their corresponding async
    // copies from global to shmem) can't start until the first iteration's set
    // has completed.

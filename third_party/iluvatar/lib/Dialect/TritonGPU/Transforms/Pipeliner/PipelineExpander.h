...
struct PipeliningOption {
    /// Lambda returning all the operation in the forOp, with their stage, in
    /// the order picked for the pipelined loop.
    ...
    /// Lambda called by the pipeliner to allow the user to annotate the IR
    /// while it is generated. The callback passes the operation created along
    /// with the part of the pipeline and the iteration index. The iteration
    /// index is always 0 for the kernel. For the prologue and epilogue, it
    /// corresponds to the iteration peeled out of the loop in the range [0,
    /// maxStage[.
    ...
    /// operations should be predicated to skip the early stages in the last
    /// loop iterations. If the epilogue is predicated; the user needs to
    /// provide a lambda to generate the predicated version of operations.
    bool peelEpilogue = true;

    /// Control whether the transformation checks that the number of iterations
    /// is greater or equal to the number of stages and skip the transformation
    /// if this is not the case. If the loop is dynamic and this is set to true
    /// the pipeliner will have to predicate operations in the the
    /// prologue/epilogue.

...
LoopPipelinerInternal::getDefiningOpAndDistance(Value value) {
    ...
    if (auto arg = dyn_cast<BlockArgument>(value)) {
        ...
        if (arg.getArgNumber() == 0)
            return {nullptr, 0};
        distance++;
        value = forOp.getBody()->getTerminator()->getOperand(
            arg.getArgNumber() - 1);
    }
...
void LoopPipelinerInternal::emitEpilogue(
    ...
    for (int64_t i = 0; i < maxStage; i++) {
        ...
        Value minusI = rewriter.create<arith::ConstantOp>(
            loc, rewriter.getIntegerAttr(t, -i));
        Value newlastIter = rewriter.create<arith::AddIOp>(
            loc, lb,
            rewriter.create<arith::MulIOp>(
                loc, step,
                rewriter.create<arith::AddIOp>(loc, totalNumIteration,
                                               minusI)));
        ...
        for (Operation *op : opOrder) {
            ...
            Operation *newOp = cloneAndUpdateOperands(
                rewriter, op, [&](OpOperand *newOperand) {
                    ...
                    if (it != valueMapping.end()) {
                        Value replacement =
                            it->second[maxStage - stages[op] + i];
                        newOperand->set(replacement);
                    }
                });
            if (annotateFn)
            ...
            for (unsigned destId :
                ...
                for (OpOperand &operand :
                    ...
                    if (version > maxStage) {
                        returnValues[operand.getOperandNumber()] =
                            newOp->getResult(destId);
                        continue;
                    }
                    setValueMapping(
                        forOp.getRegionIterArgs()[operand.getOperandNumber()],

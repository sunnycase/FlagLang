...
bool hasConvertToMMATransisitiveUse(Operation *op, Attribute encoding) {
    ...
    while (!queue.empty()) {
        ...
        for (Operation *op : forwardSlice) {
            ...
            // producing tensor smaller than the layout we would like to
            // propagate. This is to avoid stepping into the known bug.
            ...
            if (auto convertOp = dyn_cast<ConvertLayoutOp>(op)) {
                ...
                if (auto mmaLayout =
                        dyn_cast<NvidiaMmaEncodingAttr>(dstEncoding))
                ...
                if (isa<triton::gpu::DotOperandEncodingAttr>(dstEncoding)) {
                    if (auto mmaLayout =
                            dyn_cast<NvidiaMmaEncodingAttr>(encoding)) {
                        return mmaLayout.getVersionMajor() > 1;
                    } else if (isa<triton::gpu::IluvatarMmaEncodingAttr>(
                                   encoding)) {
                        return true;
                    } else {
            ...
            if (auto ifOp = dyn_cast<scf::IfOp>(yield->getParentOp())) {
                for (OpOperand &operand : yield->getOpOperands()) {
                    ...
                    if (def &&
                        (forwardSlice.count(def) ||
                         operand.get() == currentValue) &&
                        (seen.insert(operand.get()).second == true))
                        queue.push_back(
                            ifOp.getResult(operand.getOperandNumber()));
                }
            }
            auto forOp =
                dyn_cast<scf::ForOp>(yield.getOperation()->getParentOp());
            if (!forOp)
                continue;
            for (OpOperand &operand : yield->getOpOperands()) {
                Operation *def = operand.get().getDefiningOp();
...
bool LayoutPropagation::rewriteStoreOp(Operation *storeOp) {
    ...
    if (!srcEncoding || !isa<IluvatarMmaEncodingAttr>(srcEncoding) ||
        (dyn_cast<IluvatarMmaEncodingAttr>(srcEncoding).getVersionMinor() ==
             0 &&
         bitWidth == 16))
        return false;
    ...
    for (Value arg : storeOp->getOperands()) {
        if (arg.getDefiningOp() == cvt) {
            ...
            auto cvtI =
                rewriter.create<ConvertLayoutOp>(arg.getLoc(), newType, arg);
    ...
    for (Operation *op : opsToRewrite) {
        ...
        for (auto [old, newV] :
            ...
            auto newType = RankedTensorType::get(
                ...
                cast<RankedTensorType>(old.getType()).getElementType(),
                it->second);
...
void LayoutRematerialization::hoistConvertOnTopOfExtOrBroadcast(
    ...
    auto tgtSliceLayout =
        targetType.getEncoding().dyn_cast<SliceEncodingAttr>();
    if (srcSliceLayout && tgtSliceLayout) {
        if (srcSliceLayout.getNoWarpReduce() ||
            tgtSliceLayout.getNoWarpReduce())
            return;
    }
#endif
    ...
    Type newExtOrBroadcasrType = RankedTensorType::get(
        oldExtOrBroadcastType.getShape(),
        oldExtOrBroadcastType.getElementType(), dstEncoding);
...
class TritonGPURemoveLayoutConversionsPass
  ...
  public:
    void runOnOperation() override {
        ...
        if (applyPatternsAndFoldGreedily(m, std::move(cleanUpPatterns))
                .failed()) {
            signalPassFailure();
        }
        ...
        if (applyPatternsAndFoldGreedily(m, std::move(cleanUpPatterns2))
                .failed()) {
            signalPassFailure();
        }
        LLVM_DEBUG({

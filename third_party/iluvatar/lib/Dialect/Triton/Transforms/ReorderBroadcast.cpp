...
struct MoveSplatAfterElementwisePattern
    ...
    void rewrite(Operation *op, PatternRewriter &rewriter) const override {
        ...
        auto newOp = cloneWithNewArgsAndResultTypes(
            rewriter, op, scalarOperands, scalarResultTys);
...
struct MoveBroadcastAfterElementwisePattern
    ...
    void rewrite(Operation *op, PatternRewriter &rewriter) const override {
        ...
        for (auto operand : operands) {
            ...
            if (auto splatOp = llvm::dyn_cast<SplatOp>(definingOp)) {
                auto newSplat =
                    rewriter.create<SplatOp>(loc, newTy, splatOp.getSrc());
        ...
        for (unsigned iRes = 0; iRes < newResultTypes.size(); ++iRes) {
            auto newResult = rewriter.create<BroadcastOp>(

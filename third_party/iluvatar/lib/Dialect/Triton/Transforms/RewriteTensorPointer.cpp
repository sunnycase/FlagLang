...
struct RewritedInfo {
  ...
  public:
    ...
    Value getExpandedOffsetWithRange(OpBuilder &builder, const Location &loc,
        ...
        Value range = builder.create<triton::MakeRangeOp>(loc, indexI32RowType,
                                                          0, tensorShape[i]);
        Value i64Range =
            builder.create<arith::ExtSIOp>(loc, indexRowType, range);
    ...
    Value generatePtr(OpBuilder &builder, const Location &loc) {
        ...
        for (unsigned i = 0; i < tensorShape.size(); ++i) {
            ...
            // We must splat strides into the expanded shape not a row for
            // retaining the divisibility information given by strides
            Value splatStride = builder.create<triton::SplatOp>(
                loc, offsetWithRange.getType(), strides[i]);
            Value offsetWithStride = builder.create<arith::MulIOp>(
                loc, offsetWithRange, splatStride);
    ...
    Value generateMask(OpBuilder &builder, const Location &loc,
        ...
        for (auto i : boundaryCheck.value()) {
            ...
            // And and broadcast
            Value andResult =
                builder.create<arith::AndIOp>(loc, cmpLower, cmpUpper);
            Value broadcasted = builder.create<triton::BroadcastOp>(
                loc, maskTensorType, andResult);
  ...
  public:
    static bool needRewrite(Operation *op) {
        return std::any_of(op->getOperands().begin(), op->getOperands().end(),
                           [](Value operand) {
                               return triton::isTensorPointerType(
                                   operand.getType());
                           });
    }
    ...
    Operation *rewriteMakeTensorPtrOp(OpBuilder &builder,
        ...
        rewritedInfo[op.getResult()] =
            RewritedInfo(op.getBase(), op.getShape(), op.getStrides(),
                         i64Offsets, tensorType.getShape(), op.getOrderAttr());
    ...
    Operation *rewriteLoadStoreOp(OpBuilder &builder, Operation *op,
        ...
        // accessing memory, so we should set `mask` and `other` (according to
        // the padding). Also note that load with tensor pointers do not have
        // `mask` and `other` while building IR from Python AST
        ...
        if (auto loadOp = dyn_cast<triton::LoadOp>(op)) {
            ...
            if (!newMask && !newOther && resStride) {
                ...
                newResult = builder.create<triton::LoadOp>(
                    loadOp.getLoc(), loadOp.getResult().getType(), newPtr,
                    newMask, newOther, loadOp.getBoundaryCheckAttr(),
                    loadOp.getPaddingAttr(), loadOp.getCache(),
                    loadOp.getEvict(), loadOp.getIsVolatile(), matStride,
                    matStride, matStride);
            } else {
                newResult = builder.create<triton::LoadOp>(
                    loadOp.getLoc(), newPtr, newMask, newOther,
    ...
    Operation *rewriteForOp(OpBuilder &builder, scf::ForOp op,
        ...
        // mapping. It may refer to a value in the old loop, but we will rewrite
        // it later
        IRMapping mapping;
    ...
    Operation *rewriteOp(Operation *op, std::stack<Operation *> &eraser) {
        ...
        // Rewriting functions return the next operation to visit, if there is
        // no next one, simply return `nullptr`
    ...
    void visitOperation(Operation *op, std::stack<Operation *> &eraser) {
        for (auto &region : op->getRegions()) {
            for (auto &block : region) {
                // We need an extra copy because erasing operations may break
    ...
    void runOnOperation() override {
        ...
        // which converts `ptr<tensor>` into multiple types `ptr<>, int64,
        // int64, ...` (containing the base/offsets/strides...). What we can do
        // is to convert `ptr<tensor>` into a single type `Tuple<ptr<>, int64,
        // int64, ...>`. But in this way, we also have to define `PackTuple` and
        // `UnpackTuple` operations and make a canonicalization pass to
        // optimize, which is much So here we recursively build the IR, to be
        // specific, we have to rewrite `tt.make_tensor_ptr`, `tt.advance`,
        // `tt.load`, `tt.store`, `scf.for` (tensor pointer usages may be in a
        // loop fashion)
        std::stack<Operation *> eraser;

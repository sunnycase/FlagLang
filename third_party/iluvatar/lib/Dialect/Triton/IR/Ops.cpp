...
ParseResult LoadOp::parse(OpAsmParser &parser, OperationState &result) {
    ...
    if (allOperands.size() == 4) {
        ...
        operandTypes.push_back(IntegerType::get(
            parser.getBuilder().getContext(), 32)); // placeHolder0
        operandTypes.push_back(IntegerType::get(
            parser.getBuilder().getContext(), 32)); // placeHolder1
        hasStride = 1;
    }
...
LogicalResult TransOp::verify() {
    // Check that the op's `order` attribute is a permutation of the right
    // length.
    ...
    if (auto broadcast = dyn_cast<BroadcastOp>(definingOp)) {
        ...
        if (auto srcEnc = srcTy.getEncoding()) {
            if (dyn_cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                    ->inferExpandDimsOpEncoding(srcEnc, op.getAxis(),
                                                newExpandEnc, op.getLoc())
                    .failed()) {
...
LogicalResult ReshapeOp::verify() {
    ...
    if (srcEnc && !getAllowReorder()) {
        ...
        if (cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                ...
                .failed()) {
            return emitError(
                "This reshape is impossible without reordering, but "

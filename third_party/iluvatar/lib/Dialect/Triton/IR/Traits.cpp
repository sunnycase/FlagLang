    ...
    for (auto opType : llvm::drop_begin(op->getOperandTypes(), 1))
        if (failed(verifySameEncoding(opType, type, allowTensorPointerType)))
            return op->emitOpError()
                   << "requires the same encoding for all operands";
...
LogicalResult OpTrait::impl::verifySameOperandsAndResultEncoding(
    ...
    for (auto resultType : op->getResultTypes())
        if (failed(
                verifySameEncoding(resultType, type, allowTensorPointerType)))
            return op->emitOpError()
    ...
    for (auto opType : op->getOperandTypes()) {
        if (auto tensorType = dyn_cast<RankedTensorType>(opType)) {
            ...
            if ((numElements & (numElements - 1)) != 0)
                return op->emitError(
                           "Number of elements must be power-of-two, but ")
...
LogicalResult OpTrait::impl::verifyTensorLayouts(Operation *op) {
    ...
    auto checkLayout = [&](Value val, auto makeErr) -> LogicalResult {
        ...
        // TODO(jlebar): Handle the case when the encoding is nested within
        // tt.ptr.
        if (auto blocked = dyn_cast<ttg::BlockedEncodingAttr>(layout)) {
            // A different verifier should have checked that the layout itself
            // is valid, including that threads-per-warp has the same rank as
            // warps-per-block etc.
            ...
            if (blocked.getCTALayout().getCTAsPerCGA().size() > 0) {
                int moduleCTAsPerCGA =
                    ttg::TritonGPUDialect::getNumCTAs(module);
    ...
    for (size_t i = 0; i < op->getNumOperands(); i++) {
        ...
        auto err = checkLayout(operand, [&]() {
            // Stringify the operand using `printAsOperand`.  This prints e.g.
            // "%42" rather than the full definition.
...
LogicalResult OpTrait::impl::verifySameLoadStoreOperandsShape(Operation *op) {
    ...
    for (auto type : llvm::drop_begin(op->getOperandTypes(), 1))
        if (failed(
                verifyCompatibleShape(getTypeShape(type), firstOperandShape)))
            return op->emitOpError()

...
unsigned ScanLoweringHelper::getAxisNumThreadsPerWarpWithUniqueData() {
    return getThreadsPerWarpWithUniqueData(getEncoding(),
                                           getShape())[getAxis()];
}
...
unsigned ScanLoweringHelper::getNonAxisNumBlocks() {
    ...
    for (unsigned i = 0; i < sizePerThreads.size(); i++) {
        if (i == axis)
            continue;
        numBlocks *=
            ceil<unsigned>(getShape()[i], (sizePerThreads[i] *
...
bool isMmaToDotSlowShortcut(RankedTensorType &srcTy, RankedTensorType &dstTy) {
    ...
    auto dotOperandLayout =
        dstLayout.cast<triton::gpu::DotOperandEncodingAttr>();
...
void getBackwardSliceImplCorex(Operation *op,
    ...
    for (const auto &en : llvm::enumerate(op->getOperands())) {
        ...
        if (auto *definingOp = operand.getDefiningOp()) {
            ...
            // TODO: determine whether we want to recurse backward into the
            // other blocks of parentOp, which are not technically backward
            // unless they flow into us. For now, just bail.

        ...
        for (short d = 0; d < rank; ++d) {
            ...
            if (lhsInfo.getConstantValue().has_value() &&
                ...
                if ((gtPredicate(getPredicate(op)) ||
                    ...
                    // lhs ge rhs: 1, 0, 0, 0
                    // lhs gt rhs: 0, 0, 0, 0
                    constHint = std::max(constHint,
                                         gcd(rhsInfo.getContiguity(d),
  ...
  public:
    ...
    AxisInfo getAxisInfo(
        ...
        if (operands[0]->getValue().getConstantValue().has_value()) {
            ...
            for (auto d = 0; d < rank; ++d) {
                if (i1Cond) {
                    constancy.push_back(std::min(lhsInfo.getConstancy(d),
                                                 rhsInfo.getConstancy(d)));
                    divisibility.push_back(
                        std::min(lhsInfo.getDivisibility(d),
                                 rhsInfo.getDivisibility(d)));
                    contiguity.push_back(std::min(lhsInfo.getContiguity(d),
                                                  rhsInfo.getContiguity(d)));
                } else {
                    constancy.push_back(std::min(
                        gcd(lhsInfo.getConstancy(d), condConstancy[d]),
                        gcd(rhsInfo.getConstancy(d), condConstancy[d])));
                    contiguity.push_back(std::min(
                        gcd(lhsInfo.getContiguity(d), condConstancy[d]),
                    ...
                    if (contiguity.back() == lhsInfo.getContiguity(d) &&
                        ...
                        // For example, the following example should have
                        // contiguity 2 and divisibility 2
                        // [[0, 1], [4, 5]]
                        // [[16, 17, 18, 19]]
                        divisibility.push_back(std::min(
                            gcd(lhsInfo.getDivisibility(d), contiguity.back()),
                            gcd(rhsInfo.getDivisibility(d),
                                contiguity.back())));
                    }
                }
            }
    ...
    AxisInfo getAxisInfo(
        ...
        if (lhsInfo.getConstantValue().has_value() &&
            ...
            for (auto d = 0; d < rank; ++d) {
                ...
                divisibility.push_back(std::min(lhsInfo.getDivisibility(d),
                                                rhsInfo.getDivisibility(d)));
                contiguity.push_back(std::min(lhsInfo.getContiguity(d),
                                              rhsInfo.getContiguity(d)));
            }
...
/*static*/ AxisInfo AxisInfo::getPessimisticValueState(Value value) {
    ...
    if (blockArg && blockArg.getOwner()->isEntryBlock()) {
        ...
        if (isa<RegionBranchOpInterface>(op)) {
            ...
            knownDivisibility =
                DimVectorT(rank, highestPowOf2Divisor<int64_t>(0));

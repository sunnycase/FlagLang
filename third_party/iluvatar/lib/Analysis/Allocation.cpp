...
SmallVector<unsigned> getRepShapeForCvtLayout(triton::gpu::ConvertLayoutOp op) {
    ...
    if (auto srcMmaLayout =
            mlir::dyn_cast<IluvatarMmaEncodingAttr>(srcLayout)) {
        if (mlir::isa<DotOperandEncodingAttr>(dstLayout)) {
            if (isMmaToDotShortcut(srcTy, dstTy)) {
                return {};
...
getScratchConfigForCvtLayout(triton::gpu::ConvertLayoutOp op, unsigned &inVec,
    ...
    if (auto mma = mlir::dyn_cast<NvidiaMmaEncodingAttr>(srcLayout)) {
        if (mma.getVersionMajor() == 1) {
            ...
            // when storing from mma layout and loading in blocked layout
            // vectorizing the load back gives better performance even if there
            // is a transposition.
    ...
    if (auto dstBlockedLayout =
            mlir::dyn_cast<BlockedEncodingAttr>(dstLayout)) {
  ...
  private:
    ...
    void getExplicitValueSize(Operation *op) {
        ...
        for (Value result : op->getResults()) {
            if (auto alloc =
                    result.getDefiningOp<triton::gpu::LocalAllocOp>()) {
                // Bytes could be a different value once we support padding or
                ...
                // Software swizzling calculates phase based on offset, while
                // hardware swizzling do that based on physical address. Thus
                // only by setting the alignment to 1024 can ensure the
                // correctness. 
                if (bytes > 256)
                    kAlignment = 1024;
                allocation->addBuffer<BufferT::BufferKind::Explicit>(
                    result, bytes, kAlignment);
            }
        }
    }
    ...
    void getScratchValueSize(Operation *op) {
        ...
        if (auto reduceOp = dyn_cast<triton::ReduceOp>(op)) {
            ...
            // TODO: Besides of implementing ConvertLayoutOp via shared memory,
            // it's
            //       also possible to realize it with other approaches in
            //       restricted conditions, such as warp-shuffle
            unsigned inVec = 0;
            unsigned outVec = 0;
            auto smemShape =
                getScratchConfigForCvtLayout(cvtLayout, inVec, outVec);
            unsigned elems = 0;
            if (!smemShape.empty())
            ...
            auto bytes =
                isa<triton::PointerType>(srcTy.getElementType())
                    ...
                    : elems * std::max<int>(8, srcTy.getElementTypeBitWidth()) /
                          8;
            maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(
                op, bytes, scratchAlignment);
            ...
            if (dyn_cast<RankedTensorType>(value.getType())) {
                ...
                unsigned elems = std::accumulate(
                    smemShape.begin(), smemShape.end(), 1, std::multiplies{});
                auto elemTy =
                ...
                auto bytes =
                    isa<triton::PointerType>(elemTy)
                        ? elems * kPtrBitWidth / 8
                        : elems *
                              std::max<int>(8, elemTy.getIntOrFloatBitWidth()) /
                              8;
                maybeAddScratchBuffer<BufferT::BufferKind::Scratch>(
                    op, bytes, scratchAlignment);
            }
    ...
    void resolveScratchBufferLiveness(
        ...
        auto processScratchMemory = [&](const auto &container) {
            for (auto opScratchIter : container) {
                ...
                bufferRange.insert(
                    {buffer, Interval(operationId.lookup(op),

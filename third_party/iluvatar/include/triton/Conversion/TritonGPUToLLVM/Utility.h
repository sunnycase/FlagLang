...
struct SharedMemoryObject {
    ...
    // Take $a[16:32, 16:32] as an example; though we know the stride of $a[0]
    // is 32, we need to let the instruction that uses $a be aware of that.
...
inline Value getSharedMemoryBase(Location loc,
    ...
    Value base =
        gep(ptrTy, i8_ty, LLVM::getStackPointer(rewriter, func), offVal);
    return base;
}
} // namespace LLVM
...
emitBaseIndexWithinCTAForBlockedLayout(Location loc, RewriterBase &rewriter,
    ...
    for (unsigned k = 0; k < rank; ++k) {
        ...
        auto maxWarps = ceil<unsigned>(shapePerCTA[k],
                                       sizePerThread[k] * threadsPerWarp[k]);
...
emitBaseIndexWithinCTAForMmaLayoutV2V3(Location loc, RewriterBase &rewriter,
    ...
    // warpsM/N may be 0, in which case warpIDM/N is poison (division by 0),
    // which will cause LLVM to eliminate all ops that depend on the poison
    // value.  This *can* be okay, if the bad dimension is filtered out by a
    // slice layout.  So we rely on the caller to check.  Worst case we crash,
    // which is better than silently producing bad code.
    if (warpsM != 0)
    ...
    for (unsigned b = 0; b < repBatch; ++b) {
        for (unsigned i = 0; i < numReps[rank - 2]; ++i) {
            for (unsigned j = 0; j < numReps[rank - 1]; ++j) {
                emitMfmaOffsetForCTA(mfmaLayout, offsets, b * warpsPerBatch, i,
                                     j);
            }
        }
    }
    ...
    for (unsigned b = 0; b < repBatch; ++b) {
        for (unsigned i = 0; i < repM; ++i) {
            for (unsigned j = 0; j < repN; ++j) {
                emitWmmaOffsetForCTA(wmmaLayout, offsets, b * warpsPerBatch, i,
                                     j);
            }
        }
    }
...
emitBaseIndexForLayoutImpl(Location loc, RewriterBase &rewriter,
    ...
    if (auto blockedLayout = mlir::dyn_cast<BlockedEncodingAttr>(layout)) {
        ...
        if (mmaLayout.isAmpere() || mmaLayout.isHopper())
            result = emitBaseIndexWithinCTAForMmaLayoutV2V3(loc, rewriter,
                   ...
                   mlir::dyn_cast<IluvatarMmaEncodingAttr>(layout)) {
        if (mmaLayout.isVolta()) {
    ...
    if (withCTAOffset) {
        ...
        for (unsigned k = 0; k < result.size(); ++k) {
            ...
            // (emitBaseIndexForLayout), we assert that all such dimensions are
            // sliced off.
            if (!result[k])
                continue;
...
emitBaseIndexForLayout(Location loc, RewriterBase &rewriter,
    ...
    for (Value v : idx) {
        if (!v) {
            llvm::errs()
                << "Failed to generate indexing code, possibly due to bad "
...
inline DenseMap<unsigned, Value> getSwizzledSharedPtrs(
    ...
    for (unsigned elemIdx = 0; elemIdx < numElems; elemIdx += minVec) {
        ...
        if (leadingDimOffset) {
            // hopper
            offset = mul(udiv(idxCol, numElemsPerSwizzlingRowVal),
                         leadingDimOffsetVal);
        ...
        if (auto add = dyn_cast_or_null<LLVM::AddOp>(idxCol.getDefiningOp())) {
            if (auto _cst = dyn_cast_or_null<LLVM::ConstantOp>(
                ...
                unsigned cst = cast<IntegerAttr>(_cst.getValue())
                                   .getValue()
                                   .getSExtValue();
                ...
                idxCol = cacheCol[key];
                immedateOffCol =
                    cst / (outVec * maxPhase) * (outVec * maxPhase);
            }
        }
        if (auto add = dyn_cast_or_null<LLVM::AddOp>(idxRow.getDefiningOp())) {
            if (auto _cst = dyn_cast_or_null<LLVM::ConstantOp>(
                    add.getRhs().getDefiningOp())) {
                unsigned cst = mlir::cast<IntegerAttr>(_cst.getValue())
                                   .getValue()
                                   .getSExtValue();
        ...
        if (outOrder.size() == 3)
            offset =
                add(offset, mul(idx[outOrder[2]], srcStrides[outOrder[2]]));
...
inline Value packLLElements(Location loc,
    ...
    for (const auto &v : llvm::enumerate(resultVals)) {
        ...
        if (v.value().getType() != elementTypes[v.index()]) {
            ...
            LDBG("value " << v.value());
            emitError(loc)
                << "invalid element type in packLLEElements. Expected "

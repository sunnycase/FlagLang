...
static PyObject *getDeviceProperties(PyObject *self, PyObject *args) {
    ...
    CUDA_CHECK_AND_RETURN_NULL(cuDeviceGetAttribute(
        &warp_size, CU_DEVICE_ATTRIBUTE_WARP_SIZE, device));
    CUDA_CHECK_AND_RETURN_NULL(cuDeviceGetAttribute(
...
typedef CUresult (*cuOccupancyMaxActiveClusters_t)(
    int *numClusters, CUfunction func, const CUDA_LAUNCH_PARAMS *config);
...
#define defineGetFunctionHandle(name, symbolName)                              \
    static symbolName##_t name() {                                             \
        ...
        symbolName##_t funcHandle =                                            \
            (symbolName##_t)dlsym(libHandle, #symbolName);                     \
        /* Check for errors */                                                 \
        const char *err = dlerror();                                           \
        if (err) {                                                             \
            PyErr_SetString(PyExc_RuntimeError,                                \
                            "Failed to retrieve " #symbolName                  \
                            " from libcuda.so.1");                             \
            dlclose(libHandle);                                                \
            return NULL;                                                       \
        }                                                                      \

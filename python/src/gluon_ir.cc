...
struct GluonOpBuilder : public TritonOpBuilder {
    ...
    getChecked(ArgTs &&...args) {
        ...
        auto result = AttrOrType::getChecked(
...
struct GluonLayouts {
    ...
    GluonLayouts() {
        ...
        auto amdLayouts = py::module::import(
        ...
        DotOperandLayout =
            py::object(layouts.attr("DotOperandLayout")).release();
        NVMMADistributedLayout =
            py::object(layouts.attr("NVMMADistributedLayout")).release();
        NVMMASharedLayout =
            py::object(layouts.attr("NVMMASharedLayout")).release();
    ...
    if (auto blocked = dyn_cast<ttg::BlockedEncodingAttr>(layout)) {
        ...
        if (elemTypeOpt.has_value()) {
            ...
            if (elemType.isF64()) {
                ...
                // The AMDMfmaEncodingAttr mlir attribute has already verified
                // element type is fp64, fp32 or int32; so, the typeName here
                // must be int32.
                typeName = "int32";
            }
        }
        ...
        for (auto [interval, padding] : llvm::zip(paddedShared.getIntervals(),
                                                  paddedShared.getPaddings())) {
            intervalPaddingPairs.push_back({interval, padding});
        }
        return layouts.PaddedSharedLayout(
            intervalPaddingPairs, toStdVector(paddedShared.getOrder()),
            toStdVector(ctaLayout.getCTAsPerCGA()),
            toStdVector(ctaLayout.getCTASplitNum()),
            toStdVector(ctaLayout.getCTAOrder()));
    }
    ...
    py::class_<GluonOpBuilder, TritonOpBuilder>(
        ...
        .def("get_tensor_descriptor_layout_type",
             [](GluonOpBuilder &self, Type blockType, bool isSigned,
                Attribute layout) -> Type {
                 ...
                 return triton::TensorDescType::get(ctx, blockTyLayout,
                                                    isSigned);
             })
        ...
        .def("create_memdesc_index",
             [](GluonOpBuilder &self, Type resultType, Value src,
                Value index) -> Value {
                 return self.create<ttg::MemDescIndexOp>(resultType, src,
                                                         index);
             })
        ...
        .def("create_split",
             [](GluonOpBuilder &self, Value &a) -> py::tuple {
                 ...
                 auto resTy = RankedTensorType::get(
                     ArrayRef(argTy.getShape()).drop_back(),
                     argTy.getElementType(), enc);
                 auto op =
                     self.create<triton::SplitOp>(TypeRange{resTy, resTy}, a);
        ...
        .def("create_tcgen05_mma",
             [](GluonOpBuilder &self, Value a, Value b, Value acc, Value useAcc,
                ...
                std::vector<Value> &mbarrier_preds) {
                 ...
                 self.create<ttng::TCGen5MMAOp>(tokType, a, b, acc, accDep,
                                                useAcc, pred, two_ctas,
        ...
        .def("create_async_tma_scatter",
             [](GluonOpBuilder &self, Value descPtr, Value xOffsets,
                Value yOffset, Value src) {
                 self.create<ttng::AsyncTMAScatterOp>(descPtr, xOffsets,
                                                      yOffset, src);
             })
        ...
        .def("create_warp_yield",
             [](GluonOpBuilder &self,
                std::vector<Value> &values) -> Operation * {
             ...
             [](GluonOpBuilder &self, Type resultType, Value ptr, Value offsets,
                Value mask, Value other, tt::CacheModifier cache) -> Value {
                 return self.create<ttag::BufferLoadOp>(
                     resultType, ptr, offsets, Value() /*stride*/, cache, mask,
                     other);
             })
        .def("create_buffer_store",
             [](GluonOpBuilder &self, Value storedValue, Value ptr,
                Value offsets, Value mask, tt::CacheModifier cache) {

...
template <typename T> T atomic_fadd(T *loc, T value, std::memory_order order) {
    ...
    if constexpr (is_reinterpret_cast_to_atomic_safe<T>) {
        ...
        } while (!atomic_loc->compare_exchange_weak(old_value, new_value, order,
                                                    order));
    } else {
        const std::lock_guard<std::mutex> lock(atomic_op_guard);
        old_value = *loc;
...
inline uint16_t FromFloatBits(uint32_t f) {
    ...
    if (f_exp <= 0x38000000u) {
        ...
        if constexpr (round_even) {
             ...
             * shift can lose up to 11 bits, so the || checks them in the
             * original. In all other cases, we can just add one.
             */
    ...
    void applyAt(void *loc, size_t i) override final {
        if (mask[i]) {
            ...
            *(static_cast<DType *>(ret) + i) = applyAtMasked(
                ptr, *(static_cast<const DType *>(val) + i), order);
        }
    }
...
void init_triton_interpreter(py::module &&m) {
    ...
    m.def("store", [](py::array_t<uint64_t> ptr, py::array value,
        ...
        break;
              ...
              switch (rmw_op) {
                  ...
                  MAKE_ATOMIC_RMW_OP(RMWOp::AND, int32_t, uint32_t, int64_t,
                                     uint64_t)
                  MAKE_ATOMIC_RMW_OP(RMWOp::OR, int32_t, uint32_t, int64_t,
                                     uint64_t)

...
parseCommaSeparatedValues(const std::string &input,
    ...
    llvm::transform(split, std::back_inserter(result),
...
struct ConsoleReproducerStream : public mlir::ReproducerStream {
    ...
    StringRef description() override {
        return "std::errs, please share the reproducer above with Triton "
               "project.";
    }
...
py::list getTensorDescMetadata(ModuleOp &mod) {
    ...
    for (auto [i, argTy] : llvm::enumerate(kernelFunc.getArgumentTypes())) {
        ...
        auto blockSize =
            ttng::getTMABlockShape(blockType, /*packedSize=*/false);
        py::dict metadata;
...
void init_triton_ir(py::module &&m) {
             ...
             [](Block &self, int index) -> BlockArgument {
                 if (index >= self.getNumArguments())
                     throw pybind11::index_error(
                         "Block argument index out of range");
    ...
    py::class_<OpState>(m, "OpState", py::module_local())
        ...
        .def("get_operation",
             [](OpState &self) { return self.getOperation(); });
    ...
    m.def("make_attr",
          [](const std::vector<int> &values, MLIRContext &context) {
             ...
             [](TritonOpBuilder &self, Value &ptr, Value &cmp, Value &val,
                MemSemantic sem, MemSyncScope scope) -> Value {
                 ...
                 if (auto srcTensorType =
                     ...
                     auto ptrType =
                         cast<PointerType>(getElementTypeOrSelf(ptr));
        ...
        .def("create_make_block_ptr",
             [](TritonOpBuilder &self, Value &base, std::vector<Value> &shape,
                ...
                std::vector<int32_t> &order) -> Value {
                 return self.create<MakeTensorPtrOp>(
        ...
        .def("enable_debug",
             [](PassManager &self) -> bool {
                 ...
                 if (haveDump) {
                     context->disableMultithreading();
                     auto printingFlags = getOpPrintingFlags();
                     auto printAlways = [funcToDump](Pass *,
                                                     Operation *op) -> bool {
                         if (funcToDump.empty())
                             return true;
                         ...
                         if (auto func = dyn_cast<triton::FuncOp>(op)) {
                             return SymbolTable::getSymbolName(func)
                                        .getValue() == funcToDump;
                         }
        ...
        .def(
            ...
            [](PassManager &self, ModuleOp &mod) {
                ...
                if (!reproducerPath.empty()) {
                    ...
                    // But if the pass manager crashes, attempt to generate a
                    // local reproducer instead.
                    context->disableMultithreading();
                ...
                bool haveTiming =
                    ::triton::tools::getBoolEnv("MLIR_ENABLE_TIMING");
                if (haveTiming) {
                    self.enableTiming();
                }

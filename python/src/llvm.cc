...
std::string translateLLVMIRToASM(llvm::Module &module,
    ...
    if (!disableLLVMOpt) {
        ...
        if (!flagList.empty()) {
            ...
            for (auto flag : split) {
                ...
                if (optIt != options.end()) {
                    auto optPtr =
                        static_cast<llvm::cl::opt<bool> *>(optIt->second);
                    *optPtr = true;
                }
            }
        }
    ...
    auto machine =
        createTargetMachine(&module, proc, enable_fp_fusion, features);
    // set data layout
...
void init_triton_llvm(py::module &&m) {
    ...
    py::class_<llvm::Function>(m, "function", py::module_local())
        ...
        .def("remove_fn_attr",
             [](llvm::Function *fn, std::string &name) {
                 fn->removeFnAttr(name);
             })
    ...
    m.def("attach_datalayout", [](llvm::Module *mod, const std::string triple,
        ...
        std::unique_ptr<llvm::TargetMachine> machine{
            target->createTargetMachine(llvm::Triple(triple), proc, features,
    ...
    m.def(
        ...
        [](llvm::Module *mod, const llvm::OptimizationLevel &opt,
           std::string arch, std::string features,
           std::vector<std::string> flags, bool enable_fp_fusion) {
            ...
            // TODO: currently we run SLP vectorizer with an empty target
            // machine. This cause the vectorizer to create larger vector which
            // could be bad. Disabling it would currently cause regressions as
            // this pass also applies some scheduling that helps performance in
            // some cases. We should work on using NVPTX target instead and
            // address the performance regressions with some scheduling
            // solution.
            ...
            // We don't pass the targetMachine to the LLVM-IR pass builder,
            // unless `arch` is specified.
            //
            // Don't set target machine in LLVM pass builder when using LLVM IR
            // level plugins. LLVM IR level plugin passes typically want to
            // insert calls to externally generated code (i.e. precompile a
            // Cuda/Hip kernel with Clang and then insert a call to it within an
            // instrumentation pass) setting the targetMachine value here can
            // can cause a mismatch in the target machine between the MLIR and
            // Clang generated kernels and break the lowering of some target
            // specific intrinsics.
            ...
            if (!pluginFile.empty()) {
                // TODO: Add some logging here that we inserted a pass into the
                // LLVM pass pipeline
        ...
        [](std::string llvmIR, std::string triple, std::string proc,
           ...
           bool enable_fp_fusion, bool isObject) -> py::object {
            std::string obj;
            {
                ...
                obj = translateLLVMIRToASM(*module, triple, proc, features,
